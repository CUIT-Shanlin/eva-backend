...
- 🎯 读侧优先建议（保持行为不变；单类闭环）：评教写侧对课程域 `CourseMapper` 直连已清零；评教读侧已完成 `EvaTaskQueryRepository`、`EvaRecordQueryRepository` 去 `CourseMapper/SemesterMapper` 编译期直连；下一刀可继续清理 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/bcevaluation/query` 中剩余 QueryRepo（建议从 `EvaStatisticsQueryRepository` 起）。端口前置 `CourseIdsByTeacherIdAndSemesterIdQueryPort` 已就位（当前无实现/无调用点），可作为后续“按 teacherId + semesterId 查询课程ID列表”直连收敛基础。
