...
- 🎯 读侧优先建议（保持行为不变；单类闭环）：评教写侧对课程域 `CourseMapper` 直连已清零；评教读侧已完成 `EvaTaskQueryRepository`、`EvaRecordQueryRepository`、`EvaStatisticsQueryRepository` 去 `CourseMapper/SemesterMapper` 编译期直连（通过 `CourseAndSemesterObjectDirectQueryPort` 收敛）；评教侧 `EvaUpdateGatewayImpl` 也已完成去 `CourseMapper.selectById(...).getSemesterId()` 直连，且 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/gateway/impl/eva/` 目录内未再发现 `CourseMapper/SemesterMapper` 直连点。下一刀建议继续清理评教读侧对课程域 `SubjectMapper` 的编译期直连点（剩余候选：`EvaRecordQueryRepository`）：端口前置 `SubjectObjectDirectQueryPort`（含适配器 `SubjectObjectDirectQueryPortImpl`）已就位，下一刀按“改调用侧（`bc-evaluation/infrastructure`，单类）”推进。端口前置 `CourseIdsByTeacherIdAndSemesterIdQueryPort` 已就位（当前无实现/无调用点），可用于后续“按 teacherId + semesterId 查询课程ID列表”直连收敛场景。
