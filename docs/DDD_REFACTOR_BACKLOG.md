...
- ✅ 会话收口结论（2026-02-13，保持行为不变）：独立性阻塞仍集中在“跨 BC DAL 直连 + 共享依赖边界”两类：`bc-evaluation/**` 课程 mapper import 0 处（已按端口替换完成），`bc-template/**` 评教 mapper import 0 处（已清零编译期 import），`bc-course/**` 评教 mapper import 4 处（1 个文件，尚未清零）；且多个 BC 仍显式依赖 `eva-infra-shared`。
- 🔁 进行中（2026-02-13，保持行为不变）：已在 `CourseTemplateLockQueryPortImpl` 清零评教 mapper import；并在 `CourseRecommendExce/DeleteCourseRepositoryImpl/UpdateSingleCourseRepositoryImpl/AssignEvaTeachersRepositoryImpl/DeleteCoursesRepositoryImpl` 清零 `EvaTaskMapper/FormRecordMapper` 编译期 import；另已补 4 刀并提交：`e7e444a1`（`AddNotExistCoursesDetailsRepositoryImpl` 清零 `FormTemplateMapper` 编译期 import）、`bbc32ae1`（`DeleteSelfCourseRepositoryImpl` 清零 `EvaTaskMapper/FormRecordMapper` 编译期 import）、`db6d6165`（`UpdateSelfCourseRepositoryImpl` 清零 `EvaTaskMapper` 编译期 import）、`0dfe5cd6`（`CourseImportExce` 清零 `EvaTaskMapper/FormRecordMapper/CourOneEvaTemplateMapper/FormTemplateMapper` 编译期 import）；下一步继续把 `bc-course/**` 残留 1 个文件清零（`CourseQueryRepository`；见 `NEXT_SESSION_HANDOFF.md` 0.11）。
