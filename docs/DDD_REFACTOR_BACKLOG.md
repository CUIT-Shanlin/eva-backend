...
3) 写侧（bc-course，方向 A → B，保持行为不变）：`eva-app` 仍存在多处 `@CheckSemId` 写侧入口（需继续按“每次只迁 1 个入口方法簇”推进）。
   - 入口清单（Serena 盘点，2026-01-02）：
     ...
     - `edu.cuit.app.service.impl.course.IUserCourseServiceImpl`：包含 `importCourse` 写侧入口（其中课表解析已端口化为 `CourseExcelResolvePort`，但入口仍在 `eva-app`）。
   ...
   - 下一步建议（保持行为不变；每次只迁 1 个入口方法簇）：优先 `IUserCourseServiceImpl.importCourse`。
