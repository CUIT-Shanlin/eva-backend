...
- ✅ 允许“微调”：仅限结构性重构（收窄依赖/拆接口/移动默认值兜底），**不改业务语义**；缓存/日志/异常文案/副作用顺序完全不变。
  - 🎯 下一批主线建议（更新至 2026-01-15，保持行为不变）：继续推进 **S0.2 延伸（websocket：收敛 `eva-app` → `spring-boot-starter-websocket` 编译期依赖）**。前置已完成：`start/pom.xml` 已显式依赖 `spring-boot-starter-websocket`，且 websocket 通道处理器 `MessageChannel`、URI 解析支撑类 `UriUtils`、握手拦截器 `WebSocketInterceptor`、配置类 `WebSocketConfig` 已从 `eva-app` 归位到 `eva-infra-shared`，并已在 `eva-infra-shared/pom.xml` 补齐 `zym-spring-boot-starter-security` 用于闭合 `StpUtil`（均保持 `package` 不变；详见 4.2）。同时，审计（`bc-audit`）与 AI 报告（`bc-ai-report`）的 `eva-app` 编译期依赖收敛已在本会话闭环（见 4.2）。下一步可进入“评估点”：在 `rg` 证伪 `eva-app/src/main/java` 不再 `import org.springframework.web.socket.*` 后，独立提交移除 `eva-app/pom.xml` 对 `spring-boot-starter-websocket` 的编译期依赖（运行期由组合根 `start` 已显式兜底；保持行为不变）。
