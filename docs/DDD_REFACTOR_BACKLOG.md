...
- ✅ S0.2 延伸（次优先：其它依赖收敛点盘点，证伪结论，保持行为不变）：Serena + `rg` 证据化确认：当前 `eva-app` 的跨 BC 实现类型依赖主要剩余课程域（仍直接 `import edu.cuit.bc.course.application.usecase.*`），因此暂不可将 `eva-app/pom.xml` 的 `bc-course` 编译期依赖替换为更轻的协议层依赖；建议先把 `SemesterServiceImpl/ClassroomServiceImpl/ICourseTypeServiceImpl` 按“保持包名不变”搬运归位到 `bc-course-infra`，再回到 `pom.xml` 收敛（保持行为不变）。消息域方面，已完成上述前置，已具备收敛 `eva-app/pom.xml` 中 `bc-messaging` 编译期依赖的条件（保持行为不变）。
