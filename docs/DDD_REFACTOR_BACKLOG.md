...
- **S0.2 延伸（shared 收尾：仅剩 `UserConverter`，保持行为不变）**：
  ...
  - 退场路线（保持行为不变）：下一刀在 `bc-iam/infrastructure` 增加 Port Adapter（内部直接委托 `UserConverter.*`，保持异常/空值/调用顺序不变）；再逐点替换 `bc-evaluation/bc-course` 调用侧注入；待 Serena 证伪跨 BC 引用面清零后，单类搬运 `UserConverter` 归位到 `bc-iam/infrastructure`（保持 `package` 不变），最后逐 pom 去依赖并评估 `eva-infra-shared` reactor 退场（每次只改 1 个文件闭环）。
...
- 🎯 下一刀建议（shared 瘦身收尾：Convertor 收尾（当前唯一阻塞：`UserConverter`），保持行为不变；每次只改 1 个文件闭环）：
  ...
  - 推荐路线（保持行为不变）：对 `UserConverter` 先按“新增最小 Port（建议归口 `bc-iam`）→ Port Adapter 内部委托旧 `UserConverter` → 调用侧替换”的节奏逐点消除跨 BC 直接注入；待 Serena 证伪引用面仅剩 `bc-iam/**` 后，再执行“单类搬运归位（`eva-infra-shared` → `bc-iam/infrastructure`）”，最后逐 pom 去依赖并评估 `eva-infra-shared` reactor 退场。
...
- ✅ 补充（2026-02-07，保持行为不变，支撑类归位）：IAM LDAP 对象转换器 `LdapUserConvertor` 已从 `eva-infra-shared` 归位到 `bc-iam/infrastructure`（保持 `package edu.cuit.infra.convertor.user` 不变；Serena：引用面仅命中 `bc-iam/infrastructure`；最小回归通过；落地：`4d09f8da`）。
  ...
  - 🎯 补充建议（更新至 2026-02-06，保持行为不变）：当前“`eva-*` 全量整合（从 root reactor 移除）”仍被 `eva-infra-dal/eva-infra-shared/eva-base` 阻塞（多个 `bc-*` 仍编译期依赖它们）。补充：`eva-domain` 已从 root reactor 退场且其编译期依赖方已清零（口径：`rg -n "<artifactId>eva-domain</artifactId>" --glob "**/pom.xml" .` 预期无命中）。建议先按最小成本把阻塞面收敛到“仅技术共享”：
    ...
    3) **`eva-infra-dal` 路线选择（后置，保持行为不变）**：若目标是“彻底把 `eva-*` 模块全部整合进各业务 BC 并从 reactor 移除”，需要把 `edu.cuit.infra.dal` 下的 DO/Mapper/XML 按业务逐类搬运到对应 `bc-*/infrastructure` 并确保 MyBatis 扫描与资源加载行为不变（建议保持 `package/namespace` 不变）；若目标仅是“仓库不出现 eva- 前缀”，可在完成业务归位与依赖收敛后，再评估将 DAL 作为平台模块改名（artifactId 变更，需评估外部依赖影响）。
