...
- ✅ 会话收口快照（2026-02-13，保持行为不变）：当前余量为 `eva-infra-dal`=`25` Java + `6` XML、`eva-infra-shared`=`9` Java、`eva-base-common`=`2` Java；root reactor 仍包含 `eva-infra-dal/eva-infra-shared/eva-base`。
- ✅ 会话收口结论（2026-02-13，保持行为不变）：独立性阻塞仍集中在“跨 BC DAL 直连 + 共享依赖边界”两类：`bc-evaluation/**` 课程 mapper import 14 处，`bc-course/bc-template/**` 评教 mapper import 22 处；且多个 BC 仍显式依赖 `eva-infra-shared`。
...
- 🧭 方案 B（严格，保持行为不变）：彻底退场 `eva-*` 技术切片
  ...
  - 近期推进建议（仍遵守“每次只改 1 个类 / 1 个 XML / 1 个 pom”，先依赖前置再搬运）：
    1) P0（共享工具下沉）：把 `eva-infra-shared` 中“跨 BC 共享的纯工具类/常量”（例如 `ExcelUtils`）逐类下沉到 `shared-kernel`，并逐个将依赖方 `*-infra` 的依赖从 `eva-infra-shared` 迁移为 `shared-kernel`。
       ...
       - 🎯 下一刀建议（保持行为不变；严格单文件一刀）：`CourseConvertor` 本轮已完成风险评估并触发降级；下一刀优先从 `eva-infra-dal/src/main/resources/mapper/course` 继续做“单资源 XML 归位”（推荐 `SubjectMapper.xml` / `SemesterMapper.xml` / `CourseMapper.xml`），先 Serena 证据化再落地。
...
- ✅ 补充进展（2026-02-12，保持行为不变，方案 1）：课程域 `cour_inf` 的跨 BC 直连清零已推进到“评教侧阶段性清零 + 模板侧清零”；已完成清理 `bc-template/**` 对 `CourInfMapper` 的跨 BC 直连并收敛为调用 `bc-course` 端口 `CourInfIdsByCourseIdsQueryPort.findCourInfIdsByCourseIds(...)`（最小回归通过；落地：`f81b4661`；详见 4.3）。
  - 📌 `eva-*` 整合路线（更新至 2026-02-11；保持行为不变；每刀只改 1 个文件闭环）：
    1) P0（优先，DAL 拆散试点扩展）：继续按“仅单 BC 引用才允许归位”推进 `eva-infra-dal` → `bc-*/infrastructure`。✅ 已补齐 IAM `sys_role` 候选的 XML：`eva-infra-dal/src/main/resources/mapper/user/SysRoleMapper.xml` → `bc-iam/infrastructure/src/main/resources/mapper/user/SysRoleMapper.xml`（保持 MyBatis `namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变；落地：`aa1d7c6b`）。✅ 课程域补充：`CourInfMapper.xml` 已完成单资源归位（`eva-infra-dal` → `bc-course/infrastructure`，落地：`4eb6681c`）。下一刀可从 `eva-infra-dal/src/main/resources/mapper/course` 继续挑选 **1 个 XML**（如 `SubjectMapper.xml/SemesterMapper.xml/CourseMapper.xml`）作为候选，先 Serena 证据化确认对应 Mapper 引用面仅命中 `bc-course/**` 后再归位。
...
- ✅ 补充（2026-02-07，保持行为不变，支撑类归位）：IAM LDAP 对象转换器 `LdapUserConvertor` 已从 `eva-infra-shared` 归位到 `bc-iam/infrastructure`（保持 `package edu.cuit.infra.convertor.user` 不变；Serena：引用面仅命中 `bc-iam/infrastructure`；最小回归通过；落地：`4d09f8da`）。
  ...
  - 🎯 下一刀建议（保持行为不变；每次只改 1 个文件闭环）：✅ 已补齐 IAM `sys_role` 候选 XML：`eva-infra-dal/src/main/resources/mapper/user/SysRoleMapper.xml` → `bc-iam/infrastructure/src/main/resources/mapper/user/SysRoleMapper.xml`（保持 MyBatis `namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变；落地：`aa1d7c6b`）。✅ 课程域补充：`CourInfMapper.xml` 已完成归位（落地：`4eb6681c`）。下一刀建议继续从课程域推进 `eva-infra-dal` 按 BC 拆散：可从 `eva-infra-dal/src/main/resources/mapper/course` 挑选 **1 个 XML**（`SubjectMapper.xml/SemesterMapper.xml/CourseMapper.xml`）作为候选，先 Serena 证据化确认对应 Mapper 引用面仅命中 `bc-course/**` 后再归位。注意：`CourseTypeDO` 当前引用面包含 `eva-infra-shared/src/main/java/edu/cuit/infra/convertor/course/CourseConvertor.java`，不满足“仅 bc-course 引用”约束，暂不建议直接归位 DO（避免依赖/装配边界漂移）。
