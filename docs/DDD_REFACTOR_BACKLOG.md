...
3) 写侧（bc-course，方向 A → B，保持行为不变）：`eva-app` 仍存在多处 `@CheckSemId` 写侧入口（需继续按“每次只迁 1 个入口方法簇”推进）。
   ...
   - ✅ 进展（保持行为不变；每次只改 1 个方法）：已进一步压扁 `CourseDeleteGatewayImpl.deleteCourses`：新增 `DeleteCoursesGatewayEntryUseCase`，旧 gateway 不再构造命令，仅保留事务边界与委托调用（落地：`6428e685`）。下一步建议：继续压扁 `CourseDeleteGatewayImpl.deleteSelfCourse`（Serena：调用点为 `DeleteSelfCoursePortImpl.deleteSelfCourse`；每次只改 1 个方法，保持行为不变）。
