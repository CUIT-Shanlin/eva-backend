...
**已完成（更新至 2026-02-03）**
- ✅ IAM 并行（按 10.3：补齐用户目录/分页端口（前置）；保持行为不变）：在 `bc-iam-contract` 新增 `UserDirectoryQueryPort`（涵盖 `page/allUser/findAllUsername`；分页返回 `PaginationResultEntity<?>`，避免 contract 暴露旧领域实体触发 Maven 循环依赖风险），用于后续将 `bc-iam/infrastructure` 的 `UserServiceImpl` 去 `UserQueryGateway` 编译期依赖；最小回归通过；落地：`6a812cb5`。
...
- ✅ 允许“微调”：仅限结构性重构（收窄依赖/拆接口/移动默认值兜底），**不改业务语义**；缓存/日志/异常文案/副作用顺序完全不变。
  ...
  - 🎯 补充建议（更新至 2026-02-02，保持行为不变）：当前“`eva-*` 全量整合（从 root reactor 移除）”仍被 `eva-domain/eva-infra-dal/eva-infra-shared/eva-base` 阻塞（多个 `bc-*` 仍编译期依赖它们）。建议先按最小成本把阻塞面收敛到“仅技术共享”：
    1) **IAM 并行（10.3）**：继续将依赖方对 `UserQueryGateway` 的编译期依赖收敛为 `bc-iam-contract` 的最小 Port（优先目标：`bc-messaging` 的 `MessageQueryPortImpl`、`bc-evaluation/infrastructure` 的 `EvaTaskServiceImpl`、`EvaStatisticsExporter`；每次只改 1 个类；注意 `SpringUtil.getBean(...)` 次数/顺序不变）。
