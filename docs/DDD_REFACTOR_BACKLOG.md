...
- ✅ 阶段性完成（2026-02-13，保持行为不变）：已在 `CourseTemplateLockQueryPortImpl` 清零评教 mapper import；并在 `CourseRecommendExce/DeleteCourseRepositoryImpl/UpdateSingleCourseRepositoryImpl/AssignEvaTeachersRepositoryImpl/DeleteCoursesRepositoryImpl` 清零 `EvaTaskMapper/FormRecordMapper` 编译期 import；另已补 5 刀并提交：`e7e444a1`（`AddNotExistCoursesDetailsRepositoryImpl` 清零 `FormTemplateMapper` 编译期 import）、`bbc32ae1`（`DeleteSelfCourseRepositoryImpl` 清零 `EvaTaskMapper/FormRecordMapper` 编译期 import）、`db6d6165`（`UpdateSelfCourseRepositoryImpl` 清零 `EvaTaskMapper` 编译期 import）、`0dfe5cd6`（`CourseImportExce` 清零 `EvaTaskMapper/FormRecordMapper/CourOneEvaTemplateMapper/FormTemplateMapper` 编译期 import）、`be3389a2`（`CourseQueryRepository` 清零 `CourOneEvaTemplateMapper/EvaTaskMapper/FormRecordMapper/FormTemplateMapper` 编译期 import）。
