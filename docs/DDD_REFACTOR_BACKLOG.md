...
- ✅ 允许“微调”：仅限结构性重构（收窄依赖/拆接口/移动默认值兜底），**不改业务语义**；缓存/日志/异常文案/副作用顺序完全不变。
  - 📌 `eva-*` 整合路线（更新至 2026-02-07；保持行为不变；每刀只改 1 个文件闭环）：
    1) P0（优先，DAL 拆散试点扩展）：继续按“仅单 BC 引用才允许归位”推进 `eva-infra-dal` → `bc-*/infrastructure`，先从 `course_type` 的 `CourseTypeMapper.xml` 入手（XML 单资源闭环），再逐个盘点 `CourInfMapper/CourseMapper/SemesterMapper/SubjectMapper` 等候选的引用面（Serena 证据化后再动刀）。
