...
- ✅ 课程写侧（保持行为不变，单类闭环）：`bc-course/infrastructure` 的 `AssignEvaTeachersRepositoryImpl` 已清零对评教 DO（`EvaTaskDO`）的编译期依赖（改为直接调用 `EvaTaskInsertPort.insertAndReturnId(...)`；保持 `LocalDateTime.now()` 的调用次数与“先构造再插入”的时序不变；最小回归通过；落地：`8b869c33`）。
- ✅ 课程写侧（保持行为不变，单类闭环）：`bc-course/infrastructure` 的 `DeleteSelfCourseRepositoryImpl` 已清零对评教 DO（`EvaTaskDO`）的编译期依赖（移除无用 import；不改变业务语义/异常文案/副作用顺序；最小回归通过；落地：`63b92a27`）。
- ✅ 课程写侧（保持行为不变，单类闭环）：`bc-course/infrastructure` 的 `AddNotExistCoursesDetailsRepositoryImpl` 已清零对评教 DO（`FormTemplateDO`）的编译期依赖：默认模板查询仍委托 `formTemplateMapper.selectOne(...)`，仅通过反射读取 `getId` 以移除 DO 引用（不改变业务语义/异常文案/副作用顺序；最小回归通过；落地：`99a8bef1`）。
- ✅ 课程域（保持行为不变，编译期依赖收敛，单类闭环）：`bc-course/infrastructure` 的 `CourseBizConvertor` 已清零对评教 `FormTemplateDO` 的编译期依赖（`toEvaTemplateCO(Object)` 反射读取 `getId/getIsDefault` 构造 `EvaTemplateCO`；保持 `selectById` 调用次数/顺序与异常包装口径不变；最小回归通过；落地：`6ad09180`）。
...
- ✅ 模板写侧（保持行为不变，单类闭环）：`bc-template/infrastructure` 的 `CourseTemplateLockQueryPortImpl` 已移除评教 `@Qualifier("evaTaskMapper")` 运行期依赖，改走评教直查端口 `EvaTaskBriefByCourInfIdsDirectQueryPort`（保持分支/查询条件/异常文案/副作用顺序不变；最小回归通过；落地：`e7ab432f`）。注意：本刀仅去除 `evaTaskMapper`，仍残留对评教 `courOneEvaTemplateMapper/formRecordMapper` 的运行期直连与对评教 DO 的编译期依赖，需后续逐刀端口化清零。
- ✅ 评教读侧（保持行为不变，端口前置，单类）：在 `bc-evaluation/contract` 新增模板锁定判定证据直查端口 `CourseTemplateLockEvaEvidenceDirectQueryPort`，用于后续承接模板侧锁定判定对评教 `cour_one_eva_template/form_record` 的两处剩余直连（不引入缓存/切面副作用；入参为空/空列表 no-op；最小回归通过；落地：`754a0b0f`）。
- ✅ 评教读侧（保持行为不变，端口适配器，单类）：在 `bc-evaluation/infrastructure` 新增端口适配器 `CourseTemplateLockEvaEvidenceDirectQueryPortImpl`，内部仅委托 `CourOneEvaTemplateMapper/FormRecordMapper.selectCount(...)` 查询证据存在性（不引入缓存/切面副作用；查询条件与旧 Mapper 调用保持一致；入参为空/空列表 no-op；最小回归通过；落地：`10281eec`）。
- ✅ 模板写侧（保持行为不变，单类闭环）：`bc-template/infrastructure` 的 `CourseTemplateLockQueryPortImpl` 已改为注入并调用 `CourseTemplateLockEvaEvidenceDirectQueryPort`，替换对评教 `courOneEvaTemplateMapper/formRecordMapper` 的运行期直连，并清零对评教 DO 的编译期依赖（锁定判定分支/查询条件/异常文案/副作用顺序不变；最小回归通过；落地：`f757abd9`）。
...
- ✅ 课程写侧（保持行为不变，分配评教老师收敛，单类）：`bc-course/infrastructure` 的 `AssignEvaTeachersRepositoryImpl` 已清零对评教 DO（`EvaTaskDO`）的编译期依赖，改为直接调用 `EvaTaskInsertPort.insertAndReturnId(...)` 并保持 `LocalDateTime.now()` 的调用次数与“先构造再插入”的时序不变（最小回归通过；落地：`8b869c33`）。
- ✅ 课程读侧（保持行为不变，仓储收敛，单类）：`bc-course/infrastructure` 的 `CourseQueryRepository` 已移除 `@Qualifier("evaTaskMapper") Object` 运行期依赖与反射 `selectList/selectCount` 调用，统一改走评教端口 `EvaTaskBriefByCourInfIdsDirectQueryPort`（保持异常文案/缓存/日志与副作用顺序完全不变；最小回归通过；落地：`65d1c784`）。
...
- ✅ 评教读侧（保持行为不变，端口适配器，单类）：在 `bc-evaluation/infrastructure` 新增端口适配器 `EvaTaskCourInfIdsByTeacherIdAndStatusDirectQueryPortImpl`，内部仅委托 `EvaTaskMapper.selectList(eq teacher_id, status)` 并映射为 `courInfId` 列表（不引入缓存/切面副作用；结果顺序/空值语义保持不变；最小回归通过；落地：`0f4fa7d3`）。
- ✅ 课程写侧（保持行为不变，取消链路收敛，单类）：`bc-course/infrastructure` 的 `UpdateSelfCourseRepositoryImpl` 已将“自助改课时用于判断评教时间冲突”的评教任务 `courInfId` 列表查询，从评教 `evaTaskMapper.selectList(eq teacher_id, status=0)` 的反射调用收敛为调用评教直查端口 `EvaTaskCourInfIdsByTeacherIdAndStatusDirectQueryPort`（不引入缓存/切面副作用；异常文案/副作用顺序不变；最小回归通过；落地：`794bc199`）。
...
- ✅ IAM（保持行为不变，跨 BC 直连清零前置，单类）：在 `bc-iam-infra` 新增端口适配器 `UserDetailByUsernameDirectQueryPortImpl`，内部仅委托 `SysUserMapper.selectOne(eq username)` 并映射为 `UserDetailCO(id/name/username)`（不引入缓存/切面副作用；最小回归通过；落地：`644fe2c9`）。
- ✅ IAM（保持行为不变，跨 BC 直连清零前置，单类）：在 `bc-iam-infra` 新增端口适配器 `UserIdByUsernameDirectQueryPortImpl`，内部仅委托 `SysUserMapper.selectOne(eq username).getId()`（不引入缓存/切面副作用；最小回归通过；落地：`1e8c8b99`）。
...
- ✅ 课程写侧（保持行为不变，单类闭环）：`bc-course/infrastructure` 的 `DeleteSelfCourseRepositoryImpl` 已将“按用户名查用户信息”的跨 BC `sysUserMapper.selectOne(eq username)` 反射调用收敛为调用 IAM 端口 `UserDetailByUsernameDirectQueryPort.findByUsername(...)`（不引入缓存/切面副作用；最小回归通过；落地：`ff3a31c4`）。
- ✅ 课程写侧（保持行为不变，单类闭环）：`bc-course/infrastructure` 的 `UpdateSelfCourseRepositoryImpl` 已将“按用户名查用户信息/按 teacherId 查姓名”的跨 BC `sysUserMapper` 反射调用收敛为调用 IAM 直查端口 `UserDetailByUsernameDirectQueryPort/UserNameDirectQueryPort`（不引入缓存/切面副作用；最小回归通过；落地：`0439dea0`）。
- ✅ 课程写侧（保持行为不变，单类闭环）：`bc-course/infrastructure` 的 `UpdateSingleCourseRepositoryImpl` 已将“按 teacherId 取姓名”的跨 BC `sysUserMapper` 反射调用收敛为调用 IAM 端口 `UserNameDirectQueryPort.findNameById(...)`（缺失老师仍抛 `QueryException(\"老师不存在\")`；不引入缓存/切面副作用；最小回归通过；落地：`edf22c1f`）。
- ✅ 课程写侧（保持行为不变，单类闭环）：`bc-course/infrastructure` 的 `DeleteCourseRepositoryImpl` 已将“按 teacherId 取姓名”的跨 BC `sysUserMapper` 反射调用收敛为调用 IAM 端口 `UserNameDirectQueryPort.findNameById(...)`（缺失老师仍保持在删除链路后触发 NPE 的时机不变；不引入缓存/切面副作用；最小回归通过；落地：`77fd744f`）。
...
- ✅ 评教 infra（保持行为不变，删除链路前置，单类）：在 `bc-evaluation/infrastructure` 新增端口适配器 `EvaTaskCascadeDeleteByTaskIdsPortImpl`，内部委托 `EvaTaskMapper.delete(in id)` + `FormRecordMapper.delete(in task_id)` 实现 taskIds 级联删除（不引入缓存/切面副作用；删除顺序保持“先任务后记录”；最小回归通过；落地：`1d69770a`）。
- ✅ 课程写侧（保持行为不变，删除链路收敛，单类）：`bc-course/infrastructure` 的 `DeleteCourseRepositoryImpl` 已将“按 taskIds 删除评教任务 + 评教记录”的实现改为调用评教写端口 `EvaTaskCascadeDeleteByTaskIdsPort`，并移除对 `evaTaskMapper/formRecordMapper` 的反射删除调用点（保持删除顺序与缓存失效顺序不变；最小回归通过；落地：`19d9ebdc`）。
- ✅ 课程写侧（保持行为不变，删除链路收敛，单类）：`bc-course/infrastructure` 的 `DeleteCoursesRepositoryImpl` 已将“按 courInfIds 查询评教任务 + 删除评教任务/记录”的实现改为调用评教直查端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` + 写端口 `EvaTaskCascadeDeleteByTaskIdsPort`，并清零对 `edu.cuit.infra.dal.database.dataobject.eva.*` 的编译期依赖与反射删除调用点（保持删除顺序与缓存失效顺序不变；最小回归通过；落地：`55d27784`）。
- ✅ 评教 contract（保持行为不变，取消链路前置，单类）：在 `bc-evaluation/contract` 新增写侧端口 `EvaTaskCancelByTeacherIdAndCourInfIdPort`，用于课程侧“修改课次/时间导致取消评教任务”链路按 `teacherId + courInfId` 将评教任务置为取消状态（当前约定：将 `eva_task.status` 更新为 `2`；不引入缓存/切面副作用；最小回归通过；落地：`6632b3fe`）。
- ✅ 评教 infra（保持行为不变，取消链路前置，单类）：在 `bc-evaluation/infrastructure` 新增取消端口适配器 `EvaTaskCancelByTeacherIdAndCourInfIdPortImpl`，内部仅委托 `EvaTaskMapper.update(...)` 将 `eva_task.status` 更新为 `2`（不引入缓存/切面副作用；空入参 no-op；最小回归通过；落地：`656ca526`）。
- ✅ 课程写侧（保持行为不变，取消链路收敛，单类）：`bc-course/infrastructure` 的 `UpdateSingleCourseRepositoryImpl` 已将“取消评教任务”的评教 `evaTaskMapper` 反射调用收敛为调用评教端口（通过 `EvaTaskBriefByCourInfIdsDirectQueryPort` 获取任务最小视图并筛选 `status=0`，再调用 `EvaTaskCancelByTeacherIdAndCourInfIdPort` 执行取消；仍保持 `cour_inf_id = courINfo.getCourseId()` 的历史口径不变；最小回归通过；落地：`b9178e9a`）。
...
- ✅ 评教写侧（保持行为不变，单类闭环）：`bc-evaluation/infrastructure` 的 `SubmitEvaluationRepositoryImpl` 提炼提交评教后的缓存失效逻辑为私有方法 `invalidateCachesAfterSaveEvaluation(...)`，保持缓存 key、失效顺序与 `evaluatorName` 补齐口径不变（最小回归通过；落地：`02fb49f1`）。
- ✅ 评教写侧（保持行为不变，单类闭环）：`bc-evaluation/infrastructure` 的 `SubmitEvaluationRepositoryImpl` 抽取 `rethrowInvocationTargetException(...)` 复用反射调用异常解包逻辑，保持对 `RuntimeException/Error` 的原样抛出语义不变（最小回归通过；落地：`135ebc71`）。
- ✅ 评教写侧（保持行为不变，单类闭环）：`bc-evaluation/infrastructure` 的 `PostEvaTaskRepositoryImpl` 提炼发起评教任务后的缓存失效逻辑为私有方法 `invalidateTaskListCaches(...)`，保持缓存 key、失效顺序与教师名解析口径不变（最小回归通过；落地：`59c0a974`）。
- ✅ 评教写侧（保持行为不变，单类闭环）：`bc-evaluation/infrastructure` 的 `PostEvaTaskRepositoryImpl` 抽取 `rethrowInvocationTargetException(...)` 复用反射调用异常解包逻辑，保持对 `RuntimeException/Error` 的原样抛出语义不变（最小回归通过；落地：`0fcca08c`）。
...
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧收敛，单类，保持行为不变）：收敛评教统计读侧调用点：`bc-evaluation/infrastructure` 的 `EvaStatisticsQueryRepository.getEvaEdNumByTeacherId` 已在 `semId!=null` 分支改为调用 `CourseIdsByTeacherIdAndSemesterIdQueryPort` 获取课程ID列表（保持查询条件/结果顺序/空值语义不变；最小回归通过；落地：`c3dd9744`）。
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧收敛，单类，保持行为不变）：收敛评教记录读侧调用点：`bc-evaluation/infrastructure` 的 `EvaRecordQueryRepository.getEvaEdLogInfo` 已在 `courseId` 为空且 `semId!=null` 分支改为调用 `CourseIdsByTeacherIdAndSemesterIdQueryPort` 获取课程ID列表（保持查询条件/结果顺序/空值语义不变；最小回归通过；落地：`8c8a4ece`）。
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧收敛，单类，保持行为不变）：收敛评教统计读侧调用点：`bc-evaluation/infrastructure` 的 `EvaStatisticsQueryRepository.getEvaEdNumByTeacherId` 已在 `semId==null` 分支改为调用 `CourseIdsByTeacherIdQueryPort` 获取课程ID列表（保持查询条件/结果顺序/空值语义不变；最小回归通过；落地：`35b720e5`）。
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧收敛，单类，保持行为不变）：收敛评教记录读侧调用点：`bc-evaluation/infrastructure` 的 `EvaRecordQueryRepository.getEvaEdLogInfo` 已在 `courseId` 为空且 `semId==null` 分支改为调用 `CourseIdsByTeacherIdQueryPort` 获取课程ID列表（保持查询条件/结果顺序/空值语义不变；最小回归通过；落地：`510a4962`）。
...
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧前置，单类，保持行为不变）：为后续收敛评教读侧 QueryRepo 中“仅为拿课程ID列表而先查 CourseDO 再映射”的复杂条件场景，已在 `bc-course/application` 新增课程ID直查端口 `CourseIdsByCourseWrapperDirectQueryPort`（仅接口，无实现/无调用点，避免行为漂移；最小回归通过；落地：`de139ba9`）。
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧前置，单类，保持行为不变）：已在 `bc-course/infrastructure` 新增端口适配器 `CourseIdsByCourseWrapperDirectQueryPortImpl`（内部仅委托 `CourseMapper.selectList(...)` 并映射 `CourseDO::getId` 返回课程ID列表，保持查询条件/结果顺序/空值语义不变；最小回归通过；落地：`cc79d929`）。
...
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧收敛，单类，保持行为不变）：收敛评教读侧调用点：`bc-evaluation/infrastructure` 的 `EvaTaskQueryRepository` 已清零对课程域 `SubjectMapper` 的编译期直连，改为调用 `SubjectObjectDirectQueryPort` 完成科目查询（确保查询次数/顺序与空值语义不变；最小回归通过；落地：`0b11f2c4`）。
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧收敛，单类，保持行为不变）：收敛评教读侧调用点：`bc-evaluation/infrastructure` 的 `EvaTaskQueryRepository` 已将 `pageEvaUnfinishedTask/evaSelfTaskInfo` 中“按 Wrapper 查 CourseDO 列表再映射 `CourseDO::getId`”的实现收敛为调用 `CourseIdsByCourseWrapperDirectQueryPort.findCourseIds(...)` 获取课程ID列表（保持 Wrapper 构造、查询条件、结果顺序与空值语义不变；最小回归通过；落地：`7c5a70eb`）。
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧收敛，单类，保持行为不变）：收敛评教读侧调用点：`bc-evaluation/infrastructure` 的 `EvaRecordQueryRepository` 已清零对课程域 `SubjectMapper` 的编译期直连，改为调用 `SubjectObjectDirectQueryPort` 完成科目查询（确保查询次数/顺序与空值语义不变；最小回归通过；落地：`cfae3eba`）。
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧收敛，单类，保持行为不变）：收敛评教读侧调用点：`bc-evaluation/infrastructure` 的 `EvaRecordQueryRepository` 已将 `pageEvaRecord/getEvaLogInfo` 中“按 Wrapper 查 CourseDO 列表再映射 `CourseDO::getId`”的实现收敛为调用 `CourseIdsByCourseWrapperDirectQueryPort.findCourseIds(...)` 获取课程ID列表（保持 Wrapper 构造、查询条件、结果顺序与空值语义不变；最小回归通过；落地：`7d3b1844`）。
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，写侧收敛，单类，保持行为不变）：收敛评教老师分配写侧调用点：`bc-course/infrastructure` 的 `AssignEvaTeachersRepositoryImpl.judgeAlsoHasCourse(...)` 已将 `CourseMapper.selectList(eq semester_id, in teacher_id)` 的“查 CourseDO 列表再映射 `CourseDO::getId`”收敛为调用 `CourseIdsByCourseWrapperDirectQueryPort.findCourseIds(...)` 获取课程ID列表（保持 Wrapper 构造、查询条件、结果顺序与空值语义不变；最小回归通过；落地：`85768287`）。
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧前置，单类，保持行为不变）：已在 `bc-course/infrastructure` 新增端口适配器 `CourseAndSemesterObjectDirectQueryPortImpl`（内部仅原样委托 `CourseMapper/SemesterMapper` 的查询方法，保持查询条件、结果顺序与空值语义不变；最小回归通过；落地：`0241e079`）。
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧收敛，单类，保持行为不变）：收敛评教读侧调用点：`bc-evaluation/infrastructure` 的 `EvaTaskQueryRepository` 已清零对课程域 `CourseMapper/SemesterMapper` 的编译期直连，改为调用 `CourseAndSemesterObjectDirectQueryPort` 完成课程/学期查询（确保异常文案、查询条件与副作用顺序不变；最小回归通过；落地：`ccc64867`）。
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧收敛，单类，保持行为不变）：收敛评教读侧调用点：`bc-evaluation/infrastructure` 的 `EvaRecordQueryRepository` 已清零对课程域 `CourseMapper/SemesterMapper` 的编译期直连，改为调用 `CourseAndSemesterObjectDirectQueryPort` 完成课程/学期查询（确保异常文案、查询条件与副作用顺序不变；最小回归通过；落地：`75494f6c`）。
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧收敛，单类，保持行为不变）：收敛评教读侧调用点：`bc-evaluation/infrastructure` 的 `EvaStatisticsQueryRepository` 已清零对课程域 `CourseMapper` 的编译期直连，改为调用 `CourseAndSemesterObjectDirectQueryPort` 完成课程查询（确保异常文案、查询条件与副作用顺序不变；最小回归通过；落地：`f4e5b4a9`）。
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，评教侧收敛，单类，保持行为不变）：收敛评教撤回任务缓存失效 key 计算：`bc-evaluation/infrastructure` 的 `EvaUpdateGatewayImpl.cancelEvaTaskById(...)` 已清零对课程域 `CourseMapper.selectById(courseId).getSemesterId()` 的编译期直连，改为调用 `CourseAndSemesterObjectDirectQueryPort.findCourseById(courseId).getSemesterId()`（缓存失效 key 与副作用顺序不变；最小回归通过；落地：`63e3d2dc`）。
- ✅ S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛，读侧收敛，单类，保持行为不变）：收敛评教读侧调用点：`bc-evaluation/infrastructure` 的 `EvaTemplateQueryRepository` 已清零对课程域 `CourseMapper.selectById` 的编译期直连，改为调用 `CourseTeacherAndSemesterQueryPort` + `CourseTemplateIdQueryPort` 获取 `templateId` 用于模板查询（确保异常文案、缓存 key 与副作用顺序不变；最小回归通过；落地：`437a449e`）。
...
- ✅ S0.2 延伸（`eva-base` 退场收尾，目录退场前置，单文件，保持行为不变）：删除 `eva-base/eva-base-common/pom.xml`（前置：全仓库 `**/pom.xml` 不再出现 `<artifactId>eva-base-common</artifactId>`；最小回归通过；落地：`5ece67c3`）。
- ✅ S0.2 延伸（`eva-base` 退场收尾，reactor 退场，单 pom，保持行为不变）：收敛 root `pom.xml`：移除 `<module>eva-base</module>`（前置：`eva-base-common` 依赖点清零 + `eva-base-common` module/pom 已退场；最小回归通过；落地：`786fc543`）。
- ✅ S0.2 延伸（`eva-base` 退场收尾，目录退场前置，单文件，保持行为不变）：删除 `eva-base/eva-base-config/pom.xml`（前置：全仓库 `**/pom.xml` 内 `<artifactId>eva-base-config</artifactId>` 仅命中其本体；最小回归通过；落地：`1ba75ddf`）。
...
- ✅ S0.2 延伸（`eva-base` 退场前置，单类搬运，保持行为不变）：将 `LogModule` 从 `eva-base/eva-base-common` 下沉到 `shared-kernel`（保持 `package edu.cuit.common.enums` 与常量内容不变，仅改变 Maven 模块归属；最小回归通过；落地：`77f44292`）。
- ✅ 会话收口快照（2026-02-19，保持行为不变）：当前 `eva-infra-dal` / `eva-infra-shared` / `eva-base-common` 源码余量均为 `0`（口径：`fd -t f -e java ... | wc -l`）；root reactor 仅剩 `eva-base`（口径：`rg -n "<module>eva-" pom.xml`），下一阶段主线是“逐 pom 去 `eva-base-common` 依赖 → `eva-base` reactor 退场 → 删除 `eva-base-common/eva-base` 的 pom（目录清理可选）”。
...
- ✅ S0.2 延伸（引用面收敛前置，单类，保持行为不变）：清理 `bc-evaluation/**` 对 `CourseBizConvertor` 的编译期类型依赖：`MsgServiceImpl` 的测试构造器重载改为接收 `Object` 并通过反射委托构造 `SingleCourseCoConvertPort`（最小回归通过；落地：`9f7d27b0`）。
- ✅ S0.2 延伸（Convertor 归位，单类搬运，保持行为不变）：将 `CourseBizConvertor` 从 `eva-infra-shared` 搬运归位到 `bc-course/infrastructure`（保持 `package edu.cuit.app.convertor.course` 与类内容不变；最小回归通过；落地：`a082b812`）。
...
- ✅ S0.2 延伸（引用面收敛，单类，保持行为不变）：移除 `bc-course/infrastructure` 的 `CourseQueryRepository` 无用 import `MenuConvertor`（最小回归通过；落地：`4b9f5855`）。
- ✅ S0.2 延伸（支撑类归位，单类，保持行为不变）：将 `MenuConvertor` 从 `eva-infra-shared` 搬运归位到 `bc-iam/infrastructure`（保持 `package` 不变；最小回归通过；落地：`06690eee`）。
...
- ✅ S0.2 延伸（DAL 拆散试点：课程 DO 下沉，保持行为不变，单类闭环）：已将 `CourInfDO` 从 `eva-infra-dal` 下沉到 `shared-kernel`（保持 `package edu.cuit.infra.dal.database.dataobject.course` 与类内容不变，仅改变 Maven 模块归属；Serena 证据化：引用面命中 `bc-course/infrastructure`、`bc-evaluation/infrastructure` 与 `start` 测试；最小回归通过；口径更新：`eva-infra-dal` Java 余量由 `10` 变更为 `9`；落地：`542e0231`）。
- ✅ S0.2 延伸（DAL 拆散试点：课程 DO 下沉，保持行为不变，单类闭环）：已将 `CourseDO` 从 `eva-infra-dal` 下沉到 `shared-kernel`（保持 `package edu.cuit.infra.dal.database.dataobject.course` 与类内容不变，仅改变 Maven 模块归属；Serena 证据化：引用面命中 `bc-course/infrastructure`、`bc-evaluation/infrastructure` 等；最小回归通过；口径更新：`eva-infra-dal` Java 余量由 `9` 变更为 `8`；落地：`d822340e`）。
- ✅ S0.2 延伸（DAL 拆散试点：课程 DO 下沉，保持行为不变，单类闭环）：已将 `SemesterDO` 从 `eva-infra-dal` 下沉到 `shared-kernel`（保持 `package edu.cuit.infra.dal.database.dataobject.course` 与类内容不变，仅改变 Maven 模块归属；Serena 证据化：引用面命中 `bc-course/infrastructure`、`bc-evaluation/infrastructure`、`eva-infra-shared` 等；最小回归通过；口径更新：`eva-infra-dal` Java 余量由 `8` 变更为 `7`；落地：`e0680e8a`）。
- ✅ S0.2 延伸（DAL 拆散试点：IAM 用户 DO 下沉，保持行为不变，单类闭环）：已将 `SysUserDO` 从 `eva-infra-dal` 下沉到 `shared-kernel`（保持 `package edu.cuit.infra.dal.database.dataobject.user` 与类内容不变，仅改变 Maven 模块归属；Serena 证据化：引用面命中 `bc-iam/infrastructure` 与 `eva-infra-shared`；最小回归通过；口径更新：`eva-infra-dal` Java 余量由 `11` 变更为 `10`；落地：`31e157cd`）。
- ✅ S0.2 延伸（DAL 拆散试点：IAM 角色 DO 下沉，保持行为不变，单类闭环）：已将 `SysRoleDO` 从 `eva-infra-dal` 下沉到 `shared-kernel`（保持 `package edu.cuit.infra.dal.database.dataobject.user` 与类内容不变，仅改变 Maven 模块归属；Serena 证据化：引用面命中 `bc-iam/infrastructure` 与 `eva-infra-shared`；最小回归通过；口径更新：`eva-infra-dal` Java 余量由 `12` 变更为 `11`；落地：`c5ba98b1`）。
- ✅ S0.2 延伸（DAL 拆散试点：IAM 菜单 DO 下沉，保持行为不变，单类闭环）：已将 `SysMenuDO` 从 `eva-infra-dal` 下沉到 `shared-kernel`（保持 `package edu.cuit.infra.dal.database.dataobject.user` 与类内容不变，仅改变 Maven 模块归属；Serena 证据化：引用面命中 `bc-iam/infrastructure` 与 `eva-infra-shared`；最小回归通过；口径更新：`eva-infra-dal` Java 余量由 `13` 变更为 `12`；落地：`a9141bfe`）。
...
- ✅ S0.2 延伸（DAL 拆散试点：课程域 `semester`，保持行为不变，逐类归位）：`SemesterMapper` 已从 `eva-infra-dal` 搬运归位到 `bc-course/infrastructure`（保持 `package edu.cuit.infra.dal.database.mapper.course` 不变，仅改变 Maven 模块归属；最小回归通过；落地：`83c40eaa`）。口径更新：`eva-infra-dal` Java 余量为 `21`（`fd -t f -e java . eva-infra-dal/src/main/java | wc -l`）。
- ✅ S0.2 延伸（DAL 拆散试点：课程域 `cour_inf`，保持行为不变，逐类归位）：`CourInfMapper` 已从 `eva-infra-dal` 搬运归位到 `bc-course/infrastructure`（保持 `package edu.cuit.infra.dal.database.mapper.course` 不变，仅改变 Maven 模块归属；最小回归通过；落地：`7837b0fb`）。口径更新：`eva-infra-dal` Java 余量为 `20`（`fd -t f -e java . eva-infra-dal/src/main/java | wc -l`）。
- ✅ S0.2 延伸（DAL 拆散试点：课程域 `subject`，保持行为不变，逐类归位）：`SubjectMapper` 已从 `eva-infra-dal` 搬运归位到 `bc-course/infrastructure`（保持 `package edu.cuit.infra.dal.database.mapper.course` 不变，仅改变 Maven 模块归属；最小回归通过；落地：`3cf31869`）。口径更新：`eva-infra-dal` Java 余量为 `19`（`fd -t f -e java . eva-infra-dal/src/main/java | wc -l`）。
- ✅ 方案 B（严格，保持行为不变，前置解耦，单类）：为后续归位评教侧 `FormTemplateMapper` 做准备，`CourseBizConvertor` 已去 `FormTemplateMapper` 编译期依赖（改为按 `beanName` 注入 `Object` + 反射 `selectById(Serializable)`；保持调用次数/顺序不变；最小回归通过；落地：`cd014391`）。
- ✅ 方案 B（严格，保持行为不变，SysUserMapper 归位前置，单类）：为后续归位 IAM `SysUserMapper`（从 `eva-infra-dal` → `bc-iam/infrastructure`）做准备，已先移除 `CourseConvertor` 中对 `SysUserMapper` 的无用 import（不参与 `@Mapper(uses=...)` 且无任何符号引用；最小回归通过；落地：`1e0af235`）。
- ✅ 方案 B（严格，保持行为不变，SysUserMapper 归位前置，单类）：为后续归位 IAM `SysUserMapper` 做准备，已将 `bc-evaluation/infrastructure` 的 `EvaUpdateGatewayImpl` 去 `SysUserMapper` 编译期依赖（改为按 `beanName` 注入 `Object` + 反射调用 `selectById(Serializable)` + `getName()`；缓存失效 key 与副作用顺序不变；最小回归通过；落地：`364c63a0`）。
- ✅ 方案 B（严格，保持行为不变，SysUserMapper 归位前置，单类）：为后续归位 IAM `SysUserMapper` 做准备，已将 `bc-course/infrastructure` 的 `AssignEvaTeachersRepositoryImpl` 去 `SysUserMapper` 编译期依赖（改为按 `beanName` 注入 `Object` + 反射调用 `selectById(Serializable)`；日志/异常文案/副作用顺序不变；最小回归通过；落地：`712c4eb7`）。
- ✅ 方案 B（严格，保持行为不变，SysUserMapper 归位前置，单类）：为后续归位 IAM `SysUserMapper` 做准备，已将 `bc-course/infrastructure` 的 `DeleteCourseRepositoryImpl` 去 `SysUserMapper` 编译期依赖（改为按 `beanName` 注入 `Object` + 反射调用 `selectById(Serializable)`；日志/异常文案/副作用顺序不变；最小回归通过；落地：`9dd8a7d1`）。
- ✅ 方案 B（严格，保持行为不变，SysUserMapper 归位前置，单类）：为后续归位 IAM `SysUserMapper` 做准备，已将 `bc-course/infrastructure` 的 `DeleteCoursesRepositoryImpl` 去 `SysUserMapper` 编译期依赖（改为按 `beanName` 注入 `Object` + 反射调用 `selectById(Serializable)`；异常文案/日志文案/副作用顺序不变；最小回归通过；落地：`b193156d`）。
- ✅ 方案 B（严格，保持行为不变，SysUserMapper 归位前置，单类）：为后续归位 IAM `SysUserMapper` 做准备，已将 `bc-course/infrastructure` 的 `DeleteSelfCourseRepositoryImpl` 去 `SysUserMapper` 编译期依赖（改为按 `beanName` 注入 `Object` + 反射调用 `selectOne(Wrapper)`；异常文案/日志文案/副作用顺序不变；最小回归通过；落地：`71060e69`）。
- ✅ 方案 B（严格，保持行为不变，SysUserMapper 归位前置，单类）：为后续归位 IAM `SysUserMapper` 做准备，已将 `bc-course/infrastructure` 的 `UpdateCourseInfoRepositoryImpl` 去 `SysUserMapper` 编译期依赖（改为按 `beanName` 注入 `Object` + 反射调用 `selectById(Serializable)`；异常文案/缓存失效 key 与副作用顺序不变；最小回归通过；落地：`d5415b0a`）。
- ✅ 方案 B（严格，保持行为不变，SysUserMapper 归位前置，单类）：为后续归位 IAM `SysUserMapper` 做准备，已将 `bc-course/infrastructure` 的 `UpdateSelfCourseRepositoryImpl` 去 `SysUserMapper` 编译期依赖（改为按 `beanName` 注入 `Object` + 反射调用 `selectOne(Wrapper)` / `selectById(Serializable)`；异常文案、缓存/日志与副作用顺序完全不变；最小回归通过；落地：`17c4bd19`）。
- ✅ 方案 B（严格，保持行为不变，SysUserMapper 归位前置，单类）：为后续归位 IAM `SysUserMapper` 做准备，已将 `bc-course/infrastructure` 的 `UpdateSingleCourseRepositoryImpl` 去 `SysUserMapper` 编译期依赖（改为按 `beanName` 注入 `Object` + 反射调用 `selectById(Serializable)`；异常文案、缓存/日志与副作用顺序完全不变；最小回归通过；落地：`15135886`）。
- ✅ 方案 B（严格，保持行为不变，SysUserMapper 归位前置，单类）：为后续归位 IAM `SysUserMapper` 做准备，已将 `bc-course/infrastructure` 的 `CourseImportExce` 去 `SysUserMapper` 编译期依赖（改为按 `beanName` 注入 `Object` + 反射调用 `selectOne(Wrapper)`；异常文案、缓存/日志与副作用顺序完全不变；最小回归通过；落地：`e3cf8426`）。
- ✅ 方案 B（严格，保持行为不变，SysUserMapper 归位前置，单类）：为后续归位 IAM `SysUserMapper` 做准备，已将 `bc-course/infrastructure` 的 `CourseRecommendExce` 去 `SysUserMapper` 编译期依赖（改为按 `beanName` 注入 `Object` + 反射调用 `selectOne(Wrapper)` / `selectById(Serializable)` / `selectList(Wrapper)`；异常文案、缓存/日志与副作用顺序完全不变；最小回归通过；落地：`cccf259b`）。
- ✅ S0.2 延伸（DAL 拆散试点：评教域 `cour_one_eva_template`，保持行为不变，逐类归位）：`CourOneEvaTemplateMapper` 已从 `eva-infra-dal` 搬运归位到 `bc-evaluation/infrastructure`（保持 `package edu.cuit.infra.dal.database.mapper.eva` 不变，仅改变 Maven 模块归属；最小回归通过；落地：`d0eb9fa3`）。口径更新：`eva-infra-dal` Java 余量为 `18`（`fd -t f -e java . eva-infra-dal/src/main/java | wc -l`）。
- ✅ S0.2 延伸（DAL 拆散试点：评教域 `form_template`，保持行为不变，逐类归位）：`FormTemplateMapper` 已从 `eva-infra-dal` 搬运归位到 `bc-evaluation/infrastructure`（保持 `package edu.cuit.infra.dal.database.mapper.eva` 不变，仅改变 Maven 模块归属；最小回归通过；落地：`46b77dc4`）。口径更新：`eva-infra-dal` Java 余量为 `17`（`fd -t f -e java . eva-infra-dal/src/main/java | wc -l`）。
...
- ✅ 会话收口快照（2026-02-13，保持行为不变）：当前余量为 `eva-infra-dal`=`23` Java + `0` XML、`eva-infra-shared`=`9` Java、`eva-base-common`=`2` Java；root reactor 仍包含 `eva-infra-dal/eva-infra-shared/eva-base`。
- ✅ 会话收口结论（2026-02-13，保持行为不变）：独立性阻塞仍集中在“跨 BC DAL 直连 + 共享依赖边界”两类：`bc-evaluation/**` 课程 mapper import 0 处（已按端口替换完成），`bc-template/**` 评教 mapper import 0 处（已清零编译期 import），`bc-course/**` 评教 mapper import 0 处（已清零编译期 import）；且多个 BC 仍显式依赖 `eva-infra-shared`。
- ✅ 阶段性完成（2026-02-14，保持行为不变）：`CourseTemplateLockQueryPortImpl` 与 `CourseRecommendExce/DeleteCourseRepositoryImpl/UpdateSingleCourseRepositoryImpl/AssignEvaTeachersRepositoryImpl/DeleteCoursesRepositoryImpl` 已清零对评教侧 `EvaTaskMapper/FormRecordMapper` 的编译期 import，并改为按 `beanName` 注入 `Object` + 反射调用（保持 MyBatis 语义不变；最小回归通过；落地：`8bf5c164`）；另已补 5 刀并提交：`e7e444a1`（`AddNotExistCoursesDetailsRepositoryImpl` 清零 `FormTemplateMapper` 编译期 import）、`bbc32ae1`（`DeleteSelfCourseRepositoryImpl` 清零 `EvaTaskMapper/FormRecordMapper` 编译期 import）、`db6d6165`（`UpdateSelfCourseRepositoryImpl` 清零 `EvaTaskMapper` 编译期 import）、`0dfe5cd6`（`CourseImportExce` 清零 `EvaTaskMapper/FormRecordMapper/CourOneEvaTemplateMapper/FormTemplateMapper` 编译期 import）、`be3389a2`（`CourseQueryRepository` 清零 `CourOneEvaTemplateMapper/EvaTaskMapper/FormRecordMapper/FormTemplateMapper` 编译期 import）。
...
- ✅ 方案 1（业务整合优先，跨 BC 直连清零，单类，保持行为不变）：收敛评教写侧对课程域 `CourInfMapper.selectById` 的跨 BC 直连：`SubmitEvaluationRepositoryImpl.loadContext/saveEvaluation` 改为调用课程域查询端口 `CourseIdByCourInfIdQueryPort.findCourseIdByCourInfId(...)`（异常文案与查询顺序不变；最小回归通过；落地：`176ed9d4`）。
- ✅ 方案 1（业务整合优先，跨 BC 直连清零前置，单类，保持行为不变）：在 `bc-course/application` 新增 `CourInfTimeSlotQueryPort`，用于以端口方式查询 cour_inf 时间片（id/courseId/week/day/start/end）（后续用于替换评教写侧 `CourInfMapper.selectById/selectList` 的时间冲突判定；最小回归通过；落地：`ef2ab821`）。
- ✅ 方案 1（业务整合优先，跨 BC 直连清零前置，单类，保持行为不变）：在 `bc-course/infrastructure` 新增 `CourInfTimeSlotQueryPortImpl`，用于承接 `CourInfTimeSlotQueryPort`（内部仅 `CourInfMapper.selectById/selectList`；最小回归通过；落地：`343e3ecf`）。
- ✅ 方案 1（业务整合优先，跨 BC 直连清零，单类，保持行为不变）：收敛评教写侧 `PostEvaTaskRepositoryImpl.create(...)` 对课程域 `CourInfMapper.selectById/selectList` 的跨 BC 直连：改为调用课程域查询端口 `CourInfTimeSlotQueryPort.findByCourInfId/findByCourseIds/findByCourInfIds`（时间冲突判定逻辑/异常文案/查询次数与顺序不变；最小回归通过；落地：`50943f8c`）。
- ✅ 方案 1（业务整合优先，跨 BC 直连清零，单类，保持行为不变）：收敛评教侧撤回任务对课程域 `CourInfMapper.selectById` 的跨 BC 直连：`EvaUpdateGatewayImpl.cancelEvaTaskById(...)` 改为调用课程域查询端口 `CourseIdByCourInfIdQueryPort.findCourseIdByCourInfId(...)`（缓存失效 key 与副作用顺序不变；最小回归通过；落地：`6592d4ba`）。
- ✅ 方案 1（业务整合优先，跨 BC 直连清零，单类，保持行为不变）：收敛评教写侧删除评教记录对课程域 `CourInfMapper.selectById/selectList` 的跨 BC 直连：`DeleteEvaRecordRepositoryImpl.delete(...)` 改为调用课程域查询端口 `CourInfTimeSlotQueryPort.findByCourInfId(...)` + `CourInfIdsByCourseIdsQueryPort.findCourInfIdsByCourseIds(...)`（异常文案/分支/查询次数与顺序不变；最小回归通过；落地：`d2472370`）。
- ✅ 方案 1（业务整合优先，跨 BC 直连清零前置，单类，保持行为不变）：在 `bc-course/application` 新增 `CourInfObjectDirectQueryPort`，用于以端口方式直查 cour_inf 对象（返回 `Object`，避免端口暴露 DAL DataObject；后续用于替换评教读侧对 `CourInfMapper` 的直连；最小回归通过；落地：`22fba0be`）。
- ✅ 方案 1（业务整合优先，跨 BC 直连清零前置，单类，保持行为不变）：在 `bc-course/infrastructure` 新增 `CourInfObjectDirectQueryPortImpl`，用于承接 `CourInfObjectDirectQueryPort`（内部仅 `CourInfMapper.selectById/selectList`；最小回归通过；落地：`74cda7d2`）。
- ✅ 方案 1（业务整合优先，跨 BC 直连清零，单类，保持行为不变）：收敛评教读侧任务查询对课程域 `CourInfMapper.selectById/selectList(in course_id)` 的跨 BC 直连：`EvaTaskQueryRepository` 改为调用课程域查询端口 `CourInfObjectDirectQueryPort.findById/findByCourseIds(...)`（异常文案/分支/查询次数与顺序不变；最小回归通过；落地：`aec56fc9`）。
- ✅ 方案 1（业务整合优先，跨 BC 直连清零，单类，保持行为不变）：收敛评教读侧记录查询对课程域 `CourInfMapper.selectList(...)` 的跨 BC 直连：`EvaRecordQueryRepository` 改为调用课程域查询端口 `CourInfObjectDirectQueryPort.findByIds/findByCourseIds(...)`（异常文案/分支/查询次数与顺序不变；最小回归通过；落地：`13889255`）。
...
- ✅ 方案 1（业务整合优先，跨 BC 直连清零，单类，保持行为不变）：收敛评教侧对课程域 `CourInfMapper.selectList(in course_id)` 的跨 BC 直连：`EvaStatisticsQueryRepository.getEvaTaskIdS(...)`/统计相关查询改为调用课程域查询端口 `CourInfIdsByCourseIdsQueryPort.findCourInfIdsByCourseIds(...)`（查询次数与顺序不变；最小回归通过；落地：`28dd1e6b`）。
...
- ✅ S0.2 延伸（websocket：支撑类归位，保持行为不变）：将 `UriUtils` 从 `eva-infra-shared` 搬运归位到 `bc-messaging`（保持 `package edu.cuit.app.util` 不变；类内容不变；Serena：引用面仅命中 `bc-messaging` 的 `WebSocketInterceptor`；最小回归通过；落地：`3febc475`）。
- ✅ S0.2 延伸（websocket：通道处理器归位，保持行为不变）：将 `MessageChannel` 从 `eva-infra-shared` 搬运归位到 `bc-messaging`（保持 `package edu.cuit.app.websocket` 不变；类内容不变；Serena：引用面仅命中 `bc-messaging` 的 `WebSocketConfig`；最小回归通过；落地：`10248c53`）。
- ✅ S0.2 延伸（评教：编译闭合前置，单 pom，保持行为不变）：在 `bc-evaluation/infrastructure/pom.xml` 补齐对 `bc-messaging` 的 Maven 编译期依赖，用于承接后续将 `WebsocketManager` 从 `eva-infra-shared` 归位到 `bc-messaging`（最小回归通过；落地：`4dd1b34f`）。
...
- ✅ S0.2 延伸（DAL 拆散试点：评教 `eva_task`，保持行为不变，逐类归位）：将 `EvaTaskMapper` 从 `eva-infra-dal` 搬运归位到 `bc-evaluation/infrastructure`（保持 `package edu.cuit.infra.dal.database.mapper.eva` 不变；最小回归通过；落地：`b7049e4c`）。
...
- ✅ S0.2 延伸（DAL 拆散试点：IAM `sys_role`，保持行为不变，单资源闭环）：将 `SysRoleMapper.xml` 从 `eva-infra-dal` 搬运归位到 `bc-iam/infrastructure`（保持 MyBatis `namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变；最小回归通过；落地：`aa1d7c6b`）。
...
- ✅ S0.2 延伸（IAM → Course：依赖收敛，保持行为不变）：将 `bc-course` 的 `CourseQueryRepository.toUserEntity` 从“跨 BC 直连 IAM role 表（sys_user_role/sys_role）”收敛为依赖 `bc-iam-contract` 端口 `UserEntityObjectByIdDirectQueryPort`（仍保留对 `sys_user` 的直接查询；异常文案保持不变；不引入新的缓存/切面副作用；最小回归通过；落地：`c22bc75d`）。
- ✅ S0.2 延伸（IAM → Evaluation：依赖收敛，保持行为不变）：将 `bc-evaluation` 的 `EvaTaskQueryRepository.toUserEntity` 从“跨 BC 直连 IAM role 表（sys_user_role/sys_role）”收敛为依赖 `bc-iam-contract` 端口 `UserEntityObjectByIdDirectQueryPort`（异常文案保持不变；不引入新的缓存/切面副作用；最小回归通过；落地：`6c5d6bce`）。
- ✅ S0.2 延伸（IAM → Evaluation：依赖收敛，保持行为不变）：将 `bc-evaluation/infrastructure` 的 `EvaRecordQueryRepository.toUserEntity` 从“跨 BC 直连 IAM role 表（sys_user_role/sys_role）”收敛为依赖 `bc-iam-contract` 端口 `UserEntityObjectByIdDirectQueryPort`（异常文案保持不变；不引入新的缓存/切面副作用；最小回归通过；落地：`78787eee`）。
- ✅ 证据化复核（保持行为不变）：Serena 证伪 `bc-*`（排除 `bc-iam/**`）范围内无 `SysUserRoleMapper`/`SysRoleMapper` 引用点（若后续新增引用点，仍按“每次只改 1 个类”闭环收敛为调用 `UserEntityObjectByIdDirectQueryPort`）。
...
- ✅ S0.2 延伸（IAM：支撑类归位，权限加载，保持行为不变）：将 `StpInterfaceImpl` 从 `eva-infra-shared` 搬运归位到 `bc-iam/infrastructure`（保持 `package edu.cuit.app.security` 不变，仅改变 Maven 模块归属；`@Component` 由 Spring 扫描加载；最小回归通过；落地：`192e790c`）。
- ✅ S0.2 延伸（评教：支撑类归位，消息业务对象转换器，保持行为不变）：将 `MsgBizConvertor` 从 `eva-infra-shared` 搬运归位到 `bc-evaluation/infrastructure`（保持 `package edu.cuit.app.convertor` 不变，仅改变 Maven 模块归属；Serena：引用面集中在评教旧入口壳 `MsgServiceImpl`（另有 `start` 单测 mock）；最小回归通过；落地：`7077924e`）。
- ✅ S0.2 延伸（评教：支撑类归位，配置业务对象转换器，保持行为不变）：将 `EvaConfigBizConvertor` 从 `eva-infra-shared` 搬运归位到 `bc-evaluation/infrastructure`（保持 `package edu.cuit.app.convertor.eva` 不变，仅改变 Maven 模块归属；Serena：引用面仅命中 `EvaConfigService`；最小回归通过；落地：`3d374b20`）。
- ✅ S0.2 延伸（评教：支撑类归位，评教记录对象转换器，保持行为不变）：将 `EvaRecordBizConvertor` 从 `eva-infra-shared` 搬运归位到 `bc-evaluation/infrastructure`（保持 `package edu.cuit.app.convertor.eva` 不变，仅改变 Maven 模块归属；Serena：未发现显式引用面；最小回归通过；落地：`6a5430cb`）。
- ✅ S0.2 延伸（课程域：支撑类归位，事务提交后事件发布器，保持行为不变）：将 `AfterCommitEventPublisher` 从 `eva-infra-shared` 搬运归位到 `bc-course/infrastructure`（保持 `package edu.cuit.app.event` 不变，仅改变 Maven 模块归属；Serena：引用面仅命中 `bc-course/infrastructure`；最小回归通过；落地：`352b1680`）。
- ✅ S0.2 延伸（课程域：支撑类归位，学期转换器，保持行为不变）：将 `SemesterConverter` 从 `eva-infra-shared` 搬运归位到 `bc-course/infrastructure`（保持 `package edu.cuit.infra.convertor` 不变，仅改变 Maven 模块归属；Serena：引用面仅命中 `bc-course/infrastructure` 的 `SemesterGatewayImpl`；最小回归通过；落地：`99f78d40`）。
- ✅ S0.2 延伸（支撑类归位，逐类归位，保持行为不变）：将 `ClassroomCacheConstants` 从 `eva-infra-shared` 搬运归位到 `bc-course/infrastructure`（保持 `package edu.cuit.infra.enums.cache` 不变，仅改变 Maven 模块归属；Serena：引用面仅命中 `bc-course/infrastructure`；最小回归通过；落地：`eb41025e`）。
- ✅ S0.2 延伸（支撑类归位，逐类归位，保持行为不变）：将 `CalculateClassTime` 从 `eva-infra-shared` 搬运归位到 `bc-evaluation/infrastructure`（保持 `package edu.cuit.infra.gateway.impl.eva.util` 不变，仅改变 Maven 模块归属；Serena：引用面仅命中 `bc-evaluation/infrastructure` 的 `PostEvaTaskRepositoryImpl`；最小回归通过；落地：`01b13b20`）。
- ✅ S0.2 延伸（IAM：支撑类归位，头像配置属性，保持行为不变）：将 `AvatarProperties` 从 `eva-infra-shared` 搬运归位到 `bc-iam/infrastructure`（保持 `package edu.cuit.infra.property` 不变，仅改变 Maven 模块归属；Serena：引用面仅命中 `bc-iam/infrastructure` 的 `AvatarManager`；最小回归通过；落地：`e3e9a1e4`）。
- ✅ S0.2 延伸（IAM：DAL（LDAP）拆散试点，逐类归位，保持行为不变）：将 `LdapPersonDO` 从 `eva-infra-shared` 搬运归位到 `bc-iam/infrastructure`（保持 `package edu.cuit.infra.dal.ldap.dataobject` 不变；类内容不变；Serena：引用面仅命中 `bc-iam/infrastructure`；最小回归通过；落地：`2120b80d`）。
- ✅ S0.2 延伸（评教：支撑类归位，静态配置属性，保持行为不变）：将 `StaticConfigProperties` 从 `eva-infra-shared` 搬运归位到 `bc-evaluation/infrastructure`（保持 `package edu.cuit.infra.property` 不变，仅改变 Maven 模块归属；Serena：引用面仅命中 `bc-evaluation/infrastructure` 的 `EvaConfigGatewayImpl`；最小回归通过；落地：`3004217d`）。
...
- ✅ S0.2 延伸（DAL 拆散试点，单资源闭环，保持行为不变）：将 `SysMenuMapper.xml` 从 `eva-infra-dal` 搬运归位到 `bc-iam/infrastructure`（保持 MyBatis `namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变；最小回归通过；落地：`920c17d1`）。
- ✅ S0.2 延伸（DAL 拆散试点，逐类归位，保持行为不变）：将 `SysMenuMapper` 从 `eva-infra-dal` 搬运归位到 `bc-iam/infrastructure`（保持 `package edu.cuit.infra.dal.database.mapper.user` 不变，仅改变 Maven 模块归属；Serena：引用面仅命中 `bc-iam/infrastructure`；最小回归通过；落地：`b53615e5`）。
...
- ✅ S0.2 延伸（DAL 拆散试点，逐类归位，保持行为不变）：将 `SysRoleMenuDO` 从 `eva-infra-dal` 搬运归位到 `bc-iam/infrastructure`（保持 `package edu.cuit.infra.dal.database.dataobject.user` 不变，仅改变 Maven 模块归属；Serena：引用面仅命中 `bc-iam/infrastructure` 的 `MenuWritePortImpl/RoleWritePortImpl/UserMenuCacheInvalidationPortImpl/RoleQueryGatewayImpl`；最小回归通过；落地：`49fcbda7`）。
- ✅ S0.2 延伸（DAL 拆散试点，单资源闭环，保持行为不变）：将 `SysRoleMenuMapper.xml` 从 `eva-infra-dal` 搬运归位到 `bc-iam/infrastructure`（保持 MyBatis `namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变；最小回归通过；落地：`db81d674`）。
- ✅ S0.2 延伸（DAL 拆散试点，逐类归位，保持行为不变）：将 `CourseTypeCourseMapper` 从 `eva-infra-dal` 搬运归位到 `bc-course/infrastructure`（保持 `package edu.cuit.infra.dal.database.mapper.course` 不变，仅改变 Maven 模块归属；Serena：引用面仅命中 `bc-course/infrastructure`；最小回归通过；落地：`2e1cd36e`）。
- ✅ S0.2 延伸（DAL 拆散试点，单资源闭环，保持行为不变）：将 `CourseTypeCourseMapper.xml` 从 `eva-infra-dal` 搬运归位到 `bc-course/infrastructure`（保持 MyBatis `namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变；最小回归通过；落地：`45bc05d6`）。
- ✅ S0.2 延伸（DAL 拆散试点，逐类归位，保持行为不变）：将 `CourseTypeCourseDO` 从 `eva-infra-dal` 搬运归位到 `bc-course/infrastructure`（保持 `package edu.cuit.infra.dal.database.dataobject.course` 不变，仅改变 Maven 模块归属；Serena：引用面仅命中 `bc-course/infrastructure`；最小回归通过；落地：`8f410b14`）。
- ✅ S0.2 延伸（DAL 拆散试点，逐类归位，保持行为不变）：将 `CourseTypeMapper` 从 `eva-infra-dal` 搬运归位到 `bc-course/infrastructure`（保持 `package edu.cuit.infra.dal.database.mapper.course` 不变，仅改变 Maven 模块归属；Serena：引用面仅命中 `bc-course/infrastructure`；最小回归通过；落地：`241b75de`）。
- ✅ S0.2 延伸（DAL 拆散试点，单资源闭环，保持行为不变）：将 `CourseTypeMapper.xml` 从 `eva-infra-dal` 搬运归位到 `bc-course/infrastructure`（保持 MyBatis `namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变；最小回归通过；落地：`158f0bd2`）。
...
- ✅ S0.2 延伸（逐类搬运，保持行为不变）：已将 `EvaConfigEntity` 从 `eva-domain` 搬运归位到 `bc-evaluation-domain`（保持 `package` 与类内容不变，仅改变 Maven 模块归属；最小回归通过；落地：`0c7f6aae`；详见 `NEXT_SESSION_HANDOFF.md` 0.9）。
- ✅ S0.2 延伸（逐类搬运，保持行为不变）：已将 `EvaConfigGateway` 从 `eva-domain` 搬运归位到 `bc-evaluation-domain`（保持 `package`/接口签名/注解不变，仅改变 Maven 模块归属；最小回归通过；落地：`1aaff7d5`；详见 `NEXT_SESSION_HANDOFF.md` 0.9）。
- ✅ S0.2 延伸（编译闭合前置，保持行为不变）：为后续归位 `EvaTemplateEntity` 并最终收敛 `bc-evaluation/application` 去 `eva-domain` 编译期依赖做准备，已在 `bc-evaluation/application/pom.xml` 增加对 `bc-evaluation-domain` 的 Maven 编译期依赖（仅编译闭合；最小回归通过；落地：`9637eca1`；详见 `NEXT_SESSION_HANDOFF.md` 0.9）。
- ✅ S0.2 延伸（逐类搬运，保持行为不变）：已将 `EvaTemplateEntity` 从 `eva-domain` 搬运归位到 `bc-evaluation-domain`（保持 `package` 与类内容不变，仅改变 Maven 模块归属；最小回归通过；落地：`ee79ffac`；详见 `NEXT_SESSION_HANDOFF.md` 0.9）。
- ✅ S0.2 延伸（依赖收敛，单 pom，保持行为不变）：已收敛 `bc-evaluation/application/pom.xml`：移除对 `eva-domain` 的 Maven 编译期依赖，并补齐 `cola-component-exception` 以保持 `com.alibaba.cola.exception.*` 编译闭合（最小回归通过；落地：`9f4eaa06`；详见 `NEXT_SESSION_HANDOFF.md` 0.9）。
...
- ✅ IAM（保持行为不变，单类闭环）：`bc-iam/infrastructure` 的 `UserServiceImpl.getOneUserScore` 将课程域查询调用点提炼为私有方法 `getSelfTeachCourseInfoByUserId/findEvaScoreByCourseId`，保持异常文案与调用顺序不变（最小回归通过；落地：`254e0bca`）。
...
- ✅ S1 收尾（目录退场：清理 eva-app 目录，保持行为不变）：在确认 `eva-app` 无任何 tracked 文件且目录内仅剩被忽略的构建产物/IDE 元数据后，清理 `eva-app/` 目录本身（最小回归通过；落地：`3990a3b7`；注：Git 不跟踪空目录/忽略产物，因此使用空提交记录该动作）。
...
- ✅ S0.2 延伸（编译闭合前置：bc-course-domain，保持行为不变）：为后续逐类搬运 `edu.cuit.domain.*` 到 `bc-course-domain` 做前置，在 `bc-course/domain/pom.xml` 补齐 `spring-context(provided)`（仅编译期依赖；最小回归通过；落地：`3ab4b3de`）。
- ✅ S0.2 延伸（逐类搬运：ClassroomGateway，保持行为不变）：Serena 证伪 `ClassroomGateway` 引用面仅在 `bc-course/**` 后，已将其从 `eva-domain` 搬运归位到 `bc-course-domain`（保持 `package` 与接口签名/注解不变；最小回归通过；落地：`11ac2be6`）。
- ✅ S0.2 延伸（逐类搬运：SemesterGateway，保持行为不变）：Serena 证伪 `SemesterGateway` 引用面覆盖 `bc-course/**` 与 `eva-infra-shared`（AOP 依赖）后，已将其从 `eva-domain` 搬运归位到 `bc-course-domain`（保持 `package` 与接口签名/注解不变；最小回归通过；落地：`30d1c98a`）。补充：为编译闭合，已在 `bc-course/domain/pom.xml` 补齐 `shared-kernel`（落地：`6353d17e`），并在 `eva-infra-shared/pom.xml` 显式依赖 `bc-course-domain`（过渡期；落地：`46bf4c15`）。
- ✅ S0.2 延伸（编译闭合前置：eva-domain→bc-course-domain，保持行为不变）：为后续逐类把课程域类型从 `eva-domain` 归位到 `bc-course-domain`（且避免 `eva-domain` 内残留接口签名引用导致编译失败），在 `eva-domain/pom.xml` 显式增加对 `bc-course-domain` 的 Maven 编译期依赖（过渡期；最小回归通过；落地：`c425f384`）。
- ✅ S0.2 延伸（编译闭合前置：bc-course-domain→cola/lombok，保持行为不变）：为后续归位 `CourseTypeEntity` 等带 `@Entity`/Lombok 的课程域实体做前置，在 `bc-course/domain/pom.xml` 补齐 `cola-component-domain-starter` 与 `lombok(provided)`（最小回归通过；落地：`2c0626e8`）。
- ✅ S0.2 延伸（逐类搬运：CourseTypeEntity，保持行为不变）：Serena 证伪 `CourseTypeEntity` 引用面集中在课程域（含 `CourseQueryGateway` 签名）后，已将其从 `eva-domain` 搬运归位到 `bc-course-domain`（保持 `package` 与类内容不变；最小回归通过；落地：`97a8e3f3`）。
- ✅ S0.2 延伸（逐类搬运：SubjectEntity，保持行为不变）：Serena 证伪 `SubjectEntity` 引用面集中在课程域（含 `CourseQueryGateway`/`CourseConvertor`/`CourseBizConvertor`/`start` 测试签名引用）后，已将其从 `eva-domain` 搬运归位到 `bc-course-domain`（保持 `package` 与类内容不变；最小回归通过；落地：`449e08d1`）。
- ✅ S0.2 延伸（逐类搬运：SemesterEntity，保持行为不变）：Serena 证伪 `SemesterEntity` 引用面覆盖 `bc-course/**` 与 `bc-evaluation/**`（查询侧）以及 `eva-domain/eva-infra-shared` 的签名引用后，已将其从 `eva-domain` 搬运归位到 `bc-course-domain`（保持 `package` 与类内容不变；最小回归通过；落地：`f4721306`）。
- ✅ S0.2 延伸（逐类搬运：CourseDeleteGateway，保持行为不变）：Serena 证伪 `CourseDeleteGateway` 引用面集中在课程域（`bc-course/**`，含 `CourseTypeUseCase` 签名引用与端口适配器/旧 gateway 实现）后，已将其从 `eva-domain` 搬运归位到 `bc-course-domain`（保持 `package` 与接口签名/注解不变；最小回归通过；落地：`2ec12495`）。
- ✅ S0.2 延伸（逐类搬运：CourseUpdateGateway，保持行为不变）：Serena 证伪 `CourseUpdateGateway` 引用面集中在课程域（`bc-course/**`，含端口适配器与旧 `CourseUpdateGatewayImpl`）后，已将其从 `eva-domain` 搬运归位到 `bc-course-domain`（保持 `package` 与接口签名/注解不变；最小回归通过；落地：`73ccfff8`）。
- ✅ S0.2 延伸（编译闭合前置：bc-course-domain→bc-iam-domain，保持行为不变）：为后续归位 `CourseEntity/SingleCourseEntity` 提供编译闭合支撑，已在 `bc-course/domain/pom.xml` 显式增加对 `bc-iam-domain` 的 Maven 编译期依赖（承接 `CourseEntity` 对 `UserEntity` 的类型引用；最小回归通过；落地：`fa7e2270`）。
- ✅ S0.2 延伸（逐类搬运：CourseEntity，保持行为不变）：Serena 证伪 `CourseEntity` 引用面集中在课程域与评教读侧（含 `CourseQueryGateway` 签名引用、`eva-infra-shared` Convertor、`start` 测试引用等）后，已将其从 `eva-domain` 搬运归位到 `bc-course-domain`（保持 `package` 与类内容不变；最小回归通过；落地：`c94151f7`）。
- ✅ S0.2 延伸（逐类搬运：SingleCourseEntity，保持行为不变）：Serena 证伪 `SingleCourseEntity` 引用面集中在课程域与评教读侧（含 `CourseQueryGateway` 签名引用、`eva-infra-shared` Convertor、`start` 测试引用等）后，已将其从 `eva-domain` 搬运归位到 `bc-course-domain`（保持 `package` 与类内容不变；最小回归通过；落地：`16db55ff`）。
- ✅ S0.2 延伸（逐类搬运：CourseQueryGateway，保持行为不变）：Serena 证伪 `CourseQueryGateway` 引用面覆盖 `bc-course/**`、`bc-evaluation/**`、`bc-iam/**`、`eva-infra-shared`（Convertor）与 `start` 单测后，已将其从 `eva-domain` 搬运归位到 `bc-course-domain`（保持 `package` 与接口签名/注解不变；最小回归通过；落地：`e5d56d1b`）。
...
- （后续收敛，保持行为不变）：`MsgTipDO` 已进一步从 `eva-infra-dal` 归位到 `bc-messaging`（保持 `package edu.cuit.infra.dal.database.dataobject` 不变，仅改变 Maven 模块归属；最小回归通过；落地：`87b38a55`）。
...
- 🎯 下一阶段主线（写侧优先，保持行为不变；单类闭环）：持续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 contract/shared-kernel）。删除链路/取消链路/自助删课链路已阶段性闭环；✅ **分配评教老师**（`AssignEvaTeachersRepositoryImpl`）与 ✅ **课程读侧仓储**（`CourseQueryRepository`）已完成收敛；模板侧 `CourseTemplateLockQueryPortImpl` 已完成去评教 `evaTaskMapper` 运行期依赖，但仍残留对评教 `courOneEvaTemplateMapper/formRecordMapper` 的运行期直连与对评教 DO 的编译期依赖，下一刀建议继续从该类推进（详见 `NEXT_SESSION_HANDOFF.md` 0.10/0.11）。
...
- ✅ 补充进展（2026-02-23，保持行为不变）：课程写侧 `bc-course/infrastructure` 的 `UpdateSelfCourseRepositoryImpl` 已将按 `cour_inf_id` 的评教任务查询/删除从评教 `evaTaskMapper` 反射调用收敛为调用评教端口（读端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` + 写端口 `EvaTaskDeleteByCourInfIdPort`；保持异常文案/缓存失效/副作用顺序完全不变；最小回归通过；落地：`cd3a7bfe`）。
- ✅ 补充进展（2026-02-23，保持行为不变）：评教 `bc-evaluation/contract` 新增写侧端口 `EvaTaskDeleteByCourInfIdPort`，用于跨 BC 在课程侧“自助改课删除某些课次（cour_inf）”链路中按 `courInfId` 删除评教任务（保持旧调用语义：仅删除 `eva_task`，不做级联删除；不引入缓存/切面副作用；入参为空 no-op；最小回归通过；落地：`6b304d4b`）。
- ✅ 补充进展（2026-02-23，保持行为不变）：评教 `bc-evaluation/infrastructure` 新增端口适配器 `EvaTaskDeleteByCourInfIdPortImpl`，内部仅委托 `EvaTaskMapper.delete(eq cour_inf_id)` 按 `courInfId` 删除 `eva_task`（不引入缓存/切面副作用；空入参 no-op；最小回归通过；落地：`19814946`）。
...
- ✅ 补充进展（2026-02-24，保持行为不变）：课程写侧 `bc-course/infrastructure` 的 `DeleteSelfCourseRepositoryImpl` 已清零对评教 `evaTaskMapper/formRecordMapper` 的反射调用，改为调用评教端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` + `FormRecordDeleteByTaskIdsPort` + `EvaTaskDeleteByCourInfIdsPort`（保持异常文案/缓存失效/副作用顺序完全不变；最小回归通过；落地：`089be092`）。
...
- **S0.2 延伸（课程/评教：跨 BC Mapper 直连收敛前置，保持行为不变）**：
  ...
  - ✅ 进展（2026-02-20，保持行为不变，单类）：在 `bc-course/infrastructure` 新增端口适配器 `CourseIdsByTeacherIdQueryPortImpl`（内部仅委托 `CourseMapper.selectList(eq teacher_id)` 并映射 `CourseDO::getId`，保持结果顺序/空值语义不变；落地：`80747dea`）。
  - ✅ 进展（2026-02-20，保持行为不变，单类）：收敛评教写侧调用点：`bc-evaluation/infrastructure` 的 `PostEvaTaskRepositoryImpl` 已将“按 teacherId 查询课程 id 列表”的课程域 `CourseMapper.selectList(eq teacher_id)` 直连收敛为调用 `CourseIdsByTeacherIdQueryPort.findCourseIdsByTeacherId(...)`（端口适配器仍委托 `CourseMapper`，确保查询条件/结果顺序/异常文案/副作用顺序不变；落地：`a7dd8c26`）。
  - ✅ 进展（2026-02-20，保持行为不变，单类）：在 `bc-course/application` 新增最小查询端口 `SemesterStartDateQueryPort`（后续用于将评教写侧 `PostEvaTaskRepositoryImpl` 内对课程域 `SemesterMapper.selectById` 的跨 BC 直连按“补适配器（单类）→ 改调用侧（单类）”逐步收敛为调用端口；落地：`b82e9190`）。
  ...
  - 🎯 下一刀建议（保持行为不变；每次只改 1 个类闭环）：评教写侧 `bc-evaluation/infrastructure/repository` 下对课程域 `CourseMapper` 的编译期直连已清零；下一步建议转向评教读侧 QueryRepo 的直连点或进入“跨 BC 依赖边界收敛（contract 化）”阶段。
...
- **S0.2 延伸（课程/评教：DAL XML 按 BC 归位，保持行为不变）**：
  ...
  - ✅ 已闭环（保持行为不变；单资源）：`SemesterMapper.xml` 已归位到 `bc-course/infrastructure`，并同步删除 `eva-infra-dal/src/main/resources/mapper/**` 旧位置同名文件以避免 classpath 重复导致 MyBatis 启动失败（落地：`29d9b208`）。
  - ✅ 已闭环（保持行为不变；单资源）：`CourOneEvaTemplateMapper.xml` 已归位到 `bc-evaluation/infrastructure`，并同步删除 `eva-infra-dal/src/main/resources/mapper/**` 旧位置同名文件以避免 classpath 重复导致 MyBatis 启动失败（落地：`7aba53e1`）。
  ...
  - 🎯 下一步建议（保持行为不变；逐类闭环）：承接已归位 XML 后，继续拆散 `eva-infra-dal` 的剩余 Java（保持 `package` 不变，避免 classpath 重复）：
    ...
    - P2（逐类归位，保持行为不变）：课程侧剩余 `DO` 已全部按约束处理（跨模块引用的已下沉到 `shared-kernel`）；后续主线转为评估 `EntityFactory` 是否可归位（见 6 的快照与下一刀建议）。
...
- **方案 1（课程域 cour_inf：跨 BC 直连清零收尾，保持行为不变）**：
  - ✅ 状态更新（保持行为不变）：评教侧（`bc-evaluation/**`）与模板侧（`bc-template/**`）对课程域 `CourInfMapper` 的跨 BC 直连已清零（详见 4.2 与 `NEXT_SESSION_HANDOFF.md` 0.9；模板侧落地：`f81b4661`）。
  - 证据口径（可复现）：`rg -n "infra\\.dal\\.database\\.mapper\\.course\\.CourInfMapper" --glob "*.java" bc-*`
...
- ✅ 已完成（单 pom，保持行为不变）：从根 `pom.xml` 的 reactor 中移除 `<module>eva-domain</module>`（仅改变聚合构建边界；最小回归通过；落地：`6b907bc1`）。
  - ✅ 已完成（单 pom，保持行为不变）：删除 `eva-domain/pom.xml`（前置：reactor 已无 `<module>eva-domain</module>`；最小回归通过；落地：`c0035b03`）。
  - ⏳ 未完成（下一刀，保持行为不变）：若 `eva-domain/` 目录仍存在，则评估是否删除空目录（建议独立提交）。
  ...
  - ✅ 已完成（逐类归位）：已将 `EvaConfigGateway` 从 `eva-domain` 搬运归位到 `bc-evaluation-domain`（保持 `package`/接口签名/注解不变；最小回归通过；落地：`1aaff7d5`；详见 `NEXT_SESSION_HANDOFF.md` 0.9）。
  - ✅ 已完成（逐类归位）：已将 `EvaTemplateEntity` 从 `eva-domain` 搬运归位到 `bc-evaluation-domain`（保持 `package` 与类内容不变；最小回归通过；落地：`ee79ffac`；详见 `NEXT_SESSION_HANDOFF.md` 0.9）。
  ...
  - ✅ 进展（逐类归位，保持行为不变）：已将 `SysLogModuleEntity` 从 `eva-domain` 搬运归位到 `bc-audit-domain`（保持 `package` 与类内容不变，仅改变 Maven 模块归属；最小回归通过；落地：`1f8675f1`；详见 `NEXT_SESSION_HANDOFF.md` 0.9）。
  - ✅ 进展（逐类归位，保持行为不变）：已将 `SysLogEntity` 从 `eva-domain` 搬运归位到 `bc-audit-domain`（保持 `package` 与类内容不变，仅改变 Maven 模块归属；最小回归通过；落地：`9efe8f6e`；详见 `NEXT_SESSION_HANDOFF.md` 0.9）。
  - ✅ 进展（逐类归位，保持行为不变）：已将 `LogGateway` 从 `eva-domain` 搬运归位到 `bc-audit-domain`（保持 `package`/接口签名/注解不变，仅改变 Maven 模块归属；最小回归通过；落地：`44417e03`；详见 `NEXT_SESSION_HANDOFF.md` 0.9）。
  ...
  - ✅ 进展（保持行为不变；每次只改 1 个类闭环）：已将 `MessageUseCaseFacade` 从 `eva-domain` 搬运归位到 `bc-messaging-contract`（保持 `package`/接口签名不变，仅改变 Maven 模块归属；最小回归通过；落地：`31681de9`；详见 `NEXT_SESSION_HANDOFF.md` 0.9）。
  - ✅ 进展（保持行为不变；每次只改 1 个 `pom.xml` 闭环）：在 Serena 证伪 `bc-messaging/src/main/java` 无 `eva-domain` 残留引用面后，已收敛 `bc-messaging/pom.xml`：移除对 `eva-domain` 的 Maven 编译期依赖（仅收敛编译边界；最小回归通过；落地：`acecb5af`；详见 `NEXT_SESSION_HANDOFF.md` 0.9）。
  - 🎯 下一刀建议（保持行为不变；每次只改 1 个 `pom.xml` 闭环）：在 Serena 证伪 `eva-infra-shared/src/main/java` 无 `eva-domain` 残留引用面后，收敛 `eva-infra-shared/pom.xml`：移除对 `eva-domain` 的 Maven 编译期依赖（仅收敛编译边界）。
...
- **S0.2 延伸（IAM：依赖收敛 + 去 `eva-domain` 前置，保持行为不变）**：
  ...
  - ✅ 已完成（依赖收敛，保持行为不变）：在 Serena + 可复现 `rg` 证据化确认 `bc-iam/application` 不再需要 `eva-domain` 提供类型后，已收敛 `bc-iam/application/pom.xml`：移除对 `eva-domain` 的 Maven 编译期依赖，并补齐缺失的 `cola-component-exception` 编译期依赖以保持编译闭合（最小回归通过；落地：`04e8b671`）。
  - ✅ 已纠偏（保持行为不变）：`bc-iam/application` 仍 `import edu.cuit.domain.*`，但这些类型已由 `bc-iam-domain/shared-kernel` 提供，因此其 `pom.xml` **不再需要**依赖 `eva-domain`（已落地：`04e8b671`；详见上方“依赖收敛”条目）。补充：更新至 2026-02-04，`UserQueryGateway` 已删除且 `bc-iam/infrastructure` 已清零对其编译期依赖；`RoleQueryGateway` 已按“单类搬运”归位到 `bc-iam-domain`（落地：`4b3efbf7`）。另：`RoleEntity` 已按“单类搬运”归位到 `bc-iam-domain`（落地：`6f290793`），但其跨 BC 引用面仍存在，后续需再评估治理策略。
...
- ✅ 主线进展（更新至 2026-02-25，保持行为不变）：课程/模板写侧已推进“去评教 `evaTaskMapper` 运行期依赖”，其中 `CourseImportExce`、`CourseRecommendExce` 已闭环清零，模板侧 `CourseTemplateLockQueryPortImpl` 已去除 `evaTaskMapper` 但仍有后续清零项（详见 4.2/4.3）。下一步建议先跑口径快照：
  - `rg -n \"@Qualifier\\(\\\\\\\"evaTaskMapper\\\\\\\"\\)\" bc-course bc-template --glob \"**/*.java\"`（预期=0）
  - `rg -n \"@Qualifier\\(\\\\\\\"courOneEvaTemplateMapper\\\\\\\"\\)|@Qualifier\\(\\\\\\\"formRecordMapper\\\\\\\"\\)\" bc-template --glob \"**/*.java\"`
  - `rg -n \"edu\\\\.cuit\\\\.infra\\\\.dal\\\\.database\\\\.dataobject\\\\.eva\\\\.\" bc-course bc-template --glob \"**/*.java\"`
  然后按“补端口（contract，单类）→ 补适配器（infra，单类）→ 改调用侧（单类）”继续逐刀推进（保持行为不变）。
...
- 🎯 下一刀建议（shared 瘦身，LDAP 子簇续接，保持行为不变；每次只改 1 个文件闭环）：
  ...
  - 刀 3（后置，多刀编排）：✅ 已闭环：`LdapConstants` / `EvaLdapUtils` 静态初始化互引风险已按“先证据化再分刀推进”完成拆解与下沉（保持行为不变）。
    - 刀 3-1（单类，解耦前置，保持行为不变）：✅ 已完成：`LdapConstants` 去 `EvaLdapUtils` 编译期互引（仍触发 `EvaLdapUtils` 类初始化；落地：`45fa9651`）。
...
- 🧭 方案 B（严格，保持行为不变）：彻底退场 `eva-*` 技术切片
  - DoD（可复现口径）：`rg -n '<module>eva-' pom.xml` / `rg -n '<artifactId>eva-' --glob '**/pom.xml' .` / `fd -t d '^eva-' . -d 2` 均应无命中
  - 近期推进建议（仍遵守“每次只改 1 个类 / 1 个 XML / 1 个 pom”，先依赖前置再搬运）：
    1) P0（共享工具下沉）：把 `eva-infra-shared` 中“跨 BC 共享的纯工具类/常量”（例如 `ExcelUtils`）逐类下沉到 `shared-kernel`，并逐个将依赖方 `*-infra` 的依赖从 `eva-infra-shared` 迁移为 `shared-kernel`。
       ...
       - 🎯 下一刀建议（保持行为不变；严格单文件一刀）：课程域 `CourseMapper.xml/SemesterMapper.xml` 已归位后，`eva-infra-dal/src/main/resources` 当前 XML 余量为 `0`。下一刀建议转向“跨 BC DAL 直连清零”：优先清零 `bc-course/**` 对评教侧 `infra.dal.database.mapper.eva.*` 的编译期直连（单类闭环，风险更可控；详见 `NEXT_SESSION_HANDOFF.md` 0.11）。
...
- ✅ 补充进展（2026-02-12，保持行为不变，方案 1）：课程域 `cour_inf` 的跨 BC 直连清零已推进到“评教侧阶段性清零 + 模板侧清零”；已完成清理 `bc-template/**` 对 `CourInfMapper` 的跨 BC 直连并收敛为调用 `bc-course` 端口 `CourInfIdsByCourseIdsQueryPort.findCourInfIdsByCourseIds(...)`（最小回归通过；落地：`f81b4661`；详见 4.3）。
  - 📌 `eva-*` 整合路线（更新至 2026-02-11；保持行为不变；每刀只改 1 个文件闭环）：
   1) P0（优先，DAL 拆散试点扩展）：继续按“仅单 BC 引用才允许归位”推进 `eva-infra-dal` → `bc-*/infrastructure`。✅ 已补齐 IAM `sys_role` 候选的 XML：`eva-infra-dal/src/main/resources/mapper/user/SysRoleMapper.xml` → `bc-iam/infrastructure/src/main/resources/mapper/user/SysRoleMapper.xml`（保持 MyBatis `namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变；落地：`aa1d7c6b`）。✅ 课程域补充：`CourInfMapper.xml`（落地：`4eb6681c`）与 `SubjectMapper.xml`（落地：`92374d53`）均已完成单资源归位（`eva-infra-dal` → `bc-course/infrastructure`）。补充：课程域 `CourseMapper.xml/SemesterMapper.xml` 也已归位，导致 `eva-infra-dal/src/main/resources` 当前 XML 余量为 `0`；因此下一刀建议从“搬 XML”转为“清跨 BC mapper import”（详见 `NEXT_SESSION_HANDOFF.md` 0.11）。
    ...
    2) P1（关键阻塞，shared 拆分）：`eva-infra-shared` 中存在跨 BC Convertor（例如 Serena 证据化：`CourseConvertor` 引用面跨 `bc-course/**` 与 `bc-evaluation/**`），因此像 `CourseTypeDO` 这类 DO 会因共享 Convertor 被动扩大引用面；在不改行为前提下，优先选择“继续保留共享但逐步瘦身/后置改名”或“收敛依赖方到 Port/Contract（高成本，高风险）”两条路径之一推进。补充：`CourseTypeDO` 已按该风险判断下沉到 `shared-kernel`（见 4.2）。
    3) P2（模块退场/改名，后置）：当 `rg -n "<module>eva-" pom.xml` 可证伪为 0 且依赖方 `pom.xml` 不再声明 `eva-(infra-dal|infra-shared|base*)` 时，再执行“reactor 移除模块 + 删除模块 pom/目录（逐文件）”或“改名为 shared/platform 模块（artifactId 变更）”。
...
- ✅ 补充（2026-02-07，保持行为不变，支撑类归位）：IAM LDAP 支撑接口 `LdapPersonRepo` 已从 `eva-infra-shared` 归位到 `bc-iam/infrastructure`（保持 `package edu.cuit.infra.dal.ldap.repo` 不变；Serena：引用面仅命中 `bc-iam/infrastructure`；最小回归通过；落地：`ed120804`）。
- ✅ 补充（2026-02-07，保持行为不变，支撑类归位）：IAM LDAP 对象转换器 `LdapUserConvertor` 已从 `eva-infra-shared` 归位到 `bc-iam/infrastructure`（保持 `package edu.cuit.infra.convertor.user` 不变；Serena：引用面仅命中 `bc-iam/infrastructure`；最小回归通过；落地：`4d09f8da`）。
  ...
  - 🎯 下一刀建议（保持行为不变；每次只改 1 个文件闭环）：✅ 已补齐 IAM `sys_role` 候选 XML：`eva-infra-dal/src/main/resources/mapper/user/SysRoleMapper.xml` → `bc-iam/infrastructure/src/main/resources/mapper/user/SysRoleMapper.xml`（保持 MyBatis `namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变；落地：`aa1d7c6b`）。✅ 课程域补充：`CourInfMapper.xml`（落地：`4eb6681c`）、`SubjectMapper.xml`（落地：`92374d53`）以及 `CourseMapper.xml/SemesterMapper.xml`（工作区进行中，未提交）均已归位到 `bc-course/infrastructure`。下一刀建议优先清零 `bc-course/**` 对评教 mapper 的编译期直连（单类闭环），再回到“逐类归位 DO/Mapper/XML”路线。历史说明：`CourseTypeDO` 曾因引用面包含 `eva-infra-shared/src/main/java/edu/cuit/infra/convertor/course/CourseConvertor.java` 而不满足“仅 bc-course 引用”约束（避免依赖/装配边界漂移），现已下沉到 `shared-kernel`（见 4.2）。
  - 🎯 补充建议（更新至 2026-02-19，保持行为不变）：当前“`eva-*` 全量整合（从 root reactor 移除）”仅剩 `eva-base` 阻塞（`eva-infra-dal/eva-infra-shared` 已退场闭环；详见 4.2）。补充：`eva-domain` 已从 root reactor 退场且其编译期依赖方已清零（口径：`rg -n "<artifactId>eva-domain</artifactId>" --glob "**/pom.xml" .` 预期无命中）。建议按最小成本把阻塞面收敛到“仅技术共享”：
    1) **`eva-base` 退场（优先，保持行为不变）**：✅ 已完成前置：`GenericPattern/LogModule` 已下沉到 `shared-kernel`（保持 `package` 不变；见 4.2）。✅ 已完成第 1 刀：收敛 `bc-iam/contract/pom.xml` 去 `eva-base-common`（落地：`fc801525`）。✅ 已完成第 2 刀：收敛 `bc-iam/infrastructure/pom.xml` 去 `eva-base-common`（落地：`13398a74`）。✅ 已完成第 3 刀：收敛 `eva-base/pom.xml` 从 reactor 移除 `eva-base-common` module（落地：`dc8d949d`）。✅ 已完成第 4 刀：删除 `eva-base/eva-base-common/pom.xml`（落地：`5ece67c3`）。✅ 已完成第 5 刀：收敛 root `pom.xml` 移除 `<module>eva-base</module>`（落地：`786fc543`）。✅ 已完成第 6 刀：删除 `eva-base/eva-base-config/pom.xml`（落地：`1ba75ddf`）。✅ 已完成第 7 刀：删除 `eva-base/pom.xml`（落地：`45770eec`）。✅ 已完成第 8 刀：清理 `eva-base/` 目录本身（落地：`4eb50feb`）。
    2) **`eva-infra-shared`（已闭环，保持行为不变）**：✅ 已完成退场闭环（逐 pom 去依赖 + reactor 退场 + 删除 `eva-infra-shared/pom.xml`；目录清理可选），后续不再作为主线阻塞项（见 4.2）。
    3) **`eva-infra-dal`（已闭环，保持行为不变）**：✅ 已从 root reactor 退场且 `pom.xml` 已删除；目录清理可选（见 4.2 与 `NEXT_SESSION_HANDOFF.md`）。
    - 🧱 S1（`eva-*` 技术切片整合，最小试点；保持行为不变）：`CourseTemplateLockQueryPortImpl` 已从 `eva-infra` 归位到 `bc-template-infra`（落地：`9b46d5a7`），组合根 `start` 已移除 `eva-infra(runtime)` 兜底依赖（落地：`1e2ffa89`），当前 `eva-infra` 已无业务实现类（仅剩 3 个 `package-info.java`），且已在 Serena + `rg` 证伪无依赖后从 root reactor 移除 `<module>eva-infra</module>`（落地：`0aab4516`；保持行为不变）。补充：✅ 已删除 `eva-infra/pom.xml`（落地：`6c9b6224`）。下一刀建议（可选，保持行为不变）：若仍希望清理 `eva-infra/` 目录，建议按“每次只改 1 个文件”逐步删除并回归。
    - 下一刀建议（保持行为不变；每次只改 1 个类或 1 个 `pom.xml`）：将主线从“入口壳结构性收敛（Controller）”逐步切换为 **S0.2 延伸（依赖方 pom 收敛）+ `eva-*` 技术切片瘦身/依赖边界收敛**：
      1) 依赖方 `pom.xml` 收敛：优先移除无用依赖/将依赖收窄到 `*-contract`/`shared-kernel`（移除前必须 Serena + `rg` 双证据，避免误删导致增量编译误判）。
      2) `eva-*` 技术切片瘦身：优先逐类归位 `eva-infra-shared` 中“仅单 BC 引用”的支撑类/Convertor；并逐个模块证伪后收敛对 `eva-infra-(shared|dal)` 的依赖边界，为后续“整合/移除 `eva-*` reactor 模块”创造前置条件（验收口径见 `DDD_REFACTOR_PLAN.md` 10.5）。
  ...
  - ✅ 主线口径更新（滚动，更新至 2026-02-06；保持行为不变）：`bc-messaging` 的“归位 + 依赖收敛”已阶段性闭环；`bc-course` 的 S0（旧 gateway 压扁为委托壳）已推进到阶段性闭环（见 4.2/4.3 与 `NEXT_SESSION_HANDOFF.md` 0.9）。当前下一批主线：优先推进 **消息域类型逐类归位（前置：先补齐 `bc-messaging-contract` 承接 `MsgEntity` 的编译闭合依赖）**；随后再回到 **S0.2 延伸（收敛 `bc-course` 的协议承载面 + 收敛依赖方对 `bc-course` 的编译期依赖）**，按“先 Serena 证据化 → 再小步迁移协议对象到 `shared-kernel` / 依赖替换 `pom.xml` → 最小回归 → 提交 → 三文档同步”的节奏推进（保持行为不变）。
 ...
 - ✅ 并行证伪（依赖方编译期依赖收敛，保持行为不变）：最新盘点显示：当前未发现仍显式依赖 `bc-course` 且满足“仅使用 `edu.cuit.client.*` 类型”的模块；且 `eva-app` 已完成去 `bc-course` 编译期依赖（落地：`6fe8ffc8`）。因此“收敛依赖方对 `bc-course` 的编译期依赖”短期内暂无可执行的 `pom.xml` 依赖替换点（保持行为不变）。
  ...
  - ✅ 已完成（前置，DAL/Convertor 归位）：`MsgTipDO/MsgTipMapper(+xml)` → `eva-infra-dal`；`MsgConvertor` → `eva-infra-shared`（保持 `package/namespace` 不变；后续已进一步将 `MsgConvertor` 归位到 `bc-messaging`：`312756c7`；并将 `MsgTipMapper` 从 `eva-infra-dal` 归位到 `bc-messaging`：`4af9f9fc`；并将 `MsgTipDO` 从 `eva-infra-dal` 归位到 `bc-messaging`：`87b38a55`；均仅改变 Maven 模块归属，保持行为不变）。
