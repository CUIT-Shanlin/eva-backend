...
**已完成（更新至 2026-01-26）**
- ✅ S1.1（eva-adapter 退场：root reactor 退场，保持行为不变）：在 Serena 证据化确认全仓库仅 `eva-adapter/pom.xml` 声明 `<artifactId>eva-adapter</artifactId>` 后，从根 `pom.xml` 的 reactor 中移除 `eva-adapter` 模块（最小回归通过）；落地：`86842a1f`。
...
- ✅ 允许“微调”：仅限结构性重构（收窄依赖/拆接口/移动默认值兜底），**不改业务语义**；缓存/日志/异常文案/副作用顺序完全不变。
  - 🎯 下一批主线建议（更新至 2026-01-26，保持行为不变）：✅ 已闭环：**S1：收敛 `eva-adapter` 残留 `*Controller`**（残留已清零；口径见 `NEXT_SESSION_HANDOFF.md` 0.10.2）；✅ 已闭环：组合根 `start` 去 `eva-adapter` 依赖（`92a70a9e`）。下一刀建议：按“每次只改 1 个 `pom.xml`”推进 `eva-adapter` 退场（优先改 root `pom.xml` 移除 `<module>eva-adapter</module>`；保持行为不变）。
    ...
    - 并行落点（建议，保持行为不变；每次只改 1 个类或 1 个 pom）：`eva-adapter/pom.xml` 相关依赖收敛已阶段性闭环（`bc-audit` / `bc-ai-report` / `shared-kernel` / `bc-*-contract` 均已移除编译期依赖）。下一刀建议：改 root `pom.xml` 将 `eva-adapter` 从 reactor 移除（保持行为不变；闭环口径以 `NEXT_SESSION_HANDOFF.md` 0.10 为准）。
