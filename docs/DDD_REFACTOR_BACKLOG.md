...
- ✅ 允许“微调”：仅限结构性重构（收窄依赖/拆接口/移动默认值兜底），**不改业务语义**；缓存/日志/异常文案/副作用顺序完全不变。
  - 🎯 下一批主线建议（更新至 2026-02-05，保持行为不变）：✅ 已闭环：**S1：`eva-adapter` 退场**（root reactor 移除 + `start` 去依赖 + 删除模块 pom；口径见 `NEXT_SESSION_HANDOFF.md` 0.9/0.10）；✅ 已闭环：**依赖收敛纠偏**（`bc-iam-contract` 已恢复对 `bc-evaluation-contract` 的显式依赖，避免误判导致编译漂移；见 `NEXT_SESSION_HANDOFF.md` 0.9）；✅ 已补充：**依赖收敛（单 pom）**：`eva-domain` 已移除未使用的 `spring-statemachine-core`（保持行为不变；口径见 `NEXT_SESSION_HANDOFF.md` 0.9）；✅ 已补充：**IAM 去 `eva-domain` 前置拆解**：已逐类归位 `MenuQueryGateway`、`LdapPersonEntity`、`LdapPersonGateway`，并已进一步归位 `RoleEntity/UserEntity/RoleQueryGateway` 到 `bc-iam-domain`（均保持 `package`/签名不变；见 `NEXT_SESSION_HANDOFF.md` 0.9）。当前主线已切换为 **bc-course：逐类把课程域类型从 `eva-domain` 归位到 `bc-course-domain`**（`CourseQueryGateway` 已归位；`SubjectEntity/SemesterEntity/CourseDeleteGateway/CourseUpdateGateway/CourseEntity/SingleCourseEntity` 已归位）；下一刀建议：**依赖收敛（单 pom）**——在 Serena 证伪 `eva-domain/src/main/java` 无课程域引用面后，尝试移除 `eva-domain/pom.xml` 对 `bc-course-domain` 的编译期依赖（保持行为不变；最小回归通过后落地）。IAM 侧后续仅需继续按 Serena 证据化盘点推进“引用面仅在 IAM”的剩余类型即可。
