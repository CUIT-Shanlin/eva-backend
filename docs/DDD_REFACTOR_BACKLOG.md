...
4) 基础设施（S1 退场候选，保持行为不变）：`eva-infra` 仍存在多处旧 `*GatewayImpl`（需逐个用 Serena 证伪其剩余方法是否仅为委托壳；以及评估“归属到哪个 BC / shared-kernel / 继续保留在共享技术模块”）。
   - 候选清单（Serena 盘点，2026-01-10 更新；滚动修订）：`EvaConfigGatewayImpl/EvaDeleteGatewayImpl/EvaUpdateGatewayImpl/UserQueryGatewayImpl/UserUpdateGatewayImpl/RoleUpdateGatewayImpl`。
