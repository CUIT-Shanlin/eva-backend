...
- 🎯 读侧优先建议（保持行为不变；单类闭环）：评教写侧对课程域 `CourseMapper` 直连已清零；评教读侧已完成 `EvaTaskQueryRepository`、`EvaRecordQueryRepository`、`EvaStatisticsQueryRepository` 去 `CourseMapper/SemesterMapper` 编译期直连（通过 `CourseAndSemesterObjectDirectQueryPort` 收敛）；下一刀可评估 `edu/cuit/infra/gateway/impl/eva/EvaUpdateGatewayImpl` 等残留直连点。端口前置 `CourseIdsByTeacherIdAndSemesterIdQueryPort` 已就位（当前无实现/无调用点），可用于后续“按 teacherId + semesterId 查询课程ID列表”直连收敛场景。
