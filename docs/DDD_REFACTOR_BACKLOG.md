...
- **S0.2 延伸（课程/评教：DAL XML 按 BC 归位，保持行为不变）**：
  ...
  - 🎯 下一步建议（保持行为不变；逐类闭环）：承接已归位 XML 后，继续拆散 `eva-infra-dal` 的剩余 Java（保持 `package` 不变，避免 classpath 重复）：
    ...
    - P2（逐类归位，保持行为不变）：再按 Serena 证据化“仅单 BC 引用才允许归位”的规则，逐个评估并归位剩余 `DO`（如 `CourseTypeDO`）。
...
- `eva-infra-dal` 余量：`2` 个 Java + `0` 个 XML（口径：`fd -t f -e java . eva-infra-dal/src/main/java | wc -l`；目录不存在按 0：`if [ -d eva-infra-dal/src/main/resources ]; then ...; else echo 0; fi`）。
  - 剩余清单（保持行为不变；仅结构性归位，`package` 不变）：`edu.cuit.infra.convertor.EntityFactory`、`CourseTypeDO`。
- 下一刀建议（每次只改 1 个类；保持行为不变）：优先 `CourseTypeDO` 做 Serena 证据化；引用面可能跨 `bc-course/**` 与 `bc-evaluation/**`，需谨慎评估，避免依赖/装配边界漂移；若跨 BC，则下沉到 `shared-kernel`（保持 `package` 不变）。
