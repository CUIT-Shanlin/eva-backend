...
阶段性策略微调（2025-12-29）：
- ✅ 复核口径（2026-01-17，不改业务语义）：确认 `eva-app/pom.xml` 已无 `bc-messaging-contract` 依赖声明；`spring-boot-starter-websocket` 仅由组合根 `start` 显式承接；并已将 `EvaConfigBizConvertor` 从 `eva-app` 归位到 `eva-infra-shared`（保持行为不变）。当前 `eva-app` 残留源码侧口径为 7 个 Java 文件、`eva-adapter` 残留 22 个 Controller（详见 `NEXT_SESSION_HANDOFF.md` 0.9/0.10/0.11 与 `DDD_REFACTOR_PLAN.md` 10.5）。
