...
- **S0.2 延伸（`eva-domain` 退场前置：依赖收敛 + 评教域类型逐类归位，保持行为不变）**：
  ...
  - 🎯 下一刀建议（保持行为不变；每次只改 1 个 `pom.xml` 闭环）：在 Serena 证伪 `bc-messaging/src/main/java` 无 `eva-domain` 残留引用面后，收敛 `bc-messaging/pom.xml`：移除对 `eva-domain` 的 Maven 编译期依赖（仅收敛编译边界）。
