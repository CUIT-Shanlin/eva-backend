...
- ✅ S0.2 延伸（依赖收敛：bc-audit(application) 去 eva-domain 编译期依赖，保持行为不变）：在 Serena + `rg` 证伪 `bc-audit/application/src/main/java` 无 `edu.cuit.domain.*` 引用后，收敛 `bc-audit/application/pom.xml`：移除对 `eva-domain` 的 Maven 编译期依赖（最小回归通过）；落地：`bf90c040`。
