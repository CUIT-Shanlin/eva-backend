...
- ✅ 下一步小簇建议（评教统计导出基础设施归位，保持行为不变）：延续 2025-12-29 的迁移，按“每次只迁 1 个类 + 最小回归”顺序推进：✅ 已完成 `FillUserStatisticsExporterDecorator` 迁移（`e83600f6`）；✅ 已完成 `EvaStatisticsExcelFactory` 迁移（`5b2c2223`）；✅ 已完成 `BcEvaluationConfiguration.evaStatisticsExportPort()` 委托切换（`565552fa`）；✅ 已完成 `eva-app/pom.xml` 移除 `poi/poi-ooxml` Maven 直依赖（课表解析已迁移到 `bc-course-infra`，保持行为不变；落地：`383dbf33`）。
