...
- **S0.2 延伸（课程/评教：DAL XML 按 BC 归位，保持行为不变）**：
  ...
  - 🎯 下一步建议（保持行为不变；逐类闭环）：承接已归位 XML 后，继续拆散 `eva-infra-dal` 的剩余 Java（保持 `package` 不变，避免 classpath 重复）：
    ...
    - P1（单类搬运，保持行为不变）：✅ 已完成：将 `SysUserMapper.java` 从 `eva-infra-dal` 归位到 `bc-iam/infrastructure`（保持 `package edu.cuit.infra.dal.database.mapper.user` 不变，并同步删除旧位置同名类，避免 classpath 重复；落地：`ff591e46`）。下一步：再逐类评估归位 `SysUserDO/SysRoleDO`（补充：`SysMenuDO` 已下沉到 `shared-kernel`，落地：`a9141bfe`）。
