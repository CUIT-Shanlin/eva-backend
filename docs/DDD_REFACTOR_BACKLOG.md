...
阶段性策略微调（2025-12-29）：
- ✅ 复核口径（2026-01-17，不改代码）：确认 `eva-app/pom.xml` 已无 `bc-messaging-contract` 依赖声明；`spring-boot-starter-websocket` 仅由组合根 `start` 显式承接；`eva-app` 残留源码侧口径仍为 8 个 Java 文件、`eva-adapter` 残留 22 个 Controller（详见 `NEXT_SESSION_HANDOFF.md` 0.9/0.10/0.11 与 `DDD_REFACTOR_PLAN.md` 10.5）。
