...
- **S0.2 延伸（`eva-domain` 退场前置：依赖收敛 + 评教域类型逐类归位，保持行为不变）**：
  ...
  - 🎯 下一刀建议（保持行为不变；每次只改 1 个类闭环）：继续证伪并归位 `DynamicConfigGateway` → `bc-evaluation-domain`（保持 `package`/接口签名/注解不变）。
...
- ✅ 允许“微调”：仅限结构性重构（收窄依赖/拆接口/移动默认值兜底），**不改业务语义**；缓存/日志/异常文案/副作用顺序完全不变。
  ...
  - ✅ 主线口径更新（滚动，更新至 2026-02-06；保持行为不变）：`bc-messaging` 的“归位 + 依赖收敛”已阶段性闭环；`bc-course` 的 S0（旧 gateway 压扁为委托壳）已推进到阶段性闭环（见 4.2/4.3 与 `NEXT_SESSION_HANDOFF.md` 0.9）。当前下一批主线：优先推进 **评教域接口逐类归位（下一刀：证伪并归位 `DynamicConfigGateway` → `bc-evaluation-domain`）**；随后再回到 **S0.2 延伸（收敛 `bc-course` 的协议承载面 + 收敛依赖方对 `bc-course` 的编译期依赖）**，按“先 Serena 证据化 → 再小步迁移协议对象到 `shared-kernel` / 依赖替换 `pom.xml` → 最小回归 → 提交 → 三文档同步”的节奏推进（保持行为不变）。
