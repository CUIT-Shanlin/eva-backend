   ...
   - 证据（示例，均保持 `package` 不变）：
     ...
     - （以及其它 `edu.cuit.client.dto.clientobject.course/*`，此处不逐项展开，避免文档噪声）
		    - 计划（每步只迁 1 个小包/小类簇；每步闭环=Serena→最小回归→提交→三文档同步；保持行为不变）：
		      ...
		      5) ⏳ 后续延伸（课程域基础设施归位，保持行为不变）：从 `eva-infra/src/main/java/edu/cuit/infra/bccourse/adapter/*RepositoryImpl` 选择小簇逐个搬运到 `bc-course/infrastructure/src/main/java/edu/cuit/infra/bccourse/adapter/`（每次只迁 1 个类/小簇；允许“仅补齐编译期依赖”以闭合构建，不改业务语义）。已起步：`AddCourseTypeRepositoryImpl` 已归位（落地：`8426d4f2`）；当前 `eva-infra` 仍残留 14 个 `*RepositoryImpl`（Serena 证伪，见 `NEXT_SESSION_HANDOFF.md` 0.9）。
