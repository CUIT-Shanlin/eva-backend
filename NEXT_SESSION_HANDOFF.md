...
**当前残留共 7 个**（均仍在 `eva-adapter/src/main/java`；用于下一会话按 BC 逐个归位，每次只迁 1 个 Controller，保持行为不变）：
- Course（课程/课表）
- Evaluation（评教）
...
**下一刀建议（保持行为不变；每次只改 1 个 Controller）**：
1) 优先归位 `edu/cuit/adapter/controller/eva/query/EvaConfigQueryController.java` → `bc-evaluation/infrastructure`（该 Controller 体量最小，依赖面仅 `IEvaConfigService/EvaConfig/CommonResult`；Serena 已证伪其“代码级引用点”，仅由 Spring 扫描发现）。
2) 若迁移时出现编译缺依赖：先单独提交 1 次 `bc-evaluation/infrastructure/pom.xml` 做“编译闭合前置”，再单独提交 `git mv`（避免“改类 + 改 pom”混在同一提交）。
...
- `fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`（当前应为 7；用于复核“Controller 残留规模”）
- `fd -t f 'Controller\\.java$' eva-adapter/src/main/java | sort`（当前应输出 7 行；用于复核“具体残留清单”，与 0.10.2 保持一致）
...
### E 专用简版（⏳ 未闭环：eva-adapter 仍残留 7 个 *Controller；用于下一阶段迁移入口）
...
下一刀（保持行为不变）：
- **S1（入口归位，优先；每次只迁 1 个 Controller；保持 `package/注解/URL/权限/参数校验/日志与副作用顺序` 完全不变）**：下一刀建议优先将 `EvaQueryController` 从 `eva-adapter` 归位到 `bc-evaluation/infrastructure`。补充：若 `bc-evaluation-infra` 出现编译闭合缺依赖，则先单独提交 1 次 `bc-evaluation/infrastructure/pom.xml`（避免“改类 + 改 pom”混在同一提交；保持行为不变）。
