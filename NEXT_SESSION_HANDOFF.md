...
- 📊 **整体未完成清单（更新至 2026-02-11；保持行为不变）**：
  - `eva-infra-dal`：仍剩 `22` 个 Java + `9` 个 XML 未归位（可复现口径：`fd -t f -e java . eva-infra-dal/src/main/java | wc -l` + `fd -t f -e xml . eva-infra-dal/src/main/resources | wc -l`）。
...
- 🎯 **下一步重构计划（建议，保持行为不变；每次只改 1 个类/1 个 XML/1 个 pom 闭环）**：
  0) ✅ IAM（依赖方继续收敛，保持行为不变）：已将评教域读侧仓储 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/bcevaluation/query/EvaRecordQueryRepository.java` 的 `SysUserRoleMapper/SysRoleMapper` 直连收敛为依赖 `bc-iam-contract` 的 `UserEntityObjectByIdDirectQueryPort`（最小回归通过；落地：`78787eee`）。下一刀建议：继续盘点其它 BC 的同类直连点，逐类按同口径收敛。
...
- 🎯 下一刀建议（保持行为不变；每次只改 1 个文件闭环）：
  1) 优先从 **`bc-course`** 继续推进 `eva-infra-dal` 按 BC 拆散：`course_type` 候选已完成两刀——`CourseTypeMapper`（落地：`241b75de`）+ `CourseTypeMapper.xml`（落地：`158f0bd2`）均已归位到 `bc-course/infrastructure`（保持 MyBatis `namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变）。注意：`CourseTypeDO` 当前引用面包含 `eva-infra-shared/src/main/java/edu/cuit/infra/convertor/course/CourseConvertor.java`，不满足“仅 bc-course 引用”约束，暂不建议直接归位 DO（避免依赖/装配边界漂移）。下一刀建议：转而挑选 `eva-infra-dal/src/main/resources/mapper/course` 下其它 **单资源 XML** 作为候选，先 Serena 证据化确认对应 Mapper 引用面仅命中 `bc-course/**` 后再归位（每次只改 1 个文件闭环）。
  2) 若 Serena 证据化发现候选文件跨 BC 引用，则该文件暂留 `eva-infra-dal`（共享 DAL），转而挑选下一个“仅单 BC 引用”的 DAL 文件继续推进。
...
### 推荐版：续接主线：`eva-infra-dal` 按 BC 拆散（已闭环 `msg_tip` + `sys_role_menu` + `course_type_course`），保持行为不变
...
本轮主线（保持行为不变）：
1) 快照证据（保持行为不变；用于防回归/确认拆散进度）：
   - reactor：`rg -n "<module>eva-" pom.xml`（当前应仍包含 `eva-infra-dal/eva-infra-shared/eva-base`）
   - 消息域 DAL 进度（`msg_tip`，预期已闭环）：
     - `fd -t f -e java . bc-messaging/src/main/java/edu/cuit/infra/dal/database/mapper | rg -n "MsgTipMapper\\.java" || true`（预期命中 1）
     - `fd -t f -e java . bc-messaging/src/main/java/edu/cuit/infra/dal/database/dataobject | rg -n "MsgTipDO\\.java" || true`（预期命中 1）
     - `fd -t f -e xml . bc-messaging/src/main/resources/mapper | rg -n "MsgTipMapper\\.xml" || true`（预期命中 1）
     - `fd -t f -e xml . eva-infra-dal/src/main/resources/mapper | rg -n "MsgTipMapper\\.xml" || true`（预期无命中）
     - `fd -t f -e java . eva-infra-dal/src/main/java/edu/cuit/infra/dal/database/mapper | rg -n "MsgTipMapper\\.java" || true`（预期无命中）
     - `fd -t f -e java . eva-infra-dal/src/main/java/edu/cuit/infra/dal/database/dataobject | rg -n "MsgTipDO\\.java" || true`（预期无命中）
   - IAM DAL 进度（`sys_role_menu`，预期已闭环）：
     - `fd -t f -e java . bc-iam/infrastructure/src/main/java/edu/cuit/infra/dal/database/mapper/user | rg -n "SysRoleMenuMapper\\.java" || true`（预期命中 1）
     - `fd -t f -e java . bc-iam/infrastructure/src/main/java/edu/cuit/infra/dal/database/dataobject/user | rg -n "SysRoleMenuDO\\.java" || true`（预期命中 1）
     - `fd -t f -e xml . bc-iam/infrastructure/src/main/resources/mapper/user | rg -n "SysRoleMenuMapper\\.xml" || true`（预期命中 1）
     - `fd -t f -e xml . eva-infra-dal/src/main/resources/mapper/user | rg -n "SysRoleMenuMapper\\.xml" || true`（预期无命中）
     - `fd -t f -e java . eva-infra-dal/src/main/java/edu/cuit/infra/dal/database/mapper/user | rg -n "SysRoleMenuMapper\\.java" || true`（预期无命中）
     - `fd -t f -e java . eva-infra-dal/src/main/java/edu/cuit/infra/dal/database/dataobject/user | rg -n "SysRoleMenuDO\\.java" || true`（预期无命中）
   - 课程域 DAL 进度（`course_type_course`，预期已闭环）：
     - `fd -t f -e java . bc-course/infrastructure/src/main/java/edu/cuit/infra/dal/database/mapper/course | rg -n "CourseTypeCourseMapper\\.java" || true`（预期命中 1）
     - `fd -t f -e java . bc-course/infrastructure/src/main/java/edu/cuit/infra/dal/database/dataobject/course | rg -n "CourseTypeCourseDO\\.java" || true`（预期命中 1）
     - `fd -t f -e xml . bc-course/infrastructure/src/main/resources/mapper/course | rg -n "CourseTypeCourseMapper\\.xml" || true`（预期命中 1）
     - `fd -t f -e xml . eva-infra-dal/src/main/resources/mapper/course | rg -n "CourseTypeCourseMapper\\.xml" || true`（预期无命中）
     - `fd -t f -e java . eva-infra-dal/src/main/java/edu/cuit/infra/dal/database/mapper/course | rg -n "CourseTypeCourseMapper\\.java" || true`（预期无命中）
     - `fd -t f -e java . eva-infra-dal/src/main/java/edu/cuit/infra/dal/database/dataobject/course | rg -n "CourseTypeCourseDO\\.java" || true`（预期无命中）
2) Serena（必须）：继续推进 `eva-infra-dal` 按 BC 拆散时，优先选择“仅单 BC 引用”的候选文件（Mapper/DO/XML）。推荐从 `bc-course` 入手：`course_type` 候选已完成两刀——`CourseTypeMapper`（落地：`241b75de`）+ `CourseTypeMapper.xml`（落地：`158f0bd2`）已归位到 `bc-course/infrastructure`（保持 `package/namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变）。注意：`CourseTypeDO` 当前引用面包含 `eva-infra-shared/src/main/java/edu/cuit/infra/convertor/course/CourseConvertor.java`，不满足“仅 bc-course 引用”约束，暂不建议直接归位 DO（避免依赖/装配边界漂移）。
3) 最小回归命令（每步结束必跑，Java17，必须 mvnd）：  
...
4) `git commit`（代码）→ 同步三文档 → `git commit`（文档）→ `git push`
...
- ✅ 条目 26（`bc-iam-infra` 阶段 2：IAM DAL 抽离）已完成：已用 Serena 盘点 IAM DAL 依赖清单并完成迁移闭环，最终移除 `bc-iam-infra` → `eva-infra` 依赖（保持行为不变）。
  ...
  - **XML（当前仍在 `eva-infra-dal`）**：`eva-infra-dal/src/main/resources/mapper/user/SysUserMapper.xml`、`SysRoleMapper.xml`、`SysMenuMapper.xml`
...
26) ✅ **已完成：`bc-iam-infra` 依赖收敛（IAM DAL 抽离到 `eva-infra-dal`，保持行为不变）**
   ...
   - 历史拆分与里程碑（保留用于追溯）：
    1) ✅ Serena：盘点 `bc-iam/infrastructure/src/main/java/edu/cuit/infra/bciam/adapter/*` 实际依赖的 Mapper/DO 清单（按“user/role/menu”分组）。
        ...
        - XML：`eva-infra-dal/src/main/resources/mapper/user/SysUserMapper.xml`、`bc-iam/infrastructure/src/main/resources/mapper/user/SysUserRoleMapper.xml`、`SysRoleMapper.xml`、`bc-iam/infrastructure/src/main/resources/mapper/user/SysRoleMenuMapper.xml`、`SysMenuMapper.xml`
