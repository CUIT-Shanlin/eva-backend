...
- 📊 **整体未完成清单（更新至 2026-02-17；保持行为不变）**：
  ...
  - `eva-infra-shared`：当前仍有 `5` 个 Java，且多 BC 仍直依赖该模块（可复现口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l` + `rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | head`）。
  ...
  - `eva-infra-shared` 残留结构（Serena + `rg` 证据化，保持行为不变）：`CourseBizConvertor/CourseConvertor/UserConverter` 在 `eva-infra-shared/src/main/java` 内均仅自引用（无内部依赖）；LDAP 子簇当前仅剩 `LdapConstants/EvaLdapUtils`（两者存在静态初始化/互引风险，需分刀推进）；其中 `EvaLdapProperties`、`LdapGroupDO`、`LdapGroupRepo` 已下沉到 `shared-kernel`（保持包名不变）。
...
1) 证据口径（先跑快照，避免口径漂移）：
   ...
   - `fd -t f -e java . eva-infra-shared/src/main/java | wc -l`（当前预期=5）
   ...
   - 🎯 下一刀建议（优先，LDAP 子簇续接；每次只改 1 个文件闭环）：
    ...
    2) ✅ 已完成（单类，保持行为不变）：`LdapGroupRepo` 从 `eva-infra-shared` 下沉到 `shared-kernel`（保持 `package`/签名不变；落地：`7ff087ad`）。
     3) 🎯 下一刀（后置，多刀编排，保持行为不变）：`LdapConstants` / `EvaLdapUtils` 静态初始化互引风险高，需分刀推进，不建议直接动。
        ...
        - 🎯 下一刀（单类，搬运，保持行为不变）：将 `LdapConstants` 从 `eva-infra-shared` 下沉到 `shared-kernel`（保持 `package` 与类内容不变）。
