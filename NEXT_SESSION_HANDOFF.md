...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”。推荐先走 **删除链路**（改动面相对可控；当前已完成刀 1/2/3/4，下一刀建议从“删除能力端口化”继续推进）：
   ...
   - 下一刀建议（写侧优先，保持行为不变，单类闭环）：在 `bc-evaluation/contract` 新增“按 taskIds 删除评教任务 + 表单记录”的最小写端口（或按 courInfIds 删除的聚合端口），随后在 `bc-evaluation/infrastructure` 落端口适配器，再回到 `bc-course/infrastructure` 收敛 `evaTaskMapper/formRecordMapper` 的反射删除调用点（保持删除次数/顺序与缓存失效顺序不变）。
5) 每刀结束必须闭环：
