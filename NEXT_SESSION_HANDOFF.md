...
3) 🎯 本轮主线（承接已归位 XML，继续拆散 `eva-infra-dal`，保持行为不变）：
   ...
   - 下一刀建议（每次只改 1 个类；保持行为不变）：`EntityFactory`（Serena 证据化；仅当引用面可证伪为“单 BC/单模块”时再归位，否则继续留在 `eva-infra-dal` 作为共享；`package` 不变）。
   - 风险提示（保持行为不变）：`CourseTypeDO` 已下沉到 `shared-kernel`（见 0.9），后续主要风险转移为 `EntityFactory` 的引用面判断（避免误判为单 BC 导致依赖/装配边界漂移）。
