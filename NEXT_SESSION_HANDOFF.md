...
### 0.10.1 最新状态 & 下一步建议（滚动更新至 2026-02-19；聚焦：S0.2 延伸（`eva-infra-dal` 按 BC 拆散 + `eva-infra-shared` 瘦身/退场收尾），保持行为不变）

- 📊 **整体未完成清单（更新至 2026-02-19；保持行为不变）**：
  - ✅ `eva-infra-dal`：已从 root reactor 退场，且已删除 `eva-infra-dal/pom.xml`（保持行为不变；后续仅剩目录清理，若为空目录可单独一刀 `rmdir eva-infra-dal`）。
  - `eva-infra-shared`：源码已清零（Java=`0`；口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l`），但 root reactor 仍包含 `<module>eva-infra-shared</module>`，且仍有 2 处 BC 基础设施 pom 引用（当前应仅命中 `bc-evaluation/infrastructure`、`bc-iam/infrastructure`；口径：`rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | sort`）。下一步按“逐 pom 去依赖 → root reactor 移除 module → 删除 `eva-infra-shared/pom.xml`”收尾（每次只改 1 个文件闭环；保持行为不变）。
  ...
  - `eva-infra-shared` 残留结构（Serena + `rg` 证据化，保持行为不变）：已完成 `UserConverter` 的“最小 Port 收敛跨 BC 使用点 → 单类搬运归位”（`UserConverter` 已归位到 `bc-iam/infrastructure` 且跨 BC 引用面清零）；当前 `eva-infra-shared` 模块仅剩“pom 依赖残留 + reactor module 未退场”的收尾工作（按上一条逐刀执行即可）。
...
### 推荐版（2026-02-19 收口续接：`eva-infra-shared` 退场收尾（逐 pom 去依赖 → reactor 退场 → 删除 pom），保持行为不变）
...
1) 证据口径（先跑快照，避免口径漂移）：
   ...
   - `fd -t f -e java . eva-infra-shared/src/main/java | wc -l`（当前预期=0）
   - `rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | sort`（当前预期仅 2 处：`bc-evaluation/infrastructure`、`bc-iam/infrastructure`）
...
2) ✅ 阶段性完成（保持行为不变）：`UserConverter` 已按“最小 Port 收敛跨 BC 使用点 → 单类搬运归位”的路线归位到 `bc-iam/infrastructure`，且 `bc-course/infrastructure/pom.xml` 已移除对 `eva-infra-shared` 的 Maven 依赖（详见 `NEXT_SESSION_HANDOFF.md` 0.9、`DDD_REFACTOR_PLAN.md` 10.2、`docs/DDD_REFACTOR_BACKLOG.md` 4.3）。
3) 🎯 本轮主线（`eva-infra-shared` 退场收尾；每次只改 1 个文件闭环；保持行为不变）：
   - 第 1 刀（单 pom，优先）：移除 `bc-evaluation/infrastructure/pom.xml` 对 `eva-infra-shared` 的依赖（前置：Serena + `rg` 证伪 `bc-evaluation/**` 无 `eva-infra-shared` 类型引用；最小回归；代码提交；三文档同步提交；push）。
   - 第 2 刀（单 pom）：移除 `bc-iam/infrastructure/pom.xml` 对 `eva-infra-shared` 的依赖（同上闭环）。
   - 第 3 刀（单 pom）：从 root `pom.xml` 的 reactor 中移除 `<module>eva-infra-shared</module>`（仅改变聚合构建边界；保持行为不变）。
   - 第 4 刀（单文件）：删除 `eva-infra-shared/pom.xml`（前置：全仓库 `**/pom.xml` 不再出现 `<artifactId>eva-infra-shared</artifactId>`）。
   - 备注（踩坑提示）：若出现“MapStruct 旧 generated-sources 影响编译”的假阳性报错，可先执行 `mvn -pl :eva-infra-shared clean -Dmaven.repo.local=.m2/repository` 清理构建产物（仅构建产物，不入库）。
4) 每刀结束必须闭环：
