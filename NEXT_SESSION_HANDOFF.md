...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：继续推进 `eva-app` 的 **websocket** 编译期耦合面收敛，按“先归位配置/拦截器/支撑类 → 再收敛 pom 依赖”的套路推进。
  ...
  6) 下一步 1（保持行为不变；每次只改 1 个类）：归位 `eva-app/src/main/java/edu/cuit/app/config/WebSocketConfig.java` → `eva-infra-shared`（保持 `package edu.cuit.app.config` 不变）。Serena 证据：无显式引用点（Spring 扫描装配）；仅需确保搬运不改变 Bean 装配与副作用顺序。
...
- 快速证据口径（新会话先复核，避免口径漂移）：
  ...
  - websocket：`rg -n '^import\\s+org\\.springframework\\.web\\.socket' eva-app/src/main/java`（当前应命中 `WebSocketConfig`；用于后续归位与依赖收敛）。
...
2) 🎯 当前主线：继续推进 **S0.2 延伸（websocket：旧入口/装配归位 → 依赖收敛前置）**：逐步减少 `eva-app` 对 `spring-boot-starter-websocket` 的编译期耦合面，方式为“websocket 相关配置/拦截器/通道处理器逐个归位到 `eva-infra-shared`（保持 `package` 不变）”，最后再评估 `eva-app/pom.xml` 去 `spring-boot-starter-websocket` 编译期依赖（每次只改 1 个类或 1 个 pom；保持行为不变）。补充：审计（`bc-audit`）与 AI 报告（`bc-ai-report`）的 `eva-app` 编译期依赖收敛已在本会话闭环（详见 0.9）。
...
- `rg -n \"<artifactId>spring-boot-starter-websocket</artifactId>\" --glob \"**/pom.xml\" .`（当前应命中 `start/pom.xml` 与 `eva-app/pom.xml`；用于盘点 websocket 依赖边界）
- `fd -t f 'MessageChannel\\.java' eva-app/src/main/java | cat`（当前应为空；`MessageChannel` 已归位 `eva-infra-shared`）
- `rg -n '^import\\s+org\\.springframework\\.web\\.socket' eva-app/src/main/java`（当前应命中 `WebSocketConfig`；用于后续归位与依赖收敛）
