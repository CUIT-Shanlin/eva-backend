...
1) 读侧优先（保持行为不变）：评教读侧 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/bcevaluation/query` 已清零对课程域 `CourseMapper/SemesterMapper` 的编译期直连；评教侧 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/gateway/impl/eva/EvaUpdateGatewayImpl.java` 也已清零对 `CourseMapper.selectById(...).getSemesterId()` 的编译期直连（详见 0.9/4.2）。
   - 下一刀建议（保持行为不变；每次只改 1 个类闭环）：继续瘦身评教读侧对课程域查询的“中间对象”——优先把 `EvaStatisticsQueryRepository` 内剩余“查 CourseDO 列表再映射 id”的场景收敛为调用最小端口（已有 `CourseIdsBySemesterIdQueryPort` / `CourseIdsByTeacherIdQueryPort`）。建议先做 `EvaStatisticsQueryRepository` 内 `eq(semester_id)` 的课程ID列表查询改走 `CourseIdsBySemesterIdQueryPort`（单类），再做其余 `eq(teacher_id)` 的映射残留点（单类，保持行为不变）。
   - 进展提示（保持行为不变）：`CourseAndSemesterObjectDirectQueryPort`（含适配器）已就位并被评教读侧/评教侧使用；`SubjectObjectDirectQueryPort`（含适配器）已就位；`CourseIdsByTeacherIdQueryPort`（含适配器）与 `CourseIdsByTeacherIdAndSemesterIdQueryPort`（含适配器）已就位，且已被 `EvaStatisticsQueryRepository/EvaRecordQueryRepository` 使用（保持行为不变）。
