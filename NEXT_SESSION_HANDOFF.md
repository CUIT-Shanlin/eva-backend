...
- 📊 **整体未完成清单（更新至 2026-02-17；保持行为不变）**：
  ...
  - `eva-infra-shared`：当前仍有 `7` 个 Java，且多 BC 仍直依赖该模块（可复现口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l` + `rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | head`）。
  ...
  - `eva-infra-shared` 残留结构（Serena + `rg` 证据化，保持行为不变）：`CourseBizConvertor/CourseConvertor/UserConverter` 在 `eva-infra-shared/src/main/java` 内均仅自引用（无内部依赖）；LDAP 子簇 `LdapGroupDO/LdapGroupRepo/LdapConstants/EvaLdapUtils` 仍互相依赖（`EvaLdapUtils` ↔ `LdapConstants` + `LdapGroupRepo/DO`），暂不适合无前置直接单类搬运；其中配置类 `EvaLdapProperties` 已下沉到 `shared-kernel`（保持包名不变）。
...
### 推荐版（2026-02-17 收口续接：承接已归位 XML，逐类归位 `eva-infra-dal`/瘦身 `eva-infra-shared`，保持行为不变）
...
3) 🎯 本轮主线（承接已归位 XML，继续拆散 `eva-infra-dal`，保持行为不变）：
   - ✅ 已闭环（保持行为不变）：`SysUserMapper` 归位 + 非 IAM 依赖清零已完成（详见 `NEXT_SESSION_HANDOFF.md` 0.9/0.10.1；本轮无需再动）。
	   - ✅ 已闭环（更新至 2026-02-17；保持行为不变）：Serena 证据化确认 `EntityFactory` 引用面跨多个 BC，因此已将其下沉到 `shared-kernel`（保持 `package edu.cuit.infra.convertor` 与类内容不变，仅改变 Maven 模块归属），并已将 `eva-infra-dal` Java 余量清零。
	   - 🎯 下一刀建议（每次只改 1 个文件；保持行为不变）：评估是否可以让 `eva-infra-dal` 从 root reactor 退场（单 `pom.xml` 闭环；前置：Serena 证伪无模块仍显式依赖）。
4) 每刀结束必须闭环：
