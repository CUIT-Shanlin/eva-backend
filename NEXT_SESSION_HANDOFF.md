...
### 推荐版（续接主线：bc-course S0.2 延伸 —— 下一刀：证伪并归位 `CourseQueryGateway`（保持行为不变；需先处理循环依赖风险））
...
本轮主线（保持行为不变）：
1) 先跑快照证据：  
   - `rg -n "\\bCourseQueryGateway\\b" bc-* --glob "*.java" | head`
2) Serena 证伪 `edu.cuit.domain.gateway.course.CourseQueryGateway` 的引用面（含跨模块引用与签名引用），并同步评估其签名对 `CourseEntity/SingleCourseEntity` 的编译期依赖是否会触发 Maven 循环依赖风险。
3) 若成立且无循环依赖风险：将 `CourseQueryGateway` 从 `eva-domain` 搬运归位到 `bc-course-domain`（保持 `package` 与接口签名/注解不变，仅改变 Maven 模块归属；确保全仓库该 FQCN 仅存在一份）。
4) 若存在循环依赖风险：本刀暂停搬运，并在 `NEXT_SESSION_HANDOFF.md` 0.9 记录“阻塞原因 + 可复现证据口径”，后置再处理 `CourseEntity/SingleCourseEntity` 的承载面与依赖边界。
...
风险提示（保持行为不变）：
- 暂不动 `CourseEntity/SingleCourseEntity`：当前其字段仍编译期依赖 `UserEntity`，直接搬运可能引入 `bc-course-domain` → `bc-iam-domain` 的强耦合；需后续另起小步“承载面设计/依赖边界”再推进。
- 暂缓 `CourseQueryGateway`：其签名仍编译期依赖 `CourseEntity/SingleCourseEntity`（当前在 `eva-domain`），且 `eva-domain` 已过渡期显式依赖 `bc-course-domain`；直接搬运可能引入 `bc-course-domain` ↔ `eva-domain` Maven 循环依赖风险。需先解决 `CourseEntity/SingleCourseEntity` 的承载面与依赖边界，再评估归位。
