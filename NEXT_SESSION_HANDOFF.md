...
- ✅ 执行（单类，保持行为不变）：移除 `@Qualifier("evaTaskMapper") Object` 注入与反射直连；改为注入评教直查端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` 获取任务 `taskIds`，再用 `FormRecordMapper.selectCount(in task_id)` 计算是否锁定（保持原查询条件、异常文案与副作用顺序不变）。为保持既有单测构造方式不变，保留一个仅供测试使用的重载构造器（入参仍为 `Object evaTaskMapper`），内部以反射临时构造端口实现（仅填充 `id` 字段，满足测试断言），运行期由 `@Autowired` 构造器明确选择端口注入版本。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`e7ab432f`。
...
- ✅ **本会话新增闭环（更新至 2026-02-25；保持行为不变）**：
  ...
  - `bc-template/infrastructure`：`CourseTemplateLockQueryPortImpl` 已完成去评教 `evaTaskMapper` 运行期依赖，统一改走评教直查端口 `EvaTaskBriefByCourInfIdsDirectQueryPort`（保持行为不变；详见 0.9；最新落地以 Git 为准）。
...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 contract/shared-kernel）。当前下一步建议先复核残留口径，确认 `bc-course/bc-template` 是否仍存在 `@Qualifier("evaTaskMapper")`（预期=0；若不为 0，则按“补端口 → 补适配器 → 改调用侧”逐刀收敛）。
