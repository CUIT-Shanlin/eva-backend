...
> 下一步建议（读侧优先，单类闭环，保持行为不变）：继续清理 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/bcevaluation/query` 中对课程域 `CourseMapper/SemesterMapper` 的编译期直连（候选：`EvaTaskQueryRepository` → `EvaRecordQueryRepository` → `EvaStatisticsQueryRepository`），并评估 `edu/cuit/infra/gateway/impl/eva/EvaUpdateGatewayImpl` 的直连点。若端口能力不足则按“补端口（单类）→ 补适配器（单类）→ 改调用侧（单类）”推进。每刀仍遵守：Serena 证据化 → 最小回归 → 代码提交 → 三文档同步 → 文档提交 → push。
...
1) 继续清理评教读侧 QueryRepo 对课程域 DAL（`CourseMapper/SemesterMapper`）的编译期直连：优先从 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/bcevaluation/query/EvaTaskQueryRepository.java` 开始，其次 `EvaRecordQueryRepository`、`EvaStatisticsQueryRepository`；并评估 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/gateway/impl/eva/EvaUpdateGatewayImpl.java` 的直连点是否需要同路线收敛（保持行为不变）。
   - 若端口能力不足，则按“补端口（单类，bc-course/application）→ 补适配器（单类，bc-course/infrastructure）→ 改调用侧（单类，bc-evaluation/infrastructure）”推进。
   - 进展提示（保持行为不变）：已补齐端口前置 `CourseIdsByTeacherIdAndSemesterIdQueryPort`（当前无实现/无调用点），可作为后续“按 teacherId + semesterId 查询课程ID列表”直连收敛的基础。
