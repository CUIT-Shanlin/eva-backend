...
- ✅ 最小回归（保持行为不变）：已执行 `mvnd -pl start -am test -Dtest=edu.cuit.app.eva.EvaRecordServiceImplTest,edu.cuit.app.eva.EvaStatisticsServiceImplTest ...`，`BUILD SUCCESS`（对应代码提交：`9e2096fc`；本会话该回归后未再修改代码）。
...
- 🎯 **下一刀建议（保持行为不变；每次只改 1 个类闭环）**：
  ...
  3) （单 pom，前置闭合，保持行为不变）收敛 `bc-messaging-contract/pom.xml`：补齐承接 `MsgEntity` 所需的最小编译期依赖（预期包含 `bc-iam-domain` 与 `cola-component-domain-starter`；保持运行期 classpath 与业务语义不变）。
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：
  - A（优先：pom，每次只改 1 个 `pom.xml`）：消息域类型逐类归位前置 —— 收敛 `bc-messaging-contract/pom.xml` 并补齐承接 `MsgEntity` 所需的最小编译期依赖（预期包含 `bc-iam-domain` 与 `cola-component-domain-starter`；仅用于编译闭合；保持行为不变）。
...
### 推荐版（续接主线：消息域类型逐类归位前置 —— 下一刀：`bc-messaging-contract` 编译闭合补强（承接 `MsgEntity`，保持行为不变））
...
2) Serena 证据化盘点 `MsgEntity` 引用面（重点：是否集中在消息域/是否跨 BC 复用；以及其对 `UserEntity`/`@Entity` 的编译期依赖面）。
   - 背景：审计域 `SysLogModuleEntity/SysLogEntity/LogGateway` 与评教 `DynamicConfigGateway` 已在 2026-02-06 归位到对应 BC domain，本轮聚焦继续收敛 `eva-domain` 表面积（消息域残留：`MsgEntity/MsgGateway/MessageUseCaseFacade`）。
3) （单 pom，编译闭合前置，保持行为不变）收敛 `bc-messaging-contract/pom.xml`：补齐承接 `MsgEntity` 所需的最小编译期依赖（预期包含 `bc-iam-domain` 与 `cola-component-domain-starter`；仅用于编译闭合）。
