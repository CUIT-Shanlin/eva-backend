...
- 📊 口径快照（避免口径漂移，详见 0.10）：root reactor 仍包含 `eva-infra-dal/eva-infra-shared/eva-base`；`eva-infra-dal` 仍有 `23` 个 Java、`eva-infra-shared` 仍有 `9` 个 Java、`eva-base-common` 仍有 `2` 个 Java；且 `eva-infra-dal/src/main/resources` 已不存在（所有 XML 已按单资源归位到各 BC，后续证据命令需用“目录不存在=0”的口径）。
...
- 📊 **整体未完成清单（更新至 2026-02-15；保持行为不变）**：
  - `eva-infra-dal`：当前仍有 `23` 个 Java + `0` 个 XML 处于共享模块内（可复现口径：`fd -t f -e java . eva-infra-dal/src/main/java | wc -l` + `if [ -d eva-infra-dal/src/main/resources ]; then fd -t f -e xml . eva-infra-dal/src/main/resources | wc -l; else echo 0; fi`；注意：`eva-infra-dal/src/main/resources` 当前已不存在，属于“已清空并删除目录”的正常状态）。
...
- 📅 **下一步排期建议（按“刀”计，保持行为不变；按产能 2–4 刀/天粗估）**：
  ...
  - P1（约 1–2 周）：优先拆散 `eva-infra-dal`（23 个 Java）→ 逐类归位 `Mapper/DO` 到 `bc-course/bc-evaluation/bc-iam`（每刀 1 类；优先处理“XML 已归位”的对应 Mapper：`CourseMapper/SemesterMapper/CourInfMapper/SubjectMapper/CourOneEvaTemplateMapper/FormTemplateMapper/SysUserMapper`）。
...
3) 🎯 本轮主线（承接已归位 XML，逐类归位 `eva-infra-dal`，保持行为不变）：
   - 优先把“XML 已归位到 BC”的对应 `Mapper` 接口逐类搬运到相同 BC 的 `infrastructure` 子模块（保持 `package` 不变；避免 classpath 重复；每次只搬 1 个类）。
   - 次序建议（示例，按引用面/风险从低到高）：`CourseMapper` → `SemesterMapper` → `CourInfMapper` → `SubjectMapper` → `CourOneEvaTemplateMapper` → `FormTemplateMapper` → `SysUserMapper`，再逐步搬运对应 `DO` 与公共工具类（`QueryUtils/EntityFactory/Pagination*`）。
4) 每刀结束必须闭环：
