...
- ✅ 最小回归（保持行为不变）：已执行 `mvnd -pl start -am test -Dtest=edu.cuit.app.eva.EvaRecordServiceImplTest,edu.cuit.app.eva.EvaStatisticsServiceImplTest ...`，`BUILD SUCCESS`（对应代码提交：`31681de9`；本会话该回归后未再修改代码）。
...
- 🎯 **下一刀建议（保持行为不变；每次只改 1 个类闭环）**：
  ...
  2) 用 Serena 证据化确认 `bc-messaging/src/main/java` 无 `eva-domain` 残留类型引用面（消息域 `MsgEntity/MsgGateway/MessageUseCaseFacade` 已归位 `bc-messaging-contract`）。
  3) （单 pom，保持行为不变）收敛 `bc-messaging/pom.xml`：移除对 `eva-domain` 的 Maven 编译期依赖（仅收敛编译边界；最小回归通过后落地）。
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：
  - A（优先：pom，每次只改 1 个 `pom.xml`）：消息域依赖收敛 —— 在 Serena 证伪 `bc-messaging/src/main/java` 无 `eva-domain` 残留引用面后，收敛 `bc-messaging/pom.xml`：移除对 `eva-domain` 的 Maven 编译期依赖（仅收敛编译边界；保持行为不变）。
...
### 推荐版（续接主线：消息域依赖收敛 —— 下一刀：`bc-messaging` 去 `eva-domain` 编译期依赖（保持行为不变））
...
2) Serena 证据化确认 `bc-messaging/src/main/java` 无 `eva-domain` 残留类型引用面（消息域 `MsgEntity/MsgGateway/MessageUseCaseFacade` 已归位 `bc-messaging-contract`）。
3) （单 pom，保持行为不变）收敛 `bc-messaging/pom.xml`：移除对 `eva-domain` 的 Maven 编译期依赖（仅收敛编译边界）。
