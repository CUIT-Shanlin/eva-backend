...
- 🎯 **下一刀建议（保持行为不变；每次只改 1 个类闭环）**：
  1) 先跑快照证据（预期无命中，作为防回归）：
     - `rg -n "<module>eva-domain</module>" pom.xml`
     - `rg -n "<artifactId>eva-domain</artifactId>" --glob "**/pom.xml" .`
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：
  ...
  - C（优先，保持行为不变）：继续做“依赖方 pom 收敛”（每次只改 1 个 `pom.xml`）：优先评估组合根 `start/pom.xml` 是否可以将对 `eva-infra-shared` 的 Maven 依赖从编译期收敛为 `runtime`（前置：Serena 证据化确认 `start/src/main/java` 与 `start/src/test/java` 无直接类型引用；最小回归闭环）。
...
### 推荐版：续接主线：`eva-domain`/`eva-infra` 收尾清理（已完成）；下一刀：收敛 `start/pom.xml` 的编译期依赖边界（保持行为不变）
...
本轮主线（保持行为不变）：
1) 快照证据（预期无命中，保持行为不变）：  
   - `rg -n "<module>eva-domain</module>|<module>eva-infra</module>" pom.xml`
   - `rg -n "<artifactId>eva-domain</artifactId>|<artifactId>eva-infra</artifactId>" --glob "**/pom.xml" .`
2) Serena 证据化确认 `start/src/main/java` 与 `start/src/test/java` 无直接类型引用后，收敛 `start/pom.xml`：将对 `eva-infra-shared` 的 Maven 依赖从编译期收敛为 `runtime`（仅收敛编译边界；不改业务语义/装配/副作用顺序）。
