...
- 📌 下一刀建议（保持行为不变）：优先进入评教相关 Controller（例如 `EvaStatisticsController` 或 `EvaQueryController`），延续“先调用 service → 再 `CommonResult.success(...)`”模板。
...
- 下一会话优先建议（聚焦 **S0.2 延伸**，保持行为不变；每步只改 1 个小点，便于回滚）：
  0) 🎯 **优先：收敛 `eva-adapter` 残留 `*Controller`（每次只改 1 个 Controller；保持行为不变）**：
     ...
     - 推荐顺序（优先链路短、改动面可控；保持行为不变）：`AuthenticationController` → `UserUpdateController`（✅ 已起步）→ `UserQueryController` → `RoleUpdateController/RoleQueryController` → `MenuUpdateController/MenuQueryController` → `SemesterController/DepartmentController/ClassroomController` → 课程 `QueryCourseController`（✅）/`QueryUserCourseController`（✅）/`UpdateCourseController`（✅）/`DeleteCourseController`（✅）→ 评教 `EvaStatisticsController/EvaQueryController/UpdateEvaController/DeleteEvaController` → 其余。
...
下一刀（保持行为不变）：
- `EvaStatisticsController`（评教统计查询入口；延续“Controller 仅做协议适配与参数校验”的收敛模板，保持行为不变）
