...
- 🎯 **下一刀建议（保持行为不变；每次只改 1 个类闭环）**：
  1) 用 Serena 证据化盘点 `EvaTemplateEntity` 引用面（评教模板读侧端口 + 旧 gateway 实现等），确认归属评教域且无跨 BC 复用风险（保持行为不变）。
  2) 将 `EvaTemplateEntity` 从 `eva-domain` 搬运归位到 `bc-evaluation-domain`（保持 `package edu.cuit.domain.entity.eva` 与类内容不变，仅改变 Maven 模块归属；确保全仓库该 FQCN 仅存在一份）。
...
### 推荐版（续接主线：评教域类型逐类归位 —— 下一刀：证伪并归位 `EvaTemplateEntity` → `bc-evaluation-domain`（保持行为不变））
...
2) Serena 证据化盘点 `EvaTemplateEntity` 引用面（重点：评教模板读侧端口/用例、旧 gateway 实现；以及是否存在跨 BC 复用风险）。
3) 将 `EvaTemplateEntity` 从 `eva-domain` 搬运归位到 `bc-evaluation-domain`（每次只改 1 个类；保持 `package`/类内容不变；保持行为不变）。
