...
- ✅ **IAM 并行（按 10.3：补齐用户目录/分页端口（前置），保持行为不变）**：在 `bc-iam-contract` 新增 `UserDirectoryQueryPort`（涵盖 `page/allUser/findAllUsername`；分页返回 `PaginationResultEntity<?>`，避免 contract 暴露旧领域实体触发 Maven 循环依赖风险），用于后续将 `bc-iam/infrastructure` 的 `UserServiceImpl` 去 `UserQueryGateway` 编译期依赖；最小回归通过；落地提交：`6a812cb5`。
...
### 推荐版（续接主线：S0.2 依赖方 pom 收敛（含纠偏，优先 IAM）+ IAM 去 eva-domain 前置）
...
5) 🎯 **IAM 并行（按 10.3，保持行为不变）**：
   - ✅ 已闭环：`bc-messaging` 的 `MessageQueryPortImpl` 已从依赖 `UserQueryGateway` 收敛为依赖 `bc-iam-contract` 最小端口 `UserEntityByIdQueryPort`（端口适配器内部仍委托旧 `UserQueryGateway.findById` 以保持缓存/切面触发点不变；见 0.9）。
   - ✅ 已闭环：`bc-evaluation/infrastructure` 的 `EvaTaskServiceImpl` 已从依赖 `UserQueryGateway` 收敛为依赖 `UserBasicQueryPort`（见 0.9，保持行为不变）。
   - ⏳ 未完成：继续把剩余依赖方对 `UserQueryGateway` 的编译期依赖收敛为 `bc-iam-contract` 最小 Port（优先候选：`bc-evaluation/infrastructure` 的 `EvaStatisticsExporter`；注意 `SpringUtil.getBean(...)` 次数/顺序不变）。
...
- A（优先：pom，每次只改 1 个 `pom.xml`）：选择一个“仅类型引用”的依赖方模块做编译期依赖收敛；移除前先 Serena 证据化并补一条 `rg` 兜底证据，避免误判重演。
- B（次优先：类，每次只改 1 个类）：继续推进 IAM 并行（10.3）：优先处理 `bc-evaluation/infrastructure` 的 `EvaStatisticsExporter`，将其对 `UserQueryGateway` 的编译期依赖收敛为 `bc-iam-contract` 最小 Port（行为不变；注意 `SpringUtil.getBean(...)` 次数/顺序不变）。
- C（备选：类，每次只改 1 个类）：继续推进 IAM 去 `eva-domain` 的前置搬运；优先选择“引用面仅在 IAM”的 `edu.cuit.domain.gateway.user.*` 接口或 `edu.cuit.domain.entity.user.*` POJO（由 Serena 证据化确认）。
