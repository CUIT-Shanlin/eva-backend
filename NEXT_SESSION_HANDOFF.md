...
3) 🎯 本轮主线（承接已归位 XML，逐类归位 `eva-infra-dal`，保持行为不变）：
   - 优先推进：**清零非 IAM 模块对 `SysUserMapper` 的编译期依赖**（每刀 1 类；保持缓存/日志/异常文案/副作用顺序完全不变）。
     - 推荐起手：`bc-evaluation/infrastructure` 内残留 `SysUserMapper` 的类（优先从 `EvaRecordQueryRepository` 开始），改为 `@Qualifier("sysUserMapper") Object` + 反射 `selectById(Serializable)`/`getName()`。
