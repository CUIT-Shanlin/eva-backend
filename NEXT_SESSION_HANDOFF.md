...
- 📊 **整体未完成清单（更新至 2026-02-17；保持行为不变）**：
  - `eva-infra-dal`：当前仍有 `3` 个 Java + `0` 个 XML 处于共享模块内（可复现口径：`fd -t f -e java . eva-infra-dal/src/main/java | wc -l` + `if [ -d eva-infra-dal/src/main/resources ]; then fd -t f -e xml . eva-infra-dal/src/main/resources | wc -l; else echo 0; fi`；注意：`eva-infra-dal/src/main/resources` 当前已不存在，属于“已清空并删除目录”的正常状态）。
...
- 🎯 **SysUserMapper 归位（已闭环，保持行为不变）**：
  ...
  - 下一刀建议（每刀 1 类，保持行为不变）：继续按“引用面仅命中单一 BC 才允许归位”的规则，逐类评估并归位 `eva-infra-dal` 的剩余 Java（优先评教侧 DO：`EvaTaskDO`；课程侧 DO：`CourseTypeDO` 仍需谨慎评估），每刀仍需 Serena 证据化后闭环；若引用面跨 BC，则优先下沉到 `shared-kernel` 作为过渡共享。
...
- 📅 **下一步排期建议（按“刀”计，保持行为不变；按产能 2–4 刀/天粗估）**：
  ...
  - P1（约 2–4 刀）：优先继续拆散 `eva-infra-dal`（当前 `3` 个 Java）→ 逐类归位/下沉 `DO` 与少量公共工具类（每刀 1 类；保持行为不变；`package` 不变）。建议顺序：评教侧 `EvaTaskDO` → 再评估 `CourseTypeDO`（需谨慎）→ `EntityFactory`（若引用面收敛为单 BC 再归位，否则继续留作共享）。
...
1) 证据口径（先跑快照，避免口径漂移）：
   ...
   - `fd -t f -e java . eva-infra-dal/src/main/java | wc -l`（当前预期=3）
...
3) 🎯 本轮主线（承接已归位 XML，继续拆散 `eva-infra-dal`，保持行为不变）：
   ...
   - 下一刀建议（每次只改 1 个类；保持行为不变）：`EvaTaskDO`（Serena 证据化；若引用面跨 BC → 下沉到 `shared-kernel`；若可证伪为单 BC 引用 → 归位对应 BC `infrastructure`；`package` 不变）。
