  ...
  - ✅ **`eva-client` 退场**：已从 reactor 移除并从仓库删除；跨 BC 通用对象已开始沉淀 `shared-kernel`（见 10.5）。
	  - ✅ **已闭环（本阶段目标 B：基础设施旧 gateway 归位，保持行为不变）**：`eva-infra/src/main/java/edu/cuit/infra/gateway/impl` 下残留旧 `*GatewayImpl.java` 已清零（Serena 盘点口径）；归位闭环包含：
	    ...
	    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
		  - ⏳ **仍未完成（核心阻塞项）**：
		    1) `eva-app` 仍保留 **15 个** `*ServiceImpl.java`（口径：`fd -t f 'ServiceImpl\\.java$' eva-app/src/main/java/edu/cuit/app/service/impl | wc -l`），尚未全部退化为“仅切面/登录态解析/委托 UseCase”的壳（仍需要继续把业务编排逐步归位到各 BC）。
			       - 已显式委托 UseCase（共 14 个，口径：`rg -l \"\\bUseCase\\b|application\\.usecase\" eva-app/src/main/java/edu/cuit/app/service/impl`）：``
			       - 仍需继续收敛（共 1 个，按优先级建议由易到难）：`ICourseTypeServiceImpl`
			       - 下一刀建议拆分（每步只改 1 个类；保持行为不变；每步闭环=Serena→最小回归→commit(代码)→三文档同步→commit(文档)→push）：
			         1) ✅（只改 1 个类）在 `bc-course/application/usecase` 新增课程类型用例（建议单类覆盖读写：`pageCourseType/allCourseType/updateCourseType/addCourseType/deleteCourseType/deleteCoursesType/updateCoursesType`），并确保不引入 `eva-infra-shared` 等重依赖（可直接手写 `CourseTypeEntity` → `CourseType` 的字段映射，以及构造 `PaginationQueryResultCO`，逐行对齐旧入口逻辑与返回值）。
			         2) ✅（只改 1 个类）在 `bc-course` 组合根 `BcCourseConfiguration` 补齐该用例的 Bean 装配（保持行为不变）。
			         3) ✅（只改 1 个类）将 `eva-app` 的 `ICourseTypeServiceImpl` 退化为纯委托壳：仅保留接口实现与委托调用；原有“`id==null` → 传 `null` ids”的语义、`updateCoursesType` 返回 `null`（`Void`）等行为必须完全不变。
...
### D 专用简版（当前主线：收敛 eva-app 残留 *ServiceImpl，优先）
...
- ✅ 已完成：`SemesterServiceImpl` → `DepartmentServiceImpl` → `ClassroomServiceImpl`
- 下一刀：`ICourseTypeServiceImpl`

建议拆分（每步只改 1 个类；保持行为不变；每步闭环=Serena→最小回归→commit(代码)→三文档同步→commit(文档)→push）：
1) 在 `bc-course/application/usecase` 新增“课程类型”用例（建议单类覆盖读写；不引入 `eva-infra-shared` 等重依赖；手写 `CourseTypeEntity` → `CourseType` 映射 + `PaginationQueryResultCO` 组装，逐行对齐旧入口逻辑）。
2) 在 `BcCourseConfiguration` 补齐该用例的 Bean 装配（保持行为不变）。
3) 将 `eva-app` 的 `ICourseTypeServiceImpl` 退化为纯委托壳（注意 `id==null` 语义与 `updateCoursesType` 返回 `null` 必须不变）。
