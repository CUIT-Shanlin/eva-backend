...
- ✅ 执行（单类，保持行为不变）：移除 `@Qualifier("evaTaskMapper") Object` 注入与反射直连；改为注入评教直查端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` 获取任务 `taskIds`，保持锁定判定的分支与顺序不变。**注意**：本刀仅去除 `evaTaskMapper`；当前仍保留对评教侧 `courOneEvaTemplateMapper/formRecordMapper` 的运行期反射直连，以及对评教 DO（如 `CourOneEvaTemplateDO/FormRecordDO`）的编译期依赖，下一刀需继续端口化以满足“跨 BC 只依赖 contract/shared-kernel”的更强边界（详见 0.10 的下一步建议）。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`e7ab432f`。
...
- ✅ **本会话新增闭环（更新至 2026-02-25；保持行为不变）**：
  ...
  - `bc-template/infrastructure`：`CourseTemplateLockQueryPortImpl` 已完成去评教 `evaTaskMapper` 运行期依赖（改走评教直查端口 `EvaTaskBriefByCourInfIdsDirectQueryPort`；保持行为不变）。下一刀需继续清零其对评教 `courOneEvaTemplateMapper/formRecordMapper` 的运行期直连与对评教 DO 的编译期依赖（详见 0.10 的下一步建议）。
...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程/模板写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 contract/shared-kernel）。
   - 先复核残留口径（避免口径漂移）：
     ...
     - `rg -n "@Qualifier\\(\\\"courOneEvaTemplateMapper\\\"\\)|@Qualifier\\(\\\"formRecordMapper\\\"\\)" bc-template --glob "**/*.java"`（当前 `CourseTemplateLockQueryPortImpl` 仍会命中，作为下一刀目标）
     - `rg -n "edu\\.cuit\\.infra\\.dal\\.database\\.dataobject\\.eva\\." bc-course bc-template --glob "**/*.java"`（当前仍有命中，优先从本主线相关的调用侧逐类清零）
   - 下一刀优先（template，保持行为不变，单类闭环）：回到 `CourseTemplateLockQueryPortImpl`，以注入 `CourseTemplateLockEvaEvidenceDirectQueryPort` 的方式替换对评教 `courOneEvaTemplateMapper/formRecordMapper` 的运行期直连，并清零对评教 DO 的编译期依赖（每刀仍严格闭环：Serena → 最小回归 → 代码提交 → 三文档同步 → 文档提交 → push）。
   ...
   - 🎯 下一刀建议（template，保持行为不变，单类闭环）：在评教侧 contract 已补齐 `CourseTemplateLockEvaEvidenceDirectQueryPort` 的前提下，继续修改 `CourseTemplateLockQueryPortImpl`，移除对评教 `courOneEvaTemplateMapper/formRecordMapper` 的运行期直连与对评教 DO 的编译期依赖。建议按“先补适配器（infra，单类）→ 再改调用侧（template 单类）”推进，确保锁定判定的分支/查询条件/异常文案/副作用顺序完全不变。
5) 每刀结束必须闭环：
