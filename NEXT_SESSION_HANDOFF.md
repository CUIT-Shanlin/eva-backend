...
- 🔍 **阻塞提醒（更新，保持行为不变）**：当前 `eva-app/src/main/java` 仍直接 `import edu.cuit.bc.evaluation.*` 的类：`EvaTaskServiceImpl`、`UserEvaServiceImpl`、`MsgServiceImpl`，因此仍**暂不可**收敛 `eva-app/pom.xml` 对 `bc-evaluation`（application jar）的编译期依赖；证据口径：`rg -n '^import\\s+edu\\.cuit\\.bc\\.evaluation' eva-app/src/main/java`。
