...
> 下一步建议（读侧优先，单类闭环，保持行为不变）：评教读侧对课程域 `SubjectMapper` 的编译期直连已全部清零（`EvaTaskQueryRepository/EvaRecordQueryRepository` 均已改走 `SubjectObjectDirectQueryPort`）。

> 状态更新（2026-02-20，保持行为不变）：评教写侧 `repository` 与评教读侧 `query` 已清零对课程域 `CourseMapper/SemesterMapper/SubjectMapper` 的编译期直连；评教侧 `EvaUpdateGatewayImpl` 已清零对 `CourseMapper.selectById(...).getSemesterId()` 的编译期直连；当前评教读侧对课程域 Mapper 的直连点待继续盘点（若出现其它 Mapper，则按“补端口 → 补适配器 → 改调用侧”逐刀收敛）。
...
1) 读侧优先（保持行为不变）：评教读侧 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/bcevaluation/query` 已清零对课程域 `CourseMapper/SemesterMapper` 的编译期直连；评教侧 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/gateway/impl/eva/EvaUpdateGatewayImpl.java` 也已清零对 `CourseMapper.selectById(...).getSemesterId()` 的编译期直连（详见 0.9/4.2）。
   - 下一刀建议（保持行为不变；每次只改 1 个类闭环）：评教读侧对课程域 `SubjectMapper` 的编译期直连已清零（`EvaTaskQueryRepository/EvaRecordQueryRepository` 均已改走 `SubjectObjectDirectQueryPort`）。下一步建议继续盘点评教读侧对课程域其它 Mapper 的编译期直连点，若发现则按“补端口（单类）→ 补适配器（单类）→ 改调用侧（单类）”推进。
   - 进展提示（保持行为不变）：`CourseAndSemesterObjectDirectQueryPort`（含适配器）已就位并被评教读侧/评教侧使用；`SubjectObjectDirectQueryPort`（含适配器）已就位；端口前置 `CourseIdsByTeacherIdAndSemesterIdQueryPort` 仍无实现/无调用点，可按需后续补齐（用于收敛“按 teacherId + semesterId 查询课程ID列表”场景）。
