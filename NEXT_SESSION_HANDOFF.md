...
### 推荐版（续接主线：评教域 gateway 逐类归位 —— 下一刀：证伪并归位 `EvaDeleteGateway` → `bc-evaluation-domain`（保持行为不变））
...
3) Serena 证据化盘点 `EvaDeleteGateway` 引用面后，将其从 `eva-domain` 搬运归位到 `bc-evaluation-domain`（每次只改 1 个类；保持 `package`/签名/注解不变；保持行为不变）。
