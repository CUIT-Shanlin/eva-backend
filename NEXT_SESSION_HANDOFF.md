...
3) ✅ 阶段性完成（保持行为不变）：`eva-infra-shared` 退场收尾已闭环（逐 pom 去依赖 → root reactor 退场 → 删除 `eva-infra-shared/pom.xml`；最小回归通过）。
   - 可选下一刀（目录收口，保持行为不变）：若 `eva-infra-shared/` 已为空目录，可按“单目录操作一刀”执行 `rmdir eva-infra-shared`（仅目录清理；仍需最小回归 + 提交 + 三文档同步 + push）。
4) 每刀结束必须闭环：
