...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”。当前删除链路已阶段性闭环，建议转向 **取消链路**（课程改课/自助改课导致取消评教任务）：
   ...
   - 下一刀建议（写侧优先，保持行为不变，单类闭环）：回到课程侧选择 1 个调用点（推荐 `UpdateSingleCourseRepositoryImpl`）改为调用 `EvaTaskCancelByTeacherIdAndCourInfIdPort.cancelByTeacherIdAndCourInfId(...)` 并移除 `evaTaskMapper` 的反射 update 调用（注意：`UpdateSingleCourseRepositoryImpl` 现有逻辑使用 `cour_inf_id = courINfo.getCourseId()` 的历史写法，若看起来像 bug 也必须保持不变，只做“调用方式/依赖方向”重构）。
5) 每刀结束必须闭环：
