...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程/模板写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 contract/shared-kernel）。
   ...
   - 下一刀优先（evaluation，保持行为不变，单类闭环）：为 `CourseTemplateLockEvaEvidenceDirectQueryPort` 补端口适配器（infra）以承接旧 `courOneEvaTemplateMapper/formRecordMapper.selectCount(...)` 的查询语义；随后回到 `CourseTemplateLockQueryPortImpl` 替换调用点并清零对评教 DO 的编译期依赖（每刀仍严格闭环：Serena → 最小回归 → 代码提交 → 三文档同步 → 文档提交 → push）。
   - ✅ 前置已完成（保持行为不变）：评教侧已补齐并落地以下端口（contract + infrastructure）：
     ...
     - `CourseTemplateLockEvaEvidenceDirectQueryPort`（读侧：快照/记录存在性证据直查，用于承接模板侧锁定判定剩余直连）
