...
- **A（优先：pom）S0.2 延伸（依赖方 `pom.xml` 依赖收敛，每次只改 1 个 pom；保持行为不变）**：优先从“仅类型引用、无实现/副作用耦合”的依赖方模块开始（需先用 Serena + `rg` 证伪引用面），例如优先评估 `bc-ai-report/infrastructure/pom.xml` 是否可将对 `bc-evaluation`（application jar）的依赖收敛为仅依赖 `bc-evaluation-contract`（保持行为不变）。
