...
- 本次会话最新闭环（2026-01-02，便于续接）：  
  ...
  4) 下一步（保持行为不变；每次只迁 1 个入口方法簇）：优先 `ICourseDetailServiceImpl.delete`（保持 `@CheckSemId` 触发点不变；保持 `deleteCourse(...) → userId 查询 → publishAfterCommit(...)` 的既有顺序不变；异常文案/异常类型不变）；其后按清单逐簇推进（见 `docs/DDD_REFACTOR_BACKLOG.md` 4.3 的写侧入口清单）。
...
1) ✅ **bc-course（课程）写侧入口用例归位继续（方向 A → B）**：`ICourseServiceImpl.updateSingleCourse/addNotExistCoursesDetails/addExistCoursesDetails` 与 `IUserCourseServiceImpl.deleteSelfCourse/updateSelfCourse/importCourse` 已闭环，且 `ICourseDetailServiceImpl.updateCourse/updateCourses` 已完成入口用例归位/调用点端口化（见 0.9）。下一步优先做 `ICourseDetailServiceImpl.delete`（保持 `@CheckSemId` 触发点不变；保持 `deleteCourse(...) → userId 查询 → publishAfterCommit(...)` 的既有顺序不变；异常文案/异常类型不变）。建议步骤：Serena 定位入口与 Controller 调用点 → 用例/端口归位（按“入口用例 + 端口 + 端口适配器 + 委托壳”逐层收敛；保持行为不变）→ 最小回归 → commit → 三文档同步。
