...
1) 🎯 **单 pom**：从根 `pom.xml` 的 reactor 中移除 `<module>eva-adapter</module>`（前置：Serena 证伪无代码级引用点/装配漂移风险；最小回归闭环）。
...
- `rg -n "<module>eva-adapter</module>" pom.xml`（当前应命中；用于复核“reactor 仍包含 eva-adapter（下一刀待移除）”）
