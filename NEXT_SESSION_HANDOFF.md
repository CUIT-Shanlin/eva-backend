...
2) Serena（必须）：继续推进 `eva-infra-dal` 按 BC 拆散时，优先选择“仅单 BC 引用”的候选文件（Mapper/DO/XML）。推荐从 `bc-course` 入手：`course_type` 候选已完成第一刀——`CourseTypeMapper` 已归位到 `bc-course/infrastructure`（落地：`241b75de`；保持 `package` 不变）。下一刀建议优先归位 `CourseTypeMapper.xml`（保持 `namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变）。注意：`CourseTypeDO` 当前引用面包含 `eva-infra-shared/src/main/java/edu/cuit/infra/convertor/course/CourseConvertor.java`，不满足“仅 bc-course 引用”约束，暂不建议直接归位 DO（避免依赖/装配边界漂移）。
