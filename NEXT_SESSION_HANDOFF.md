...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”。推荐先走 **删除链路**（改动面相对可控；当前已完成刀 1，下一刀从刀 2 开始）：
   ...
   - 刀 2（contract，单类）：在 `bc-evaluation/contract` 新增最小端口（例如 `EvaTaskBriefByCourInfIdsDirectQueryPort`），返回上述 CO 列表（约束：不引入缓存/切面副作用；语义保持与旧 SQL 条件/顺序一致）。
