...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：
  ...
  - 📌 **当前状态快照（截至 2026-01-28，口径=可复现命令；保持行为不变）**：
		  ...
		  - ✅ **已闭环（本阶段目标 B：基础设施旧 gateway 归位，保持行为不变）**：`eva-infra/src/main/java/edu/cuit/infra/gateway/impl` 下残留旧 `*GatewayImpl.java` 已清零（Serena 盘点口径）；归位闭环包含：
		    ...
		    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
			  - ⏳ **仍未完成（核心阻塞项）**：
			    ...
			    2.2) ⏳ **`eva-*` 技术切片仍未具备“全量整合进各 BC”的前置条件**：root reactor 仍包含 `eva-domain` / `eva-infra-dal` / `eva-infra-shared` / `eva-infra` / `eva-base`（可复现口径：`rg -n "<module>eva-" pom.xml`）；且组合根 `start` 仍 `runtime` 依赖 `eva-infra`（可复现口径：`rg -n "<artifactId>eva-infra</artifactId>" start/pom.xml`）。保持行为不变前提下，当前不能直接移除这些模块。
			    2.3) ⏳ **`eva-domain` 仍被多个 BC 编译期依赖（因此暂不能移除/整合）**：当前至少包含 `bc-course/application`、`bc-evaluation/application`、`bc-iam/application`、`bc-audit/application`、`bc-messaging`、`bc-ai-report/application` 等仍显式依赖 `eva-domain`（可复现口径：`rg -n "<artifactId>eva-domain</artifactId>" --glob "**/pom.xml" .`）。下一阶段的“整合 eva-*”需要先把 `edu.cuit.domain.*` 的类型/接口按 BC 归位（每次只改 1 个类或 1 个 pom；保持行为不变）。
