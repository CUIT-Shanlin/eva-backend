...
1) 从 `docs/DDD_REFACTOR_BACKLOG.md` 的 4.3 里选 1 个“仍未收敛”的目标（写侧优先、影响面清晰、可做到单类/单 XML/单 pom），用 Serena 做入口定位 + 引用面证伪后开刀。
   - 推荐优先（写侧，保持行为不变）：继续清理 `bc-evaluation/infrastructure` 写侧端口适配器对课程域 `CourseMapper` 的编译期直连（`PostEvaTaskRepositoryImpl` 已完成清零，下一刀建议从 `SubmitEvaluationRepositoryImpl` 开始）。
