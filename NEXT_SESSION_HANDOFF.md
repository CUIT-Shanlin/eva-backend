...
下一刀（保持行为不变）：
- **S1（入口归位，优先；每次只迁 1 个 Controller；保持 `package/注解/URL/权限/参数校验/日志与副作用顺序` 完全不变）**：下一刀优先将 `ClassroomController` 从 `eva-adapter` 归位到 `bc-course/infrastructure`。若编译闭合缺依赖，则先单独提交 1 次 `bc-course/infrastructure/pom.xml` 的编译闭合，再做 `git mv`（避免“改类 + 改 pom”混在同一提交）。
