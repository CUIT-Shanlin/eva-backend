...
- 🧾 本会话新增闭环（代码，按时间顺序；保持行为不变）：`0a691bb1`（QueryCourse）→ `d49976cf`（QueryUserCourse）→ `58b1b763`（UpdateCourse）→ `20e1214c`（DeleteCourse）→ `7c465c5d`（EvaStatistics）→ `74a34133`（EvaQuery）→ `87fc0475`（UpdateEva）→ `7809cc2d`（DeleteEva）→ `d055c271`（EvaConfigQuery）→ `c7ab9633`（EvaConfigUpdate）→ `aa99c775`（Message）。
...
- 📌 下一刀建议（保持行为不变）：`LogController`（审计/日志入口；优先固化返回包装顺序表达，保持行为不变）。
  ...
  - ✅ **`eva-client` 退场**：已从 reactor 移除并从仓库删除；跨 BC 通用对象已开始沉淀 `shared-kernel`（见 10.5）。
	  - ✅ **已闭环（本阶段目标 B：基础设施旧 gateway 归位，保持行为不变）**：`eva-infra/src/main/java/edu/cuit/infra/gateway/impl` 下残留旧 `*GatewayImpl.java` 已清零（Serena 盘点口径）；归位闭环包含：
	    ...
	    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
		  - ⏳ **仍未完成（核心阻塞项）**：
		    1) `eva-app` 仍保留 **15 个** `*ServiceImpl.java`（口径：`fd -t f 'ServiceImpl\\.java$' eva-app/src/main/java/edu/cuit/app/service/impl | wc -l`），现已全部退化为“仅切面/登录态解析/委托 UseCase”的壳（保持行为不变）。
			       - 已显式委托 UseCase（共 15 个，口径：`rg -l \"\\bUseCase\\b|application\\.usecase\" eva-app/src/main/java/edu/cuit/app/service/impl`）：``
		    ...
		    2) `eva-adapter` 仍保留 **22 个** `*Controller.java`（口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`），Controller 仍需逐步收敛为“纯 HTTP 协议适配”（避免直接依赖基础设施实现细节；保持行为不变）。
		       - 补充进展（已闭环，保持行为不变）：已完成课程相关 Controller “注入类型从 `*ServiceImpl` 收窄为接口”的试点（落地：`47a6b06c`）。本次复盘显示：当前 `eva-adapter` 目录下 Controller 已不再直接 `import edu.cuit.app.service.impl.*`（口径：`rg -l \"service\\.impl\\.\" eva-adapter/src/main/java` 返回空）。
		       - 补充进展（进行中，保持行为不变）：已开始按“每次只改 1 个 Controller”推进 Controller 收敛；首刀选择 `UserUpdateController`（含 `StpUtil` 登录态读取）先固化风险更高的调用顺序约束：提取 `currentUserId()`，保持 `StpUtil.getLoginId()` → `userService.getIdByUsername(...)` 调用顺序与次数不变（落地：`09cb6454`；见 0.9）。补充闭环：`AuthenticationController`（登录入口）已完成 `login` 表达式收敛，用“局部变量 + return”显式固化 `userAuthService.login(...)` → `CommonResult.success(...)` 的顺序（落地：`102a0cbb`；见 0.9）。补充闭环：`UserQueryController` 已完成“先调用 service → 再 `CommonResult.success(...)`”的返回表达式收敛（落地：`18e0bb29`；见 0.9）。补充闭环：`RoleQueryController` 已完成“先调用 service → 再 `CommonResult.success(...)`”的返回表达式收敛（落地：`fa04e48e`；见 0.9）。补充闭环：`MenuQueryController` 已完成“先调用 service → 再 `CommonResult.success(...)`”的返回表达式收敛（落地：`8521f72c`；见 0.9）。补充闭环：`MenuUpdateController` 已完成 `create` 内日志内容构造表达式收敛（保持 `menuService.create(...)` → `LogUtils.logContent(...)` → `CommonResult.success()` 顺序不变；落地：`9a2fc3ff`；见 0.9）。补充闭环：`RoleUpdateController` 已完成 `create` 内日志内容构造表达式收敛（保持 `roleService.create(...)` → `LogUtils.logContent(...)` → `CommonResult.success()` 顺序不变；落地：`9f07698a`；见 0.9）。补充闭环：`SemesterController` 已完成“先调用 service → 再 `CommonResult.success(...)`”的返回表达式收敛（落地：`24e5ad3f`；见 0.9）。补充闭环：`DepartmentController` 已完成“先调用 service → 再 `CommonResult.success(...)`”的返回表达式收敛（落地：`e57c0e41`；见 0.9）。补充闭环：`ClassroomController` 已完成“先调用 service → 再 `CommonResult.success(...)`”的返回表达式收敛（落地：`f1606018`；见 0.9）。下一刀建议：进入课程相关 Controller（如 `UpdateCourseController` / `DeleteCourseController`），每次只改 1 个并保持行为不变。
...
### E 专用简版（当前主线：收敛 eva-adapter 残留 *Controller，优先）
...
- 每次只改 1 个 Controller（或只改 1 个 pom）
...
下一刀（保持行为不变）：
- `LogController`（审计/日志入口；延续“Controller 仅做协议适配与参数校验”的收敛模板，保持行为不变）
