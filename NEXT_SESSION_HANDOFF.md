...
1) 证据口径（先跑快照，避免口径漂移）：
   ...
   - `fd -t f -e java . eva-infra-dal/src/main/java | wc -l`（当前预期=1）
...
3) 🎯 本轮主线（承接已归位 XML，继续拆散 `eva-infra-dal`，保持行为不变）：
   ...
   - ✅ Serena 证据化结论（更新至 2026-02-17；保持行为不变）：`EntityFactory` 引用面跨 `bc-iam/**`、`bc-evaluation/**`、`bc-audit/**`、`bc-messaging/**`，且仍被 `eva-infra-shared` 多个 Convertor/BizConvertor 使用；因此不满足“单 BC/单模块才允许归位到对应 infrastructure”的条件。
   - 🎯 下一刀建议（每次只改 1 个文件；保持行为不变）：优先将 `EntityFactory` 下沉到 `shared-kernel`（保持 `package edu.cuit.infra.convertor` 与类内容不变，仅改变 Maven 模块归属），以便后续彻底退场 `eva-infra-dal`。
     - 若 `shared-kernel/pom.xml` 缺少编译依赖（例如 `spring-context`、`hutool-all`、`mapstruct`、`cola-component-exception`），先做“单 pom 前置”闭环；再做“单类搬运”闭环（不要同一刀做两件事）。
4) 每刀结束必须闭环：
