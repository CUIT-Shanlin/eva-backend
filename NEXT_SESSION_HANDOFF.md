...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：开始推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”。推荐先走 **删除链路**（改动面相对可控）：
   - 刀 1（contract，单类）：在 `bc-evaluation/contract` 新增“按 courInfIds 读取评教任务最小视图”的 CO（例如 `EvaTaskBriefCO`：仅含 `id/teacherId/status/courInfId` 等必要字段；不引入 Mapper/DO）。
