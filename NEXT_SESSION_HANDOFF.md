...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：继续推进 **S0.2 延伸（依赖方编译期依赖收敛）**：选择下一条“依赖方编译期依赖收敛”目标（优先从 `eva-domain` / `eva-infra-shared` 等依赖方开始），用 Serena 证据化盘点引用面与依赖闭包后再收敛 `pom.xml`（每次只改 1 个 pom；保持行为不变）。
  - 📌 **当前仍未完成（截至 2026-01-16，口径=可复现命令；保持行为不变）**：
    ...
    - `eva-app` 仍作为“过渡装配/支撑类”模块存在：当前源码侧仅剩 **12 个** Java 文件（口径：`fd -t f -e java . eva-app/src/main/java | wc -l`；包括 Sa-Token 配置、切面配置、若干 Convertor/Exec 等）；组合根 `start` 仍对 `eva-app` 有编译期依赖（见 `start/pom.xml`）。
...
- 快速证据口径（新会话先复核，避免口径漂移）：
  ...
  - bc-messaging-contract：`rg -n \"<artifactId>bc-messaging-contract</artifactId>\" eva-app/pom.xml`（当前应为 0；用于复核“eva-app 已去消息契约依赖”）。
  - bc-messaging-contract：`rg -n '^import\\s+edu\\.cuit\\.client\\.api\\.IMsgService' eva-app/src/main/java`（当前应为 0；用于复核“未引用消息契约关键类型”）。
...
3) 🎯 当前主线：推进 **S1（eva-* 退场/整合）前置**（保持行为不变）：优先把“过渡模块”`eva-app` 的残留支撑类逐步归位（每次只搬 1 个类/或只改 1 个 `pom.xml`），并让组合根 `start` 的装配责任更清晰，从而为后续“移除 `start` 对 `eva-app/eva-adapter` 的依赖”创造条件。补充：**S0.2 延伸（依赖方编译期依赖收敛）** 仍作为并行主线：优先从 `eva-domain` / `eva-infra-shared` 等依赖方挑选 1 个 `pom.xml` 做证据化盘点后收敛（保持行为不变）。

下一刀（保持行为不变）：
- 下一刀（每次只改 1 个 pom.xml 或 1 个类；保持行为不变）：
  1) Serena 证据化盘点：对 `eva-app/src/main/java` 残留支撑类（当前口径为 12 个 Java 文件）做“引用面 + 依赖闭包”盘点，优先挑选 1 个最独立、最不易引起装配漂移的类先归位（建议优先从 Sa-Token 相关配置类开始：`SaTokenConfig` / `SaTokenInterceptorConfig`）。
  2) 只搬 1 个类（允许跨模块搬运，保持 `package` 与行为不变；必要时仅做编译闭合的最小依赖补齐）→ 最小回归闭环。
...
- `rg -n \"<artifactId>bc-messaging-contract</artifactId>\" eva-app/pom.xml`（当前应为 0；用于复核“eva-app 已去消息契约依赖”）
- `rg -n '^import\\s+edu\\.cuit\\.client\\.api\\.IMsgService' eva-app/src/main/java`（当前应为 0；用于复核“未引用消息契约关键类型”）
