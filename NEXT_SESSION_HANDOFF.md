...
- 📌 **当前仍未完成（进入 S1 前置的硬事实口径，保持行为不变）**：
  - `eva-app/src/main/java` 已清零（口径：`fd -t f -e java . eva-app/src/main/java | wc -l` 当前为 0），但组合根 `start` 仍对 `eva-app` 有编译期依赖（见 `start/pom.xml`），因此 `eva-app` 模块暂不可直接从 reactor 移除。
...
- 🎯 **下一步建议（保持行为不变；每步只改 1 个类或 1 个 pom）**：
  1) 下一刀（优先，单 pom）：评估并尝试收敛 `start/pom.xml` 对 `eva-app` 的编译期依赖（目标：移除或至少降为 `runtime`），使组合根装配责任更清晰，并为后续 `eva-app` 模块退场创造条件（必须先 Serena 证据化：确认 `start` 运行期 classpath 已显式兜底所有被搬走类的装配；保持行为不变）。
  2) 并行主线（依赖方 pom 收敛）：继续挑选 1 个依赖方模块（优先 `eva-domain` / `eva-infra-shared`）做“Serena 证据化盘点 → 单 pom 收敛”（每次只改 1 个 `pom.xml`）。
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：继续推进 **S0.2 延伸（依赖方编译期依赖收敛）**：选择下一条“依赖方编译期依赖收敛”目标（优先从 `eva-domain` / `eva-infra-shared` 等依赖方开始），用 Serena 证据化盘点引用面与依赖闭包后再收敛 `pom.xml`（每次只改 1 个 pom；保持行为不变）。
  - 📌 **当前仍未完成（截至 2026-01-17，口径=可复现命令；保持行为不变）**：
    ...
    - `eva-app` 仍作为“过渡装配/支撑类”模块存在：当前源码侧已清零（口径：`fd -t f -e java . eva-app/src/main/java | wc -l` 当前为 0）；组合根 `start` 仍对 `eva-app` 有编译期依赖（见 `start/pom.xml`），因此下一步应优先评估 `start/pom.xml` 的依赖收敛（保持行为不变）。
  ...
  - ✅ **`eva-client` 退场**：已从 reactor 移除并从仓库删除；跨 BC 通用对象已开始沉淀 `shared-kernel`（见 10.5）。
		  - ✅ **已闭环（本阶段目标 B：基础设施旧 gateway 归位，保持行为不变）**：`eva-infra/src/main/java/edu/cuit/infra/gateway/impl` 下残留旧 `*GatewayImpl.java` 已清零（Serena 盘点口径）；归位闭环包含：
		    ...
		    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
			  - ⏳ **仍未完成（核心阻塞项）**：
				    1) `eva-app` 已无 `*ServiceImpl.java`（口径：`fd -t f 'ServiceImpl\\.java$' eva-app/src/main/java | wc -l` → 0），但仍残留 **2 个**支撑类（口径：`fd -t f -e java . eva-app/src/main/java | wc -l`；示例：`*Exec`）。下一步建议进入 **S1 前置**：继续逐个归位这些支撑类（每步只搬 1 个类；保持 `package` 与行为不变）。
...
- 下一刀（每次只改 1 个 pom.xml 或 1 个类；保持行为不变）：
  1) Serena 证据化盘点：确认 `eva-app/src/main/java` 已清零（口径：`fd -t f -e java . eva-app/src/main/java | wc -l` 当前为 0），并盘点 `start/pom.xml` 中对 `eva-app` 的依赖用途（是否仅为“历史过渡 classpath”）。
  2) 单 pom 收敛：在证据化确认“运行期装配已由组合根显式兜底”的前提下，尝试将 `start/pom.xml` 对 `eva-app` 的依赖移除或降为 `runtime`（保持行为不变）→ 最小回归闭环。
