...
**2026-01-13（补充：课程域去编译期耦合推进——学期旧入口归位，保持行为不变）**
...
- ✅ **课程（依赖收敛前置：旧入口归位，学期，保持行为不变）**：将 `SemesterServiceImpl` 从 `eva-app` 搬运归位到 `bc-course-infra`（保持 `package edu.cuit.app.service.impl` 不变；仍实现 `ISemesterService` 并委托 `SemesterQueryUseCase`；保留 `@Transactional`；仅改变类所在 Maven 模块以减少 `eva-app` → `bc-course` 的编译期耦合面；最小回归通过）；落地提交：`8eddc643`。
- ✅ **S0.2 延伸（次优先：其它依赖收敛点盘点，证伪结论，保持行为不变）**：当前阻塞主要剩余课程域：Serena + `rg` 证据化确认 `eva-app` 仍直接 `import edu.cuit.bc.course.application.usecase.*`（`ClassroomServiceImpl/ICourseTypeServiceImpl`），因此暂不可将 `eva-app/pom.xml` 的 `bc-course` 编译期依赖移除/替换为更轻的协议层依赖；建议优先把剩余 2 个旧入口类按“保持包名不变”的方式搬运归位到 `bc-course-infra`，再回到 `pom.xml` 收敛（保持行为不变）。
  ...
  - ✅ **`eva-client` 退场**：已从 reactor 移除并从仓库删除；跨 BC 通用对象已开始沉淀 `shared-kernel`（见 10.5）。
	  - ✅ **已闭环（本阶段目标 B：基础设施旧 gateway 归位，保持行为不变）**：`eva-infra/src/main/java/edu/cuit/infra/gateway/impl` 下残留旧 `*GatewayImpl.java` 已清零（Serena 盘点口径）；归位闭环包含：
	    ...
	    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
		  - ⏳ **仍未完成（核心阻塞项）**：
		    1) `eva-app` 仍保留 **14 个** `*ServiceImpl.java`（口径：`fd -t f 'ServiceImpl\\.java$' eva-app/src/main/java/edu/cuit/app/service/impl | wc -l`），现已全部退化为“仅切面/登录态解析/委托 UseCase/网关”的壳（保持行为不变）。
			       - 已显式委托 UseCase（共 13 个，口径：`rg -l \"\\bUseCase\\b|application\\.usecase\" eva-app/src/main/java/edu/cuit/app/service/impl`）：`EvaRecordServiceImpl/EvaStatisticsServiceImpl/EvaTaskServiceImpl/EvaTemplateServiceImpl/UserEvaServiceImpl/RoleServiceImpl/MenuServiceImpl/LogServiceImpl/UserServiceImpl/UserAuthServiceImpl/DepartmentServiceImpl/ClassroomServiceImpl/ICourseTypeServiceImpl`
...
本轮目标（F，优先；保持行为不变）：
1) 🎯 **课程（优先：S0.2 延伸——继续削减 `eva-app` 对 `bc-course` 的编译期耦合）**：通过“只搬运 1 个旧入口类（保持包名不变）”逐步清空 `eva-app` 对 `edu.cuit.bc.course.application.usecase.*` 的直接引用，最终让 `eva-app/pom.xml` 可移除 `bc-course` 编译期依赖（替换为更轻的 `shared-kernel`；保持行为不变）
2) ⏳ 次优先：继续推进 **S0.2 延伸（依赖方 `pom.xml` 依赖收敛）**：逐步让组合根 `start` 显式承接装配责任，并减少依赖方对业务实现模块的编译期耦合（每次只改 1 个 `pom.xml`；保持行为不变；需先 Serena + `rg` 证据化）

下一刀（保持行为不变）：
- **只改 1 个类（优先）**：将 `eva-app` 的 `ClassroomServiceImpl` 搬运归位到 `bc-course-infra`（保持 `package edu.cuit.app.service.impl` 不变；仍实现原接口并委托 `ClassroomQueryUseCase`；不改缓存/日志/异常文案/副作用顺序；最小回归通过后提交）。
- **下一刀（只改 1 个类）**：将 `eva-app` 的 `ICourseTypeServiceImpl` 搬运归位到 `bc-course-infra`（保持 `package edu.cuit.app.service.impl.course` 不变；行为不变）。
- **在上述两类完成后（只改 1 个 pom）**：Serena + `rg` 证伪 `eva-app` 不再 `import edu.cuit.bc.course.*` 后，再收敛 `eva-app/pom.xml`：移除对 `bc-course` 的编译期依赖（改为显式依赖 `shared-kernel`；保持行为不变）。
...
下一刀（保持行为不变）：
- **S0.2 延伸（依赖方 `pom.xml` 依赖收敛，每次只改 1 个 pom；保持行为不变）**：优先从“仍显式依赖 `bc-course` 但仅使用 `edu.cuit.client.*` 类型”的依赖方模块开始（需先用 Serena + `rg` 证伪引用面）。
