...
- ✅ **S0.2 延伸（次优先：其它依赖收敛点盘点，证伪结论，保持行为不变）**：当前阻塞主要剩余课程域：Serena + `rg` 证据化确认 `eva-app` 仍直接 `import edu.cuit.bc.course.application.usecase.*`（`SemesterServiceImpl/ClassroomServiceImpl/ICourseTypeServiceImpl`），因此暂不可将 `eva-app/pom.xml` 的 `bc-course` 编译期依赖移除/替换为更轻的协议层依赖；建议优先把这 3 个旧入口类按“保持包名不变”的方式搬运归位到 `bc-course-infra`，再回到 `pom.xml` 收敛（保持行为不变）。消息域方面，已具备把 `eva-app/pom.xml` 中 `bc-messaging` 的编译期依赖收敛为仅保留 `bc-messaging-contract` 的前置条件（每次只改 1 个 `pom.xml`；保持行为不变）。
