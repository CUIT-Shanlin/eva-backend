...
**2026-01-12（本次会话：LogServiceImpl 写侧收敛 + 文档同步）**
	    ...
	    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
		  - ⏳ **仍未完成（核心阻塞项）**：
		    1) `eva-app` 仍保留 **15 个** `*ServiceImpl.java`（口径：`fd -t f 'ServiceImpl\\.java$' eva-app/src/main/java/edu/cuit/app/service/impl | wc -l`），尚未全部退化为“仅切面/登录态解析/委托 UseCase”的壳（仍需要继续把业务编排逐步归位到各 BC）。
		       - 已显式委托 UseCase（共 8 个，口径：`rg -l \"\\bUseCase\\b|application\\.usecase\" eva-app/src/main/java/edu/cuit/app/service/impl`）：`EvaRecordServiceImpl/EvaStatisticsServiceImpl/EvaTaskServiceImpl/EvaTemplateServiceImpl/UserEvaServiceImpl/RoleServiceImpl/MenuServiceImpl/LogServiceImpl`
		       - 仍需继续收敛（共 7 个，按优先级建议由易到难）：`UserServiceImpl` → `UserAuthServiceImpl` → `MsgServiceImpl` → `SemesterServiceImpl` → `DepartmentServiceImpl` → `ClassroomServiceImpl` → `ICourseTypeServiceImpl`
