...
- 🎯 **SysUserMapper 归位（进行中，保持行为不变）**：
  ...
  - 下一刀建议（单类搬运，保持行为不变）：将 `eva-infra-dal/.../SysUserMapper.java` 归位到 `bc-iam/infrastructure`（保持 `package` 不变，避免 classpath 重复）。
...
3) 🎯 本轮主线（承接已归位 XML，逐类归位 `eva-infra-dal`，保持行为不变）：
   ...
   - 下一刀建议（单类搬运，保持行为不变）：`SysUserMapper.java` 从 `eva-infra-dal` 归位到 `bc-iam/infrastructure`（保持 `package` 不变，避免 classpath 重复）。
