...
### 推荐版（续接主线：bc-course S0.2 延伸 —— 下一刀：证伪并归位 `SingleCourseEntity`（保持行为不变））
...
本轮主线（保持行为不变）：
1) 先跑快照证据：  
   - `rg -n "\\bSingleCourseEntity\\b" bc-* --glob "*.java" | head`
2) Serena 证伪 `edu.cuit.domain.entity.course.SingleCourseEntity` 的引用面与签名引用面，确认其属于“课程域可归位类型”。
3) 若成立：将 `SingleCourseEntity` 从 `eva-domain` 搬运归位到 `bc-course-domain`（保持 `package` 与类内容不变，仅改变 Maven 模块归属；确保全仓库该 FQCN 仅存在一份）。
...
风险提示（保持行为不变）：
- `CourseEntity` 当前编译期依赖 `UserEntity`（已在 `bc-iam-domain`），因此 `bc-course-domain` 已补齐对 `bc-iam-domain` 的编译期依赖作为前置（保持行为不变；见 0.9）。
- `CourseQueryGateway` 暂缓：其签名编译期依赖 `CourseEntity/SingleCourseEntity`；在两者归位到 `bc-course-domain` 前，直接搬运可能引入 Maven 循环依赖风险。建议顺序：先归位 `CourseEntity` → `SingleCourseEntity` → 再归位 `CourseQueryGateway`（每步只改 1 个类闭环）。
...
- `rg -n \"\\bSingleCourseEntity\\b\" bc-* --glob \"*.java\" | head`（下一刀证据口径：确认课程域类型引用面分布，为 `eva-domain` → `bc-course-domain` 归位做前置）
