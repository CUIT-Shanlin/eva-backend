...
- 🎯 **SysUserMapper 归位（进行中，保持行为不变）**：
  ...
  - ✅ 已完成（保持行为不变）：`CourseConvertor` 去 `SysUserMapper` 无用 import（`1e0af235`）；`EvaUpdateGatewayImpl` 去 `SysUserMapper` 编译期依赖（`364c63a0`）；`DeleteEvaRecordRepositoryImpl` 去 `SysUserMapper` 编译期依赖（`c0f94380`）。
  - 下一刀建议（每刀 1 类，保持行为不变）：优先在 `bc-evaluation/infrastructure` 逐个清理 `SysUserMapper` 编译期依赖（改为 `@Qualifier("sysUserMapper") Object` + 反射调用 `selectById(Serializable)`/`getName()`），推荐顺序：
    1) `PostEvaTaskRepositoryImpl`
    2) `SubmitEvaluationRepositoryImpl`
    3) `EvaTaskQueryRepository`
    4) `EvaStatisticsQueryRepository`
    5) `EvaRecordQueryRepository`
...
3) 🎯 本轮主线（承接已归位 XML，逐类归位 `eva-infra-dal`，保持行为不变）：
   - 优先推进：**清零非 IAM 模块对 `SysUserMapper` 的编译期依赖**（每刀 1 类；保持缓存/日志/异常文案/副作用顺序完全不变）。
     - 推荐起手：`bc-evaluation/infrastructure` 内残留 `SysUserMapper` 的类（优先从 `PostEvaTaskRepositoryImpl` 开始），改为 `@Qualifier("sysUserMapper") Object` + 反射 `selectById(Serializable)`/`getName()`。
