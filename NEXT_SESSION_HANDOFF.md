...
- ✅ 本会话提交链（按发生顺序，便于回溯/回滚；保持行为不变）：`c38e30f0`（代码：bc-course/application 去 lombok）→ `d28f0cff`（三文档同步）→ `ec6b200e`（docs：纠偏 eva-* 整合现状与路线）→ `c901e3a6`（代码：SysLogModuleMapper 归位）→ `7534337b`（三文档同步）→ `5de32a6c`（代码：SysLogMapper 归位）→ `221a4050`（三文档同步）→ `02c85909`（代码：LogConverter 归位）→ `84998087`（三文档同步；本会话末尾 `HEAD`）。
...
### 0.10.1 最新状态 & 下一步建议（滚动更新至 2026-02-06；聚焦：审计域 DAL 按 BC 拆散试点（`sys_log`），保持行为不变）
...
- 🎯 **本次会话最新增量（2026-02-06，保持行为不变）**：
  ...
  - ⏳ 未完成（下一刀建议，保持行为不变；每次只改 1 个类/1 个资源文件）：`SysLogDO`、`SysLogModuleDO` 仍在 `eva-infra-dal`，且 `SysLogMapper.xml`、`SysLogModuleMapper.xml` 仍在 `eva-infra-dal/src/main/resources/mapper/log/`；建议顺序：先搬 DO（2 刀），再搬 XML（2 刀），最后再评估 `bc-audit/infrastructure/pom.xml` 是否可去 `eva-infra-shared`（需 Serena 证伪；单 pom 闭环）。
...
- 🎯 **下一刀建议（保持行为不变；主线：审计 `sys_log` 的 DAL 按 BC 拆散试点；每次只改 1 个类/1 个资源文件闭环）**：
  1) 先跑快照证据（作为防回归；口径同 0.11）：
     ...
     - `fd -t f -e java . eva-infra-dal/src/main/java/edu/cuit/infra/dal/database/dataobject/log`（当前应仅剩 `SysLogDO/SysLogModuleDO`）
     - `fd -t f -e xml . eva-infra-dal/src/main/resources/mapper/log`（当前应仍有 `SysLogMapper.xml/SysLogModuleMapper.xml`）
  2) Serena（必须）：分别证伪 `SysLogDO` / `SysLogModuleDO` 引用面仅审计域后，按顺序做 4 刀（每刀都需闭环一次）：
     - `SysLogDO`（类）→ `SysLogModuleDO`（类）→ `SysLogMapper.xml`（资源）→ `SysLogModuleMapper.xml`（资源）
...
### 推荐版：续接主线：`eva-infra-dal` 按 BC 拆散（审计 `sys_log` 试点），保持行为不变
...
1) 快照证据（保持行为不变；用于防回归/确认拆散进度）：
   - reactor：`rg -n "<module>eva-" pom.xml`（当前应仍包含 `eva-infra-dal/eva-infra-shared/eva-base`）
   - 审计 DAL 进度：
     - `rg -n "interface\\s+SysLogModuleMapper\\b|interface\\s+SysLogMapper\\b" --glob "*.java" bc-audit/infrastructure/src/main/java`
     - `fd -t f -e java . eva-infra-dal/src/main/java/edu/cuit/infra/dal/database/dataobject/log`（当前应仅剩 `SysLogDO/SysLogModuleDO`）
     - `fd -t f -e xml . eva-infra-dal/src/main/resources/mapper/log`（当前应仍有 `SysLogMapper.xml/SysLogModuleMapper.xml`）
2) Serena（必须）：证据化盘点 `SysLogDO` / `SysLogModuleDO` 引用面（确保仅审计域使用）后，按“一次只改 1 个类”的规则逐步归位：
   - 下一刀建议顺序：先 `SysLogDO` → 再 `SysLogModuleDO`（均保持 `package edu.cuit.infra.dal.database.dataobject.log` 不变）
