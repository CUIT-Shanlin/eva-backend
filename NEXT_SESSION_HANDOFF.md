...
- 🎯 **SysUserMapper 归位（进行中，保持行为不变）**：
  ...
  - 下一刀建议（每刀 1 类，保持行为不变）：继续清零 `bc-course/infrastructure` 内对 `SysUserMapper` 的编译期依赖（改为 `@Qualifier("sysUserMapper") Object` + 反射调用 `selectById(Serializable)` / `selectList(...)` + `getId()`/`getName()`），推荐顺序：
    1) `DeleteSelfCourseRepositoryImpl`
    2) `UpdateCourseInfoRepositoryImpl`
    3) `UpdateSelfCourseRepositoryImpl`
    4) `UpdateSingleCourseRepositoryImpl`
    5) `CourseImportExce`
    6) `CourseRecommendExce`
...
3) 🎯 本轮主线（承接已归位 XML，逐类归位 `eva-infra-dal`，保持行为不变）：
   ...
   - 下一刀建议（每刀 1 类，保持行为不变）：继续清零 `bc-course/infrastructure` 内对 `SysUserMapper` 的编译期依赖（优先 `DeleteSelfCourseRepositoryImpl`；`@Qualifier("sysUserMapper") Object` + 反射调用）；完成清零后再做“单类搬运”：`SysUserMapper.java` 从 `eva-infra-dal` 归位到 `bc-iam/infrastructure`（保持 `package` 不变，避免 classpath 重复）。
