...
- 📌 **当前仍未完成（进入 S1 前置的硬事实口径，保持行为不变）**：
  - `eva-adapter` 仍残留 17 个 `*Controller.java`（口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`）。
...
- 当前重构进度汇报（截至当前 `HEAD`，用于“还没完成什么/下一步怎么排”）：
  ...
  - ✅ **`eva-client` 退场**：已从 reactor 移除并从仓库删除；跨 BC 通用对象已开始沉淀 `shared-kernel`（见 10.5）。
		  - ✅ **已闭环（本阶段目标 B：基础设施旧 gateway 归位，保持行为不变）**：`eva-infra/src/main/java/edu/cuit/infra/gateway/impl` 下残留旧 `*GatewayImpl.java` 已清零（Serena 盘点口径）；归位闭环包含：
		    ...
		    2) `eva-adapter` 仍保留 **17 个** `*Controller.java`（口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`），且 ✅ 已开始逐个归位到对应 BC（已完成：`AuthenticationController`（`94a00022`）、`UserUpdateController`（`367f781d`）、`UserQueryController`（`f7c5d219`）、`RoleQueryController`（`e7d51beb`）、`MenuQueryController`（`76e19a47`）→ `bc-iam-infra`；保持行为不变）。
...
- 下一会话优先建议（聚焦 **S0.2 延伸**，保持行为不变；每步只改 1 个小点，便于回滚）：
  0) 🎯 **优先：收敛 `eva-adapter` 残留 `*Controller`（每次只改 1 个 Controller；保持行为不变）**：
     ...
     - 推荐顺序（优先链路短、改动面可控；保持行为不变；**✅ 仅表示“已归位迁出 eva-adapter”**）：`AuthenticationController`（✅）→ `UserUpdateController`（✅）→ `UserQueryController`（✅）→ `RoleQueryController`（✅）→ `MenuQueryController`（✅）→ `MenuUpdateController`（⏳）→ `RoleUpdateController`（⏳）→ 其余（⏳）。
...
**当前残留共 17 个**（均仍在 `eva-adapter/src/main/java`；用于下一会话按 BC 逐个归位，每次只迁 1 个 Controller，保持行为不变）：
...
- `fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`（当前应为 17；用于复核“Controller 残留规模”）
- `fd -t f 'Controller\\.java$' eva-adapter/src/main/java | sort`（当前应输出 17 行；用于复核“具体残留清单”，与 0.10.2 保持一致）
...
### E 专用简版（⏳ 未闭环：eva-adapter 仍残留 17 个 *Controller；用于下一阶段迁移入口）
...
下一刀（保持行为不变）：
- **S1（入口归位，优先；每次只迁 1 个 Controller；保持 `package/注解/URL/权限/参数校验/日志与副作用顺序` 完全不变）**：下一刀优先将 `MenuUpdateController` 从 `eva-adapter` 归位到 `bc-iam/infrastructure`；随后 `RoleUpdateController`。若编译闭合缺依赖，则先单独提交 1 次 `bc-iam/infrastructure/pom.xml` 的编译闭合，再做 `git mv`（避免“改类 + 改 pom”混在同一提交）。
