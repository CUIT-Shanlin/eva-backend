...
- 📌 **当前仍未完成（进入 S1 前置的硬事实口径，保持行为不变）**：
  - `eva-app/src/main/java` 已清零（口径：`fd -t f -e java . eva-app/src/main/java | wc -l` 当前为 0），且组合根 `start` 已不再依赖 `eva-app`（口径：`rg -n "<artifactId>eva-app</artifactId>" --glob "**/pom.xml" .` 仅命中 `eva-app/pom.xml` 的模块自身 artifactId 声明；落地：`0a9ff564`），因此下一步可评估 `eva-app` 是否可从 reactor 移除（保持行为不变；每次只改 1 个 pom）。
...
- 🎯 **下一步建议（保持行为不变；每步只改 1 个类或 1 个 pom）**：
  1) 下一刀（优先，单 pom）：在 Serena + `rg` 证据化确认全仓库已无对 `eva-app` 的 Maven 依赖后，评估从根 `pom.xml` 的 reactor 中移除 `eva-app` 模块（保持行为不变）→ 最小回归闭环。
...
- 下一刀（每次只改 1 个 pom.xml 或 1 个类；保持行为不变）：
  1) Serena 证据化盘点：确认 `eva-app/src/main/java` 已清零（口径：`fd -t f -e java . eva-app/src/main/java | wc -l` 当前为 0），并确认为“全仓库已无对 `eva-app` 的 Maven 依赖”（口径：`rg -n "<artifactId>eva-app</artifactId>" --glob "**/pom.xml" .` 仅命中 `eva-app/pom.xml` 的模块自身 artifactId 声明）。
  2) 单 pom 收敛：在证据化确认“已无依赖方引用”的前提下，评估从根 `pom.xml` 的 reactor 中移除 `eva-app` 模块（保持行为不变）→ 最小回归闭环。
