...
- ✅ 已证据化（Serena + `rg` 兜底）：`bc-iam/infrastructure/src/main/java/edu/cuit/infra/gateway/impl/user/UserQueryGatewayImpl.java` 仍承载 `@LocalCached` 缓存触发点；当前 `bc-iam/infrastructure` 内对旧 `UserQueryGateway` 的 import/注入引用面已清零（含 `UserQueryGatewayImpl` 已不再显式实现旧接口；落地：`fb3b2e40`）；详见 `docs/DDD_REFACTOR_BACKLOG.md` 4.3（证据化起点：`c4a32bde`）。
...
5) 🎯 **下一刀聚焦（保持行为不变）**：`bc-iam/infrastructure` 端口适配器对旧 `UserQueryGateway`（eva-domain）的编译期依赖已清零，且 `UserQueryGatewayImpl` 已不再显式实现旧接口（仍承载 `@LocalCached` 触发点）。下一步建议：优先证据化评估并处置 `UserQueryGatewayImpl` 对旧 `UserEntity` 的编译期依赖（风险高；需拆步；必须确保缓存 key/area 与命中/回源语义不漂移；每次仍只改 1 个类或 1 个 pom）。
