...
5) 🎯 **IAM 并行（按 10.3，保持行为不变）**：
   ...
   - ⏳ 未完成：继续把剩余依赖方对 `UserQueryGateway` 的编译期依赖收敛为 `bc-iam-contract` 最小 Port（优先候选：`bc-evaluation/infrastructure` 的 `EvaTaskServiceImpl`、`EvaStatisticsExporter`；注意 `SpringUtil.getBean(...)` 次数/顺序不变）。
...
- B（次优先：类，每次只改 1 个类）：继续推进 IAM 并行（10.3）：优先处理 `bc-evaluation/infrastructure` 的 `EvaTaskServiceImpl` 或 `EvaStatisticsExporter`，将其对 `UserQueryGateway` 的编译期依赖收敛为 `bc-iam-contract` 最小 Port（行为不变；注意 `SpringUtil.getBean(...)` 次数/顺序不变）。
