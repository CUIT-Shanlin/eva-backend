...
- 🎯 **SysUserMapper 归位（进行中，保持行为不变）**：
  ...
  - 下一刀建议（每刀 1 类，保持行为不变）：继续清零 `bc-course/**` 内对 `SysUserMapper` 的编译期依赖（Serena 证据：仍有 `CourseQueryRepository` 直连）。推荐顺序：
    1) `CourseQueryRepository`
  - 完成全仓库“非 IAM 模块清零”后再做“单类搬运”（保持行为不变）：将 `eva-infra-dal/.../SysUserMapper.java` 归位到 `bc-iam/infrastructure`（保持 `package` 不变，避免 classpath 重复）。
...
3) 🎯 本轮主线（承接已归位 XML，逐类归位 `eva-infra-dal`，保持行为不变）：
   ...
   - 下一刀建议（每刀 1 类，保持行为不变）：继续清零 `bc-course/**` 内对 `SysUserMapper` 的编译期依赖（优先 `CourseQueryRepository`；`@Qualifier("sysUserMapper") Object` + 反射调用）。完成清零后再做“单类搬运”：`SysUserMapper.java` 从 `eva-infra-dal` 归位到 `bc-iam/infrastructure`（保持 `package` 不变，避免 classpath 重复）。
