...
- 快速证据口径（新会话先复核，避免口径漂移）：
  ...
  - 审计（当前主线）：`rg -n '^import\\s+edu\\.cuit\\.bc\\.audit' eva-app/src/main/java`（当前应命中 `LogServiceImpl`；用于后续归位与依赖收敛）。
...
- `rg -n '^import\\s+edu\\.cuit\\.bc\\.audit' eva-app/src/main/java`（当前应命中 `LogServiceImpl`；用于盘点审计“待归位类清单”）
