...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程/模板写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 contract/shared-kernel）。
   ...
   - 下一刀优先（course，保持行为不变，单类闭环）：继续清零课程写侧对评教 DO 的编译期依赖（口径见上方 `edu.cuit.infra.dal.database.dataobject.eva.` 命中点），建议先从 `bc-course/infrastructure` 的 `DeleteSelfCourseRepositoryImpl` 开始逐类收敛（每刀仍严格闭环：Serena → 最小回归 → 代码提交 → 三文档同步 → 文档提交 → push）。
