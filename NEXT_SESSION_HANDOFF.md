...
- ✅ **评教（依赖收敛：eva-app 去 bc-evaluation 编译期依赖，保持行为不变）**：在 Serena + `rg` 证伪 `eva-app/src/main/java` 已不再引用 `edu.cuit.bc.evaluation.*` 后，收敛 `eva-app/pom.xml`：移除对 `bc-evaluation`（application jar）的 Maven 编译期依赖（最小回归通过；证据口径：`rg -n '^import\\s+edu\\.cuit\\.bc\\.evaluation' eva-app/src/main/java` → 0）；落地提交：`2b42db5d`。
...
### 0.10.1 本次会话（2026-01-14）最新状态 & 下一步建议（聚焦：S0.2 延伸/依赖收敛）
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类）**：S0.2 延伸（评教：旧入口归位 → 依赖收敛前置）已阶段性闭环：评教发布器/监听器/旧入口壳均已归位到 `bc-evaluation-infra`，且 `rg -n '^import\\s+edu\\.cuit\\.bc\\.evaluation' eva-app/src/main/java` 已命中为 0，并已完成 `eva-app/pom.xml` 去 `bc-evaluation` 编译期依赖。下一步建议转入“收敛其它依赖方 pom.xml（每次只改 1 个 pom）”或继续推进其它 BC 的入口/装配归位（仍保持行为不变）。
  1) ✅ 支撑类前置（保持行为不变；每次只改 1 个类）：已将 `WebsocketManager`、`MsgBizConvertor` 从 `eva-app` 归位到 `eva-infra-shared`（保持 `package` 不变；落地：`406186ae` / `c69f494f`）。
...
- 快速证据口径（新会话先复核，避免口径漂移）：`rg -n '^import\\s+edu\\.cuit\\.bc\\.evaluation' eva-app/src/main/java`（当前应为 0；以及 Serena `search_for_pattern` 同口径）。
...
3) 🎯 本轮目标（保持行为不变；每次只改 1 个类闭环）：✅ 已归位 `SpringAfterCommitDomainEventPublisher`（见 0.9）；✅ 已归位评教领域事件监听器 `EvaluationSubmittedMessageCleanupListener`、`EvaluationTaskPostedMessageSenderListener`；✅ 已归位评教旧入口壳 `EvaStatisticsServiceImpl/EvaTemplateServiceImpl/EvaRecordServiceImpl/EvaTaskServiceImpl/UserEvaServiceImpl/MsgServiceImpl`；并已完成 `eva-app/pom.xml` 去 `bc-evaluation` 编译期依赖（保持行为不变；详见 0.9 与 0.10.1）。
