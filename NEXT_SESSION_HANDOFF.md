...
- 🔍 **阻塞提醒（更新，保持行为不变）**：当前 `eva-app/src/main/java` 仍直接 `import edu.cuit.bc.evaluation.*` 的类：`UserEvaServiceImpl`、`MsgServiceImpl`，因此仍**暂不可**收敛 `eva-app/pom.xml` 对 `bc-evaluation`（application jar）的编译期依赖；证据口径：`rg -n '^import\\s+edu\\.cuit\\.bc\\.evaluation' eva-app/src/main/java`。
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类）**：继续推进 **S0.2 延伸（评教：旧入口归位 → 依赖收敛前置）**。截至当前基线：评教发布器/监听器/四个旧入口壳（统计/模板/记录/任务）均已归位到 `bc-evaluation-infra`；`eva-app/src/main/java` 仍直接 `import edu.cuit.bc.evaluation.*` 的类只剩 **2 个**：`UserEvaServiceImpl`、`MsgServiceImpl`（证据口径：`rg -n '^import\\s+edu\\.cuit\\.bc\\.evaluation' eva-app/src/main/java`）。建议按“依赖闭包最小 + 易回滚”顺序推进：
  1) 先搬运 `eva-app/src/main/java/edu/cuit/app/service/impl/eva/UserEvaServiceImpl.java` → `bc-evaluation/infrastructure`（保持 `package` 不变；不改 `@CheckSemId`/`StpUtil` 登录态解析次数与顺序；不改异常文案/副作用顺序）。
  2) 再处理 `eva-app/src/main/java/edu/cuit/app/service/impl/MsgServiceImpl.java`：优先仍按“归位到 `bc-evaluation-infra`（保持 `package` 不变）”推进，以清零 `eva-app` 对评教端口类型的直接引用面；若出现 Maven 循环依赖风险，先用 Serena 盘点依赖闭包并在 0.9 记录证据与降级方案。
...
3) 🎯 本轮目标（保持行为不变；每次只改 1 个类闭环）：✅ 已归位 `SpringAfterCommitDomainEventPublisher`（见 0.9）；✅ 已归位评教领域事件监听器 `EvaluationSubmittedMessageCleanupListener`、`EvaluationTaskPostedMessageSenderListener`；✅ 已归位评教旧入口壳 `EvaStatisticsServiceImpl/EvaTemplateServiceImpl/EvaRecordServiceImpl/EvaTaskServiceImpl`；下一步继续逐个归位 `UserEvaServiceImpl` 与 `MsgServiceImpl`；完成后再用 Serena + `rg` 证伪 `eva-app/src/main/java` 对 `edu.cuit.bc.evaluation.*` 的直接引用面，评估是否可以收敛 `eva-app/pom.xml` 的 `bc-evaluation` 编译期依赖（每次只改 1 个 `pom.xml`）。

下一刀（保持行为不变）：
- 下一刀（每次只改 1 个类；保持行为不变）：
  1) Serena 证据化盘点：`eva-app/src/main/java` 对 `edu.cuit.bc.evaluation.*` 的引用清单与“依赖闭包”（用于决定先搬哪一个类最省前置）。
  2) 搬运评教旧入口壳 `UserEvaServiceImpl` 到 `bc-evaluation-infra`（保持 `package` 不变；类内容不变；副作用顺序不变；若遇支撑类依赖，先逐个搬运支撑类；仍遵守“每次只改 1 个类”）。
  3) 继续处理并归位 `MsgServiceImpl`：目标是让 `eva-app/src/main/java` 不再直接 `import edu.cuit.bc.evaluation.*`（保持业务语义/异常文案/副作用顺序完全不变；每次只改 1 个类闭环）。
  4) 当 Serena + `rg` 证伪 `eva-app/src/main/java` 不再 `import edu.cuit.bc.evaluation.*` 后，再评估是否可以收敛 `eva-app/pom.xml` 对 `bc-evaluation` 的编译期依赖（每次只改 1 个 `pom.xml`；保持行为不变）。

快速证据口径（新会话先跑一次，避免口径漂移）：
- `rg -n '^import\\s+edu\\.cuit\\.bc\\.evaluation' eva-app/src/main/java`（当前应只命中 `UserEvaServiceImpl` 与 `MsgServiceImpl`；用于盘点“待归位类清单”）
