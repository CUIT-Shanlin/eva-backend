...
- 🔍 **阻塞提醒（保持行为不变）**：`eva-app/src/main/java` 仍直接 `import edu.cuit.bc.evaluation.*`（例如 `EvaRecordServiceImpl/EvaTaskServiceImpl/EvaTemplateServiceImpl/EvaStatisticsServiceImpl/UserEvaServiceImpl`、`MsgServiceImpl` 以及评教领域事件监听器等），因此**暂不可**收敛 `eva-app/pom.xml` 对 `bc-evaluation`（application jar）的编译期依赖；证据口径：`rg -n '^import\\s+edu\\.cuit\\.bc\\.evaluation' eva-app/src/main/java`。
...
### 0.10.1 本次会话（2026-01-13）最新状态 & 下一步建议（聚焦：S0.2 延伸/依赖收敛）
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类）**：继续推进 **S0.2 延伸（评教：旧入口归位 → 为后续收敛 eva-app → bc-evaluation 编译期依赖创造前置）**：逐步把 `eva-app` 中仍直接依赖 `edu.cuit.bc.evaluation.*` 的评教入口壳/监听器/发布器归位到 `bc-evaluation-infra`（保持 `package` 不变；不改缓存/日志/异常文案/副作用顺序）。推荐先从“依赖闭包最小 + 易回滚”的类开始：
  1) `eva-app/src/main/java/edu/cuit/app/event/SpringAfterCommitDomainEventPublisher.java`
  2) `eva-app/src/main/java/edu/cuit/app/event/listener/EvaluationSubmittedMessageCleanupListener.java`
  3) `eva-app/src/main/java/edu/cuit/app/event/listener/EvaluationTaskPostedMessageSenderListener.java`
  4) `eva-app/src/main/java/edu/cuit/app/service/impl/eva/*ServiceImpl.java`（`EvaRecord/EvaTask/EvaTemplate/EvaStatistics/UserEva` 逐个归位；若出现支撑类依赖，先逐个归位支撑类，仍遵守“每次只改 1 个类”）
  5) （后置）`eva-app/src/main/java/edu/cuit/app/service/impl/MsgServiceImpl.java` 对评教端口的依赖收敛（通常需要先明确“端口归属/契约落点”，避免跨域副作用顺序漂移）
...
### G 推荐版（当前主线：S0.2 延伸（评教：旧入口归位 → 依赖收敛前置），优先）
...
本轮目标（G，优先；保持行为不变）：
1) 🎯 当前主线：继续推进 **S0.2 延伸（评教：旧入口归位 → 为后续依赖收敛创造前置）**：逐步减少 `eva-app` 对 `bc-evaluation`（application jar）的编译期耦合面，方式为“评教旧入口壳/监听器/发布器逐个归位到 `bc-evaluation-infra`（保持 `package` 不变）”。
2) ✅ 已完成（保持行为不变，依赖收敛前置）：`BcEvaluationConfiguration` 已从 `eva-app` 归位到 `bc-evaluation-infra`；`start` 已显式依赖 `bc-evaluation-infra(runtime)`；`eva-app` 已移除对 `bc-evaluation-infra` 的编译期依赖（详见 0.9）。
3) 🎯 本轮目标（保持行为不变；每次只改 1 个类闭环）：✅ 已归位 `SpringAfterCommitDomainEventPublisher`（见 0.9）；下一步先归位评教领域事件监听器 `EvaluationSubmittedMessageCleanupListener`、`EvaluationTaskPostedMessageSenderListener`，再逐个归位 `EvaRecordServiceImpl/EvaTaskServiceImpl/EvaTemplateServiceImpl/EvaStatisticsServiceImpl/UserEvaServiceImpl`；完成后再用 Serena + `rg` 证伪 `eva-app/src/main/java` 对 `edu.cuit.bc.evaluation.*` 的直接引用面，评估是否可以收敛 `eva-app/pom.xml` 的 `bc-evaluation` 编译期依赖（每次只改 1 个 `pom.xml`）。

下一刀（保持行为不变）：
- 下一刀（每次只改 1 个类；保持行为不变）：
  1) Serena 证据化盘点：`eva-app/src/main/java` 对 `edu.cuit.bc.evaluation.*` 的引用清单与“依赖闭包”（用于决定先搬哪一个类最省前置）。
  2) 先搬运评教领域事件监听器 `EvaluationSubmittedMessageCleanupListener`（保持 `package` 不变；类内容不变；副作用顺序不变）。
  3) 再搬运评教领域事件监听器 `EvaluationTaskPostedMessageSenderListener`（保持 `package` 不变；类内容不变；副作用顺序不变）。
  4) 再逐个搬运评教旧入口壳 `EvaRecordServiceImpl/EvaTaskServiceImpl/EvaTemplateServiceImpl/EvaStatisticsServiceImpl/UserEvaServiceImpl`（若遇支撑类依赖，先逐个搬运支撑类；仍遵守“每次只改 1 个类”）。
  5) 当 Serena + `rg` 证伪 `eva-app/src/main/java` 不再 `import edu.cuit.bc.evaluation.*` 后，再评估是否可以收敛 `eva-app/pom.xml` 对 `bc-evaluation` 的编译期依赖（每次只改 1 个 `pom.xml`；保持行为不变）。

快速证据口径（新会话先跑一次，避免口径漂移）：
- `rg -n '^import\\s+edu\\.cuit\\.bc\\.evaluation' eva-app/src/main/java`（当前应非 0；用于盘点“待归位类清单”）
