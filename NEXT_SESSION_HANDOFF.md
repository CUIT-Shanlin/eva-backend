...
- ✅ **S1.1（eva-adapter 退场：root reactor 移除模块，保持行为不变）**：在 Serena 证据化确认全仓库仅 `eva-adapter/pom.xml` 声明 `<artifactId>eva-adapter</artifactId>`，且根 `pom.xml` 仅残留 `<module>eva-adapter</module>` 的前提下，从根 `pom.xml` 的 reactor 中移除 `eva-adapter` 模块（最小回归通过）；落地提交：`86842a1f`。
...
### 0.10.1 最新状态 & 下一步建议（滚动更新至 2026-01-26；聚焦：S1.1（eva-adapter 退场）+ 并行“依赖方 pom 收敛”）
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：继续推进 **S0.2 延伸（依赖方编译期依赖收敛）**：选择下一条“依赖方编译期依赖收敛”目标（优先从 `eva-domain` / `eva-infra-shared` 等依赖方开始），用 Serena 证据化盘点引用面与依赖闭包后再收敛 `pom.xml`（每次只改 1 个 pom；保持行为不变）。
  - 📌 **当前状态快照（截至 2026-01-26，口径=可复现命令；保持行为不变）**：
    ...
    - ✅ `eva-adapter/pom.xml` 已进一步收敛为“仅保留运行时基础依赖”（并已移除 `bc-audit` / `bc-ai-report` / `shared-kernel` / `bc-iam-contract` / `bc-evaluation-contract` / `bc-messaging-contract` 的 Maven 编译期依赖；均以 Serena 证据化盘点为前置，并在最小回归下闭环；保持行为不变）。
		  ...
		  - ✅ **已闭环（本阶段目标 B：基础设施旧 gateway 归位，保持行为不变）**：`eva-infra/src/main/java/edu/cuit/infra/gateway/impl` 下残留旧 `*GatewayImpl.java` 已清零（Serena 盘点口径）；归位闭环包含：
		    ...
		    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
			  - ⏳ **仍未完成（核心阻塞项）**：
			    ...
			    2.1) ✅ `eva-adapter/pom.xml` 已完成依赖收敛：已移除 `bc-audit` / `bc-ai-report` / `shared-kernel` / `bc-iam-contract` / `bc-evaluation-contract` / `bc-messaging-contract` 的 Maven 编译期依赖（保持行为不变；口径见 0.10.1）。
...
### 0.10.2 `eva-adapter` 残留 Controller 清单（口径=可复现命令；更新至 2026-01-26）
...
2) （后置，可选）在 reactor 已移除后，再评估是否需要删除 `eva-adapter/` 目录及其 `pom.xml`（建议独立提交，避免与 pom 变更混在同一提交里）。
...
3) 🎯 当前主线：推进 **S1（eva-* 退场/整合）**（保持行为不变）：`eva-app` 已完成退场；`eva-adapter` 也已完成关键退场闭环（`*Controller` 清零 + `start` 去依赖 + root reactor 移除）。下一阶段建议：并行推进 **S0.2 延伸（依赖方编译期依赖收敛）**（每次只改 1 个 pom），或评估“删除 `eva-adapter` 空模块目录”（需 Serena 证伪后独立提交；保持行为不变）。
...
- 下一刀（每次只改 1 个 pom.xml 或 1 个类；保持行为不变）：
  ...
  4) ✅ 已完成：单 pom 收敛：从根 `pom.xml` 的 reactor 中移除 `eva-adapter` 模块（保持行为不变）→ 最小回归通过；落地提交以 `git log -n 1 -- pom.xml` 为准（提示词里不固化 commitId）。
...
- `rg -n "<artifactId>eva-adapter</artifactId>" start/pom.xml`（当前应为空；用于复核“组合根已移除 eva-adapter 依赖”）
- `rg -n "<module>eva-adapter</module>" pom.xml`（当前应为空；用于复核“reactor 已移除 eva-adapter 模块”）
- `fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`（当前应为 0；用于复核“Controller 残留规模”）
- `fd -t f 'Controller\\.java$' eva-adapter/src/main/java | sort`（当前应为空；用于复核“具体残留清单”，与 0.10.2 保持一致）
