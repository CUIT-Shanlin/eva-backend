...
**当前残留共 2 个**（均仍在 `eva-adapter/src/main/java`；用于下一会话按 BC 逐个归位，每次只迁 1 个 Controller，保持行为不变）：
- Course（课程/课表）
- Messaging（消息）
...
1) 优先归位 `edu/cuit/adapter/controller/MessageController.java` → `bc-messaging`（先 Serena 证据化盘点引用面/依赖闭包；保持 `package/注解/URL/权限校验/参数校验/日志与副作用顺序` 完全不变）。
2) 若迁移时出现编译缺依赖：先单独提交 1 次“目标模块的 `pom.xml`”做“编译闭合前置”，再单独提交 `git mv`（避免“改类 + 改 pom”混在同一提交）。
...
- `fd -t f 'Controller\\.java$' eva-adapter/src/main/java | sort`（当前应输出 2 行；用于复核“具体残留清单”，与 0.10.2 保持一致）
...
下一刀（保持行为不变）：
- **S1（入口归位，优先；每次只迁 1 个 Controller；保持 `package/注解/URL/权限/参数校验/日志与副作用顺序` 完全不变）**：下一刀建议优先将 `MessageController` 从 `eva-adapter` 归位到 `bc-messaging`。补充：若目标模块出现编译闭合缺依赖，则先单独提交 1 次对应 `pom.xml`（避免“改类 + 改 pom”混在同一提交；保持行为不变）。
