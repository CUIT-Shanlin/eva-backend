...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类）**：继续推进 **S0.2 延伸（IAM：旧入口归位 → 为后续依赖收敛创造前置）**：按顺序把 `eva-app` 残留的 IAM 旧入口壳归位到 `bc-iam-infra`（保持 `package` 不变；不改缓存/日志/异常文案/副作用顺序），推荐顺序：`UserServiceImpl` → `BcIamConfiguration`。完成后再用 Serena + `rg` 证伪 `eva-app` 对 `bc-iam` 的直接引用面是否已足够收敛，再评估是否可收敛 `eva-app/pom.xml` 的 `bc-iam` 编译期依赖（仍遵守“每次只改 1 个 pom.xml”）。
  - 快速口径（新会话先复核，避免口径漂移）：`fd -t f 'ServiceImpl\\.java$' eva-app/src/main/java/edu/cuit/app/service/impl | sort`；`fd -t f 'BcIamConfiguration\\.java' eva-app/src/main/java | cat`
...
### F 推荐版（当前主线：S0.2 延伸（IAM 旧入口归位 → 依赖收敛前置），优先）
...
本轮目标（F，优先；保持行为不变）：
1) 🎯 当前主线：继续推进 **S0.2 延伸（IAM：旧入口归位 → 为后续依赖收敛创造前置）**：逐步减少 `eva-app` 对 `bc-iam` 的编译期耦合面，方式为“旧入口壳归位到 `bc-iam-infra`（保持 `package` 不变）”。
2) ✅ 已完成（保持行为不变）：IAM 旧入口归位已推进到 `UserAuthServiceImpl`（以及 `DepartmentServiceImpl/MenuServiceImpl/RoleServiceImpl` 等；详见 0.9）。
3) 🎯 本轮目标（保持行为不变；每次只改 1 个类闭环）：按顺序完成 `UserServiceImpl` → `BcIamConfiguration` 的归位；然后再用 Serena + `rg` 证伪 `eva-app/src/main/java` 对 `bc-iam` 的直接引用面，评估是否可以收敛 `eva-app/pom.xml` 的 `bc-iam` 编译期依赖（每次只改 1 个 `pom.xml`）。

下一刀（保持行为不变）：
- ✅ 已完成（保持行为不变）：将 `UserAuthServiceImpl` 从 `eva-app` 搬运归位到 `bc-iam-infra`（保持 `package` 不变；类内容不变；详见 0.9）。
- 下一刀（每次只改 1 个类；保持行为不变）：
  1) 优先恢复 Serena（符号级引用分析口径）；若持续 `TimeoutError`，按 0.9 的“降级原因 + 可复现 `rg` 证据”口径记录并继续。
  2) 盘点并搬运 `UserServiceImpl`（若存在支撑类依赖，先逐个搬运支撑类；仍遵守“每次只改 1 个类”）。
  3) 搬运 `BcIamConfiguration` 到 `bc-iam-infra`（保持 `package edu.cuit.app.config` 不变；Bean 装配/副作用顺序不变）。
  4) 完成上述两步后，再评估是否可以收敛 `eva-app/pom.xml` 对 `bc-iam` 的编译期依赖（每次只改 1 个 `pom.xml`；保持行为不变）。

快速证据口径（新会话先跑一次，避免口径漂移）：
- `fd -t f 'ServiceImpl\\.java$' eva-app/src/main/java/edu/cuit/app/service/impl | sort`（当前应仅剩 `UserServiceImpl` + 评教/消息/日志相关）
- `fd -t f 'BcIamConfiguration\\.java' eva-app/src/main/java | cat`
