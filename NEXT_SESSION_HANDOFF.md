...
- 本次会话最新闭环（2026-01-02，便于续接）：  
  ...
  4) 下一步（保持行为不变；每次只改 1 个类/小点）：优先清理 `ICourseServiceImpl` 残留的未使用注入依赖（例如 `CourseQueryGateway/CourseUpdateGateway`），并用 Serena 证伪无引用面后再移除（避免误删导致装配漂移）；其后再评估是否开始“旧 gateway 压扁为委托壳”的 S0（每次只压 1 个方法）。
