...
- ✅ 以上每步最小回归均已通过（Java17）：  
  - （JDK 由 mise 管理，见 4.1；若当前 shell 的 `java -version` 不是 17，则用 `mise exec java@temurin-17 -- <cmd>` 兜底）`mvnd -pl start -am test -Dtest=edu.cuit.app.eva.EvaRecordServiceImplTest,edu.cuit.app.eva.EvaStatisticsServiceImplTest -Dsurefire.failIfNoSpecifiedTests=false -Dmaven.repo.local=.m2/repository`
...
- 最小回归命令（每步结束必跑，Java17）：  
  - 前置：本仓库已提供 `mise.toml`（固定 `java@temurin-17`）。若你的交互 shell 已启用 mise 激活，可在仓库根目录执行 `mise use java@temurin-17` 并确认 `java -version` 为 17；若当前 shell 中 `java` 仍非 17，则用 `mise exec` 兜底执行回归命令。  
...
最小回归命令（Java17）：
- 前置：本仓库已提供 `mise.toml`（固定 `java@temurin-17`）。若当前 shell 的 `java -version` 不是 17，使用 `mise exec java@temurin-17 -- <cmd>` 兜底。
...
- 切换 JDK（本仓库使用 mise，优先让 `java` 命令直接生效）：`mise use java@temurin-17`（或：`mise exec java@temurin-17 -- java -version` 复核）
 ...
 - 已验证：`mvnd -o -pl eva-infra -am test -q -Dmaven.repo.local=.m2/repository`（前置：`mise use java@temurin-17` 且 `java -version` 为 17）
...
- 建议命令（每次新 shell，在仓库根目录执行一次即可）：  
  - `mise use java@temurin-17`  
  - 验证：`java -version`（应为 17）
- 兜底（避免 shell 未启用 mise 激活导致 `java` 仍指向其它版本）：  
  - 验证：`mise exec java@temurin-17 -- java -version`  
  - 执行 Maven：`mise exec java@temurin-17 -- mvn ...` / `mise exec java@temurin-17 -- mvnd ...`
