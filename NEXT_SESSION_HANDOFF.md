...
1) **C5-2（读侧实现继续拆，记录主题）**：抽出 `EvaRecordQueryRepository` 承接 `EvaRecordQueryRepo` 实现，并让 `EvaQueryRepository` 的记录主题方法退化为委托（保持口径/异常文案不变）。
2) **C5-3（读侧实现继续拆，任务主题）**：抽出 `EvaTaskQueryRepository` 承接 `EvaTaskQueryRepo` 实现（保持口径/异常文案不变）。
3) **C5-4（读侧实现继续拆，模板主题）**：抽出 `EvaTemplateQueryRepository` 承接 `EvaTemplateQueryRepo` 实现（保持口径/异常文案不变）。
...
- **bc-evaluation（评教）**：写侧主链路（任务发布/删除/模板）已按“用例+端口+适配器+委托壳”收敛；读侧已完成 QueryPort 拆分与 `EvaQueryRepo` 的四主题接口拆分（统计/记录/任务/模板），且已完成 C5-1（统计主题实现抽离），下一步继续拆记录/任务/模板实现。
...
下一步提交点（建议优先级）：
1) C5-2：继续拆 `EvaQueryRepository` 的实现（记录主题，行为不变）
2) C5-3：继续拆 `EvaQueryRepository` 的实现（任务主题，行为不变）
3) C5-4：继续拆 `EvaQueryRepository` 的实现（模板主题，行为不变）
