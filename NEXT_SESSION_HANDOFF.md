...
3) 🎯 当前主线：继续推进 **S0.2 延伸（依赖方编译期依赖收敛）**：`eva-app/pom.xml` 已完成移除 `bc-messaging-contract` 编译期依赖；下一刀建议转向 `eva-domain` / `eva-infra-shared` 等依赖方的编译期依赖收敛评估（每次只改 1 个 `pom.xml` 或 1 个类；保持行为不变）。补充：审计（`bc-audit`）与 AI 报告（`bc-ai-report`）的 `eva-app` 编译期依赖收敛已闭环（详见 0.9）。
