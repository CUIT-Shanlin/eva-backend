...
5) 🎯 **下一刀聚焦（保持行为不变）**：`bc-iam/infrastructure` 已清零 `UserQueryGateway` 与 `UserEntity` 的直接 import（含 `UserQueryGatewayImpl` 已不再显式实现旧接口、并已去 `UserEntity` 编译期依赖）。下一步建议：在保持 `@LocalCached` 触发点不变的前提下，评估是否可进一步收敛 `eva-domain` 中旧 `UserEntity` 的 Maven 归属/替代方案（高风险；多步；每次仍只改 1 个类或 1 个 pom）。
...
- `rg -n \"import\\s+edu\\.cuit\\.domain\\.entity\\.user\\.biz\\.UserEntity;\" bc-* --glob \"*.java\" | head`（应命中为 0：BC 范围已清零）
