...
- 🎯 **下一刀建议（保持行为不变；单类闭环；用于减少口径漂移）**：
  - `bc-course/infrastructure/src/main/java/edu/cuit/infra/gateway/impl/course/operate/CourseImportExce.java`：清理历史注释中残留的 `CourseMapper.selectList(...).map(CourseDO::getId)` 旧实现（注释清理不改变任何业务语义/异常文案/副作用顺序，但可避免后续 `rg`/Serena 盘点时被“注释命中”干扰）。
...
1) ✅ 已完成（保持行为不变）：评教读侧与课程域读侧中“仅为拿课程ID列表而先查 CourseDO 再映射”的关键残留点已按“最小端口”路线收敛（详见 0.9/`DDD_REFACTOR_PLAN.md` 10.2/`docs/DDD_REFACTOR_BACKLOG.md` 4.2）。
   - 下一刀建议（保持行为不变；单类闭环；减少口径漂移）：清理 `bc-course/infrastructure/src/main/java/edu/cuit/infra/gateway/impl/course/operate/CourseImportExce.java` 内历史注释残留的旧实现（避免 `rg` 命中注释导致盘点误判）。
