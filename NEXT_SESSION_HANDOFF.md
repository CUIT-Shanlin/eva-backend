...
- 📌 **当前仍未完成（进入 S1 前置的硬事实口径，保持行为不变）**：
  - `eva-app` 仍残留 1 个 Java 文件（口径：`fd -t f -e java . eva-app/src/main/java | wc -l`）：`package-info.java`。
...
- 🎯 **下一步建议（保持行为不变；每步只改 1 个类或 1 个 pom）**：
  1) 下一刀（优先）：`package-info.java` 归位到合适落点（建议：随课程 Exec 一并归位到 `bc-course-infra` / `bc-course/infrastructure`，保持包名不变），使 `eva-app/src/main/java` 清零。
  2) 在 `eva-app` 源码清零后：评估 `start/pom.xml` 是否可移除对 `eva-app` 的编译期依赖（或至少降为 `runtime`），为后续 `eva-app` 模块退场创造条件（每次只改 1 个 `pom.xml`，保持行为不变）。
  3) 并行主线（依赖方 pom 收敛）：继续挑选 1 个依赖方模块（优先 `eva-domain` / `eva-infra-shared`）做“Serena 证据化盘点 → 单 pom 收敛”。
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：继续推进 **S0.2 延伸（依赖方编译期依赖收敛）**：选择下一条“依赖方编译期依赖收敛”目标（优先从 `eva-domain` / `eva-infra-shared` 等依赖方开始），用 Serena 证据化盘点引用面与依赖闭包后再收敛 `pom.xml`（每次只改 1 个 pom；保持行为不变）。
  - 📌 **当前仍未完成（截至 2026-01-17，口径=可复现命令；保持行为不变）**：
    ...
    - `eva-app` 仍作为“过渡装配/支撑类”模块存在：当前源码侧仅剩 **1 个** Java 文件（口径：`fd -t f -e java . eva-app/src/main/java | wc -l`；仅 `package-info.java`）；组合根 `start` 仍对 `eva-app` 有编译期依赖（见 `start/pom.xml`）。
...
- 下一刀（每次只改 1 个 pom.xml 或 1 个类；保持行为不变）：
  1) Serena 证据化盘点：对 `eva-app/src/main/java` 残留支撑类（当前口径为 1 个 Java 文件）做“引用面 + 依赖闭包”盘点，并将 `package-info.java` 归位到合适落点（建议：随课程相关包一并归位到 `bc-course/infrastructure`；保持包名与行为不变）。
