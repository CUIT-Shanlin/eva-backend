...
- 🎯 **下一刀建议（保持行为不变；每次只改 1 个 `pom.xml` 闭环；主线继续做“依赖边界收敛”）**：
  1) 先跑快照证据（作为防回归；口径同 0.11 的 4 条 rg）：
     ...
     - `rg -n "class\\s+CourInfTimeOverlapQuery\\b" --glob "*.java" .`（应命中 1）
     - `rg -n "class\\s+ClassroomOccupancyChecker\\b" --glob "*.java" .`（应命中 1）
  2) 选择 1 个依赖方模块（优先 `bc-*/application/pom.xml` 或 `bc-*/contract/pom.xml`）做“无引用面证伪 → 依赖收敛”：
     - 推荐候选（从低风险开始，单 pom）：优先尝试移除 `lombok(provided)` 或无用 `junit-jupiter(test)`；
     - 必须用 Serena 在对应 `src/main/java`（以及必要时 `src/test/java`）证伪“无 Lombok/JUnit 引用面”后再改 pom；
     - 若 Serena 持续 `TimeoutError`：在 0.9 追加“降级原因 + 可复现 rg 证据”，并将“恢复 Serena”置为下一会话最高优先级。
  3) 最小回归命令（Java17 + mvnd）通过后：`git commit`（代码）→ 同步三文档 → `git commit`（文档）→ `git push`。
...
### 推荐版：续接主线：课程域支撑类归位已补齐；下一刀：继续做依赖方 pom 收敛（保持行为不变）
...
1) 快照证据（预期无命中，保持行为不变）：  
   ...
   - `rg -n "class\\s+CourInfTimeOverlapQuery\\b" --glob "*.java" .`（应命中 1）
   - `rg -n "class\\s+ClassroomOccupancyChecker\\b" --glob "*.java" .`（应命中 1）
2) Serena 证据化选择一个依赖方模块（优先 `bc-*/application/pom.xml` 或 `bc-*/contract/pom.xml`），证伪其源码无对应类型引用面后，再收敛其 Maven 依赖边界（优先移除冗余 `lombok(provided)` / 无用 `junit-jupiter(test)`；保持行为不变）。
