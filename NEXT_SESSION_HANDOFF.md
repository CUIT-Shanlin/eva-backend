...
### 0.10.1 最新状态 & 下一步建议（滚动更新至 2026-02-19；聚焦：S0.2 延伸（`eva-infra-dal` 按 BC 拆散 + `eva-infra-shared` 瘦身），保持行为不变）
...
- 📊 **整体未完成清单（更新至 2026-02-17；保持行为不变）**：
  ...
  - `eva-infra-shared`：当前仍有 `3` 个 Java，且多 BC 仍直依赖该模块（可复现口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l` + `rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | head`）。
  ...
  - `eva-infra-shared` 残留结构（Serena + `rg` 证据化，保持行为不变）：仅剩 `CourseBizConvertor/CourseConvertor/UserConverter` 3 个 MapStruct Convertor，**三者之间无互相依赖**（shared 内部不互引），但其引用面跨 `bc-course/bc-evaluation/bc-iam`，且编译期依赖 `bc-course-domain/bc-iam-domain` 的实体类型；因此 **不能直接下沉 `shared-kernel`**（否则会形成 Maven 环依赖）。LDAP 子簇已完成 `EvaLdapProperties/LdapGroupDO/LdapGroupRepo/LdapConstants/EvaLdapUtils` 下沉到 `shared-kernel`（保持包名不变）。
...
### 推荐版（2026-02-19 收口续接：瘦身 `eva-infra-shared` 收尾（Convertor 退场路径证据化 + 逐点收敛），保持行为不变）
...
1) 证据口径（先跑快照，避免口径漂移）：
   ...
   - `git stash list`（确认是否存在未落地 WIP；后续只能按“指定文件路径”逐刀 restore）
	   - `fd -t f -e java . eva-infra-dal/src/main/java | wc -l`（当前预期=0）
   ...
   - `fd -t f -e java . eva-infra-shared/src/main/java | wc -l`（当前预期=3）
...
3) 🎯 本轮主线（瘦身 `eva-infra-shared`，保持行为不变）：
   - ✅ 已闭环（保持行为不变）：`eva-infra-dal` 已从 root reactor 退场，且已删除 `eva-infra-dal/pom.xml`（详见 0.10.1 / `docs/DDD_REFACTOR_BACKLOG.md` 6；本轮无需再动）。
   - ✅ 已闭环（保持行为不变）：LDAP 配置类 `EvaLdapProperties` 与 LDAP DO `LdapGroupDO` 已下沉到 `shared-kernel`（保持包名不变；详见 0.9）。
   - 🎯 下一刀建议（优先，LDAP 子簇续接；每次只改 1 个文件闭环）：
     1) ✅ 已完成（单 pom，依赖前置，保持行为不变）：`shared-kernel/pom.xml` 已补齐 `spring-data-ldap(optional)`（或等价依赖）以承接 `LdapGroupRepo` 的 `LdapRepository` 编译期依赖（落地：`9dad61a8`）。
    2) ✅ 已完成（单类，保持行为不变）：`LdapGroupRepo` 从 `eva-infra-shared` 下沉到 `shared-kernel`（保持 `package`/签名不变；落地：`7ff087ad`）。
     3) ✅ 已完成（后置，多刀编排，保持行为不变）：`LdapConstants` / `EvaLdapUtils` 静态初始化互引风险已分刀闭环：
        - ✅ 已完成（单类，解耦前置，保持行为不变）：`LdapConstants` 去 `EvaLdapUtils` 编译期互引（落地：`45fa9651`）。
        - ✅ 已完成（单类，搬运，保持行为不变）：将 `LdapConstants` 从 `eva-infra-shared` 下沉到 `shared-kernel`（保持 `package` 与类内容不变；落地：`3dc2e8ff`）。
        - ✅ 已完成（单类，后置，保持行为不变）：将 `EvaLdapUtils` 从 `eva-infra-shared` 下沉到 `shared-kernel`（保持 `package` 与类内容不变；落地：`05cc3039`）。
   - 备选主线（更稳，非 LDAP）：对 `CourseBizConvertor/CourseConvertor/UserConverter` 逐个做 Serena 引用面证伪后再决定“归位到单 BC”（前置：先消除跨 BC 引用面）或“继续暂留作为过渡共享模块”（**不建议** 直接下沉 `shared-kernel`：存在 Maven 环依赖风险，详见 0.9/0.10.1 的证据化结论）。
   - 🎯 Convertor 收尾路线（推荐，保持行为不变；按“先收敛引用面 → 再搬运归位”）：
     1) **先收敛 `CourseBizConvertor` 的跨 BC 使用点（优先，风险最低）**：其当前跨 BC 使用主要集中在 `bc-evaluation/infrastructure` 的 `MsgServiceImpl`（仅 1 处调用），可先把“`SingleCourseEntity -> SingleCourseCO` 的转换能力”收敛为 `bc-course/application` 的一个最小 Port（每刀 1 个文件闭环）：
        - 第 1 刀（单类）：新增 `bc-course/application` Port（建议命名：`SingleCourseCoConvertPort`），签名示例：`SingleCourseCO toSingleCourseCO(SingleCourseEntity courInf, Integer evaNum)`（不改业务语义）。
        - 第 2 刀（单类）：在 `bc-course/infrastructure` 增加 Port Adapter 实现（例如 `SingleCourseCoConvertPortImpl`），内部 **直接委托** 既有 `CourseBizConvertor.toSingleCourseCO(...)`（确保映射行为、空值/异常形态一致）。
        - 第 3 刀（单类）：修改 `bc-evaluation/infrastructure` 的 `MsgServiceImpl`，改为依赖该 Port（而非直接注入 `CourseBizConvertor`）；调用参数与异常文案保持不变。
        - 第 4 刀（单类搬运）：Serena 证伪 `CourseBizConvertor` 引用面仅剩 `bc-course/**` 后，将其从 `eva-infra-shared` 搬运归位到 `bc-course/infrastructure`（保持 `package` 不变），随后再逐 pom 收敛依赖。
     2) **再收敛 `CourseConvertor/UserConverter` 的跨 BC 使用点**：优先从 `bc-evaluation/infrastructure` 的 `EvaRecordQueryRepository/EvaTaskQueryRepository` 与 `bc-course/infrastructure` 的 `CourseQueryRepository` 入手，按“新增最小 Port → Port Adapter 原样复刻旧逻辑 → 调用侧替换”的节奏逐点清零对 Convertor 的跨 BC 直接依赖（每次只改 1 个文件闭环）。
4) 每刀结束必须闭环：
