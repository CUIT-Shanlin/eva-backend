...
> 下一步建议（读侧优先，单类闭环，保持行为不变）：继续清理评教读侧/评教侧对课程域其它 Mapper 的编译期直连，优先 `SubjectMapper`（候选：`EvaTaskQueryRepository`、`EvaRecordQueryRepository`）。端口前置 `SubjectObjectDirectQueryPort`（含适配器 `SubjectObjectDirectQueryPortImpl`）已就位，下一刀按“改调用侧（`bc-evaluation/infrastructure`，单类）”推进。端口前置 `CourseIdsByTeacherIdAndSemesterIdQueryPort` 仍无实现/无调用点，可按需后续补齐。

> 状态更新（2026-02-20，保持行为不变）：评教写侧 `repository` 与评教读侧 `query` 已清零对课程域 `CourseMapper/SemesterMapper` 的编译期直连；评教侧 `EvaUpdateGatewayImpl` 已清零对 `CourseMapper.selectById(...).getSemesterId()` 的编译期直连；当前评教读侧仍存在对课程域 `SubjectMapper` 的编译期直连点（待按同路线收敛）。
...
1) 读侧优先（保持行为不变）：评教读侧 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/bcevaluation/query` 已清零对课程域 `CourseMapper/SemesterMapper` 的编译期直连；评教侧 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/gateway/impl/eva/EvaUpdateGatewayImpl.java` 也已清零对 `CourseMapper.selectById(...).getSemesterId()` 的编译期直连（详见 0.9/4.2）。
   - 下一刀建议（保持行为不变；每次只改 1 个类闭环）：继续清理评教读侧对课程域 `SubjectMapper` 的编译期直连（候选：`EvaTaskQueryRepository`、`EvaRecordQueryRepository`），端口前置 `SubjectObjectDirectQueryPort`（含适配器 `SubjectObjectDirectQueryPortImpl`）已就位，下一刀直接按“改调用侧（`bc-evaluation/infrastructure`，单类）”推进。
