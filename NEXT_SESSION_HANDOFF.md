...
- 📊 **整体未完成清单（更新至 2026-02-17；保持行为不变）**：
  ...
  - `eva-infra-shared` 残留结构（Serena + `rg` 证据化，保持行为不变）：`CourseBizConvertor/CourseConvertor/UserConverter` 在 `eva-infra-shared/src/main/java` 内均仅自引用（无内部依赖）；LDAP 子簇已完成 `EvaLdapProperties/LdapGroupDO/LdapGroupRepo/LdapConstants/EvaLdapUtils` 下沉到 `shared-kernel`（保持包名不变）。
