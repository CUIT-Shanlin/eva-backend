...
**2026-01-13（本次会话：Controller 收敛推进（课程 + 评教 + 消息 + 日志），保持行为不变）**
	    ...
	    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
		  - ⏳ **仍未完成（核心阻塞项）**：
		    ...
		    2) `eva-adapter` 仍保留 **22 个** `*Controller.java`（口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`），但 ✅ **已阶段性闭环**：已按“每次只改 1 个 Controller”的模板完成这 22 个 Controller 的最小化收敛（固化返回/日志表达顺序、清理无用依赖/无用 import 等；保持行为不变；详见 0.9）。
		       - 补充说明（保持行为不变）：当前 `start` 仅显式依赖 `eva-adapter`，`eva-app` 通过传递依赖进入 classpath。若下一步要继续收敛“装配责任与依赖边界”，建议优先按 **S0.2 延伸（每次只改 1 个 pom）** 推进：先在 `start/pom.xml` 显式依赖 `eva-app`（保持行为不变），再在 `eva-adapter/pom.xml` 移除对 `eva-app` 的依赖（减少编译期耦合；保持行为不变）。
...
下一刀（保持行为不变）：
- `start/pom.xml`：显式增加对 `eva-app` 的依赖（保持行为不变；为后续在 `eva-adapter/pom.xml` 移除对 `eva-app` 的依赖做前置）
