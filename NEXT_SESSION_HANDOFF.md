...
- 下一会话优先建议（聚焦 **S0.2 延伸**，保持行为不变；每步只改 1 个小点，便于回滚）：
  1) **Serena 证据化盘点（先证伪再动手）**：优先选 1 个“疑似仅为类型引用而依赖 `bc-course`”的模块（建议从 `eva-adapter` / `eva-app` / `bc-evaluation-infrastructure` 中择一），证伪该模块是否还引用 `bc-course` 内部实现类（而不仅仅是 `edu.cuit.client.*` 类型）。
  2) **收敛 Maven 依赖（每次只改 1 个 pom）**：若证伪该模块仅使用 `edu.cuit.client.*`（现已由 `shared-kernel` 承载），则将其对 `bc-course` 的编译期依赖替换为显式依赖 `shared-kernel`（保持行为不变；最小回归通过）。
  3) **路线 B（后置，成本更高）**：若后续发现 `shared-kernel` 承载课程域接口/CO 规模继续膨胀，可再引入 `bc-course-contract`（或更中立的 contract 模块）承载这些接口/CO（保持 `package` 不变），并逐步把依赖方从 `shared-kernel` 切到该 contract（每步闭环；保持行为不变）。
...
1) 🎯 当前主线（**S0.2 延伸：收敛依赖方对 `bc-course` 的编译期依赖**，保持行为不变）：
   ...
   - 先用 Serena 选择 1 个模块做证伪（优先：`eva-app` 或 `eva-infra`；当前仓库内仅这两处仍显式依赖 `bc-course`，其余依赖方已按 0.9 收敛）：
