...
- 📌 **当前仍未完成（进入 S1 前置的硬事实口径，保持行为不变）**：
  - `eva-app` 仍残留 7 个 Java 文件（口径：`fd -t f -e java . eva-app/src/main/java | wc -l`）：3 个评教 Convertor（`EvaRecordBizConvertor/EvaTaskBizConvertor/EvaTemplateBizConvertor`）、`EvaConfigService`、2 个课程 Exec（`UserCourseDetailQueryExec/FileImportExec`）以及 `package-info.java`。
  - `eva-adapter` 仍残留 22 个 `*Controller.java`（口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`）。
- 🎯 **下一步建议（保持行为不变；每步只改 1 个类或 1 个 pom）**：优先从 `eva-app` 残留支撑类里挑 1 个最独立的类先归位（建议从 `EvaRecordBizConvertor` 或 `EvaConfigService` 起步），按“Serena → 最小回归 → 提交 → 三文档同步”的节奏推进；并行主线继续选择 1 个“依赖方 pom 收敛”目标（优先 `eva-domain` / `eva-infra-shared`）。
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：继续推进 **S0.2 延伸（依赖方编译期依赖收敛）**：选择下一条“依赖方编译期依赖收敛”目标（优先从 `eva-domain` / `eva-infra-shared` 等依赖方开始），用 Serena 证据化盘点引用面与依赖闭包后再收敛 `pom.xml`（每次只改 1 个 pom；保持行为不变）。
  - 📌 **当前仍未完成（截至 2026-01-16，口径=可复现命令；保持行为不变）**：
    ...
    - `eva-app` 仍作为“过渡装配/支撑类”模块存在：当前源码侧仅剩 **7 个** Java 文件（口径：`fd -t f -e java . eva-app/src/main/java | wc -l`；包括评教 Convertor、若干 Exec/Service 等）；组合根 `start` 仍对 `eva-app` 有编译期依赖（见 `start/pom.xml`）。
  ...
  - ✅ **`eva-client` 退场**：已从 reactor 移除并从仓库删除；跨 BC 通用对象已开始沉淀 `shared-kernel`（见 10.5）。
		  - ✅ **已闭环（本阶段目标 B：基础设施旧 gateway 归位，保持行为不变）**：`eva-infra/src/main/java/edu/cuit/infra/gateway/impl` 下残留旧 `*GatewayImpl.java` 已清零（Serena 盘点口径）；归位闭环包含：
		    ...
		    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
			  - ⏳ **仍未完成（核心阻塞项）**：
				    1) `eva-app` 已无 `*ServiceImpl.java`（口径：`fd -t f 'ServiceImpl\\.java$' eva-app/src/main/java | wc -l` → 0），但仍残留 **7 个**支撑类（口径：`fd -t f -e java . eva-app/src/main/java | wc -l`；示例：`Eva*BizConvertor/*Exec/EvaConfigService`）。下一步建议进入 **S1 前置**：继续逐个归位这些支撑类（每步只搬 1 个类；保持 `package` 与行为不变）。
...
### G 推荐版（当前主线：S1 前置（清空 eva-app 残留支撑类）+ 并行“依赖方 pom 收敛”，优先）
...
- 下一刀（每次只改 1 个 pom.xml 或 1 个类；保持行为不变）：
  1) Serena 证据化盘点：对 `eva-app/src/main/java` 残留支撑类（当前口径为 7 个 Java 文件）做“引用面 + 依赖闭包”盘点，优先挑选 1 个最独立、最不易引起装配漂移的类先归位（建议优先从 `EvaRecordBizConvertor` 或 `EvaConfigService` 开始）。
...
### E 专用简版（⏳ 未闭环：eva-adapter 仍残留 22 个 *Controller；用于下一阶段迁移入口）
