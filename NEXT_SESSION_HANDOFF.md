...
2) 🎯 下一步：继续推进 **S0.2 延伸（依赖方 `pom.xml` 依赖收敛）**：逐步减少依赖方对业务实现模块的编译期耦合（每次只改 1 个 `pom.xml`；保持行为不变；需先 Serena + `rg` 证据化确认仅类型引用、无实现/副作用耦合）
...
- 下一刀：优先从“仅使用 `edu.cuit.client.*` 类型”的依赖方模块入手，逐个证伪后每次只改 1 个 `pom.xml` 收敛编译期依赖边界（保持行为不变）。
