...
- ✅ 会话收口结论（保持行为不变）：已确认“完全独立（仅通过 contract/port 调用）”的关键阻塞仍是跨 BC 的 DAL 直连与共享模块依赖面：`bc-evaluation/**` 对课程 Mapper import 尚有 0 处（已按端口替换完成），`bc-template/**` 对评教 Mapper import 尚有 0 处（已清零编译期 import），`bc-course/**` 对评教 Mapper import 尚有 11 处（4 个文件，尚未清零）；且 `bc-course/bc-evaluation/bc-iam/bc-audit/bc-template/bc-messaging` 仍编译期依赖 `eva-infra-shared`。
- ⚠️ 工作区状态（重要，避免下个会话误判）：本会话仍处于“已暂存新增文件 + 未暂存代码改动”的混合状态，且尚有大量变更未提交（另已补 1 刀并提交：`e7e444a1`）。其中“新增端口/适配器/XML 归位文件”已 `git add`（可用 `git diff --cached --name-status` 复核）；其余 Java 改动仍在工作区未暂存（可用 `git diff --name-status` 复核）。
...
1) 证据口径（先跑快照，避免口径漂移）：
   ...
   - `rg -n "infra\\.dal\\.database\\.mapper\\.eva" bc-course | wc -l`（当前预期=11；文件数预期=4）
