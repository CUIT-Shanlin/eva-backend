...
>
> 📌 本会话关键增量（2026-02-24~2026-02-25，均已闭环且已推送；保持行为不变）：
...
> - ✅ 评教任务插入端口：在 `bc-evaluation/contract` 新增写侧端口 `EvaTaskInsertPort`，用于后续将课程写侧（如 `AssignEvaTeachersRepositoryImpl`）中对评教 `evaTaskMapper.insert` 的反射调用收敛为端口调用，并显式回传生成的 `taskId`（不引入缓存/切面副作用；入参为空 no-op；保持旧 insert + 主键回填语义不变；代码：`3facb7f1`；文档：本提交）。
> - ✅ 评教任务插入端口适配器：在 `bc-evaluation/infrastructure` 新增 `EvaTaskInsertPortImpl`，内部仅委托 `EvaTaskMapper.insert` 并回传主键 `taskId`（不引入缓存/切面副作用；入参为空 no-op；保持旧 insert + 主键回填语义不变；代码：`d7a31571`；文档：本提交）。
> - ✅ 课程分配评教老师链路去 `evaTaskMapper` 反射直连：`bc-course/infrastructure` 的 `AssignEvaTeachersRepositoryImpl` 已移除 `@Qualifier(\"evaTaskMapper\") Object` 注入与反射工具方法，统一改走评教端口 `EvaTaskByTeacherIdsAndStatusDirectQueryPort` + `EvaTaskInsertPort`（异常文案/日志文案/缓存失效与副作用顺序完全不变；尤其保持 `LogUtils.logContent(...)` 与 `invalidateCache(...)` 的相对顺序；最小回归通过；代码：`6926ada2`；文档：本提交）。
> - ✅ 课程读侧仓储去 `evaTaskMapper` 运行期依赖：`bc-course/infrastructure` 的 `CourseQueryRepository` 已移除 `@Qualifier(\"evaTaskMapper\") Object` 注入与反射 `selectList/selectCount` 调用，统一改走评教端口 `EvaTaskBriefByCourInfIdsDirectQueryPort`（保持异常文案/缓存/日志与副作用顺序完全不变；最小回归通过；代码：`65d1c784`；文档：本提交）。
...
> - ✅ 课程自助改课“评教时间冲突检查”改走端口：`UpdateSelfCourseRepositoryImpl` 已将上述 `courInfId` 列表查询从评教 `evaTaskMapper` 反射调用收敛为调用 `EvaTaskCourInfIdsByTeacherIdAndStatusDirectQueryPort`（代码：`794bc199`；文档：`42252083`）。
...
> - ✅ 课程自助删课链路评教端口化：`DeleteSelfCourseRepositoryImpl` 已清零对评教 `evaTaskMapper/formRecordMapper` 的反射调用，改为调用评教端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` + `FormRecordDeleteByTaskIdsPort` + `EvaTaskDeleteByCourInfIdsPort`（保持异常文案/缓存失效/副作用顺序完全不变；最小回归通过；代码：`089be092`）。
...
> - ✅ 评教任务按 courInfIds 批量删除端口适配器：在 `bc-evaluation/infrastructure` 新增 `EvaTaskDeleteByCourInfIdsPortImpl`（内部委托 `EvaTaskMapper.delete(in cour_inf_id)`；入参为空应为 no-op；不引入缓存/切面副作用；最小回归通过；代码：`73d98f77`）。
...
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：评教 `bc-evaluation/contract` 新增“按 courInfIds 读取任务最小视图”的协议对象 `EvaTaskBriefCO`（仅含 `id/teacherId/courInfId/status`；不引入 Mapper/DO；最小回归通过；代码落地：`017b16e3`）。后续已按下条完成刀 2（contract 端口）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：评教 `bc-evaluation/contract` 新增最小端口 `EvaTaskBriefByCourInfIdsDirectQueryPort`，用于跨 BC 按 courInfIds 读取 `EvaTaskBriefCO` 列表（约束：不引入缓存/切面副作用；最小回归通过；代码落地：`9457edb1`）。下一刀建议（写侧优先，保持行为不变）：按 0.11 的刀 3 在 `bc-evaluation/infrastructure` 落端口适配器实现（内部仍委托现有 Mapper/SQL）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：评教 `bc-evaluation/infrastructure` 新增端口适配器 `EvaTaskBriefByCourInfIdsDirectQueryPortImpl`，内部仅委托 `EvaTaskMapper.selectList(in cour_inf_id)` 并映射为 `EvaTaskBriefCO`（不引入缓存/切面副作用；查询条件/结果顺序与旧调用口径一致；最小回归通过；代码落地：`a2b5abdf`）。下一刀建议（写侧优先，保持行为不变）：按 0.11 的刀 4 回到 `bc-course/infrastructure` 替换一个删除类调用点，移除对 `EvaTaskDO` 编译期依赖。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：评教 `bc-evaluation/contract` 新增写侧端口 `EvaTaskCancelByTeacherIdAndCourInfIdPort`，用于课程侧“修改课次/时间导致取消评教任务”链路按 `teacherId + courInfId` 将评教任务置为取消状态（当前约定：将 `eva_task.status` 更新为 `2`；不引入缓存/切面副作用；最小回归通过；代码落地：`6632b3fe`）。后续已按下条完成刀 2（infra 适配器）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：评教 `bc-evaluation/infrastructure` 新增取消端口适配器 `EvaTaskCancelByTeacherIdAndCourInfIdPortImpl`，内部仅委托 `EvaTaskMapper.update(...)` 将 `eva_task.status` 更新为 `2`（不引入缓存/切面副作用；空入参 no-op；最小回归通过；代码落地：`656ca526`）。下一刀建议（写侧优先，保持行为不变）：回到课程写侧选择 1 个调用点（推荐 `UpdateSingleCourseRepositoryImpl`）移除 `evaTaskMapper` 反射调用并改走该端口（注意：其现有逻辑使用 `cour_inf_id = courINfo.getCourseId()` 的历史口径，必须保持不变，仅替换调用方式与依赖方向）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：课程写侧 `bc-course/infrastructure` 的 `UpdateSingleCourseRepositoryImpl` 已将“取消评教任务”的评教 `evaTaskMapper` 反射调用收敛为调用评教端口：通过 `EvaTaskBriefByCourInfIdsDirectQueryPort` 获取任务最小视图并筛选 `status=0`，再调用 `EvaTaskCancelByTeacherIdAndCourInfIdPort` 执行取消（不引入缓存/切面副作用；仍保持 `cour_inf_id = courINfo.getCourseId()` 的历史口径不变；最小回归通过；代码落地：`b9178e9a`）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：课程写侧 `bc-course/infrastructure` 的 `UpdateSelfCourseRepositoryImpl` 已将“自助改课时用于判断评教时间冲突”的评教任务 `courInfId` 列表查询，从评教 `evaTaskMapper.selectList(eq teacher_id, status=0)` 的反射调用收敛为调用评教直查端口 `EvaTaskCourInfIdsByTeacherIdAndStatusDirectQueryPort`（不引入缓存/切面副作用；异常文案/副作用顺序不变；最小回归通过；代码落地：`794bc199`）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：评教 `bc-evaluation/contract` 新增写侧端口 `EvaTaskDeleteByCourInfIdPort`，用于跨 BC 在课程侧“自助改课删除某些课次（cour_inf）”链路中按 `courInfId` 删除评教任务（保持旧调用语义：仅删除 `eva_task`，不做级联删除；不引入缓存/切面副作用；入参为空 no-op；最小回归通过；代码落地：`6b304d4b`）。下一刀建议（保持行为不变）：在 `bc-evaluation/infrastructure` 落端口适配器实现（内部委托现有 `EvaTaskMapper.delete(eq cour_inf_id)`）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：评教 `bc-evaluation/contract` 新增最小直查端口 `EvaTaskCourInfIdsByTeacherIdAndStatusDirectQueryPort`，用于跨 BC 以 `teacherId + status` 获取评教任务关联的 `courInfId` 列表（不引入缓存/切面副作用；入参为空 no-op；最小回归通过；代码落地：`d9846645`）。后续已按下条完成刀 2（infra 适配器）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：评教 `bc-evaluation/infrastructure` 新增端口适配器 `EvaTaskCourInfIdsByTeacherIdAndStatusDirectQueryPortImpl`，内部仅委托 `EvaTaskMapper.selectList(eq teacher_id, status)` 并映射为 `courInfId` 列表（不引入缓存/切面副作用；查询条件、结果顺序与空值语义保持不变；最小回归通过；代码落地：`0f4fa7d3`）。后续已按本会话推进：课程写侧 `UpdateSelfCourseRepositoryImpl` 已清零评教 `evaTaskMapper` 的反射调用并改走评教端口（见 0.9 关键增量）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：为承接“课程写侧删课链路改走评教 contract 端口”的编译闭合前置，在 `bc-course-infra` 增加对 `bc-evaluation-contract` 的 Maven 编译期依赖（最小回归通过；代码落地：`fad6220f`）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：课程写侧删除链路 `DeleteCourseRepositoryImpl` 已将“按 courInfIds 查询评教任务列表”的实现改为调用评教端口 `EvaTaskBriefByCourInfIdsDirectQueryPort`，并清零对 `edu.cuit.infra.dal.database.dataobject.eva.*`（`EvaTaskDO/FormRecordDO`）的编译期依赖（异常文案/缓存/日志/副作用顺序不变；最小回归通过；代码落地：`08fe8079`）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：评教 `bc-evaluation/contract` 新增写侧最小端口 `EvaTaskCascadeDeleteByTaskIdsPort`，用于在删除链路中按 taskIds 级联删除 `eva_task` 与 `form_record`（约束：不引入缓存/切面副作用；保持删除顺序“先任务后记录”；最小回归通过；代码落地：`975cd9fb`）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：评教 `bc-evaluation/infrastructure` 新增端口适配器 `EvaTaskCascadeDeleteByTaskIdsPortImpl`，内部委托 `EvaTaskMapper.delete(in id)` + `FormRecordMapper.delete(in task_id)` 实现 taskIds 级联删除（不引入缓存/切面副作用；删除顺序保持“先任务后记录”；最小回归通过；代码落地：`1d69770a`）。下一刀建议（写侧优先，保持行为不变）：回到 `bc-course/infrastructure` 将反射删除调用点收敛为调用该写端口（保持删除次数/顺序与缓存失效顺序不变）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：课程写侧删除链路 `DeleteCourseRepositoryImpl` 已将“按 taskIds 删除评教任务 + 评教记录”的实现改为调用评教写端口 `EvaTaskCascadeDeleteByTaskIdsPort`，并移除对 `evaTaskMapper/formRecordMapper` 的反射删除调用点（保持删除顺序与缓存失效顺序不变；最小回归通过；代码落地：`19d9ebdc`）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：课程写侧删除链路 `DeleteCoursesRepositoryImpl` 已将“按 courInfIds 查询评教任务 + 删除评教任务/记录”的实现改为调用评教直查端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` + 写端口 `EvaTaskCascadeDeleteByTaskIdsPort`，并清零对 `edu.cuit.infra.dal.database.dataobject.eva.*` 的编译期依赖与反射删除调用点（保持删除顺序与缓存失效顺序不变；最小回归通过；代码落地：`55d27784`）。

> ⚠️ 本会话执行环境注意（2026-02-23）：本地已提交但可能尚未推送到远端（以 `git status -sb` 是否出现 `ahead N` 为准）；当前环境对 GitHub `git push` 可能因 SSH Key/Agent 权限问题失败（现象：`Permission denied (publickey)` 或连接被关闭）。若出现该情况：先在本机解锁/授权 SSH Agent 或改用 HTTPS + PAT，再执行 `git push`；不要通过 `reset/checkout` 回滚工作区以规避 push。
...
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：IAM 新增用户详情直查端口 `UserDetailByUsernameDirectQueryPort`（约束：不走缓存/切面副作用），用于后续将课程写侧等调用点从 `sysUserMapper.selectOne(eq username)` 的跨 BC 直连写法收敛为调用端口（最小回归通过；代码落地：`c1cc2a56`）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：IAM 已新增端口适配器 `UserDetailByUsernameDirectQueryPortImpl`，内部仅委托 `SysUserMapper.selectOne(eq username)` 并映射为 `UserDetailCO(id/name/username)`（不引入缓存/切面副作用；最小回归通过；代码落地：`644fe2c9`）。
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：IAM 已新增端口适配器 `UserIdByUsernameDirectQueryPortImpl`，内部仅委托 `SysUserMapper.selectOne(eq username).getId()`（不引入缓存/切面副作用；最小回归通过；代码落地：`1e8c8b99`）。
...
> ✅ 会话快照（2026-02-23，保持行为不变）：课程写侧 `bc-course/infrastructure/src/main/java/edu/cuit/infra/bccourse/adapter/` 内已不再出现 `@Qualifier(\"sysUserMapper\")`（口径：`rg -n '@Qualifier\\(\"sysUserMapper\"\\)' bc-course/infrastructure/src/main/java/edu/cuit/infra/bccourse/adapter` = 0）；即“课程写侧跨 BC 直连 IAM Mapper（哪怕反射）”已阶段性清零，统一改走 `bc-iam-contract` 的直查端口（不引入缓存/切面副作用）。
> ⚠️ 下一阶段主线（写侧优先，保持行为不变）：继续推进“跨 BC 只允许依赖 contract/shared-kernel”的更强边界——课程写侧仍存在对评教侧 Mapper/DO 的运行期直连（例如 `@Qualifier(\"evaTaskMapper\")` + 反射调用、以及 `edu.cuit.infra.dal.database.dataobject.eva.*` 的编译期引用），需按“补评教 contract 最小端口 → 补端口适配器 → 改课程调用侧”的逐刀路线收敛（每刀只改 1 个类闭环）。
...
**2026-02-25（评教读侧：新增按状态列表直查 courInfIds 端口；保持行为不变）**
- ✅ 执行（单类，保持行为不变）：在 `bc-evaluation/contract` 新增直查端口 `EvaTaskCourInfIdsByStatusesDirectQueryPort`，用于后续将课程侧 `CourseRecommendExce` 中“按 status(0/1) 查询任务列表”的评教 `evaTaskMapper` 反射调用收敛为端口调用（约束：不引入缓存/切面副作用；入参为空 no-op）。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`2814143d`。

**2026-02-25（评教读侧：补齐按状态列表直查 courInfIds 端口适配器；保持行为不变）**
- ✅ 执行（单类，保持行为不变）：在 `bc-evaluation/infrastructure` 新增端口适配器 `EvaTaskCourInfIdsByStatusesDirectQueryPortImpl`，内部仅委托 `EvaTaskMapper.selectList(in status)` 并映射为 `courInfId` 列表（不引入缓存/切面副作用；查询条件/结果顺序/空值语义保持与旧 Mapper 调用一致）。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`db28b332`。

**2026-02-25（课程域写侧：`CourseRecommendExce` 去评教 `evaTaskMapper` 运行期依赖；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`CourseRecommendExce` 内仍通过 `@Qualifier("evaTaskMapper") Object` + 反射 `selectList/selectCount` 读取评教任务列表与计数；需收敛为调用评教 `contract` 端口以满足“跨 BC 只依赖 contract/shared-kernel”的边界约束。
- ✅ 执行（单类，保持行为不变）：移除 `@Qualifier("evaTaskMapper") Object` 注入与反射调用；将“teacherId+status=0 的 courInfIds 查询”改走评教直查端口 `EvaTaskCourInfIdsByTeacherIdAndStatusDirectQueryPort`，将“status(0/1) 的 courInfIds 查询”改走 `EvaTaskCourInfIdsByStatusesDirectQueryPort`，将“按 courInfIds 统计 status(0/1) 任务数”改走 `EvaTaskBriefByCourInfIdsDirectQueryPort` 并在调用侧过滤；同时清零本类对评教 DO（`EvaTaskDO`）的编译期依赖（异常文案/副作用顺序保持不变）。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`af084dec`。
...
**2026-02-25（template：`CourseTemplateLockQueryPortImpl` 去评教 `evaTaskMapper` 运行期依赖；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`CourseTemplateLockQueryPortImpl` 内仍通过 `@Qualifier("evaTaskMapper") Object` + 反射 `selectList` 直连评教侧实现，用于计算“模板是否锁定”；需收敛为调用评教 `contract` 端口以满足“跨 BC 只依赖 contract/shared-kernel”的边界约束。
- ✅ 执行（单类，保持行为不变）：移除 `@Qualifier("evaTaskMapper") Object` 注入与反射直连；改为注入评教直查端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` 获取任务 `taskIds`，保持锁定判定的分支与顺序不变。**注意**：本条记录为当时状态；后续已通过端口化清零 `courOneEvaTemplateMapper/formRecordMapper` 的运行期直连与对评教 DO 的编译期依赖（见 `f757abd9`）。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`e7ab432f`。
- 📝 交接与计划同步：本会话结束前需将“已完成项 + 下一步计划 + 新对话提示词”同步到三文档；请以下一会话开始时的 `git log -n 1 -- NEXT_SESSION_HANDOFF.md`/`DDD_REFACTOR_PLAN.md`/`docs/DDD_REFACTOR_BACKLOG.md` 为准（不要在提示词中固化 commitId）。

**2026-02-25（评教读侧：模板锁定判定证据直查端口前置；保持行为不变）**
- ✅ 执行（单类，保持行为不变）：在 `bc-evaluation/contract` 新增直查端口 `CourseTemplateLockEvaEvidenceDirectQueryPort`，用于后续承接模板侧 `CourseTemplateLockQueryPortImpl` 对评教 `cour_one_eva_template/form_record` 的两处剩余直连（不引入缓存/切面副作用；入参为空/空列表按 no-op 处理并返回 false）。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`754a0b0f`。

**2026-02-25（评教读侧：模板锁定判定证据直查端口适配器落地；保持行为不变）**
- ✅ 执行（单类，保持行为不变）：在 `bc-evaluation/infrastructure` 新增端口适配器 `CourseTemplateLockEvaEvidenceDirectQueryPortImpl`，内部仅委托 `CourOneEvaTemplateMapper/FormRecordMapper.selectCount(...)` 查询证据存在性（不引入缓存/切面副作用；查询条件与旧 Mapper 调用保持一致；入参为空/空列表 no-op）。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`10281eec`。

**2026-02-25（template：`CourseTemplateLockQueryPortImpl` 去评教快照/记录直连与 DO 编译期依赖；保持行为不变）**
- ✅ 执行（单类，保持行为不变）：`CourseTemplateLockQueryPortImpl` 改为注入并调用评教证据直查端口 `CourseTemplateLockEvaEvidenceDirectQueryPort`，替换对 `courOneEvaTemplateMapper/formRecordMapper` 的运行期直连，并清零对评教 DO 的编译期依赖；锁定判定分支与顺序保持不变。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`f757abd9`。
...
**2026-02-25（课程域：`CourseBizConvertor` 去评教 `FormTemplateDO` 编译期依赖；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`CourseBizConvertor` 仍需要在 course 侧构造 `EvaTemplateCO(id,isDefault)` 用于 `CourseModelCO.templateMsg`；但其对评教 `FormTemplateDO` 的编译期类型依赖会阻碍“跨 BC 只允许依赖 contract/shared-kernel”的边界收敛。
- ✅ 执行（单类，保持行为不变）：将 `toEvaTemplateCO(...)` 改为接收 `Object` 并通过反射读取 `getId/getIsDefault` 构造 `EvaTemplateCO`；`selectFormTemplateById(...)` 返回 `Object`，保持 MyBatis `selectById` 的调用次数/顺序不变；异常包装口径保持 `RuntimeException` 不变。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`6ad09180`。
...
**2026-02-21（审计日志：`LogGatewayImpl` 提炼日志映射列表构建；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`LogGatewayImpl.page` 的调用点在 `LogServiceImpl.page`；本刀仅做类内重构，不改 SQL 条件/排序/异常文案与副作用顺序。
- ✅ 执行（单类，保持行为不变）：提炼 `toSysLogEntityList(...)`，保持 `page` 内 `SysLogDO -> SysLogEntity` 映射顺序不变；并简化 `insertLog` 的异步 lambda 写法（仍使用同一 `executor`，不改变异步触发时机）。
- 🧪 最小回归通过（Java17）：按 0.11 命令执行；`mvnd` 启动阶段报 `java.lang.ExceptionInInitializerError`，已按约束降级使用 `mvn` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`63565d73`。
...
**2026-02-23（评教写侧：`DeleteEvaRecordRepositoryImpl` 反射异常处理去重复；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`selectSysUserNameById(...)` 内 `InvocationTargetException` 目标异常解包逻辑为内联重复写法；可抽取公共方法而不改变异常语义与反射调用位置。
- ✅ 执行（单类，保持行为不变）：抽取 `rethrowInvocationTargetException(...)` 并在 `selectSysUserNameById(...)` 复用；保持对 `RuntimeException/Error` 的原样抛出与其余异常包装口径不变。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`a1773f2b`。

**2026-02-22（评教读侧：`EvaStatisticsQueryRepository` 反射异常处理去重复；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`selectSysUserList/selectSysUserById/selectSysUserId/selectSysUserNameById/selectSysUserDepartmentById` 的 `InvocationTargetException` 解包逻辑重复；可抽取公共方法而不改变异常语义与反射调用位置。
- ✅ 执行（单类，保持行为不变）：抽取 `rethrowInvocationTargetException(...)` 并在上述五处复用；保持对 `RuntimeException/Error` 的原样抛出与其余异常包装口径不变。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`bf79e34e`。

**2026-02-22（评教读侧：`EvaRecordQueryRepository` 反射异常处理去重复；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`selectSysUserList/selectSysUserById` 的 `InvocationTargetException` 解包逻辑重复；可抽取公共方法而不改变异常语义与反射调用位置。
- ✅ 执行（单类，保持行为不变）：抽取 `rethrowInvocationTargetException(...)` 并在两处复用；保持对 `RuntimeException/Error` 的原样抛出与其余异常包装口径不变。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`479ce62a`。

**2026-02-22（评教读侧：`EvaTaskQueryRepository` 反射异常处理去重复；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`selectSysUserList/selectSysUserById/selectSysUserId/selectSysUserNameById` 的 `InvocationTargetException` 解包逻辑重复；可抽取公共方法而不改变异常语义与反射调用位置。
- ✅ 执行（单类，保持行为不变）：抽取 `rethrowInvocationTargetException(...)` 并在上述四处复用；保持对 `RuntimeException/Error` 的原样抛出与其余异常包装口径不变。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`84f5d6b0`。

**2026-02-22（评教写侧：`PostEvaTaskRepositoryImpl` 反射异常处理去重复；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`selectSysUserById/selectSysUserId/selectSysUserNameById` 的 `InvocationTargetException` 解包逻辑重复；调用点集中在 `create/invalidateTaskListCaches` 相关链路；可抽取公共方法而不改变异常语义。
- ✅ 执行（单类，保持行为不变）：抽取 `rethrowInvocationTargetException(...)` 并在上述三处复用；保持对 `RuntimeException/Error` 的原样抛出与其余异常包装口径不变。
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`0fcca08c`。
...
**2026-02-22（评教写侧：`DeleteEvaRecordRepositoryImpl` 提炼删除后置流程；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`DeleteEvaRecordRepositoryImpl.delete` 被 `DeleteEvaRecordUseCase` 与 `EvaDeleteGatewayImpl.deleteEvaRecord` 调用；本刀仅做类内重构，不改任何 DB 查询条件/异常文案/日志文案与副作用顺序。
- ✅ 执行（单类，保持行为不变）：将“删记录后：判断课程下是否仍有评教记录 → 可能删除课程模板分配 → 失效缓存”的后置流程提炼为 `deleteCourseTemplateIfNoEvaRecordExists(...)` 与 `invalidateEvaRecordCaches(...)`，保持执行顺序与异常文案不变。
- 🧪 最小回归通过（Java17）：按 0.11 命令执行；使用 `mise exec java@temurin-17 -- mvn ...` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`4ef05cb2`。

**2026-02-22（评教写侧：`EvaDeleteGatewayImpl` 提炼删除异常适配逻辑；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`EvaTaskServiceImpl` 注入并调用 `EvaDeleteGateway`；本刀仅在 `EvaDeleteGatewayImpl` 内部抽取辅助方法，不改变事务边界（`@Transactional` 仍在公开方法）、异常文案与副作用顺序。
- ✅ 执行（单类，保持行为不变）：将 `deleteEvaRecord/deleteEvaTemplate` 内重复的 try/catch 异常映射逻辑提炼为 `deleteEvaRecordOrThrow/deleteEvaTemplateOrThrow`；同时仅做代码格式统一，不改任何业务语义。
- 🧪 最小回归通过（Java17）：按 0.11 命令执行；使用 `mise exec java@temurin-17 -- mvn ...` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`6126ddcb`。

**2026-02-22（评教写侧：`DeleteEvaTemplateRepositoryImpl` 提炼模板缓存失效调用；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`DeleteEvaTemplateRepositoryImpl.delete` 被 `DeleteEvaTemplateUseCase` 与 `EvaDeleteGatewayImpl.deleteEvaTemplate` 调用；本刀仅做类内重构，不改任何 DB 查询条件/异常文案/日志文案与副作用顺序。
- ✅ 执行（单类，保持行为不变）：将模板删除后的两处缓存失效调用提炼为 `invalidateTemplateCaches(...)`，保持调用顺序与参数构造口径不变。
- 🧪 最小回归通过（Java17）：按 0.11 命令执行；使用 `mise exec java@temurin-17 -- mvn ...` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`479c5500`。
...
**2026-02-22（评教写侧：`PostEvaTaskRepositoryImpl` 提炼发起评教任务缓存失效逻辑；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`PostEvaTaskRepositoryImpl.create` 为发起评教任务写侧端口适配器方法；本刀仅做类内重构，不改 DB 插入、异常文案与冲突校验逻辑。
- ✅ 执行（单类，保持行为不变）：将发起评教任务后的缓存失效调用提炼为 `invalidateTaskListCaches(...)`，保持失效顺序不变：按 `semesterId` 失效 `TASK_LIST_BY_SEM` → 按 `teacherId` 失效 `TASK_LIST_BY_TEACH`（`selectSysUserNameById(...)` 仍在第二次失效时触发）。
- 🧪 最小回归通过（Java17）：按 0.11 命令执行；使用 `mise exec java@temurin-17 -- mvn ...` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`59c0a974`。
...
**2026-02-21（课程域：新增 `CourseIdsByCourseWrapperDirectQueryPort` 的端口适配器；保持行为不变）**
- ✅ 执行（单类，保持行为不变）：在 `bc-course/infrastructure` 新增端口适配器 `CourseIdsByCourseWrapperDirectQueryPortImpl`，内部仅委托 `CourseMapper.selectList(...)` 并映射 `CourseDO::getId` 返回课程ID列表（保持查询条件、结果顺序与空值语义不变；不引入新副作用）。
- 🧪 最小回归通过（Java17）：按 0.11 命令执行；`mvnd` 启动阶段报 `java.lang.ExceptionInInitializerError`，已按约束降级使用 `mvn` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`cc79d929`。
...
**2026-02-21（评教读侧：`EvaRecordQueryRepository` 收敛课程ID列表查询改走 `CourseIdsByCourseWrapperDirectQueryPort`；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`EvaRecordQueryRepository` 内存在“按 Wrapper 查 CourseDO 列表后映射 `CourseDO::getId`”的残留点（涉及 `pageEvaRecord` 与 `getEvaLogInfo`），且其查询条件由调用方构造 Wrapper（含 `in/eq` 等组合），适合改为调用“按 Wrapper 直查课程ID列表”的端口以保持 SQL 条件与结果顺序不变。
- ✅ 执行（单类，保持行为不变）：将 `pageEvaRecord`、`getEvaLogInfo` 中课程ID列表获取逻辑改为调用 `CourseIdsByCourseWrapperDirectQueryPort.findCourseIds(wrapper)`，保持 Wrapper 构造、分支返回与后续查询顺序不变。
- 🧪 最小回归通过（Java17）：按 0.11 命令执行；`mvnd` 启动阶段报 `java.lang.ExceptionInInitializerError`，已按约束降级使用 `mvn` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`7d3b1844`。
...
- ✅ 执行（单类，保持行为不变）：将 `getEvaTaskIdS` 在 `semId!=null` 分支改为调用 `CourseIdsBySemesterIdQueryPort.findCourseIdsBySemesterId(semId)`；其余逻辑与返回条件不变。
- 🧪 最小回归通过（Java17）：按 0.11 命令执行；`mvnd` 启动阶段报 `java.lang.ExceptionInInitializerError`，已按约束降级使用 `mvn` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`265c1301`。

**2026-02-20（课程域：新增科目基础对象直查端口 `SubjectObjectDirectQueryPort`，为评教读侧去 `SubjectMapper` 编译期直连做前置；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：评教读侧 `bc-evaluation/infrastructure` 当前仍存在对课程域 `SubjectMapper` 的编译期直连点（候选：`EvaTaskQueryRepository`、`EvaRecordQueryRepository`），需按“端口化”逐刀收敛以避免跨 BC DAL 依赖扩散。
- ✅ 执行（单类，保持行为不变）：在 `bc-course/application` 新增最小查询端口 `SubjectObjectDirectQueryPort`（仅接口，无实现/无调用点，避免行为漂移）；后续按“补适配器（单类）→ 改调用侧（单类）”继续推进。
- 🧪 最小回归通过（Java17）：`mvnd` 启动阶段报 `java.lang.ExceptionInInitializerError`；已按约束降级 `mvn` 完成最小回归（`EvaRecordServiceImplTest/EvaStatisticsServiceImplTest` 通过）。
- 📌 代码落地：`a68c1b74`。

**2026-02-20（课程域：新增 `SubjectObjectDirectQueryPort` 的端口适配器；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`SubjectMapper` 位于 `bc-course/infrastructure`，可按既有 adapter 范式原样委托查询（不引入新副作用）。
- ✅ 执行（单类，保持行为不变）：新增 `SubjectObjectDirectQueryPortImpl`，内部仅委托 `SubjectMapper.selectList/selectById` 并返回（保持查询条件、结果顺序与空值语义不变）。
...
**2026-02-20（评教读侧：`EvaTaskQueryRepository` 去课程域 `SubjectMapper` 编译期直连，改走 `SubjectObjectDirectQueryPort`；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`EvaTaskQueryRepository` 内对 `SubjectMapper` 的 import/注入/调用点已清零，改为注入并调用 `SubjectObjectDirectQueryPort.findSubjectList/findSubjectById`；查询次数/顺序与空值语义保持不变。
- ✅ 执行（单类，保持行为不变）：将 `pageEvaUnfinishedTask/evaSelfTaskInfo/toCourseEntity` 中涉及科目查询的调用统一收敛为调用端口（端口适配器侧仍原样委托课程域 `SubjectMapper`，确保查询条件、结果顺序与空值语义不漂移）。
- 🧪 最小回归通过（Java17）：`mvnd` 启动阶段报 `java.lang.ExceptionInInitializerError`；已按约束降级使用 `mvn` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`0b11f2c4`。

**2026-02-20（评教读侧：`EvaRecordQueryRepository` 去课程域 `SubjectMapper` 编译期直连，改走 `SubjectObjectDirectQueryPort`；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`EvaRecordQueryRepository` 内对 `SubjectMapper` 的 import/注入/调用点已清零，改为注入并调用 `SubjectObjectDirectQueryPort.findSubjectList/findSubjectById`；查询次数/顺序与空值语义保持不变。
- ✅ 执行（单类，保持行为不变）：将 `pageEvaRecord/getEvaLogInfo/toCourseEntity` 中涉及科目查询的调用统一收敛为调用端口（端口适配器侧仍原样委托课程域 `SubjectMapper`，确保查询条件、结果顺序与空值语义不漂移）。
- 🧪 最小回归通过（Java17）：`mvnd` 启动阶段报 `java.lang.ExceptionInInitializerError`；已按约束降级使用 `mvn` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`cfae3eba`。

**2026-02-20（课程域：补齐 `CourseIdsByTeacherIdAndSemesterIdQueryPort` 的端口适配器；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`CourseIdsByTeacherIdAndSemesterIdQueryPort` 已存在但此前无适配器实现；该端口用于后续收敛“按 teacherId + semesterId 查询课程ID列表”的跨 BC DAL 直连场景（保持查询条件、结果顺序与空值语义不变）。
- ✅ 执行（单类，保持行为不变）：在 `bc-course/infrastructure` 新增 `CourseIdsByTeacherIdAndSemesterIdQueryPortImpl`，内部仅委托 `CourseMapper.selectList(eq teacher_id, semester_id)` 并映射 `CourseDO::getId` 返回课程ID列表（不引入新副作用）。
- 🧪 最小回归通过（Java17）：`mvnd` 启动阶段报 `java.lang.ExceptionInInitializerError`；已按约束降级 `mvn` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`7c8fc78f`。

**2026-02-20（评教读侧：`EvaStatisticsQueryRepository` 改走 `CourseIdsByTeacherIdAndSemesterIdQueryPort` 获取课程ID列表；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`EvaStatisticsQueryRepository.getEvaEdNumByTeacherId` 原实现为 `CourseAndSemesterObjectDirectQueryPort.findCourseList(eq teacher_id, semester_id)` 后映射 `CourseDO::getId`；可在不改变查询条件/结果顺序/空值语义的前提下，收敛为调用最小端口返回课程ID列表。
- ✅ 执行（单类，保持行为不变）：将 `getEvaEdNumByTeacherId` 在 `semId!=null` 分支改为调用 `CourseIdsByTeacherIdAndSemesterIdQueryPort.findCourseIdsByTeacherIdAndSemesterId(teacherId, semId)`；其余逻辑与返回条件不变。
- 🧪 最小回归通过（Java17）：`mvnd` 启动阶段报 `java.lang.ExceptionInInitializerError`；已按约束降级 `mvn` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`c3dd9744`。

**2026-02-20（评教读侧：`EvaRecordQueryRepository` 改走 `CourseIdsByTeacherIdAndSemesterIdQueryPort` 获取课程ID列表；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`EvaRecordQueryRepository.getEvaEdLogInfo` 原实现为 `CourseAndSemesterObjectDirectQueryPort.findCourseList(eq teacher_id, semester_id)` 后映射 `CourseDO::getId`；可在不改变查询条件/结果顺序/空值语义的前提下，收敛为调用最小端口返回课程ID列表。
- ✅ 执行（单类，保持行为不变）：将 `getEvaEdLogInfo` 在 `courseId` 为空且 `semId!=null` 分支改为调用 `CourseIdsByTeacherIdAndSemesterIdQueryPort.findCourseIdsByTeacherIdAndSemesterId(userId, semId)`；其余逻辑与返回条件不变。
- 🧪 最小回归通过（Java17）：`mvnd` 启动阶段报 `java.lang.ExceptionInInitializerError`；已按约束降级 `mvn` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`8c8a4ece`。
...
**2026-02-20（评教读侧：`EvaRecordQueryRepository` 改走 `CourseIdsByTeacherIdQueryPort` 获取课程ID列表；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`EvaRecordQueryRepository.getEvaEdLogInfo` 在 `courseId` 为空且 `semId==null` 分支原实现为 `CourseAndSemesterObjectDirectQueryPort.findCourseList(eq teacher_id)` 后映射 `CourseDO::getId`；可在不改变查询条件/结果顺序/空值语义的前提下，收敛为调用最小端口返回课程ID列表。
- ✅ 执行（单类，保持行为不变）：将 `getEvaEdLogInfo` 在 `courseId` 为空且 `semId==null` 分支改为调用 `CourseIdsByTeacherIdQueryPort.findCourseIdsByTeacherId(userId)`；其余逻辑与返回条件不变。
- 🧪 最小回归通过（Java17）：`mvnd` 启动阶段报 `java.lang.ExceptionInInitializerError`；已按约束降级 `mvn` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`510a4962`。
...
**2026-02-20（评教读侧：`EvaRecordQueryRepository` 去课程域 `CourseMapper/SemesterMapper` 编译期直连，改走 `CourseAndSemesterObjectDirectQueryPort`；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`EvaRecordQueryRepository` 内对 `CourseMapper/SemesterMapper` 的 import/注入/调用点已清零，改为注入并调用 `CourseAndSemesterObjectDirectQueryPort.findCourseList/findCourseById/findOneCourse/findSemesterById`；查询条件、异常文案与副作用顺序保持不变。
- ✅ 执行（单类，保持行为不变）：将 `pageEvaRecord/evaUserEvaRecords/getEvaEdLogInfo/getRecordByCourse/getEvaNumByCourse/toCourseEntity/getListCurInfoEntities` 中涉及课程/学期查询的调用统一收敛为调用端口（端口适配器侧仍原样委托课程域 Mapper，确保空值/NPE 触发语义不漂移）。
- 🧪 最小回归通过（Java17）：按 0.11 命令执行；`mvnd` 启动阶段报 `java.lang.ExceptionInInitializerError`，已按约束降级使用 `mvn` 完成最小回归（测试用例集保持不变）。
- 📌 代码落地：`75494f6c`。
...
**2026-02-20（课程域：新增按学期ID查询课程ID列表端口 `CourseIdsBySemesterIdQueryPort`，为评教读侧去 `CourseMapper.selectList(eq semester_id)` 直连做前置；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：评教读侧 `EvaTemplateQueryRepository.getEvaTaskIdS(semId)` 当前通过 `CourseMapper.selectList(new QueryWrapper<CourseDO>().eq(\"semester_id\", semId))` 获取课程ID列表，再映射为评教任务列表；该调用点属于跨 BC DAL 编译期直连入口。
- ✅ 执行（单类，保持行为不变）：在 `bc-course/application` 新增最小查询端口 `CourseIdsBySemesterIdQueryPort.findCourseIdsBySemesterId(Integer)`，仅定义接口，不引入实现与调用点，避免行为漂移；后续按“补适配器（单类）→ 改调用侧（单类）”继续推进。
- 🧪 最小回归通过（Java17）：`mvnd` 启动阶段仍报 `java.lang.ExceptionInInitializerError`；已按约束降级 `mvn` 完成最小回归（`EvaRecordServiceImplTest/EvaStatisticsServiceImplTest` 通过）。
- 📌 代码落地：`183aae12`。

**2026-02-20（课程域：新增 `CourseIdsBySemesterIdQueryPort` 的端口适配器；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`CourseMapper` 位于 `bc-course/infrastructure`，可按既有 adapter 范式直接委托查询。
- ✅ 执行（单类，保持行为不变）：新增 `CourseIdsBySemesterIdQueryPortImpl`，内部仅委托 `CourseMapper.selectList(new QueryWrapper<CourseDO>().eq(\"semester_id\", semId))` 并映射 `CourseDO::getId` 返回课程ID列表（不引入新副作用、不改变结果顺序/空值语义）。
- 🧪 最小回归通过（Java17）：`mvnd` 启动阶段仍报 `java.lang.ExceptionInInitializerError`；已按约束降级 `mvn` 完成最小回归（`EvaRecordServiceImplTest/EvaStatisticsServiceImplTest` 通过）。
- 📌 代码落地：`d91536a0`。

**2026-02-20（评教读侧：`EvaTemplateQueryRepository` 去课程 `CourseMapper.selectList(eq semester_id)` 编译期直连，改走 `CourseIdsBySemesterIdQueryPort`；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`EvaTemplateQueryRepository.getEvaTaskIdS(semId)` 在 `semId != null` 分支内通过 `CourseMapper.selectList(new QueryWrapper<CourseDO>().eq(\"semester_id\", semId))` 拉取课程列表并映射 `courseIdS`，属于跨 BC DAL 编译期直连入口。
- ✅ 执行（单类，保持行为不变）：将上述调用点收敛为调用 `bc-course/application` 查询端口 `CourseIdsBySemesterIdQueryPort.findCourseIdsBySemesterId(semId)` 获取 `courseIdS`，后续逻辑与缓存 key/写入时机保持不变。
- 🧪 最小回归通过（Java17）：`mvnd` 启动阶段仍报 `java.lang.ExceptionInInitializerError`；已按约束降级 `mvn` 完成最小回归（`EvaRecordServiceImplTest/EvaStatisticsServiceImplTest` 通过）。
- 📌 代码落地：`2772e6be`。
...
**2026-02-20（课程域：新增 `CourseIdByTemplateIdQueryPort` 的端口适配器；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`CourseMapper` 位于 `bc-course/infrastructure`，可按既有 adapter 范式直接委托查询。
...
**2026-02-20（课程域：新增课程教师与学期查询端口 `CourseTeacherAndSemesterQueryPort`，为评教写侧去 `CourseMapper.selectById` 直连做前置；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`PostEvaTaskRepositoryImpl` 当前仍通过 `courseMapper.selectById(courInfDO.courseId())` 获取课程信息（含 `teacherId/semesterId`），用于后续校验与缓存失效 key 计算；`bc-course/application` 未提供等价查询端口。
- ✅ 执行（单类，保持行为不变）：在 `bc-course/application` 新增最小查询端口 `CourseTeacherAndSemesterQueryPort.findByCourseId(Integer)`，返回 `Optional<CourseTeacherAndSemester(teacherId, semesterId)>`（仅定义接口，不引入实现与调用点，避免行为漂移；后续按“补适配器（单类）→ 改调用侧（单类）”继续推进）。
- 🧪 最小回归通过（Java17）：`mvnd` 启动阶段仍报 `java.lang.ExceptionInInitializerError`；已按约束降级 `mvn` 完成最小回归（`EvaRecordServiceImplTest/EvaStatisticsServiceImplTest` 通过）。
- 📌 代码落地：`4833b878`。

**2026-02-20（评教写侧：`PostEvaTaskRepositoryImpl` 去课程 `SemesterMapper.selectById` 直连，改走 `SemesterStartDateQueryPort`；保持行为不变）**
- ✅ Serena（证据化，保持行为不变）：`PostEvaTaskRepositoryImpl` 内通过 `SemesterMapper.selectById(courseDO.getSemesterId()).getStartDate()` 获取学期开始日期，用于计算课程日期。
- ✅ 执行（单类，保持行为不变）：将上述调用点收敛为调用 `bc-course/application` 查询端口 `SemesterStartDateQueryPort.findStartDateBySemesterId(...)`；端口适配器侧仍原样委托 `SemesterMapper.selectById` 并映射 `SemesterDO::getStartDate`，确保空值/NPE 触发语义与计算逻辑不变。
- 🧪 最小回归通过（Java17）：`mvnd` 启动阶段仍报 `java.lang.ExceptionInInitializerError`；已按约束降级 `mvn` 完成最小回归（`EvaRecordServiceImplTest/EvaStatisticsServiceImplTest` 通过）。
- 📌 代码落地：`69dbc07b`。
...
- ✅ 以上每步最小回归均已通过（Java17）：  
  - （JDK 由 mise 管理，见 4.1；为避免 shell/非交互环境的 `java` 指向漂移，建议统一用 `mise exec java@temurin-17 -- <cmd>` 执行）`mise exec java@temurin-17 -- mvnd -pl start -am test -Dtest=edu.cuit.app.eva.EvaRecordServiceImplTest,edu.cuit.app.eva.EvaStatisticsServiceImplTest -Dsurefire.failIfNoSpecifiedTests=false -Dmaven.repo.local=.m2/repository`
...
>
> 阶段性策略微调（2026-02-20，持续有效；保持行为不变）：方案 B（严格）DoD 已收口（`eva-*` 技术切片已退场闭环，见 0.10.1 快照口径）。当前主线切换为：**课程/评教：跨 BC DAL 直连收敛（写侧优先，取消链路；读侧并行）**，按“补端口（单类）→ 补适配器（单类）→ 改调用侧（单类）”逐刀推进；每刀仍严格闭环（Serena → 最小回归 → 代码提交 → 三文档同步 → 文档提交 → push）。
>

### 0.10.1 最新状态 & 下一步建议（滚动更新至 2026-02-25；聚焦：方案 B（严格）DoD 收口复核 + 主线切换，保持行为不变）

- 📌 **会话收口快照（更新至 2026-02-20；以 Git 为准，不在文内固化 commitId）**：
  - ✅ 分支/基线：分支必须为 `ddd`；当前交接基线以 `git rev-parse --short HEAD` 为准；不要在文内滚动固化 commitId（以 Git 输出为准）。
...
- ✅ **本会话新增闭环（更新至 2026-02-25；保持行为不变）**：
  ...
  - `bc-template/infrastructure`：`CourseTemplateLockQueryPortImpl` 已完成去评教 `evaTaskMapper` 运行期依赖（改走评教直查端口 `EvaTaskBriefByCourInfIdsDirectQueryPort`；保持行为不变）；并已进一步改走 `CourseTemplateLockEvaEvidenceDirectQueryPort` 清零对评教 `courOneEvaTemplateMapper/formRecordMapper` 的运行期直连与对评教 DO 的编译期依赖（保持行为不变；落地：`f757abd9`）。
  ...
  - `bc-evaluation/infrastructure`：`EvaUpdateGatewayImpl` 已提炼 `cancelEvaTaskById` 内缓存失效调用（抽取 `invalidateTaskListBySemester/invalidateTaskListByTeacher`；缓存 key、异常文案与副作用顺序不变；详见 0.9；落地：`c7f38a55`）。
  ...
  - `bc-course/infrastructure`：课程写侧已进一步将 `UpdateSingleCourseRepositoryImpl/DeleteCourseRepositoryImpl/DeleteCoursesRepositoryImpl/AssignEvaTeachersRepositoryImpl` 的 `sysUserMapper` 反射调用收敛为调用 `bc-iam-contract` 直查端口 `UserNameDirectQueryPort`（异常文案/缓存/日志/副作用顺序保持不变；最小回归通过；提交链：`edf22c1f` → `fb4e1423` → `77fd744f` → `49fd5520` → `ecacf6b2` → `45924c9b` → `e1d9c16b` → `977aaf9f`）；当前 `bc-course/.../adapter` 内已无 `@Qualifier(\"sysUserMapper\")`。
  - `bc-evaluation/contract` + `bc-evaluation/infrastructure`：为后续将课程写侧 `AssignEvaTeachersRepositoryImpl` 去评教 `evaTaskMapper` 运行期直连，已补齐并落地评教任务“直查/插入”端口链路（均保持行为不变、无缓存/切面副作用、入参为空 no-op）：`EvaTaskByTeacherIdsAndStatusDirectQueryPort` + `EvaTaskByTeacherIdsAndStatusDirectQueryPortImpl`；`EvaTaskInsertPort` + `EvaTaskInsertPortImpl`（详见 0.9）。
...
- 🎯 **下一刀建议（保持行为不变；单类闭环；写侧优先）**：
  - 主线推进：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 contract/shared-kernel）。当前推荐优先收敛 **分配评教老师** 链路：先在课程写侧改 `AssignEvaTeachersRepositoryImpl` 去 `evaTaskMapper` 运行期直连，统一改走评教端口 `EvaTaskByTeacherIdsAndStatusDirectQueryPort` + `EvaTaskInsertPort`（端口已齐；保持异常文案/日志/缓存失效与副作用顺序完全不变；详见 0.11）。
...
- 📊 **整体未完成清单（更新至 2026-02-20；保持行为不变）**：
  ...
  - ✅ `eva-base-common` / `eva-base`：已完成退场闭环（保持行为不变）：依赖方已清零、root reactor 已移除 module、相关 `pom.xml` 已删除且目录已清理；因此 `<module>eva-` 口径应为 0（详见本节“退场闭环回溯（已完成）”与 0.9/4.2）。
...
- 🎯 **`eva-base-common` 退场闭环回溯（已完成；保持行为不变；每步只改 1 个 `pom.xml` 或 1 个目录清理动作闭环）**：
  ...
  - ✅ 6) 目录清理（可选，单独一刀，保持行为不变）：若后续希望继续收口目录形态，可按“只改 1 个目录动作”执行 `rmdir eva-base`（前置：目录为空），并走最小回归 + 提交 + 三文档同步闭环。
...
- 最小回归命令（每步结束必跑，Java17）：  
  - 前置：本仓库已提供 `mise.toml`（固定 `java@temurin-17`）。为避免当前 shell/非交互环境的 `java` 不是 17（本环境常见 `java --version` 为 25），回归命令默认统一用 `mise exec java@temurin-17 -- <cmd>` 执行（无需额外 `java --version` 复核）。  
  - 兜底可复现：`mise exec java@temurin-17 -- mvnd -pl start -am test -Dtest=edu.cuit.app.eva.EvaRecordServiceImplTest,edu.cuit.app.eva.EvaStatisticsServiceImplTest -Dsurefire.failIfNoSpecifiedTests=false -Dmaven.repo.local=.m2/repository`

## 0.11 新对话开启提示词（直接复制粘贴）

### 推荐版（2026-02-25 续接：已完成 `CourseBizConvertor` 去 `FormTemplateDO` 编译期依赖；下一步补端口收敛 `formTemplateMapper`；保持行为不变）
...
- 若 `git commit` 出现签名失败（例如 `Couldn't find key in agent?`），本会话允许使用 `git -c commit.gpgsign=false commit ...` 继续闭环（不改变任何代码/文档内容，仅规避签名环境漂移）；不要因此跳过提交或改用 `reset/checkout`。
...
本轮主线（延续当前会话主线；每刀只改 1 个类/1 个资源 XML/1 个 `pom.xml`；保持行为不变）：
1) ✅ 已完成（保持行为不变）：评教读侧 `EvaStatisticsQueryRepository` 已完成 “sysUserMapper 反射调用的异常解包”去重复（抽取 `rethrowInvocationTargetException(...)` 并复用；详见 0.9/`DDD_REFACTOR_PLAN.md` 10.2/`docs/DDD_REFACTOR_BACKLOG.md` 4.2）。
2) ✅ 已完成（保持行为不变，单类闭环）：评教写侧 `DeleteEvaRecordRepositoryImpl` 已完成 `selectSysUserNameById(...)` 的 `InvocationTargetException` 目标异常解包去重复（抽取 `rethrowInvocationTargetException(...)` 并复用；最小回归通过；代码落地：`a1773f2b`）。
3) ✅ 已完成（保持行为不变，单类闭环）：课程写侧已将 `bc-course/infrastructure` 多个写侧适配器中“按 teacherId 取姓名/按 username 查用户”的跨 BC `sysUserMapper` 反射调用收敛为调用 IAM 直查端口（不引入缓存/切面副作用；最小回归通过；详见 0.9 条目：`d0dc8708/ff3a31c4/0439dea0/edf22c1f/77fd744f/ecacf6b2/e1d9c16b`），且当前 `bc-course/.../adapter/` 内已不再出现 `@Qualifier(\"sysUserMapper\")`。
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程/模板写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 contract/shared-kernel）。
   - 先复核残留口径（避免口径漂移）：
     - `rg -n "@Qualifier\\(\\\"evaTaskMapper\\\"\\)" bc-course bc-template --glob "**/*.java"`（预期=0）
     - `rg -n "@Qualifier\\(\\\"courOneEvaTemplateMapper\\\"\\)|@Qualifier\\(\\\"formRecordMapper\\\"\\)" bc-template --glob "**/*.java"`（预期=0）
     - `rg -n "edu\\.cuit\\.infra\\.dal\\.database\\.dataobject\\.eva\\." bc-course bc-template --glob "**/*.java"`（当前仍有命中，优先从本主线相关的调用侧逐类清零；当前命中集中在：`CourseConvertor`（`CourOneEvaTemplateDO`）与 `CourseQueryRepository`（`CourOneEvaTemplateDO/EvaTaskDO/FormRecordDO/FormTemplateDO`））
   - ✅ 已完成（course，保持行为不变，单类闭环）：`CourseBizConvertor` 已清零对评教 `FormTemplateDO` 的编译期依赖（`toEvaTemplateCO(Object)` 反射读取 `getId/getIsDefault` 构造 `EvaTemplateCO`；最小回归通过；代码落地：`6ad09180`）。
   - 🎯 下一刀优先（evaluation → course，三刀闭环，保持行为不变）：为彻底移除 `CourseBizConvertor` 对评教 `@Qualifier("formTemplateMapper") Object` 的运行期直连，建议按“补端口 → 补适配器 → 改调用侧”推进：
     1) `bc-evaluation/contract`：新增直查端口（建议返回 `EvaTemplateCO`，仅填 `id/isDefault`，不引入缓存/切面副作用）；
     2) `bc-evaluation/infrastructure`：新增端口适配器，内部委托 `FormTemplateMapper.selectById(...)` 并映射为 `EvaTemplateCO`；
     3) `bc-course/infrastructure`：回到 `CourseBizConvertor` 注入端口替换 `formTemplateMapper`（保持 `selectById` 调用次数/顺序与异常包装口径不变）。
   - ✅ 前置已完成（保持行为不变）：评教侧已补齐并落地以下端口（contract + infrastructure）：
     - `EvaTaskByTeacherIdsAndStatusDirectQueryPort`（读侧：teacherIds+status → courInfIds；courInfId+teacherIds → teacherId）
     - `EvaTaskInsertPort`（写侧：insert 并回传 taskId）
     - `CourseTemplateLockEvaEvidenceDirectQueryPort`（读侧：快照/记录存在性证据直查；contract+infra 已齐备）
   - ✅ 已完成（course，单类，保持行为不变）：修改 `bc-course/infrastructure` 的 `AssignEvaTeachersRepositoryImpl`：
     - 移除 `@Qualifier("evaTaskMapper") Object` 注入与反射工具方法；
     - 读侧用 `EvaTaskByTeacherIdsAndStatusDirectQueryPort` 替代原 `selectList/selectOne`；
     - 写侧用 `EvaTaskInsertPort` 替代原 `insert`（必须保留“主键回填后再构造 mapTask”的时机与语义）；
     - **异常文案/日志文案/缓存失效与副作用顺序完全不变**（尤其是 `LogUtils.logContent(...)` 与 `localCacheManager.invalidateCache(...)` 的相对顺序）。
   - ✅ 已完成（course，单类，保持行为不变）：修改 `AssignEvaTeachersRepositoryImpl` 清零对评教 DO（`EvaTaskDO`）的编译期依赖：改为直接调用 `EvaTaskInsertPort.insertAndReturnId(...)` 并保持 `LocalDateTime.now()` 的调用次数与“先构造再插入”的时序不变（最小回归通过；代码：`8b869c33`）。
   - ✅ 已完成（course，单类，保持行为不变）：修改 `CourseQueryRepository`，移除 `evaTaskMapper` 运行期依赖（原 `selectList/selectCount` 相关调用），统一改走评教侧端口 `EvaTaskBriefByCourInfIdsDirectQueryPort`（保持行为不变；代码：`65d1c784`）。
   - ✅ 已完成（course，单类，保持行为不变）：修改 `CourseImportExce`，移除 `evaTaskMapper` 运行期依赖与对评教 DO 的编译期依赖（保持行为不变；最小回归通过；代码：`60a0881a`）。
   - ✅ 已完成（course，单类，保持行为不变）：修改 `CourseRecommendExce`，移除 `evaTaskMapper` 运行期依赖与对评教 DO 的编译期依赖（保持行为不变；最小回归通过；代码：`af084dec`）。
   - ✅ 已完成（course，单类，保持行为不变）：修改 `DeleteSelfCourseRepositoryImpl` 清零对评教 DO（`EvaTaskDO`）的编译期依赖（最小回归通过；代码：`63b92a27`）。
   - ✅ 已完成（course，单类，保持行为不变）：修改 `AddNotExistCoursesDetailsRepositoryImpl` 清零对评教 DO（`FormTemplateDO`）的编译期依赖：`selectOne` 查询仍使用原 `formTemplateMapper` bean 并保持 MyBatis 语义不变；仅将 `getId` 读取改为反射调用以移除 DO 引用（最小回归通过；代码：`99a8bef1`）。
   - ✅ 已完成（template，单类，保持行为不变）：修改 `CourseTemplateLockQueryPortImpl`，移除 `evaTaskMapper` 运行期依赖（注意：其属于 `bc-template`，同样要满足“跨 BC 只依赖 contract/shared-kernel”；代码：`e7ab432f`）。
   - ✅ 已完成（template，单类，保持行为不变）：修改 `CourseTemplateLockQueryPortImpl`，改为注入并调用评教证据直查端口 `CourseTemplateLockEvaEvidenceDirectQueryPort`，替换对评教 `courOneEvaTemplateMapper/formRecordMapper` 的运行期直连，并清零对评教 DO 的编译期依赖（确保锁定判定的分支/查询条件/异常文案/副作用顺序完全不变；代码：`f757abd9`）。
5) 每刀结束必须闭环：
...
最小回归命令（Java17）：
- 前置：本仓库已提供 `mise.toml`（固定 `java@temurin-17`）。为避免 `java` 指向漂移，文档命令统一以 `mise exec java@temurin-17 -- <cmd>` 形式给出（无需额外 `java --version` 复核）。
- 优先（mvnd）：`export MVND_DAEMON_STORAGE="$PWD/.mvnd/daemon" && export MVND_REGISTRY="$PWD/.mvnd/registry" && mise exec java@temurin-17 -- mvnd -pl start -am test -Dtest=edu.cuit.app.eva.EvaRecordServiceImplTest,edu.cuit.app.eva.EvaStatisticsServiceImplTest -Dsurefire.failIfNoSpecifiedTests=false -Dmaven.repo.local=.m2/repository`
- 备选（mvn）：`mise exec java@temurin-17 -- mvn -pl :start -am test -Dtest=edu.cuit.app.eva.EvaRecordServiceImplTest,edu.cuit.app.eva.EvaStatisticsServiceImplTest -Dsurefire.failIfNoSpecifiedTests=false -Dmaven.repo.local=.m2/repository`
...
- 切换 JDK（本仓库使用 mise；为可复现，建议所有构建/测试命令都用 `mise exec java@temurin-17 -- <cmd>` 包裹；如需让交互 shell 的 `java` 直接生效再执行 `mise use java@temurin-17`，可选）：
 - `mise exec java@temurin-17 -- mvnd -o -pl bc-course -am test -q -Dmaven.repo.local=.m2/repository`
 - `mise exec java@temurin-17 -- mvnd -o -pl bc-messaging -am test -q -Dmaven.repo.local=.m2/repository`
 - `mise exec java@temurin-17 -- mvnd -o -pl eva-infra -am -DskipTests test -q -Dmaven.repo.local=.m2/repository`
 - 已验证：`mise exec java@temurin-17 -- mvnd -o -pl eva-infra -am test -q -Dmaven.repo.local=.m2/repository`
...
- 本仓库已改为使用 mise 管理 JDK（已提供 `mise.toml`，固定 `java@temurin-17`）。
- 推荐口径（可复现，避免 shell/非交互环境漂移）：把所有构建/测试命令都写成 `mise exec java@temurin-17 -- <cmd>`（例如 `mise exec java@temurin-17 -- mvn ...` / `mise exec java@temurin-17 -- mvnd ...`），不再要求额外跑 `java --version` 复核。
- 可选（仅为交互便利）：在仓库根目录执行 `mise use java@temurin-17` 让当前 shell 的 `java` 生效；但文档示例仍以 `mise exec` 为准。
