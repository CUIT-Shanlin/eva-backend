...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”。推荐先走 **删除链路**（改动面相对可控；当前已完成刀 1/2/3，下一刀从刀 4 开始）：
   ...
   - 刀 4（course，单类）：选择 `bc-course/infrastructure` 的一个删除类（例如 `DeleteCourseRepositoryImpl` 或 `DeleteCoursesRepositoryImpl`）改为调用该端口，并同步移除对 `edu.cuit.infra.dal.database.dataobject.eva.*` 的编译期依赖（异常文案/缓存/日志/副作用顺序完全不变）。
5) 每刀结束必须闭环：
