...
- 📊 **整体未完成清单（更新至 2026-02-12；保持行为不变）**：
  - `eva-infra-dal`：当前仍有 `24` 个 Java + `8` 个 XML 处于共享模块内（可复现口径：`fd -t f -e java . eva-infra-dal/src/main/java | wc -l` + `fd -t f -e xml . eva-infra-dal/src/main/resources | wc -l`）。
  - `eva-infra-shared`：当前仍有 `15` 个 Java，且多 BC 仍直依赖该模块（可复现口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l` + `rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | head`）。
...
- 🎯 **下一步重构计划（建议，保持行为不变；每次只改 1 个类/1 个 XML/1 个 pom 闭环）**：
  ...
  1) 🎯 主线优先（shared 瘦身，下一刀建议，保持行为不变）：将 `EntityFactory` 从 `eva-infra-shared` 搬运归位到 `eva-infra-dal`（保持 `package edu.cuit.infra.convertor` 不变；类内容不变）。前置已完成：`eva-infra-dal/pom.xml` 已显式补齐 `hutool-all` / `cola-component-exception` / `mapstruct` 依赖以保证编译闭合（详见 0.9）。
     ...
     - 关键约束：不引入新缓存/切面副作用；不改异常文案；不改日志；仅改变 Maven 模块归属。
  2) 并行推进（dal 拆散）：继续推进 `eva-infra-dal` 按 BC 拆散（Mapper/DO/XML）。做法：先用 Serena 对候选文件做“引用面证伪”，只挑“引用面仅命中单一 BC（例如 `bc-iam/**` 或 `bc-course/**`）”的文件搬运归位；每刀严格闭环（Serena → mvnd → commit → 三文档 → commit → push）。
...
1) 🎯 **下一刀（单类，推荐，shared 瘦身）**：将 `EntityFactory` 从 `eva-infra-shared` 搬运归位到 `eva-infra-dal`（保持 `package edu.cuit.infra.convertor` / 类内容不变）。前置已完成：`eva-infra-dal/pom.xml` 已补齐 `hutool-all` / `cola-component-exception` / `mapstruct` 依赖用于编译闭合（不改变任何业务语义/副作用顺序）。
   - `eva-infra-shared/src/main/java/edu/cuit/infra/convertor/EntityFactory.java` → `eva-infra-dal/src/main/java/edu/cuit/infra/convertor/EntityFactory.java`
