...
> ✅ 本会话补充进展（2026-02-23，保持行为不变）：评教 `bc-evaluation/contract` 新增写侧端口 `EvaTaskCancelByTeacherIdAndCourInfIdPort`，用于课程侧“修改课次/时间导致取消评教任务”链路按 `teacherId + courInfId` 将评教任务置为取消状态（当前约定：将 `eva_task.status` 更新为 `2`；不引入缓存/切面副作用；最小回归通过；代码落地：`6632b3fe`）。下一刀建议（写侧优先，保持行为不变）：在 `bc-evaluation/infrastructure` 落端口适配器实现（内部委托现有 Mapper/SQL）。
...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”。当前删除链路已阶段性闭环，建议转向 **取消链路**（课程改课/自助改课导致取消评教任务）：
   ...
   - 下一刀建议（写侧优先，保持行为不变，单类闭环）：实现取消链路端口适配器：在 `bc-evaluation/infrastructure` 新增 `EvaTaskCancelByTeacherIdAndCourInfIdPort` 的适配器实现（内部委托现有 `EvaTaskMapper.update(...)`；不引入缓存/切面副作用；空入参 no-op）。随后回到课程侧选择 1 个调用点（推荐 `UpdateSingleCourseRepositoryImpl`）改为调用该端口并移除 `evaTaskMapper` 反射调用（注意：`UpdateSingleCourseRepositoryImpl` 现有逻辑使用 `cour_inf_id = courINfo.getCourseId()` 的历史写法，若看起来像 bug 也必须保持不变，只做“调用方式”重构）。
5) 每刀结束必须闭环：
