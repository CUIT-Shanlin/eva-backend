...
### 推荐版（续接主线：消息域依赖收敛 —— 下一刀：`eva-infra-shared` 去 `eva-domain` 编译期依赖（保持行为不变））
...
2) Serena 证据化确认 `eva-infra-shared/src/main/java` 无 `eva-domain` 残留类型引用面（消息域 `MsgEntity/MsgGateway/MessageUseCaseFacade` 已归位 `bc-messaging-contract`）。
3) （单 pom，保持行为不变）收敛 `eva-infra-shared/pom.xml`：移除对 `eva-domain` 的 Maven 编译期依赖（仅收敛编译边界）。
