...
### 0.10.1 最新状态 & 下一步建议（滚动更新至 2026-01-28；聚焦：S1（BC Controller 收敛已阶段性完成）+ 并行“依赖方 pom 收敛”）
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：
  ...
  - B（并行：IAM 方向，准备后续“去 eva-domain”，保持行为不变）：`bc-iam/application` 仍 `import edu.cuit.domain.*`，因此暂不能直接移除 `bc-iam/application/pom.xml` 的 `eva-domain` 依赖；下一步需要先把“仅 IAM 使用”的 `edu.cuit.domain.entity.user.*` / `edu.cuit.domain.gateway.user.*` 等类型逐个归位到 `bc-iam-domain`（保持 `package` 不变），再收敛 `pom.xml` 依赖边界（每步闭环）。
...
4) 🎯 **IAM 优先（保持行为不变）**：已完成 `bc-iam-infra` 去 `bc-evaluation` 编译期依赖（详见 0.9）；下一步优先继续推进“依赖收敛”与“去 `eva-domain` 的前置搬运”（见 0.10.1 与 `DDD_REFACTOR_PLAN.md` 10.3 的 IAM 小节）。

下一刀（保持行为不变，二选一，先做 A 再做 B）：
- A（优先：pom，每次只改 1 个 `pom.xml`）：选择一个“仅类型引用”的依赖方模块做编译期依赖收敛；移除前先 Serena 证据化并补一条 `rg` 兜底证据，避免误判重演。
- B（备选：Controller，每次只改 1 个类）：仅在发现“新增/归位的新 Controller”或“漏网未收敛 Controller”时再继续；当前 `bc-*` Controller 清单已全部完成结构性收敛（见下方 `fd` 口径）。
