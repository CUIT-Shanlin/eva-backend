...
- 📊 口径快照（避免口径漂移，详见 0.10）：root reactor 仍包含 `eva-infra-dal/eva-infra-shared/eva-base`；`eva-infra-dal` 当前仍有 `17` 个 Java、`eva-infra-shared` 仍有 `9` 个 Java、`eva-base-common` 仍有 `2` 个 Java；且 `eva-infra-dal/src/main/resources` 已不存在（所有 XML 已按单资源归位到各 BC，后续证据命令需用“目录不存在=0”的口径）。
...
### 0.10.1 最新状态 & 下一步建议（滚动更新至 2026-02-16；聚焦：S0.2 延伸（`eva-infra-dal` 按 BC 拆散 + `eva-infra-shared` 瘦身），保持行为不变）

- 📊 **整体未完成清单（更新至 2026-02-16；保持行为不变）**：
  - `eva-infra-dal`：当前仍有 `17` 个 Java + `0` 个 XML 处于共享模块内（可复现口径：`fd -t f -e java . eva-infra-dal/src/main/java | wc -l` + `if [ -d eva-infra-dal/src/main/resources ]; then fd -t f -e xml . eva-infra-dal/src/main/resources | wc -l; else echo 0; fi`；注意：`eva-infra-dal/src/main/resources` 当前已不存在，属于“已清空并删除目录”的正常状态）。
...
- 📅 **下一步排期建议（按“刀”计，保持行为不变；按产能 2–4 刀/天粗估）**：
  ...
  - P1（约 1–2 周）：优先拆散 `eva-infra-dal`（17 个 Java）→ 逐类归位 `Mapper/DO` 到 `bc-course/bc-evaluation/bc-iam`（每刀 1 类；优先处理“XML 已归位”的对应 Mapper：`CourseMapper/SemesterMapper/CourInfMapper/SubjectMapper/CourOneEvaTemplateMapper/FormTemplateMapper/SysUserMapper`）。
...
### 推荐版（2026-02-16 收口续接：承接已归位 XML，逐类归位 `eva-infra-dal`/瘦身 `eva-infra-shared`，保持行为不变）
...
本轮主线（严格单文件一刀，保持行为不变）：
1) 证据口径（先跑快照，避免口径漂移）：
   - `rg -n "<module>eva-" pom.xml`
   - `git stash list`（确认是否存在未落地 WIP；后续只能按“指定文件路径”逐刀 restore）
	   - `fd -t f -e java . eva-infra-dal/src/main/java | wc -l`（当前预期=17）
   - `if [ -d eva-infra-dal/src/main/resources ]; then fd -t f -e xml . eva-infra-dal/src/main/resources | wc -l; else echo 0; fi`（当前预期=0；注意：目录可能不存在，按“目录不存在=0”口径）
   - `fd -t f -e java . eva-infra-shared/src/main/java | wc -l`（当前预期=9）
   - `rg -n "infra\\.dal\\.database\\.mapper\\.eva" bc-template | wc -l`（当前预期=0）
   - `rg -n "infra\\.dal\\.database\\.mapper\\.eva" bc-course | wc -l`（当前预期=0；文件数预期=0）
2) ✅ 阶段性完成：`bc-course/**` 对评教侧 `infra.dal.database.mapper.eva.*` 的编译期直连已清零（保持行为不变）。
3) 🎯 本轮主线（承接已归位 XML，逐类归位 `eva-infra-dal`，保持行为不变）：
   - 优先推进：**清零非 IAM 模块对 `SysUserMapper` 的编译期依赖**（每刀 1 类；保持缓存/日志/异常文案/副作用顺序完全不变）。
     - ✅ 已完成（保持行为不变）：`bc-evaluation/infrastructure` 内对 `SysUserMapper` 的编译期依赖已清零。
      - ✅ 已完成（保持行为不变）：`bc-course/infrastructure` 的 `AssignEvaTeachersRepositoryImpl` 已去 `SysUserMapper` 编译期依赖（改为 `@Qualifier("sysUserMapper") Object` + 反射 `selectById(Serializable)`；日志/异常文案/副作用顺序不变）。
      - ✅ 已完成（保持行为不变）：`bc-course/infrastructure` 的 `DeleteCourseRepositoryImpl` 已去 `SysUserMapper` 编译期依赖（改为 `@Qualifier("sysUserMapper") Object` + 反射 `selectById(Serializable)`；日志/异常文案/副作用顺序不变）。
      - ✅ 已完成（保持行为不变）：`bc-course/infrastructure` 的 `DeleteCoursesRepositoryImpl` 已去 `SysUserMapper` 编译期依赖（改为 `@Qualifier("sysUserMapper") Object` + 反射 `selectById(Serializable)`；日志/异常文案/副作用顺序不变）。
      - ✅ 已完成（保持行为不变）：`bc-course/infrastructure` 的 `DeleteSelfCourseRepositoryImpl` 已去 `SysUserMapper` 编译期依赖（改为 `@Qualifier("sysUserMapper") Object` + 反射调用 `selectOne(Wrapper)`；异常文案/日志文案/副作用顺序不变）。
      - ✅ 已完成（保持行为不变）：`bc-course/infrastructure` 的 `UpdateCourseInfoRepositoryImpl` 已去 `SysUserMapper` 编译期依赖（改为 `@Qualifier("sysUserMapper") Object` + 反射调用 `selectById(Serializable)`；异常文案/缓存失效 key 与副作用顺序不变）。
      - ✅ 已完成（保持行为不变）：`bc-course/infrastructure` 的 `UpdateSelfCourseRepositoryImpl` 已去 `SysUserMapper` 编译期依赖（改为 `@Qualifier("sysUserMapper") Object` + 反射调用 `selectOne(Wrapper)` / `selectById(Serializable)`；异常文案/缓存/日志与副作用顺序完全不变）。
      - ✅ 已完成（保持行为不变）：`bc-course/infrastructure` 的 `UpdateSingleCourseRepositoryImpl` 已去 `SysUserMapper` 编译期依赖（改为 `@Qualifier("sysUserMapper") Object` + 反射调用 `selectById(Serializable)`；异常文案/缓存/日志与副作用顺序完全不变）。
      - ✅ 已完成（保持行为不变）：`bc-course/infrastructure` 的 `CourseImportExce` 已去 `SysUserMapper` 编译期依赖（改为 `@Qualifier("sysUserMapper") Object` + 反射调用 `selectOne(Wrapper)`；异常文案/缓存/日志与副作用顺序完全不变）。
      - ✅ 已完成（保持行为不变）：`bc-course/infrastructure` 的 `CourseRecommendExce` 已去 `SysUserMapper` 编译期依赖（改为 `@Qualifier("sysUserMapper") Object` + 反射调用 `selectOne(Wrapper)` / `selectById(Serializable)` / `selectList(Wrapper)`；异常文案/缓存/日志与副作用顺序完全不变）。
      - ✅ 已完成（保持行为不变）：`bc-course/infrastructure` 的 `CourseQueryRepository` 已去 `SysUserMapper` 编译期依赖（改为 `@Qualifier("sysUserMapper") Object` + 反射调用 `selectOne(Wrapper)` / `selectList(Wrapper)` / `selectById(Serializable)`；异常文案与副作用顺序完全不变）。
   - 下一刀建议（每刀 1 类，保持行为不变）：在 Serena + `rg` 证据化确认“非 IAM 模块对 `SysUserMapper` 的编译期依赖已清零”后，开始“单类搬运”：将 `SysUserMapper.java` 从 `eva-infra-dal` 归位到 `bc-iam/infrastructure`（保持 `package` 不变，并同步删除旧位置同名类，避免 classpath 重复）。
   - 随后再逐类评估并归位剩余 `DO` 与公共工具类（例如 `SysUserDO/SysRoleDO/SysMenuDO`、`CourInfDO/CourseDO/SemesterDO/SubjectDO`、`CourOneEvaTemplateDO/FormTemplateDO/FormRecordDO/EvaTaskDO`、`QueryUtils`），每刀仍需 Serena 证据化后闭环。
4) 每刀结束必须闭环：
   - 跑最小回归（优先 `mvnd`；若遇 `~/.m2/mvnd/.../registry.bin AccessDenied`，先确保 `MVND_REGISTRY` 指向工作区 `.mvnd/`，仍失败则降级用 `mvn`）
...
### 推荐版（2026-02-12 主线：方案 B（严格）——清零 `eva-*` 技术切片，保持行为不变）
...
本轮主线（严格单文件一刀，保持行为不变）：
1) 证据口径（状态确认）：  
   - `fd -t f -e java . eva-infra-shared/src/main/java | wc -l`（预期=9）  
...
- `fd -t f -e java . eva-infra-shared/src/main/java | wc -l`（统计 shared 仍剩 Java 文件数）
- `fd -t f -e java . eva-infra-dal/src/main/java | wc -l` 与 `if [ -d eva-infra-dal/src/main/resources ]; then fd -t f -e xml . eva-infra-dal/src/main/resources | wc -l; else echo 0; fi`（统计 dal 残留；注意：目录可能不存在，按“目录不存在=0”口径）

### 推荐版：续接主线（先 IAM，保持行为不变）
...
本轮优先主线（保持行为不变；严格单文件一刀）：
A) ✅ 阶段性完成：其它 BC 对 IAM role 表（`sys_user_role/sys_role`）的跨 BC 直连已清零（保持行为不变）  
   - 证据口径：Serena 证伪 `bc-*`（排除 `bc-iam/**`）范围内无 `SysUserRoleMapper`/`SysRoleMapper` 引用点；如后续新增/回归引用点，仍按“每次只改 1 个类”将直连装配/查询收敛为调用 `bc-iam-contract` 端口 `UserEntityObjectByIdDirectQueryPort`（异常文案/副作用顺序完全不变；不引入新缓存/切面副作用）。
B) `eva-infra-dal` 按 BC 拆散：补齐 `sys_role` 候选的 XML（单资源 XML）  
   - ✅ 已闭环：`eva-infra-dal/src/main/resources/mapper/user/SysRoleMapper.xml` → `bc-iam/infrastructure/src/main/resources/mapper/user/SysRoleMapper.xml`（保持 MyBatis `<mapper namespace>`、`resultMap type`、SQL 与资源路径 `mapper/**` 不变；落地：`aa1d7c6b`）。
...
推送提示（保持行为不变，仅协作效率）：若 `git push` 卡住无输出，优先怀疑 GitHub SSH `22` 端口被阻断，可改用 `ssh.github.com:443`（示例：`GIT_SSH_COMMAND='ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o Hostname=ssh.github.com -p 443' git push origin ddd`）。

风险提示（保持行为不变）：
- **DAL 拆散必须保持 `package/namespace` 不变**：Java 类/Mapper/DO 归位后，MyBatis XML 的 `<mapper namespace="...">` 与 `resultMap type="..."` 必须仍指向同一 FQCN；搬运 XML 时建议保持资源路径 `mapper/**` 不变，避免 `mapper-locations` 漂移。
- `pom.xml` 依赖收敛容易被增量编译/缓存掩盖误判：移除依赖前必须 Serena 证据化“无引用面”，并在失败时优先通过 `clean` 复现后再判断（保持行为不变）。
...
- ✅ 条目 26（`bc-iam-infra` 阶段 2：IAM DAL 抽离）已完成：已用 Serena 盘点 IAM DAL 依赖清单并完成迁移闭环，最终移除 `bc-iam-infra` → `eva-infra` 依赖（保持行为不变）。
  ...
  - **DO（当前仍在 `eva-infra-dal`）**：`SysUserDO`、`SysRoleDO`、`SysMenuDO`
