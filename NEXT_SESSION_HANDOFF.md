...
### 推荐版（2026-02-22 续接：读侧“课程ID列表查询端口化”收尾 + 写侧主线择一；保持行为不变）
...
1) ✅ 已完成（保持行为不变）：评教读侧与课程域读侧中“仅为拿课程ID列表而先查 CourseDO 再映射”的关键残留点已按“最小端口”路线收敛（详见 0.9/`DDD_REFACTOR_PLAN.md` 10.2/`docs/DDD_REFACTOR_BACKLOG.md` 4.2）。
   - 状态补充（保持行为不变）：已清理 `CourseImportExce` 内历史注释残留旧实现（避免 `rg`/Serena 盘点时被注释命中干扰；不改变任何业务语义/异常文案/副作用顺序）。
   - 快速复核口径（可复现）：`rg -n "CourseDO::getId" --glob "**/*.java" .`（应仅命中 `CourseRecommendExce` 的“复用 CourseDO 列表的本地映射”、`CourseIdsBy*QueryPortImpl` 端口适配器内部映射，以及端口注释/文档引用）。
2) 下一条业务主线选择（保持行为不变）：从 `docs/DDD_REFACTOR_BACKLOG.md` 的 4.3“未收敛清单”或第 6 节建议中选 1 个可单刀闭环目标继续推进（写侧优先）。
3) 每刀结束必须闭环：
