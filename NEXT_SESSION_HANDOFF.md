...
- 📌 **当前仍未完成（进入 S1 前置的硬事实口径，保持行为不变）**：
  - `eva-adapter` 仍残留 15 个 `*Controller.java`（口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`）。
...
**当前残留共 15 个**（均仍在 `eva-adapter/src/main/java`；用于下一会话按 BC 逐个归位，每次只迁 1 个 Controller，保持行为不变）：
...
- `fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`（当前应为 15；用于复核“Controller 残留规模”）
- `fd -t f 'Controller\\.java$' eva-adapter/src/main/java | sort`（当前应输出 15 行；用于复核“具体残留清单”，与 0.10.2 保持一致）
...
### E 专用简版（⏳ 未闭环：eva-adapter 仍残留 15 个 *Controller；用于下一阶段迁移入口）
...
下一刀（保持行为不变）：
- **S1（入口归位，优先；每次只迁 1 个 Controller；保持 `package/注解/URL/权限/参数校验/日志与副作用顺序` 完全不变）**：下一刀优先将 `DepartmentController` 从 `eva-adapter` 归位到 `bc-iam/infrastructure`。若编译闭合缺依赖，则先单独提交 1 次 `bc-iam/infrastructure/pom.xml` 的编译闭合，再做 `git mv`（避免“改类 + 改 pom”混在同一提交）。
