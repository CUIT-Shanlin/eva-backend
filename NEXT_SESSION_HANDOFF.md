...
- ✅ **S1（入口归位：DepartmentController，保持行为不变）**：将 `DepartmentController` 从 `eva-adapter` 归位到 `bc-iam-infra`（保持 `package`/接口签名/URL/注解与行为不变，仅搬运归位；最小回归通过）；落地提交：`3e66a7b4`。当前 `eva-adapter` 残留 Controller 口径更新为 **14**（见 0.10.2）。
...
- ✅ **S1（入口归位：ClassroomController，保持行为不变）**：将 `ClassroomController` 从 `eva-adapter` 归位到 `bc-course-infra`（保持 `package`/接口签名/URL/注解与行为不变，仅搬运归位；最小回归通过）；落地提交：`132f32f5`。当前 `eva-adapter` 残留 Controller 口径更新为 **13**（见 0.10.2）。
...
- 📌 **当前仍未完成（进入 S1 前置的硬事实口径，保持行为不变）**：
  - `eva-adapter` 仍残留 13 个 `*Controller.java`（口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`）。
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：继续推进 **S0.2 延伸（依赖方编译期依赖收敛）**：选择下一条“依赖方编译期依赖收敛”目标（优先从 `eva-domain` / `eva-infra-shared` 等依赖方开始），用 Serena 证据化盘点引用面与依赖闭包后再收敛 `pom.xml`（每次只改 1 个 pom；保持行为不变）。
  - 📌 **当前仍未完成（截至 2026-01-17，口径=可复现命令；保持行为不变）**：
    - `eva-adapter` 仍保留 **18 个** `*Controller.java`（口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`）；组合根 `start` 对 `eva-adapter` 已收敛为 **runtime** 依赖（落地：`045891d1`；见 `start/pom.xml`；运行期装配不变，仅收敛编译期边界）。
    - `eva-adapter` 仍编译期依赖 `bc-audit` / `bc-ai-report`（口径：`rg -n '<artifactId>bc-(audit|ai-report)</artifactId>' eva-adapter/pom.xml`），根因是 `LogController` 仍引用 `edu.cuit.client.api.ILogService`、`EvaStatisticsController` 仍引用 `edu.cuit.client.api.ai.IAiCourseAnalysisService`，且这两类协议类型当前落在对应 BC 的 application 模块中（保持行为不变；后续建议按“协议下沉到 shared-kernel/contract”收敛依赖边界）。
...
- 当前重构进度汇报（截至当前 `HEAD`，用于“还没完成什么/下一步怎么排”）：
  ...
  - ✅ **`eva-client` 退场**：已从 reactor 移除并从仓库删除；跨 BC 通用对象已开始沉淀 `shared-kernel`（见 10.5）。
		  - ✅ **已闭环（本阶段目标 B：基础设施旧 gateway 归位，保持行为不变）**：`eva-infra/src/main/java/edu/cuit/infra/gateway/impl` 下残留旧 `*GatewayImpl.java` 已清零（Serena 盘点口径）；归位闭环包含：
		    ...
		    2) `eva-adapter` 仍保留 **13 个** `*Controller.java`（口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`），且 ✅ 已开始逐个归位到对应 BC（已完成：`AuthenticationController`（`94a00022`）、`UserUpdateController`（`367f781d`）、`UserQueryController`（`f7c5d219`）、`RoleQueryController`（`e7d51beb`）、`MenuQueryController`（`76e19a47`）、`MenuUpdateController`（`d29b565b`）、`RoleUpdateController`（`80888bed`）、`DepartmentController`（`3e66a7b4`）、`ClassroomController`（`132f32f5`）→ 对应 BC；保持行为不变）。
		    2.1) `eva-adapter` 仍编译期依赖 `bc-audit` / `bc-ai-report`（根因：`LogController` 引用 `ILogService`、`EvaStatisticsController` 引用 `IAiCourseAnalysisService`，且协议类型落在对应 BC 的 application 模块中；保持行为不变）。
	       - 补充说明（保持行为不变）：当前组合根 `start` 已显式依赖 `eva-adapter(runtime)` 与 `eva-infra(runtime)`（装配责任由组合根兜底；保持行为不变，仅收敛编译期边界）。若下一步要继续收敛“依赖方 `pom.xml` 的编译期依赖面”，仍按 **S0.2 延伸（每次只改 1 个 pom）** 推进：先 Serena + `rg` 证伪“仅类型引用”，再替换依赖（保持行为不变）。
...
- 下一会话优先建议（聚焦 **S0.2 延伸**，保持行为不变；每步只改 1 个小点，便于回滚）：
  0) 🎯 **优先：收敛 `eva-adapter` 残留 `*Controller`（每次只改 1 个 Controller；保持行为不变）**：
     - 目标：Controller 仅做协议适配与参数校验，不额外编排业务流程；不直接耦合基础设施实现细节；不改变异常文案/副作用顺序。
     - 推荐顺序（优先链路短、改动面可控；保持行为不变；**✅ 仅表示“已归位迁出 eva-adapter”**）：`AuthenticationController`（✅）→ `UserUpdateController`（✅）→ `UserQueryController`（✅）→ `RoleQueryController`（✅）→ `MenuQueryController`（✅）→ `MenuUpdateController`（✅）→ `RoleUpdateController`（✅）→ `DepartmentController`（✅）→ `ClassroomController`（✅）→ 其余（⏳）。

### 0.10.2 `eva-adapter` 残留 Controller 清单（口径=可复现命令；更新至 2026-01-19）

> 口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | sort`（与 `| wc -l` 同步）。

**当前残留共 13 个**（均仍在 `eva-adapter/src/main/java`；用于下一会话按 BC 逐个归位，每次只迁 1 个 Controller，保持行为不变）：
- Course（课程/课表）
  - `edu/cuit/adapter/controller/SemesterController.java`
  - `edu/cuit/adapter/controller/course/query/QueryCourseController.java`
  - `edu/cuit/adapter/controller/course/query/QueryUserCourseController.java`
  - `edu/cuit/adapter/controller/course/update/DeleteCourseController.java`
  - `edu/cuit/adapter/controller/course/update/UpdateCourseController.java`
- Evaluation（评教）
  - `edu/cuit/adapter/controller/eva/query/EvaConfigQueryController.java`
  - `edu/cuit/adapter/controller/eva/query/EvaQueryController.java`
  - `edu/cuit/adapter/controller/eva/query/EvaStatisticsController.java`
  - `edu/cuit/adapter/controller/eva/update/DeleteEvaController.java`
  - `edu/cuit/adapter/controller/eva/update/EvaConfigUpdateController.java`
  - `edu/cuit/adapter/controller/eva/update/UpdateEvaController.java`
- Messaging（消息）
  - `edu/cuit/adapter/controller/MessageController.java`
- Audit（审计/日志）
  - `edu/cuit/adapter/controller/LogController.java`
...
### 推荐版（当前主线：收敛 eva-adapter *Controller + 并行“依赖方 pom 收敛”）
...
本轮目标（推荐版，优先；保持行为不变）：
1) ✅ 已完成（保持行为不变，评教 S0.2 延伸闭环）：`eva-app/src/main/java` 对 `edu.cuit.bc.evaluation.*` 的直接引用面已清零，且 `eva-app/pom.xml` 已移除对 `bc-evaluation`（application jar）的 Maven 编译期依赖（详见 0.9 与 0.10.1）。
2) ✅ 已完成（保持行为不变，websocket S0.2 延伸闭环）：`WebSocketConfig/WebSocketInterceptor/MessageChannel/UriUtils` 已归位 `eva-infra-shared`，且 `eva-app/src/main/java` 不再 `import org.springframework.web.socket.*`，并已移除 `eva-app/pom.xml` 对 `spring-boot-starter-websocket` 的编译期依赖（运行期由组合根 `start` 显式兜底；详见 0.9 与 0.10.1）。
3) 🎯 当前主线：推进 **S1（eva-* 退场/整合）**（保持行为不变）：`eva-app` 已完成退场（组合根去依赖 + reactor 移除 + 删除 `eva-app/pom.xml`），下一阶段优先回到 `eva-adapter` 的 `*Controller` 收敛（每次只改 1 个 Controller）。补充：**S0.2 延伸（依赖方编译期依赖收敛）** 仍作为并行主线：优先从 `eva-domain` / `eva-infra-shared` 等依赖方挑选 1 个 `pom.xml` 做证据化盘点后收敛（保持行为不变）。

下一刀（保持行为不变）：
- 下一刀（每次只改 1 个 pom.xml 或 1 个类；保持行为不变）：
  1) ✅ 已完成：Serena 证据化盘点确认“全仓库已无对 `eva-app` 的 Maven 依赖”（口径：`rg -n "<artifactId>eva-app</artifactId>" --glob "**/pom.xml" .` 命中为空）。
  2) ✅ 已完成：单 pom 收敛：从根 `pom.xml` 的 reactor 中移除 `eva-app` 模块（保持行为不变）→ 最小回归通过；落地：`b5f15a4b`。
  3) ✅ 已完成：删除 `eva-app/pom.xml`（目录无其它文件；保持行为不变）→ 最小回归通过；落地：`4bfa9d40`。

快速证据口径（新会话先跑一次，避免口径漂移）：
- `rg -n "<module>eva-app</module>" pom.xml`（当前应为空；用于复核“reactor 已移除 eva-app 模块”）
- `rg -n "<artifactId>eva-app</artifactId>" --glob "**/pom.xml" .`（当前应为空；用于复核“全仓库已无 eva-app Maven 依赖/声明”）
- `rg -n "<artifactId>eva-app</artifactId>" start/pom.xml`（当前应为空；用于复核“组合根已去 eva-app 依赖”）
- `test -e eva-app/pom.xml && echo 1 || echo 0`（当前应输出 0；用于复核“eva-app/pom.xml 已删除”）
- `rg -n "<artifactId>eva-infra</artifactId>" start/pom.xml`（当前应命中；用于复核“组合根运行期显式兜底”）
- `rg -n "<artifactId>eva-adapter</artifactId>" start/pom.xml`（当前应命中；用于复核“组合根仍显式依赖适配层”）
- `fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`（当前应为 13；用于复核“Controller 残留规模”）
- `fd -t f 'Controller\\.java$' eva-adapter/src/main/java | sort`（当前应输出 13 行；用于复核“具体残留清单”，与 0.10.2 保持一致）
- `rg -n \"<artifactId>spring-boot-starter-websocket</artifactId>\" --glob \"**/pom.xml\" .`（当前应仅命中 `start/pom.xml`；用于盘点 websocket 依赖边界）
...
### E 专用简版（⏳ 未闭环：eva-adapter 仍残留 13 个 *Controller；用于下一阶段迁移入口）
...
本轮目标（E，优先；保持行为不变）：
1) 🎯 收敛 `eva-adapter` 残留 `*Controller`（每次只改 1 个 Controller）：Controller 仅做协议适配与参数校验，不额外承载业务编排；避免直接依赖基础设施实现细节
2) ⏳ 次优先：继续做 S0.2 延伸（依赖方 `pom.xml` 依赖收敛，每次只改 1 个 pom；保持行为不变）

下一刀（保持行为不变）：
- **S1（入口归位，优先；每次只迁 1 个 Controller；保持 `package/注解/URL/权限/参数校验/日志与副作用顺序` 完全不变）**：下一刀优先将 `SemesterController` 从 `eva-adapter` 归位到 `bc-course/infrastructure`。补充：`bc-course-infra` 已完成编译闭合前置（`spring-boot-starter-web` + `zym-spring-boot-starter-common`；落地：`8915db14`），因此下一刀可直接 `git mv`（仍保持“只改 1 个类”）。
- **S0.2 延伸（依赖方 `pom.xml` 依赖收敛，每次只改 1 个 pom；保持行为不变）**：优先从“仅类型引用、无实现/副作用耦合”的依赖方模块开始（需先用 Serena + `rg` 证伪引用面），例如优先评估 `eva-infra/pom.xml` 中 `bc-evaluation/bc-iam/bc-audit` 等依赖是否可收敛到 `*-contract`/`*-domain`。
...
本轮目标（D，优先；保持行为不变）：
1) 🎯 收敛 `eva-app` 残留 `*ServiceImpl`（每次只改 1 个类）：让旧入口退化为“仅切面/登录态解析/委托 UseCase”的壳；业务编排归位到对应 BC 的 `bc-*/application/usecase`
2) ⏳ 次优先：收敛 `eva-adapter` 残留 `*Controller`（每次只改 1 个 Controller；保持行为不变）

下一刀建议（从易到难，保持行为不变；清单以 0.10 为准）：
- ✅ 已完成：`SemesterServiceImpl` → `DepartmentServiceImpl` → `ClassroomServiceImpl` → `ICourseTypeServiceImpl`
- 下一刀：优先转向收敛 `eva-adapter` 残留 `*Controller`
