...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：转入“依赖方编译期依赖收敛”下一候选：优先从 `eva-app` 的 **bc-audit** 入手（当前仍直接 `import edu.cuit.bc.audit.*`，见下方证据口径），按“先归位旧入口/组合根 → 再收敛 pom 依赖”的套路推进。
...
- 快速证据口径（新会话先复核，避免口径漂移）：
  ...
  - 审计（下一候选）：`rg -n '^import\\s+edu\\.cuit\\.bc\\.audit' eva-app/src/main/java`（当前应命中 `BcAuditConfiguration` 与 `LogServiceImpl`；用于后续归位与依赖收敛）。
...
### G 推荐版（当前主线：S0.2 延伸（审计：旧入口归位 → 依赖收敛前置），优先）
...
2) 🎯 当前主线：继续推进 **S0.2 延伸（审计：旧入口归位 → 依赖收敛前置）**：逐步减少 `eva-app` 对 `bc-audit`（application jar）的编译期耦合面，方式为“审计组合根/旧入口壳逐个归位到 `bc-audit-infra`（保持 `package` 不变）”，最后再评估 `eva-app/pom.xml` 去 `bc-audit` 编译期依赖（每次只改 1 个 pom；保持行为不变）。
...
- `rg -n '^import\\s+edu\\.cuit\\.bc\\.audit' eva-app/src/main/java`（当前应命中 `BcAuditConfiguration` 与 `LogServiceImpl`；用于盘点审计“待归位类清单”）
