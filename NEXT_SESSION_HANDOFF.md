...
- 🎯 **下一刀建议（保持行为不变；单类闭环；写侧优先）**：
  - 主线切换：开始推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 contract/shared-kernel）。推荐优先从 **删除链路** 开始（改动面较可控）：先在 `bc-evaluation/contract` 补最小 CO/端口，再落 `bc-evaluation/infrastructure` 端口适配器，最后回到 `bc-course/infrastructure` 替换一个调用点（详见 0.11 的下一刀建议）。
...
### 推荐版（2026-02-23 续接：取消链路端口化已推进到课程写侧；下一步收敛自助改课剩余评教反射调用；保持行为不变）
...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”，当前聚焦 **取消链路/自助改课链路**：
   - 刀 1（contract，单类）：✅ 已完成：在 `bc-evaluation/contract` 新增“按 courInfIds 读取评教任务最小视图”的 CO `EvaTaskBriefCO`（仅含 `id/teacherId/courInfId/status`；不引入 Mapper/DO）。
   - 刀 2（contract，单类）：✅ 已完成：在 `bc-evaluation/contract` 新增最小端口 `EvaTaskBriefByCourInfIdsDirectQueryPort`，返回上述 CO 列表（约束：不引入缓存/切面副作用；语义保持与旧 SQL 条件/顺序一致）。
   - 刀 3（infra，单类）：✅ 已完成：在 `bc-evaluation/infrastructure` 落端口适配器实现（内部仍委托现有 Mapper/SQL；保持查询条件/结果顺序不变）。
   - 刀 4（course，单类）：✅ 已完成：`DeleteCourseRepositoryImpl`、`DeleteCoursesRepositoryImpl` 均已改为调用评教端口，并同步移除对 `edu.cuit.infra.dal.database.dataobject.eva.*` 的编译期依赖（异常文案/缓存/日志/副作用顺序完全不变）。
   - 刀 5（contract，单类）：✅ 已完成：在 `bc-evaluation/contract` 新增写侧端口 `EvaTaskCascadeDeleteByTaskIdsPort`，用于按 taskIds 级联删除评教任务与表单记录（保持删除顺序“先任务后记录”）。
   - 刀 6（infra，单类）：✅ 已完成：在 `bc-evaluation/infrastructure` 落 `EvaTaskCascadeDeleteByTaskIdsPortImpl`（内部委托现有 Mapper/SQL；保持删除顺序不变）。
   - 刀 7（course，单类）：✅ 已完成：课程写侧删除链路已改走端口（`DeleteCourseRepositoryImpl/DeleteCoursesRepositoryImpl`）。
   - ✅ 已完成（course，单类，保持行为不变）：`UpdateSingleCourseRepositoryImpl` 已将“取消评教任务”的评教 `evaTaskMapper` 反射调用收敛为调用评教端口（保持 `cour_inf_id = courINfo.getCourseId()` 历史口径不变）。
   - ✅ 已完成（course，单类，保持行为不变）：`UpdateSelfCourseRepositoryImpl` 已将“评教时间冲突检查”的评教任务 `courInfId` 列表查询收敛为调用评教端口 `EvaTaskCourInfIdsByTeacherIdAndStatusDirectQueryPort`。
   - ✅ 已完成（contract，单类，保持行为不变）：评教已新增写侧端口 `EvaTaskDeleteByCourInfIdPort`（仅删除 `eva_task`，不级联）。
   - 🎯 下一刀建议（infra，单类，保持行为不变）：在 `bc-evaluation/infrastructure` 落 `EvaTaskDeleteByCourInfIdPort` 的端口适配器实现（内部委托 `EvaTaskMapper.delete(eq cour_inf_id)`；空入参 no-op；不引入缓存/切面副作用）。
   - 🎯 再下一刀建议（course，单类，保持行为不变）：回到 `UpdateSelfCourseRepositoryImpl`，将其“按 cour_inf_id 查询任务/删除任务”的评教 `evaTaskMapper` 反射调用，分别收敛为调用 `EvaTaskBriefByCourInfIdsDirectQueryPort`（构造 `taskMap` 口径保持不变）与 `EvaTaskDeleteByCourInfIdPort`（保持删除语义与副作用顺序不变）。
5) 每刀结束必须闭环：
