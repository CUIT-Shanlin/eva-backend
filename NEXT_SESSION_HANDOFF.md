...
- 🎯 **本次会话最新增量（2026-02-07，保持行为不变）**：
  ...
  - ✅ DAL 拆散试点（审计）：`SysLogModuleDO` → `bc-audit/infrastructure`（保持 `package` 不变；最小回归通过；落地：`960d2bbb`；三文档同步：本提交）。
  - ⏳ 未完成（下一刀建议，保持行为不变；每次只改 1 个资源文件）：`SysLogMapper.xml`、`SysLogModuleMapper.xml` 仍在 `eva-infra-dal/src/main/resources/mapper/log/`；建议顺序：先搬 XML（2 刀），最后再评估 `bc-audit/infrastructure/pom.xml` 是否可去 `eva-infra-shared`（需 Serena 证伪；单 pom 闭环）。
...
- 🎯 **下一刀建议（保持行为不变；主线：审计 `sys_log` 的 DAL 按 BC 拆散试点；每次只改 1 个类/1 个资源文件闭环）**：
  ...
  2) Serena（必须）：证据化盘点 `SysLogMapper.xml` / `SysLogModuleMapper.xml` 的引用面与装配方式后，按顺序做 2 刀（每刀都需闭环一次）：
     - `SysLogMapper.xml`（资源）→ `SysLogModuleMapper.xml`（资源）
...
1) 快照证据（保持行为不变；用于防回归/确认拆散进度）：
   ...
   - 审计 DAL 进度：
	     ...
	     - `fd -t f -e java . eva-infra-dal/src/main/java/edu/cuit/infra/dal/database/dataobject/log`（当前应为空：`SysLogDO/SysLogModuleDO` 均已归位）
	     - `fd -t f -e xml . eva-infra-dal/src/main/resources/mapper/log`（当前应仍有 `SysLogMapper.xml/SysLogModuleMapper.xml`）
2) Serena（必须）：证据化盘点 `SysLogMapper.xml` / `SysLogModuleMapper.xml` 的装配与命中范围后，按“一次只改 1 个资源文件”的规则逐步归位：
   - 下一刀建议顺序：先 `SysLogMapper.xml` → 再 `SysLogModuleMapper.xml`（均保持 `namespace/resultMap type` 与资源路径 `mapper/**` 不变）
