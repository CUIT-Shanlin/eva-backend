...
- ✅ 本次会话提交链（按发生顺序，便于回溯/回滚）：`51301d23`（消息查询适配器去 `UserEntity` 代码）→ `f3fb4743`（三文档同步）→ `8fa053ed`（`LogConverter` 桥接前置）→ `c52a99fc`（三文档同步）→ `a86f6520`（`LogGatewayImpl` 去 `UserEntity` 代码）→ `902bb3ab`（三文档同步）。
...
- 🎯 **下一刀建议（保持行为不变；每次只改 1 个类闭环）**：
  ...
  9) ⏳ I（类，前置，建议下一会话第一刀）：`eva-infra-shared/src/main/java/edu/cuit/infra/convertor/user/UserConverter.java` 增加“Spring Bean 桥接”方法（返回 `Object`，内部仍 `SpringUtil.getBean(UserEntity.class)`）+ “setName 桥接”方法（入参 `Object`，内部仍强转 `UserEntity` 调 `setName`），用于后续让 `bc-course` 读侧仓储去 `UserEntity` 编译期依赖时，保持 `getBean` 与 `setName` 的调用语义/异常表现尽量一致（每次只改 1 个类闭环）。
  10) ⏳ J（类，主线，建议 I 完成后再做）：`bc-course/infrastructure/src/main/java/edu/cuit/infra/bccourse/query/CourseQueryRepository.java` 去 `UserEntity` import：将 `UserEntity`/`Supplier<UserEntity>` 替换为 `Object`/`Supplier<?>`；调用侧改走 `userConverter.toUserEntityObject(...)`、`courseConvertor.toCourseEntityWithTeacherObject(...)`；并将 `SpringUtil.getBean(UserEntity.class)` 部分改为走 I 的桥接方法（保持缓存/查询/遍历/异常文案与副作用顺序完全不变）。
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：
  - A（优先：类，每次只改 1 个类）：继续 IAM S0.2 延伸——收敛依赖方对 `edu.cuit.domain.entity.user.biz.UserEntity` 的编译期依赖，优先处理 `bc-course/infrastructure` 的读侧仓储 `CourseQueryRepository`（见上方 0.10.1 的 I/J 两刀）。
  - B（次优先：pom，每次只改 1 个 `pom.xml`）：继续做“依赖方 pom 收敛”，优先选择“仅类型引用”的依赖方模块；移除前必须 Serena + `rg` 双证据，避免增量编译/缓存导致误判（保持行为不变）。
  - C（并行注意事项，保持行为不变）：不要直接把 `bc-ai-report/infrastructure` 对 `bc-evaluation` 的依赖替换为 `bc-evaluation-contract` 来“省依赖”；评教导出端口签名当前会触发 Maven 循环依赖（原因见 0.9 / `DDD_REFACTOR_PLAN.md` 10.3）。
...
### 推荐版（续接主线：IAM S0.2 延伸 —— 继续收敛依赖方对旧 `UserEntity` 的编译期依赖（优先 `bc-course/infrastructure` 读侧仓储；行为不变））
...
4) 🎯 **依赖方收敛（本轮主线，保持行为不变）**：目标是让非 IAM 依赖方（如 `bc-evaluation/bc-messaging/bc-audit/bc-course`）逐步不再“编译期依赖”旧 `edu.cuit.domain.entity.user.biz.UserEntity`，而是仅编译期依赖 `bc-iam-contract` 的最小 Port/基础类型；端口适配器内部仍委托旧 gateway，确保缓存/切面触发点不变。现状：✅ `bc-evaluation/infrastructure` 已清零 `UserEntity` import；✅ `bc-messaging` 已清零；✅ `bc-audit(infrastructure)` 已清零；⏳ `bc-course/infrastructure` 仍残留（见“快速证据口径”）。
5) 🎯 **下一刀聚焦（保持行为不变）**：优先处理 `bc-course/infrastructure/src/main/java/edu/cuit/infra/bccourse/query/CourseQueryRepository.java` 去 `UserEntity` 编译期依赖。该类包含 `SpringUtil.getBean(UserEntity.class)` + `user.setName(...)` 的显式调用，为避免引入反射导致异常形态漂移，建议先在 `UserConverter` 增加“Spring Bean 桥接 + setName 桥接”方法（每次只改 1 个类闭环），再改调用侧（每次只改 1 个类闭环）。
...
- A（优先：类，每次只改 1 个类）：`eva-infra-shared/src/main/java/edu/cuit/infra/convertor/user/UserConverter.java` 增加“Spring Bean 桥接 + setName 桥接”方法（返回/入参均以 `Object` 表达；内部仍使用 `UserEntity` 强转与 `SpringUtil.getBean(UserEntity.class)`，以尽量保持历史异常形态与副作用顺序一致）。
- B（次优先：类，每次只改 1 个类）：`bc-course/infrastructure/src/main/java/edu/cuit/infra/bccourse/query/CourseQueryRepository.java` 去 `UserEntity` import：`UserEntity`/`Supplier<UserEntity>` → `Object`/`Supplier<?>`；调用侧改走 `userConverter.toUserEntityObject(...)`、`courseConvertor.toCourseEntityWithTeacherObject(...)`；并将 `SpringUtil.getBean(UserEntity.class)` + `setName` 改为调用 A 的桥接方法（缓存/查询/遍历顺序与异常文案完全不变）。
- C（备选：盘点下一批候选）：用 Serena + `rg` 证据化盘点其它 `bc-*` 对 `UserEntity` 的编译期依赖点（口径见下方），择一闭环（每次只改 1 个类）。
...
- `rg -n \"import\\s+edu\\.cuit\\.domain\\.entity\\.user\\.biz\\.UserEntity;\" bc-course/infrastructure/src/main/java`（当前预期仍命中：`CourseQueryRepository`）
- `rg -n \"\\bUserEntity\\b\" bc-course/infrastructure/src/main/java/edu/cuit/infra/bccourse/query/CourseQueryRepository.java`（用于盘点该类中所有 `UserEntity` 使用点，确保改造仅做类型桥接不改流程）
- `rg -n \"import\\s+edu\\.cuit\\.domain\\.entity\\.user\\.biz\\.UserEntity;\" bc-* --glob \"*.java\" | head`（快速快照：除 `bc-iam/**` 外，目标是逐步清零）
