...
>
> 阶段性策略微调（2026-02-20，持续有效；保持行为不变）：方案 B（严格）DoD 已收口（`eva-*` 技术切片已退场闭环，见 0.10.1 快照口径）。当前主线切换为：**课程/评教：跨 BC DAL 直连收敛（优先读侧 QueryRepo）**，按“补端口（单类）→ 补适配器（单类）→ 改调用侧（单类）”逐刀推进；每刀仍严格闭环（Serena → 最小回归 → 代码提交 → 三文档同步 → 文档提交 → push）。
>

### 0.10.1 最新状态 & 下一步建议（滚动更新至 2026-02-21；聚焦：方案 B（严格）DoD 收口复核 + 主线切换，保持行为不变）
...
### 推荐版（2026-02-23 续接：课程写侧 sysUserMapper 直连已清零；下一步推进课程写侧去评教 infrastructure/Mapper/DO/XML 直连；保持行为不变）
...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”。当前删除链路已阶段性闭环，建议转向 **取消链路**（课程改课/自助改课导致取消评教任务）：
   ...
   - 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续在课程写侧选择 1 个调用点，移除评教 `evaTaskMapper` 的运行期反射调用并改走评教端口（可从 `UpdateSelfCourseRepositoryImpl` / `DeleteSelfCourseRepositoryImpl` / `AssignEvaTeachersRepositoryImpl` 中任选其一，优先改写侧“取消/删除/分配”链路；注意保持异常文案/缓存失效/日志与副作用顺序不变）。
5) 每刀结束必须闭环：
