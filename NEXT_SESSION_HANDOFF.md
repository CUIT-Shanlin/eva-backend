...
- 🧪 最小回归通过（Java17 + mvnd）：按 0.11 命令执行（测试用例集保持不变）。
- 📌 代码落地：`e7ab432f`。
- 📝 交接与计划同步：本会话已将“已完成项 + 下一步计划 + 新对话提示词”同步到三文档；请以下一会话开始时的 `git log -n 1 -- NEXT_SESSION_HANDOFF.md`/`DDD_REFACTOR_PLAN.md`/`docs/DDD_REFACTOR_BACKLOG.md` 为准（不要在提示词中固化 commitId）。
...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程/模板写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 contract/shared-kernel）。
   ...
   - 下一刀优先（template，保持行为不变，单类闭环）：先补评教侧最小端口（contract）承接 `CourseTemplateLockQueryPortImpl` 的两处剩余直连，再补端口适配器（infra），最后回到 `CourseTemplateLockQueryPortImpl` 替换调用点并清零对评教 DO 的编译期依赖（每刀仍严格闭环：Serena → 最小回归 → 代码提交 → 三文档同步 → 文档提交 → push）。
   ...
   - 🎯 下一刀建议（template，保持行为不变，单类闭环）：继续修改 `CourseTemplateLockQueryPortImpl`，移除对评教 `courOneEvaTemplateMapper/formRecordMapper` 的运行期直连与对评教 DO 的编译期依赖。建议按“先补端口（contract，单类）→ 再补适配器（infra，单类）→ 再改调用侧（template 单类）”推进，确保锁定判定的分支/查询条件/异常文案/副作用顺序完全不变。
5) 每刀结束必须闭环：
