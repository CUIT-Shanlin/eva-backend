...
### 推荐版（续接主线：bc-course S0.2 延伸 —— 下一刀：证伪并归位 `CourseQueryGateway`（保持行为不变））
...
2) Serena 证伪 `edu.cuit.domain.gateway.course.CourseQueryGateway` 的引用面（含跨模块引用与签名引用），确认其属于“课程域可归位类型”（不引入新的 Maven 反向依赖/循环依赖风险）。
3) 若成立：将 `CourseQueryGateway` 从 `eva-domain` 搬运归位到 `bc-course-domain`（保持 `package` 与接口签名/注解不变，仅改变 Maven 模块归属；确保全仓库该 FQCN 仅存在一份）。
...
- `CourseQueryGateway` 签名依赖 `CourseEntity/SingleCourseEntity`：在两者均已归位到 `bc-course-domain` 后，再搬运 `CourseQueryGateway`（每步只改 1 个类闭环）。
