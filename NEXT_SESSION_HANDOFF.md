...
- 下一会话优先建议（聚焦 **S0.2 延伸**，保持行为不变；每步只改 1 个小点，便于回滚）：
  0) 🎯 **优先：收敛 `eva-adapter` 残留 `*Controller`（每次只改 1 个 Controller；保持行为不变）**：
     ...
     - 推荐顺序（优先链路短、改动面可控；保持行为不变）：`AuthenticationController` → `UserUpdateController`（✅ 已起步）→ `UserQueryController` → `RoleUpdateController/RoleQueryController` → `MenuUpdateController/MenuQueryController` → `SemesterController/DepartmentController/ClassroomController` → 课程 `UpdateCourseController/DeleteCourseController/QueryCourseController/QueryUserCourseController` → 评教 `EvaStatisticsController/EvaQueryController/UpdateEvaController/DeleteEvaController` → 其余。
...
下一刀（保持行为不变）：
- `QueryCourseController`（课程查询入口；优先链路短、改动面可控；延续“Controller 仅做协议适配与参数校验”的收敛模板，保持行为不变）
