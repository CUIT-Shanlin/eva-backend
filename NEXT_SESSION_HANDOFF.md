...
- ✅ 已完成（保持行为不变，DAL 拆散试点，逐类归位）：将 `MsgTipDO` 从 `eva-infra-dal` 搬运归位到 `bc-messaging`（保持 `package edu.cuit.infra.dal.database.dataobject` 不变，仅改变 Maven 模块归属；Serena：引用面仅命中 `bc-messaging`；最小回归通过；代码落地：`87b38a55`）。
