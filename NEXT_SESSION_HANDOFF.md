...
- 快速证据口径（新会话先复核，避免口径漂移）：
  ...
  - websocket：`rg -n '^import\\s+org\\.springframework\\.web\\.socket' eva-app/src/main/java`（当前应命中 `WebSocketConfig/WebSocketInterceptor`；用于后续归位与依赖收敛）。
...
- `rg -n '^import\\s+org\\.springframework\\.web\\.socket' eva-app/src/main/java`（当前应命中 `WebSocketConfig/WebSocketInterceptor`；用于后续归位与依赖收敛）
