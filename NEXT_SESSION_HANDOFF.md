...
**当前残留共 6 个**（均仍在 `eva-adapter/src/main/java`；用于下一会话按 BC 逐个归位，每次只迁 1 个 Controller，保持行为不变）：
- Course（课程/课表）
- Evaluation（评教）
...
1) 优先归位 `edu/cuit/adapter/controller/eva/query/EvaQueryController.java` → `bc-evaluation/infrastructure`（先 Serena 证据化盘点引用面/依赖闭包；保持 `package/注解/URL/权限校验/参数校验/日志与副作用顺序` 完全不变）。
...
- `fd -t f 'Controller\\.java$' eva-adapter/src/main/java | sort`（当前应输出 6 行；用于复核“具体残留清单”，与 0.10.2 保持一致）
