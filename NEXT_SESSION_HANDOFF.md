...
### 推荐版（续接主线：bc-course S0.2 延伸 —— 下一刀：证伪并归位 `CourseEntity`（保持行为不变））
...
本轮主线（保持行为不变）：
1) 先跑快照证据：  
   - `rg -n "\\bCourseEntity\\b" bc-* --glob "*.java" | head`
2) Serena 证伪 `edu.cuit.domain.entity.course.CourseEntity` 的引用面与签名引用面，确认其属于“课程域可归位类型”。
3) 若成立：将 `CourseEntity` 从 `eva-domain` 搬运归位到 `bc-course-domain`（保持 `package` 与类内容不变，仅改变 Maven 模块归属；确保全仓库该 FQCN 仅存在一份）。
...
- `rg -n \"\\bCourseEntity\\b\" bc-* --glob \"*.java\" | head`（下一刀证据口径：确认课程域类型引用面分布，为 `eva-domain` → `bc-course-domain` 归位做前置）
