...
- 📊 **整体未完成清单（更新至 2026-02-19；保持行为不变）**：
  ...
  - `eva-infra-shared`：源码已清零（Java=`0`；口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l`），且 BC 基础设施 pom 引用已清零（口径：`rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | sort` 预期无命中）；root reactor 也已移除 `<module>eva-infra-shared</module>`。下一步按“删除 `eva-infra-shared/pom.xml`”收尾（每次只改 1 个文件闭环；保持行为不变）。
  ...
  - `eva-infra-shared` 残留结构（Serena + `rg` 证据化，保持行为不变）：已完成 `UserConverter` 的“最小 Port 收敛跨 BC 使用点 → 单类搬运归位”（`UserConverter` 已归位到 `bc-iam/infrastructure` 且跨 BC 引用面清零）；且已完成“依赖方逐 pom 去依赖 + root reactor 移除 module”的收尾前置；当前仅剩“删除 `eva-infra-shared/pom.xml`（单文件）”。
...
本轮主线（严格单文件一刀，保持行为不变）：
1) 证据口径（先跑快照，避免口径漂移）：
   - `rg -n "<module>eva-" pom.xml`（当前预期仅命中 `eva-base`）
...
3) 🎯 本轮主线（`eva-infra-shared` 退场收尾；每次只改 1 个文件闭环；保持行为不变）：
   - 第 1 刀（单文件）：删除 `eva-infra-shared/pom.xml`（前置：全仓库 `**/pom.xml` 不再出现 `<artifactId>eva-infra-shared</artifactId>`）。
   - 备注（踩坑提示）：若出现“MapStruct 旧 generated-sources 影响编译”的假阳性报错，可在删除前先执行 `mvn -f eva-infra-shared/pom.xml clean -Dmaven.repo.local=.m2/repository` 清理构建产物（仅构建产物，不入库）。
4) 每刀结束必须闭环：
