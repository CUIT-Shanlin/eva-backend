...
- 🎯 **本次会话最新增量（2026-02-07，保持行为不变）**：
  ...
  - 🎯 下一步建议（保持行为不变；主线：按 BC 拆散 `eva-infra-dal` + 瘦身 `eva-infra-shared`；每次只改 1 个文件闭环）：继续以消息域 `msg_tip` 为试点，下一刀只搬 `MsgTipMapper.xml`（资源）。约束：保持 MyBatis XML `namespace/resultMap type` 指向的 FQCN、SQL 与资源路径 `mapper/**` 不变。
  - 📌 下一刀执行清单（单资源：`MsgTipMapper.xml`，保持行为不变）：
    1) Serena（必须）：证据化盘点 `MsgTipMapper.xml` 的引用面与装配位置（若 LSP 无法直接解析 XML，则用 Serena `search_for_pattern` 证据化 + 额外给出可复现 `rg/fd` 兜底证据）。
    2) 搬运（只改 1 个资源文件）：`git mv eva-infra-dal/src/main/resources/mapper/MsgTipMapper.xml bc-messaging/src/main/resources/mapper/MsgTipMapper.xml`（确保资源路径仍为 `mapper/**`）。
    3) 最小回归（Java17，必须 mvnd）通过。
    4) `git commit`（资源）→ 三文档同步 → `git commit`（文档）→ `git push`。
...
### 推荐版：续接主线：`eva-infra-dal` 按 BC 拆散（消息 `msg_tip` 试点），保持行为不变
...
1) 快照证据（保持行为不变；用于防回归/确认拆散进度）：
   - reactor：`rg -n "<module>eva-" pom.xml`（当前应仍包含 `eva-infra-dal/eva-infra-shared/eva-base`）
   - 消息域 DAL 进度（`msg_tip`）：
     - `fd -t f -e java . bc-messaging/src/main/java/edu/cuit/infra/dal/database/mapper | rg -n "MsgTipMapper\\.java" || true`（当前应已归位）
     - `fd -t f -e java . bc-messaging/src/main/java/edu/cuit/infra/dal/database/dataobject | rg -n "MsgTipDO\\.java" || true`（当前应已归位）
     - `fd -t f -e java . eva-infra-dal/src/main/java/edu/cuit/infra/dal/database/dataobject | rg -n "MsgTipDO\\.java" || true`（当前应为空：`MsgTipDO` 已归位）
     - `fd -t f -e xml . eva-infra-dal/src/main/resources/mapper | rg -n "MsgTipMapper\\.xml" || true`（当前仍在 `eva-infra-dal`，待归位）
     - `fd -t f -e xml . bc-messaging/src/main/resources/mapper | rg -n "MsgTipMapper\\.xml" || true`（当前应为空：待下一刀归位后应命中 1）
   - 共享基础设施瘦身（消息域前置）：
     - `fd -t f -e java . bc-messaging/src/main/java/edu/cuit/infra/convertor | rg -n "MsgConvertor\\.java" || true`（当前应已归位）
2) Serena（必须）：证据化盘点 `MsgTipMapper.xml` 引用面（确保仅消息域使用）后，按“一次只改 1 个文件”的规则逐步归位：
   - 下一刀建议：搬 `MsgTipMapper.xml`（资源）（保持 MyBatis `namespace/resultMap type` 指向的 FQCN、SQL 与资源路径 `mapper/**` 不变）
