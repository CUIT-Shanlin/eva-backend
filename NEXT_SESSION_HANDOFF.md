	    ...
	    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
		  - ⏳ **仍未完成（核心阻塞项）**：
		    ...
		    2) `eva-adapter` 仍保留 **22 个** `*Controller.java`（口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`），Controller 仍需逐步收敛为“纯 HTTP 协议适配”（避免直接依赖基础设施实现细节；保持行为不变）。
		       ...
		       - 补充进展（进行中，保持行为不变）：已开始按“每次只改 1 个 Controller”推进 Controller 收敛；首刀选择 `UserUpdateController`（含 `StpUtil` 登录态读取）先固化风险更高的调用顺序约束：提取 `currentUserId()`，保持 `StpUtil.getLoginId()` → `userService.getIdByUsername(...)` 调用顺序与次数不变（落地：`09cb6454`；见 0.9）。补充闭环：`AuthenticationController`（登录入口）已完成 `login` 表达式收敛，用“局部变量 + return”显式固化 `userAuthService.login(...)` → `CommonResult.success(...)` 的顺序（落地：`102a0cbb`；见 0.9）。补充闭环：`UserQueryController` 已完成“先调用 service → 再 `CommonResult.success(...)`”的返回表达式收敛（落地：`18e0bb29`；见 0.9）。补充闭环：`RoleQueryController` 已完成“先调用 service → 再 `CommonResult.success(...)`”的返回表达式收敛（落地：`fa04e48e`；见 0.9）。补充闭环：`MenuQueryController` 已完成“先调用 service → 再 `CommonResult.success(...)`”的返回表达式收敛（落地：`8521f72c`；见 0.9）。补充闭环：`MenuUpdateController` 已完成 `create` 内日志内容构造表达式收敛（保持 `menuService.create(...)` → `LogUtils.logContent(...)` → `CommonResult.success()` 顺序不变；落地：`9a2fc3ff`；见 0.9）。补充闭环：`RoleUpdateController` 已完成 `create` 内日志内容构造表达式收敛（保持 `roleService.create(...)` → `LogUtils.logContent(...)` → `CommonResult.success()` 顺序不变；落地：`9f07698a`；见 0.9）。补充闭环：`SemesterController` 已完成“先调用 service → 再 `CommonResult.success(...)`”的返回表达式收敛（落地：`24e5ad3f`；见 0.9）。补充闭环：`DepartmentController` 已完成“先调用 service → 再 `CommonResult.success(...)`”的返回表达式收敛（落地：`e57c0e41`；见 0.9）。下一刀建议：`ClassroomController`，再逐步推进课程/评教 Controller。
...
下一刀（保持行为不变）：
- `ClassroomController`（教室查询入口；延续“Controller 仅做协议适配与参数校验”的收敛模板，保持行为不变）
