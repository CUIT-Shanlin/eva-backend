...
- 🎯 **本次会话最新增量（2026-02-06，保持行为不变）**：
  ...
  - ✅ 可复现快照：`eva-domain` 残留 Java 文件数口径：`fd -t f -e java . eva-domain/src/main/java | wc -l`（当前为 10）。
...
- 🎯 **下一刀建议（保持行为不变；每次只改 1 个类闭环）**：
  1) 用 Serena 证据化盘点 `EvaConfigGateway` 的引用面（重点：`bc-evaluation-infra` 旧入口壳与旧 gateway 实现）。
...
2) Serena 证据化盘点 `EvaConfigGateway` 引用面（重点：`bc-evaluation-infra` 旧入口壳与旧 gateway 实现）。
