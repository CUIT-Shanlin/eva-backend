...
- ✅ 最小回归（保持行为不变）：已执行 `mvnd -pl start -am test -Dtest=edu.cuit.app.eva.EvaRecordServiceImplTest,edu.cuit.app.eva.EvaStatisticsServiceImplTest ...`，`BUILD SUCCESS`（本会话该回归后未再修改代码）。
...
- 🎯 **本次会话最新增量（2026-02-06，保持行为不变）**：
  ...
  - ✅ 可复现快照：`eva-domain` 残留 Java 文件数口径：`fd -t f -e java . eva-domain/src/main/java | wc -l`（当前为 6；清单口径：`fd -t f -e java . eva-domain/src/main/java`）。
...
- 🎯 **下一刀建议（保持行为不变；每次只改 1 个类闭环）**：
  ...
  2) 用 Serena 证据化盘点 `SysLogEntity` 引用面（重点：`LogGateway`/`LogGatewayImpl`/`LogConverter`/`LogServiceImpl` 等审计链路与是否跨 BC 复用）。
  3) 将 `SysLogEntity` 从 `eva-domain` 搬运归位到 `bc-audit-domain`（保持 `package` 与类内容不变，仅改变 Maven 模块归属；确保全仓库该 FQCN 仅存在一份）。
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：
  - A（优先：类，每次只改 1 个类）：审计域类型逐类归位 —— 证伪并归位 `SysLogEntity` → `bc-audit-domain`（保持 `package`/类内容不变；确保全仓库该 FQCN 仅存在一份；最小回归通过后落地提交并同步三文档）。
...
### 推荐版（续接主线：审计域类型逐类归位 —— 下一刀：证伪并归位 `SysLogEntity` → `bc-audit-domain`（保持行为不变））
...
2) Serena 证据化盘点 `SysLogEntity` 引用面（重点：审计日志链路与是否跨 BC 复用）。
   - 注意：`SysLogModuleEntity` 已在 2026-02-06 归位到 `bc-audit-domain`，本轮只处理 `SysLogEntity`。
3) 将 `SysLogEntity` 从 `eva-domain` 搬运归位到 `bc-audit-domain`（每次只改 1 个类；保持 `package`/类内容不变；保持行为不变）。
