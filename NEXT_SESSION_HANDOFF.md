...
### 推荐版（2026-02-24 续接：已补齐评教任务直查/插入端口；下一步改 AssignEvaTeachersRepositoryImpl 去 evaTaskMapper 反射直连；保持行为不变）
...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 contract/shared-kernel），优先从 **分配评教老师** 链路收敛开始（改动面较可控，且端口已齐）：
   ...
   - 刀 1（course，单类，保持行为不变）：修改 `bc-course/infrastructure` 的 `AssignEvaTeachersRepositoryImpl`：
   ...
   - 刀 2（course，单类，保持行为不变）：修改 `CourseQueryRepository`，移除 `evaTaskMapper` 运行期依赖（当前仍有 `selectList/selectCount` 相关调用），改走评教侧端口（若端口能力不足，按“先补端口（单类）→ 再补适配器（单类）→ 再改调用侧（单类）”推进）。
