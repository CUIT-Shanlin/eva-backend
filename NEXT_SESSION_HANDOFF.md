...
- ✅ 会话收口快照（保持行为不变）：当前 root reactor 仍包含 `eva-infra-dal/eva-infra-shared/eva-base`（口径：`rg -n '<module>eva-' pom.xml`）；`eva-infra-dal` 余量为 `25` 个 Java + `5` 个 XML；`eva-infra-shared` 余量为 `9` 个 Java；`eva-base-common` 余量为 `2` 个 Java（口径见 0.10.1）。
- ✅ 会话收口结论（保持行为不变）：已确认“完全独立（仅通过 contract/port 调用）”的关键阻塞仍是跨 BC 的 DAL 直连与共享模块依赖面：`bc-evaluation/**` 对课程 Mapper import 尚有 14 处，`bc-course/bc-template/**` 对评教 Mapper import 尚有 22 处；且 `bc-course/bc-evaluation/bc-iam/bc-audit/bc-template/bc-messaging` 仍编译期依赖 `eva-infra-shared`。
...
- 📊 **整体未完成清单（更新至 2026-02-13；保持行为不变）**：
  - `eva-infra-dal`：当前仍有 `25` 个 Java + `5` 个 XML 处于共享模块内（可复现口径：`fd -t f -e java . eva-infra-dal/src/main/java | wc -l` + `fd -t f -e xml . eva-infra-dal/src/main/resources | wc -l`）。
...
### 推荐版（2026-02-13 收口续接：先 XML、后跨 BC Mapper 收敛，保持行为不变）
...
1) 证据口径（先跑快照）：
   ...
   - `fd -t f -e java . eva-infra-dal/src/main/java | wc -l`（当前预期=25）
   - `fd -t f -e xml . eva-infra-dal/src/main/resources | wc -l`（当前预期=5）
   - `fd -t f -e java . eva-infra-shared/src/main/java | wc -l`（当前预期=9）
2) 🎯 下一刀（单资源 XML，优先课程域）：从 `eva-infra-dal/src/main/resources/mapper/course` 继续归位 1 个 XML（优先 `SemesterMapper.xml`，备选 `CourseMapper.xml`）。  
   - 前置必须 Serena 证伪对应 Mapper 的引用面与风险；搬运后保持 MyBatis `namespace/resultMap type`、SQL、资源路径 `mapper/**` 不变。
3) 若 XML 归位风险超阈值：降级到“单 pom 前置”或“单类无行为差异搬运”，但仍必须单文件闭环。
...
1) 🎯 **下一刀（单资源 XML，推荐，dal 拆散，课程域）**：从 `eva-infra-dal/src/main/resources/mapper/course` 选择 **1 个 XML**（优先 `SemesterMapper.xml`，备选 `CourseMapper.xml`）归位到 `bc-course/infrastructure/src/main/resources/mapper/course`（保持 MyBatis `namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变）；前置必须 Serena 证据化确认对应 Mapper 引用面与风险（允许跨 BC 引用，但需确保 MyBatis 扫描与资源加载行为不变）。
...
- `fd -t f -e java . eva-infra-dal/src/main/java | wc -l` 与 `fd -t f -e xml . eva-infra-dal/src/main/resources | wc -l`（统计 dal 残留）
