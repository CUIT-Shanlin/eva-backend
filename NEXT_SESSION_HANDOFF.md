...
- 🎯 **下一刀建议（保持行为不变；每次只改 1 个类闭环）**：
  1) 用 Serena 证伪 `bc-evaluation/application` 是否仍需要 `eva-domain` 提供的任何类型（尤其是 `eva-domain` 残留的 `MsgEntity/SysLog*/MsgGateway/LogGateway/DynamicConfigGateway/MessageUseCaseFacade`）。
  2) 若已证伪：收敛 `bc-evaluation/application/pom.xml`：移除对 `eva-domain` 的 Maven 编译期依赖（已具备 `bc-evaluation-domain` 依赖作为承接；保持行为不变；每次只改 1 个 `pom.xml` 闭环）。
...
### 推荐版（续接主线：依赖收敛（单 pom） —— 下一刀：`bc-evaluation/application` 去 `eva-domain` 编译期依赖（保持行为不变））
...
2) Serena 证伪 `bc-evaluation/application` 是否仍需要 `eva-domain` 提供的任何类型（尤其是 `eva-domain` 残留类型列表）。
3) 若已证伪：修改 `bc-evaluation/application/pom.xml`：移除 `<artifactId>eva-domain</artifactId>` 依赖（保留 `bc-evaluation-domain` 依赖作为承接；每次只改 1 个 `pom.xml`；保持行为不变）。
