	  ...
	  - ✅ **已闭环（本阶段目标 B：基础设施旧 gateway 归位，保持行为不变）**：`eva-infra/src/main/java/edu/cuit/infra/gateway/impl` 下残留旧 `*GatewayImpl.java` 已清零（Serena 盘点口径）；归位闭环包含：
	    ...
	    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
		  - ⏳ **仍未完成（核心阻塞项）**：
	       ...
	       - 补充说明（保持行为不变）：当前 `start` 已显式依赖 `eva-adapter` 与 `eva-app`（其中 `eva-app` 同时仍会经由 `eva-adapter` 传递进入 classpath；保持行为不变）。若下一步要继续收敛“装配责任与依赖边界”，建议仍按 **S0.2 延伸（每次只改 1 个 pom）** 推进：先用 Serena + `rg` 证伪 `eva-adapter` 不再引用 `edu.cuit.app.*` 的实现类型（或先将注入类型接口化/下沉到 contract/shared-kernel），再考虑在 `eva-adapter/pom.xml` 收敛对 `eva-app` 的编译期依赖（保持行为不变）。
...
下一刀（保持行为不变）：
- `eva-adapter/pom.xml`：收敛对 `eva-app` 的编译期依赖（减少编译期耦合；**必须**先 Serena + `rg` 证伪 `eva-adapter` 不再引用 `edu.cuit.app.*` 实现类型；若仍引用，先做“注入类型接口化/接口下沉”，再动 `pom.xml`；保持行为不变）
