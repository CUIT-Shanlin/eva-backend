...
- 🎯 下一刀建议（保持行为不变；每次只改 1 个文件闭环）：
  1) 优先从 **`bc-course`** 继续推进 `eva-infra-dal` 按 BC 拆散：以 `course_type`（非 `course_type_course`）为候选，先用 Serena 证据化盘点 `CourseTypeMapper` / `CourseTypeDO` / `CourseTypeMapper.xml` 的引用面；若仅命中 `bc-course/**`，再按“Mapper → XML → DO”（或反向）逐文件归位（每刀最小回归 + 提交 + 三文档同步 + push）。
...
2) Serena（必须）：继续推进 `eva-infra-dal` 按 BC 拆散时，优先选择“仅单 BC 引用”的候选文件（Mapper/DO/XML）。推荐从 `bc-course` 入手：先证据化盘点 `CourseTypeMapper` / `CourseTypeDO` / `CourseTypeMapper.xml` 引用面；若仅命中 `bc-course/**`，再按“一次只改 1 个文件”的规则逐步归位（保持 `package/namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变）。
