...
### 推荐版（当前主线：IAM Controller 结构性收敛 + 并行“依赖方 pom 收敛”（含纠偏））
...
本轮目标（推荐版，优先；保持行为不变）：
1) 🎯 **S1（入口壳收敛：IAM Controller）**：对 `bc-iam/infrastructure` 下剩余 Controller 做“纯结构性重构”（每次只改 1 个类）：优先 `DepartmentController`、`AuthenticationController`、`MenuUpdateController`、`RoleUpdateController`（URL/注解/异常文案/日志/缓存/副作用顺序完全不变）。
2) 🎯 **S0.2 延伸（依赖方 pom 收敛，含纠偏）**：每次只改 1 个 `pom.xml`，且移除依赖前必须先用 Serena 证据化“无类型引用面”，并用 `rg` 做可复现的兜底证据，避免增量编译/缓存导致误判。
3) ⚠️ 关键注意：`bc-iam-contract` 当前必须显式依赖 `bc-evaluation-contract`（`IUserService#getOneUserScore` 返回 `UserSingleCourseScoreCO`，定义于 `bc-evaluation/contract`；本次会话已纠偏恢复依赖，详见 0.9）。

下一刀（保持行为不变，二选一，先做 A 再做 B）：
- A（优先：Controller，每次只改 1 个类）：继续推进 IAM 入口壳结构性收敛，建议从 `DepartmentController` 开始（最小化改动：收敛返回表达式/抽取 `success()`/清理无用 import；不改任何副作用顺序）。
- B（备选：pom，每次只改 1 个 `pom.xml`）：选择一个“仅类型引用”的依赖方模块做编译期依赖收敛；移除前先 Serena 证据化并补一条 `rg` 兜底证据，避免误判重演。
