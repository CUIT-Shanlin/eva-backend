...
- ✅ 会话收口快照（保持行为不变）：当前 root reactor 仍包含 `eva-infra-dal/eva-infra-shared/eva-base`（口径：`rg -n '<module>eva-' pom.xml`）；`eva-infra-dal` 余量为 `25` 个 Java + `6` 个 XML；`eva-infra-shared` 余量为 `9` 个 Java；`eva-base-common` 余量为 `2` 个 Java（口径见 0.10.1）。
...
1) 证据口径（先跑快照）：
   ...
   - `fd -t f -e xml . eva-infra-dal/src/main/resources | wc -l`（当前预期=6）
   - `fd -t f -e java . eva-infra-shared/src/main/java | wc -l`（当前预期=9）
2) 🎯 下一刀（单资源 XML，优先课程域）：从 `eva-infra-dal/src/main/resources/mapper/course` 继续归位 1 个 XML（优先 `SubjectMapper.xml`，备选 `SemesterMapper.xml`/`CourseMapper.xml`）。  
...
1) 🎯 **下一刀（单资源 XML，推荐，dal 拆散，课程域）**：从 `eva-infra-dal/src/main/resources/mapper/course` 选择 **1 个 XML**（如 `SubjectMapper.xml`）归位到 `bc-course/infrastructure/src/main/resources/mapper/course`（保持 MyBatis `namespace/resultMap type`、SQL 与资源路径 `mapper/**` 不变）；前置必须 Serena 证伪对应 Mapper 引用面仅命中 `bc-course/**`。
