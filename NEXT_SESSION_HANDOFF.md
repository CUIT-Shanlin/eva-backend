...
- 🎯 **下一刀建议（保持行为不变；单类闭环；写侧优先）**：
  - 从 `docs/DDD_REFACTOR_BACKLOG.md` 4.3 或第 6 节候选中选 1 个写侧目标继续推进（例如优先在 `bc-course/infrastructure/src/main/java/edu/cuit/infra/gateway/impl/course/CourseUpdateGatewayImpl.java` 或 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/gateway/impl/eva/EvaUpdateGatewayImpl.java` 中挑 1 个方法簇做“旧 gateway 退化为委托壳”收敛；保持缓存/日志/异常文案/副作用顺序完全不变）。
...
### 推荐版（2026-02-22 续接：反射异常解包去重复收尾 + 写侧主线择一；保持行为不变）
...
3) 🎯 下一刀建议（写侧优先，保持行为不变）：从 `docs/DDD_REFACTOR_BACKLOG.md` 的 4.3“未收敛清单”或第 6 节建议中选 1 个可单刀闭环目标继续推进（每次只改 1 个类/1 个资源 XML/1 个 `pom.xml` 闭环）。
4) 每刀结束必须闭环：
