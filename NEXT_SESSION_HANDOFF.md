...
下一刀（保持行为不变）：
- `MenuUpdateController`（菜单写入口；延续“Controller 仅做协议适配与参数校验”的收敛模板，保持行为不变）
