...
### 推荐版（2026-02-25 续接：已完成 CourseQueryRepository 去 evaTaskMapper 运行期依赖；下一步改 CourseImportExce 去 evaTaskMapper 运行期依赖；保持行为不变）
...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 contract/shared-kernel）。当前下一刀建议优先处理 `CourseImportExce`：
   ...
   - 🎯 下一刀建议（course，单类，保持行为不变）：修改 `CourseImportExce`，移除 `evaTaskMapper` 运行期依赖与对评教 DO 的编译期依赖（若端口能力不足，按“先补端口（单类）→ 再补适配器（单类）→ 再改调用侧（单类）”推进）。
   - 刀 4（course，单类，保持行为不变）：修改 `CourseRecommendExce`，移除 `evaTaskMapper` 运行期依赖。
