...
### 推荐版（续接主线：IAM S0.2 延伸 —— 收尾：继续收敛依赖方对旧 `UserEntity` 的编译期依赖（优先确认/处置 `bc-iam/infrastructure` 残留 `UserQueryGatewayImpl`；行为不变））
...
5) 🎯 **下一刀聚焦（保持行为不变）**：继续收敛 `bc-iam/infrastructure` 端口适配器对旧 `UserQueryGateway`（eva-domain）的编译期依赖：通过内部过渡接口 `UserQueryCacheGateway` 逐类把“注入类型”从 `UserQueryGateway` 收敛为内部接口，但**调用仍必须最终进入旧 `UserQueryGatewayImpl`** 以保持 `@LocalCached` 缓存/切面触发点不变（每次只改 1 个类）。
...
- 🎯 F（下一刀建议，保持行为不变）：继续按“每次只改 1 个类”推进，将下列残留类的注入从 `UserQueryGateway` 收敛为 `UserQueryCacheGateway`（方法体仍委托同名方法，确保调用最终进入旧 `UserQueryGatewayImpl`）：
  - `bc-iam/infrastructure/src/main/java/edu/cuit/infra/bciam/adapter/UserDirectoryPageQueryPortImpl.java`
  - `bc-iam/infrastructure/src/main/java/edu/cuit/infra/bciam/adapter/UserEntityQueryPortImpl.java`
...
- `rg -n \"import\\s+edu\\.cuit\\.domain\\.entity\\.user\\.biz\\.UserEntity;\" bc-* --glob \"*.java\" | head`（快速快照：当前预期仅命中 `bc-iam/infrastructure/**/UserQueryGatewayImpl.java`）
- `rg -n \"import\\s+edu\\.cuit\\.domain\\.entity\\.user\\.biz\\.UserEntity;\" bc-iam/infrastructure/src/main/java --glob \"*.java\"`（预期命中：`UserQueryGatewayImpl`）
- `rg -n \"import\\s+edu\\.cuit\\.domain\\.gateway\\.user\\.UserQueryGateway;\" bc-iam/infrastructure/src/main/java --glob \"*.java\"`（当前应命中 3 个文件：`UserQueryGatewayImpl` + 2 个待收敛端口适配器；收敛完成后应仅剩 `UserQueryGatewayImpl`）
