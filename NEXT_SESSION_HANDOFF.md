...
- 📊 口径快照（避免口径漂移，详见 0.10）：root reactor 仍包含 `eva-infra-dal/eva-infra-shared/eva-base`；`eva-infra-dal` 当前仍有 `10` 个 Java、`eva-infra-shared` 仍有 `9` 个 Java、`eva-base-common` 仍有 `2` 个 Java；且 `eva-infra-dal/src/main/resources` 已不存在（所有 XML 已按单资源归位到各 BC，后续证据命令需用“目录不存在=0”的口径）。
...
- 📊 **整体未完成清单（更新至 2026-02-16；保持行为不变）**：
  - `eva-infra-dal`：当前仍有 `10` 个 Java + `0` 个 XML 处于共享模块内（可复现口径：`fd -t f -e java . eva-infra-dal/src/main/java | wc -l` + `if [ -d eva-infra-dal/src/main/resources ]; then fd -t f -e xml . eva-infra-dal/src/main/resources | wc -l; else echo 0; fi`；注意：`eva-infra-dal/src/main/resources` 当前已不存在，属于“已清空并删除目录”的正常状态）。
...
- 🎯 **SysUserMapper 归位（已闭环，保持行为不变）**：
  ...
  - 下一刀建议（每刀 1 类，保持行为不变）：继续按“引用面仅命中单一 BC 才允许归位”的规则，逐类评估并归位 `eva-infra-dal` 的剩余 Java（优先课程侧 DO：`CourInfDO/CourseDO/SemesterDO/SubjectDO`；评教侧 DO：`CourOneEvaTemplateDO/FormTemplateDO/FormRecordDO/EvaTaskDO`；`CourseTypeDO` 仍需谨慎评估），每刀仍需 Serena 证据化后闭环；若引用面跨 BC，则继续留在 `eva-infra-dal` 作为过渡共享。
...
1) 证据口径（先跑快照，避免口径漂移）：
   ...
   - `git stash list`（确认是否存在未落地 WIP；后续只能按“指定文件路径”逐刀 restore）
	   - `fd -t f -e java . eva-infra-dal/src/main/java | wc -l`（当前预期=10）
...
3) 🎯 本轮主线（承接已归位 XML，逐类归位 `eva-infra-dal`，保持行为不变）：
   ...
   - 随后再逐类评估并归位剩余 `DO` 与公共工具类（例如 `SysUserDO`、`CourInfDO/CourseDO/SemesterDO/SubjectDO`、`CourOneEvaTemplateDO/FormTemplateDO/FormRecordDO/EvaTaskDO`），每刀仍需 Serena 证据化后闭环。
4) 每刀结束必须闭环：
