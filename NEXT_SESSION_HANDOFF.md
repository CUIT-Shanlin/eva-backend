...
- 🎯 **本次会话最新增量（2026-02-07，保持行为不变）**：
  ...
  - ✅ DAL 拆散试点（审计）：`SysLogMapper.xml` → `bc-audit/infrastructure`（保持 `namespace/type/SQL` 与资源路径 `mapper/**` 不变；最小回归通过；落地：`b6f05784`；三文档同步：本提交）。
  - ⏳ 未完成（下一刀建议，保持行为不变；每次只改 1 个资源文件）：`SysLogModuleMapper.xml` 仍在 `eva-infra-dal/src/main/resources/mapper/log/`；建议顺序：先搬 XML（1 刀），最后再评估 `bc-audit/infrastructure/pom.xml` 是否可去 `eva-infra-shared`（需 Serena 证伪；单 pom 闭环）。
...
- 🎯 **下一刀建议（保持行为不变；主线：审计 `sys_log` 的 DAL 按 BC 拆散试点；每次只改 1 个类/1 个资源文件闭环）**：
  1) 先跑快照证据（作为防回归；口径同 0.11）：
     ...
     - `fd -t f -e xml . eva-infra-dal/src/main/resources/mapper/log`（当前应仅剩 `SysLogModuleMapper.xml`）
  2) Serena（必须）：证据化盘点 `SysLogModuleMapper.xml` 的引用面与装配方式后，做 1 刀（闭环一次）：
     - `SysLogModuleMapper.xml`（资源）
...
1) 快照证据（保持行为不变；用于防回归/确认拆散进度）：
   ...
   - 审计 DAL 进度：
	     ...
	     - `fd -t f -e java . eva-infra-dal/src/main/java/edu/cuit/infra/dal/database/dataobject/log`（当前应为空：`SysLogDO/SysLogModuleDO` 均已归位）
	     - `fd -t f -e xml . eva-infra-dal/src/main/resources/mapper/log`（当前应仅剩 `SysLogModuleMapper.xml`）
2) Serena（必须）：证据化盘点 `SysLogModuleMapper.xml` 的装配与命中范围后，按“一次只改 1 个资源文件”的规则逐步归位：
   - 下一刀建议顺序：`SysLogModuleMapper.xml`（保持 `namespace/resultMap type` 与资源路径 `mapper/**` 不变）
