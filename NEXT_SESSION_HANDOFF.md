...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：继续推进 `eva-app` 的 **bc-audit** 编译期耦合面收敛，按“先归位旧入口/组合根 → 再收敛 pom 依赖”的套路推进（见下方证据口径）。
...
- 快速证据口径（新会话先复核，避免口径漂移）：
  ...
  - 审计（当前主线）：`rg -n '^import\\s+edu\\.cuit\\.bc\\.audit' eva-app/src/main/java`（当前应为 0；用于评估 `eva-app/pom.xml` 是否可去 `bc-audit` 编译期依赖）。
...
- `rg -n '^import\\s+edu\\.cuit\\.bc\\.audit' eva-app/src/main/java`（当前应为 0；用于盘点审计“待收敛依赖”清单）
