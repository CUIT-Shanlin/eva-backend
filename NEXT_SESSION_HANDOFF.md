...
- 📊 **整体未完成清单（更新至 2026-02-19；保持行为不变）**：
  ...
  - `eva-infra-shared`：源码已清零（Java=`0`；口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l`），但 root reactor 仍包含 `<module>eva-infra-shared</module>`，且 BC 基础设施 pom 引用已清零（口径：`rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | sort` 预期无命中）。下一步按“root reactor 移除 module → 删除 `eva-infra-shared/pom.xml`”收尾（每次只改 1 个文件闭环；保持行为不变）。
...
1) 证据口径（先跑快照，避免口径漂移）：
   ...
   - `rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | sort`（当前预期=0）
...
3) 🎯 本轮主线（`eva-infra-shared` 退场收尾；每次只改 1 个文件闭环；保持行为不变）：
   - 第 1 刀（单 pom）：从 root `pom.xml` 的 reactor 中移除 `<module>eva-infra-shared</module>`（仅改变聚合构建边界；保持行为不变）。
   - 第 2 刀（单文件）：删除 `eva-infra-shared/pom.xml`（前置：全仓库 `**/pom.xml` 不再出现 `<artifactId>eva-infra-shared</artifactId>`）。
