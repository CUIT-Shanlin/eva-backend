...
**当前残留共 5 个**（均仍在 `eva-adapter/src/main/java`；用于下一会话按 BC 逐个归位，每次只迁 1 个 Controller，保持行为不变）：
- Course（课程/课表）
- Evaluation（评教）
...
1) 优先归位 `edu/cuit/adapter/controller/eva/update/EvaConfigUpdateController.java` → `bc-evaluation/infrastructure`（先 Serena 证据化盘点引用面/依赖闭包；保持 `package/注解/URL/权限校验/参数校验/日志与副作用顺序` 完全不变）。
...
- `fd -t f 'Controller\\.java$' eva-adapter/src/main/java | sort`（当前应输出 5 行；用于复核“具体残留清单”，与 0.10.2 保持一致）
...
下一刀（保持行为不变）：
- **S1（入口归位，优先；每次只迁 1 个 Controller；保持 `package/注解/URL/权限/参数校验/日志与副作用顺序` 完全不变）**：下一刀建议优先将 `EvaConfigUpdateController` 从 `eva-adapter` 归位到 `bc-evaluation/infrastructure`。补充：若 `bc-evaluation-infra` 出现编译闭合缺依赖，则先单独提交 1 次 `bc-evaluation/infrastructure/pom.xml`（避免“改类 + 改 pom”混在同一提交；保持行为不变）。
