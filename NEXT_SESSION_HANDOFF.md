...
- 🧾 文档同步：已将上述变更同步到 `NEXT_SESSION_HANDOFF.md` / `DDD_REFACTOR_PLAN.md` / `docs/DDD_REFACTOR_BACKLOG.md`，以 `git log -n 1 -- NEXT_SESSION_HANDOFF.md` 为准（不在文内固化 commitId）。
- ✅ **S0.2 延伸（依赖方 pom 收敛：组合根显式承接 eva-app 装配责任，保持行为不变）**：Serena 证实：`start` 当前显式依赖 `eva-adapter`，`eva-app` 仅经由 `eva-adapter/pom.xml` 传递进入 classpath；因此在 `start/pom.xml` 增加对 `eva-app` 的 `runtime` 依赖，作为后续收敛 `eva-adapter` 编译期耦合的前置（不改业务语义/装配顺序；最小回归通过）；落地提交：`0a69dfb6`。
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：继续推进 **S0.2 延伸（依赖方编译期依赖收敛）**：优先评估 `eva-app/pom.xml` 中的 `bc-messaging-contract` 是否仍必要；若引用面已清零，则独立提交移除该编译期依赖（只改 1 个 `pom.xml`）。
  1) Serena 证据化盘点：确认 `eva-app/src/main/java` 已无消息域相关类型引用（例如 `edu.cuit.client.api.msg.*`、`IMsgService` 等），并检查是否存在“仅通过 transitive 引入但未使用”的依赖。
  2) 若引用面为 0：收敛 `eva-app/pom.xml` 并移除 `bc-messaging-contract` 编译期依赖（每次只改 1 个 pom）。
  3) 最小回归 → commit（代码/pom）→ 三文档同步 → commit（文档）→ push。
- ✅ **完成条件（评估点）**：
...
- ✅ 已满足（websocket）：`eva-app/src/main/java` 不再 `import org.springframework.web.socket.*`，且 `eva-app/pom.xml` 已移除 `spring-boot-starter-websocket` 依赖（运行期由组合根兜底；保持行为不变）。
- 快速证据口径（新会话先复核，避免口径漂移）：
  ...
  - 审计：`rg -n '^import\\s+edu\\.cuit\\.bc\\.audit' eva-app/src/main/java`（当前应为 0；用于复核“审计引用面已清零”）。
  - AI 报告：`rg -n '^import\\s+edu\\.cuit\\.bc\\.aireport' eva-app/src/main/java`（当前应为 0；用于复核“AI 报告引用面已清零”）。
  - websocket：`rg -n '^import\\s+org\\.springframework\\.web\\.socket' eva-app/src/main/java`（当前应为 0；用于复核“引用面清零”）。
  - bc-messaging-contract：`rg -n \"<artifactId>bc-messaging-contract</artifactId>\" eva-app/pom.xml`（当前应为 1；用于下一刀评估是否可收敛为 0）。
  - bc-messaging-contract：`rg -n '^import\\s+edu\\.cuit\\.client\\.api\\.msg' eva-app/src/main/java`（当前应为 0；用于证伪“未引用消息契约”）。
  ...
  - ✅ **`eva-client` 退场**：已从 reactor 移除并从仓库删除；跨 BC 通用对象已开始沉淀 `shared-kernel`（见 10.5）。
	  - ✅ **已闭环（本阶段目标 B：基础设施旧 gateway 归位，保持行为不变）**：`eva-infra/src/main/java/edu/cuit/infra/gateway/impl` 下残留旧 `*GatewayImpl.java` 已清零（Serena 盘点口径）；归位闭环包含：
	    ...
	    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
		  - ⏳ **仍未完成（核心阻塞项）**：
			    1) `eva-app` 仍保留 **12 个** `*ServiceImpl.java`（口径：`fd -t f 'ServiceImpl\\.java$' eva-app/src/main/java/edu/cuit/app/service/impl | wc -l`），现已全部退化为“仅切面/登录态解析/委托 UseCase/网关”的壳（保持行为不变）。
				       - 已显式委托 UseCase（共 11 个，口径：`rg -l \"\\bUseCase\\b|application\\.usecase\" eva-app/src/main/java/edu/cuit/app/service/impl`）：`EvaRecordServiceImpl/EvaStatisticsServiceImpl/EvaTaskServiceImpl/EvaTemplateServiceImpl/UserEvaServiceImpl/RoleServiceImpl/MenuServiceImpl/LogServiceImpl/UserServiceImpl/UserAuthServiceImpl/DepartmentServiceImpl`
			       - 下一刀建议拆分（每步只改 1 个类；保持行为不变；每步闭环=Serena→最小回归→commit(代码)→三文档同步→commit(文档)→push）：
			         1) ✅（只改 1 个类；落地：`325f221a`）在 `bc-course/application/usecase` 新增课程类型用例（建议单类覆盖读写：`pageCourseType/allCourseType/updateCourseType/addCourseType/deleteCourseType/deleteCoursesType/updateCoursesType`），并确保不引入 `eva-infra-shared` 等重依赖（可直接手写 `CourseTypeEntity` → `CourseType` 的字段映射，以及构造 `PaginationQueryResultCO`，逐行对齐旧入口逻辑与返回值）。
			         2) ✅（只改 1 个类；落地：`55eb322e`）在 `bc-course` 组合根 `BcCourseConfiguration` 补齐该用例的 Bean 装配（保持行为不变）。
			         3) ✅（只改 1 个类；落地：`1aebda24`）将 `eva-app` 的 `ICourseTypeServiceImpl` 退化为纯委托壳：仅保留接口实现与委托调用；原有“`id==null` → 传 `null` ids”的语义、`updateCoursesType` 返回 `null`（`Void`）等行为必须完全不变。
		    2) `eva-adapter` 仍保留 **22 个** `*Controller.java`（口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`），但 ✅ **已阶段性闭环**：已按“每次只改 1 个 Controller”的模板完成这 22 个 Controller 的最小化收敛（固化返回/日志表达顺序、清理无用依赖/无用 import 等；保持行为不变；详见 0.9）。
	       - 补充说明（保持行为不变）：当前 `start` 已显式依赖 `eva-adapter` 与 `eva-app`，且 `eva-adapter` 已不再传递 `eva-app`（装配责任由组合根兜底；保持行为不变）。若下一步要继续收敛“依赖方 `pom.xml` 的编译期依赖面”，仍按 **S0.2 延伸（每次只改 1 个 pom）** 推进：先 Serena + `rg` 证伪“仅类型引用”，再替换依赖（保持行为不变）。
...
- 下一会话优先建议（聚焦 **S0.2 延伸**，保持行为不变；每步只改 1 个小点，便于回滚）：
  0) 🎯 **优先：收敛 `eva-adapter` 残留 `*Controller`（每次只改 1 个 Controller；保持行为不变）**：
     ...
     - 推荐顺序（优先链路短、改动面可控；保持行为不变）：`AuthenticationController`（✅）→ `UserUpdateController`（✅）→ `UserQueryController`（✅）→ `RoleUpdateController/RoleQueryController`（✅）→ `MenuUpdateController/MenuQueryController`（✅）→ `SemesterController/DepartmentController/ClassroomController`（✅）→ 课程 `QueryCourseController`（✅）/`QueryUserCourseController`（✅）/`UpdateCourseController`（✅）/`DeleteCourseController`（✅）→ 评教 `EvaStatisticsController`（✅）/`EvaQueryController`（✅）/`UpdateEvaController`（✅）/`DeleteEvaController`（✅）→ `EvaConfigQueryController`（✅）→ `EvaConfigUpdateController`（✅）→ `MessageController`（✅）→ `LogController`（✅）→ 其余。
...
### G 推荐版（当前主线：S0.2 延伸（依赖收敛：websocket 已闭环，继续收敛 eva-app 依赖），优先）
...
3) 🎯 当前主线：继续推进 **S0.2 延伸（依赖方编译期依赖收敛）**：优先评估 `eva-app/pom.xml` 中的 `bc-messaging-contract` 是否仍必要；若引用面已清零，则独立提交移除该编译期依赖（每次只改 1 个 `pom.xml`；保持行为不变）。补充：审计（`bc-audit`）与 AI 报告（`bc-ai-report`）的 `eva-app` 编译期依赖收敛已闭环（详见 0.9）。

下一刀（保持行为不变）：
- 下一刀（每次只改 1 个 pom.xml 或 1 个类；保持行为不变）：
  1) Serena 证据化盘点：选择下一条“依赖方编译期依赖收敛”目标（优先从 `eva-app`/`eva-infra` 等依赖方开始），明确引用面与依赖闭包。
  2) 若引用面已清零：再做 `pom.xml` 依赖收敛（每次只改 1 个 pom），并跑最小回归闭环。

快速证据口径（新会话先跑一次，避免口径漂移）：
- `rg -n '^import\\s+edu\\.cuit\\.bc\\.evaluation' eva-app/src/main/java`（当前应为 0；用于复核“评教引用面已清零”）
- `rg -n '^import\\s+edu\\.cuit\\.infra\\.bcevaluation' eva-app/src/main/java`（当前应为 0；用于复核“已去 bc-evaluation-infra 编译期耦合面”）
- `fd -t f 'BcEvaluationConfiguration\\.java' eva-app/src/main/java | cat`（当前应为空）
- `rg -n '^import\\s+edu\\.cuit\\.bc\\.audit' eva-app/src/main/java`（当前应为 0；用于复核“审计引用面已清零”）
- `rg -n \"<artifactId>bc-audit\" eva-app/pom.xml`（当前应为 0；用于复核“eva-app 已去 bc-audit 依赖”）
- `rg -n \"<artifactId>bc-ai-report\" eva-app/pom.xml`（当前应为 0；用于复核“eva-app 已去 bc-ai-report 依赖”）
- `rg -n \"<artifactId>spring-boot-starter-websocket</artifactId>\" --glob \"**/pom.xml\" .`（当前应仅命中 `start/pom.xml`；用于盘点 websocket 依赖边界）
- `fd -t f 'MessageChannel\\.java' eva-app/src/main/java | cat`（当前应为空；`MessageChannel` 已归位 `eva-infra-shared`）
- `rg -n '^import\\s+org\\.springframework\\.web\\.socket' eva-app/src/main/java`（当前应为 0；用于复核“websocket 引用面清零”）
- `rg -n \"<artifactId>bc-messaging-contract</artifactId>\" eva-app/pom.xml`（当前应为 1；用于下一刀评估是否可收敛为 0）
- `rg -n '^import\\s+edu\\.cuit\\.client\\.api\\.msg' eva-app/src/main/java`（当前应为 0；用于证伪“未引用消息契约”）
...
### E 专用简版（历史：收敛 eva-adapter 残留 *Controller，✅ 已阶段性闭环）
