...
### 0.10.1 最新状态 & 下一步建议（滚动更新至 2026-02-23；聚焦：方案 B（严格）DoD 收口复核 + 主线切换，保持行为不变）
...
### 推荐版（2026-02-25 续接：已完成 CourseRecommendExce 去 evaTaskMapper 运行期依赖；下一步改 CourseTemplateLockQueryPortImpl 去 evaTaskMapper 运行期依赖；保持行为不变）
...
4) 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 contract/shared-kernel）。当前下一刀建议优先处理 `CourseTemplateLockQueryPortImpl`：
   ...
   - 🎯 下一刀建议（template，单类，保持行为不变）：修改 `CourseTemplateLockQueryPortImpl`，移除 `evaTaskMapper` 运行期依赖（注意：其属于 `bc-template`，同样要满足“跨 BC 只依赖 contract/shared-kernel”）。
5) 每刀结束必须闭环：
