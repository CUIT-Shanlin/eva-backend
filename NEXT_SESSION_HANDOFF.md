...
### 0.10.1 最新状态 & 下一步建议（滚动更新至 2026-01-29；聚焦：S1（BC Controller 收敛已阶段性完成）+ 并行“依赖方 pom 收敛”）
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：
  ...
  - C（S1：`eva-*` 技术切片整合，短期可落地；保持行为不变）：优先清空 `eva-infra` 的最后业务实现类 `CourseTemplateLockQueryPortImpl`（Serena 证据化引用面后搬运归位到 `bc-template` 的基础设施落点；每次只改 1 个类），为后续“从 root reactor 移除 `eva-infra` 模块”做前置。
  ...
  - 📌 **当前状态快照（截至 2026-01-29，口径=可复现命令；保持行为不变）**：
    ...
    - ⏳ “全量整合 `eva-*`（并从 reactor 移除）”仍未具备落地条件：root reactor 仍包含 `eva-domain/eva-infra/eva-infra-dal/eva-infra-shared/eva-base`（口径：`rg -n "<module>eva-" pom.xml`），且组合根 `start` 仍显式依赖 `eva-infra`，并有多个 `bc-*` 模块编译期依赖 `eva-domain/eva-infra-*`（口径：`rg -n "<artifactId>eva-domain</artifactId>" --glob "**/pom.xml" .` 与 `rg -n "<artifactId>eva-infra-(shared|dal)</artifactId>" --glob "**/pom.xml" .`）。整合判定标准与路线见 `DDD_REFACTOR_PLAN.md` 10.5（坚持“每步只改 1 个类/1 个 pom + 最小回归 + 可回滚”）。
  - ✅ **完成条件（评估点）**：
		  ...
		  - ✅ **已闭环（本阶段目标 B：基础设施旧 gateway 归位，保持行为不变）**：`eva-infra/src/main/java/edu/cuit/infra/gateway/impl` 下残留旧 `*GatewayImpl.java` 已清零（Serena 盘点口径）；归位闭环包含：
		    ...
		    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
			  - ⏳ **仍未完成（核心阻塞项）**：
			    ...
			    2.2) ⏳ **`eva-*` 技术切片仍未具备“全量整合进各 BC”的前置条件**：root reactor 仍包含 `eva-domain` / `eva-infra-dal` / `eva-infra-shared` / `eva-infra` / `eva-base`（可复现口径：`rg -n "<module>eva-" pom.xml`）；且组合根 `start` 仍 `runtime` 依赖 `eva-infra`（可复现口径：`rg -n "<artifactId>eva-infra</artifactId>" start/pom.xml`）。保持行为不变前提下，当前不能直接移除这些模块。补充“可量化快照”（口径=可复现命令）：`eva-domain` 约 29 个 Java 文件、`eva-infra-dal` 约 36 个、`eva-infra-shared` 约 47 个；`eva-infra` 仅剩 4 个 Java 文件，其中 3 个为 `package-info.java`，唯一业务实现类为 `CourseTemplateLockQueryPortImpl`（适合作为 S1 的“最小整合试点”优先清空）。粗略工作量估算：若坚持“root reactor 不含任何 `eva-*` module（含 `eva-infra-dal/eva-infra-shared/eva-base`）”，仅按当前 `eva-*` 下的 Java 文件计，至少约 116 次“单文件闭环”；若按 `DDD_REFACTOR_PLAN.md` 10.5 推荐先把共享技术模块（`eva-infra-dal/eva-infra-shared`）视为平台模块保留，则可先聚焦“清空 `eva-infra` + 逐 BC 清空对 `eva-domain` 的编译期依赖”，阶段性收益更高且风险更低（保持行为不变）。
		       ...
		       - 补充说明（保持行为不变）：当前组合根 `start` 已显式依赖 `eva-infra(runtime)` 与各 `bc-*-infra(runtime)`（装配责任由组合根兜底；保持行为不变，仅上推依赖边界）。`eva-adapter` 依赖已移除（落地：`92a70a9e`）。若下一步要继续收敛“依赖方 `pom.xml` 的编译期依赖面”，仍按 **S0.2 延伸（每次只改 1 个 pom）** 推进：先 Serena + `rg` 证伪“仅类型引用”，再替换依赖（保持行为不变）。
...
下一刀（保持行为不变，二选一，先做 A 再做 B）：
- A（优先：pom，每次只改 1 个 `pom.xml`）：选择一个“仅类型引用”的依赖方模块做编译期依赖收敛；移除前先 Serena 证据化并补一条 `rg` 兜底证据，避免误判重演。
- B（备选：类，每次只改 1 个类）：继续推进 IAM 去 `eva-domain` 的前置搬运；优先选择“引用面仅在 IAM”的 `edu.cuit.domain.gateway.user.*` 接口或 `edu.cuit.domain.entity.user.*` POJO（由 Serena 证据化确认）。
...
- **S0.2 延伸（依赖方 `pom.xml` 依赖收敛，每次只改 1 个 pom；保持行为不变）**：可优先评估 `eva-adapter/pom.xml` 中 `bc-audit` / `bc-ai-report` 的编译期依赖是否仍必要（先 Serena 证据化盘点引用面/依赖闭包，再按“单 pom 收敛”推进；保持行为不变）。
