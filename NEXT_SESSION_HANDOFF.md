...
- 🎯 **本次会话最新增量（2026-02-04，保持行为不变）**：
  ...
  - ⏳ 当前 `bc-evaluation/infrastructure` 仍残留 `UserEntity` import：`EvaRecordQueryRepository`（可复现口径：`rg -n "import\\s+edu\\.cuit\\.domain\\.entity\\.user\\.biz\\.UserEntity;" bc-evaluation/infrastructure/src/main/java`）。
