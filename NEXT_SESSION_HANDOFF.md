...
### 推荐版（续接主线：评教域 gateway 逐类归位 —— 下一刀：证伪并归位 `EvaUpdateGateway` → `bc-evaluation-domain`（保持行为不变））
...
2) Serena 证伪 `eva-domain/src/main/java` 内是否仍存在对课程域类型的引用（如 `edu.cuit.domain.entity.course.*` / `edu.cuit.domain.gateway.course.*`）。  
   - 预期无命中：上一刀已闭环；若仍有引用：按“每次只改 1 个类”先逐类归位/收敛后再继续。
3) Serena 证据化盘点 `EvaUpdateGateway` 引用面后，将其从 `eva-domain` 搬运归位到 `bc-evaluation-domain`（每次只改 1 个类；保持 `package`/签名/注解不变；保持行为不变）。
