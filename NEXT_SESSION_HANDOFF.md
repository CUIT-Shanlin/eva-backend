	    ...
	    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
		  - ⏳ **仍未完成（核心阻塞项）**：
		    ...
		    2) `eva-adapter` 仍保留 **22 个** `*Controller.java`（口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`），Controller 仍需逐步收敛为“纯 HTTP 协议适配”（避免直接依赖基础设施实现细节；保持行为不变）。
		       ...
		       - 补充进展（进行中，保持行为不变）：已开始按“每次只改 1 个 Controller”推进 Controller 收敛；首刀选择 `UserUpdateController`（含 `StpUtil` 登录态读取）先固化风险更高的调用顺序约束：提取 `currentUserId()`，保持 `StpUtil.getLoginId()` → `userService.getIdByUsername(...)` 调用顺序与次数不变（落地：`09cb6454`；见 0.9）。下一刀建议：`AuthenticationController`（登录入口），再逐步推进用户/角色/菜单与评教查询/写侧 Controller。
...
下一刀（保持行为不变）：
- `AuthenticationController`（登录入口，风险偏高但链路短；先固化 Controller 侧对登录态/异常/返回的“顺序不变”口径）
