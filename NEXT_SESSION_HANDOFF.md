...
- ✅ **S0.2 延伸（次优先：其它依赖收敛点盘点，证伪结论，保持行为不变）**：Serena + `rg` 证据化确认：`eva-app` 仍直接 `import edu.cuit.bc.course.application.usecase.*`（`SemesterServiceImpl/ClassroomServiceImpl/ICourseTypeServiceImpl`）与 `import edu.cuit.bc.messaging.application.usecase.*`（`MsgServiceImpl`），因此 **不满足** “仅使用 `edu.cuit.client.*` 类型、可替换依赖” 的前提；本阶段不动 `eva-app/pom.xml` 中对 `bc-course/bc-messaging` 的编译期依赖，避免引入编译/装配漂移（最小回归通过；证据命令：`rg -n '^import edu\\.cuit\\.bc\\.course' eva-app/src/main/java` 与 `rg -n '^import edu\\.cuit\\.bc\\.messaging' eva-app/src/main/java`）。
...
本轮目标（F，优先；保持行为不变）：
1) 🎯 **S0.2 延伸（依赖方 `pom.xml` 依赖收敛）**：逐步让组合根 `start` 显式承接装配责任，并减少 `eva-adapter` 对 `eva-app` 的编译期耦合（每次只改 1 个 `pom.xml`；保持行为不变）
2) ⏳ 次优先：继续盘点其它可收敛的依赖（必须先 Serena + `rg` 证据化：确认仅类型引用、无实现/副作用耦合后再动 `pom.xml`；保持行为不变）

下一刀（保持行为不变）：
- 继续推进 **S0.2 延伸（依赖方 `pom.xml` 依赖收敛）**：先用 Serena + `rg` 盘点“仍显式依赖 `bc-course`/`bc-messaging` 等业务实现模块、但仅使用 `edu.cuit.client.*` 类型”的依赖方模块，选择 1 个满足前提的模块后再动对应 `pom.xml`（每次只改 1 个 `pom.xml`；保持行为不变）

执行模板（每步闭环，保持行为不变）：
1) Serena：证据化盘点（pom 依赖链/引用面），确认“只改 1 个 pom”不会引入 Maven 循环依赖或运行时装配漂移
2) 最小化改动：每次只改 1 个 `pom.xml`（不改业务语义）
