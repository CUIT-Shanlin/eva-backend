...
3) 🎯 本轮目标（保持行为不变；每次只改 1 个类闭环）：先归位 `SpringAfterCommitDomainEventPublisher`（以及评教领域事件监听器），再逐个归位 `EvaRecordServiceImpl/EvaTaskServiceImpl/EvaTemplateServiceImpl/EvaStatisticsServiceImpl/UserEvaServiceImpl`；完成后再用 Serena + `rg` 证伪 `eva-app/src/main/java` 对 `edu.cuit.bc.evaluation.*` 的直接引用面，评估是否可以收敛 `eva-app/pom.xml` 的 `bc-evaluation` 编译期依赖（每次只改 1 个 `pom.xml`）。

下一刀（保持行为不变）：
- 下一刀（每次只改 1 个类；保持行为不变）：
  1) Serena 证据化盘点：`eva-app/src/main/java` 对 `edu.cuit.bc.evaluation.*` 的引用清单与“依赖闭包”（用于决定先搬哪一个类最省前置）。
  2) 先搬运 `SpringAfterCommitDomainEventPublisher` 到 `bc-evaluation-infra`（保持 `package edu.cuit.app.event` 不变；类内容不变；Bean 装配/副作用顺序不变）。
  3) 再逐个搬运评教领域事件监听器 `EvaluationSubmittedMessageCleanupListener`、`EvaluationTaskPostedMessageSenderListener`（保持 `package` 不变；类内容不变；副作用顺序不变）。
