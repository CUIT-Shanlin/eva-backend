...
- 🎯 **下一刀建议（保持行为不变；每次只改 1 个类闭环）**：
  1) 用 Serena 证据化盘点 `SysLogEntity` 或 `SysLogModuleEntity` 的引用面（重点：审计日志链路与是否存在跨 BC 复用风险）。
  2) 将选定的 1 个类从 `eva-domain` 搬运归位到 `bc-audit-domain`（保持 `package` 与类内容不变，仅改变 Maven 模块归属；确保全仓库该 FQCN 仅存在一份）。
...
### 推荐版（续接主线：审计域类型逐类归位 —— 下一刀：证伪并归位 `SysLogEntity`（或 `SysLogModuleEntity`）→ `bc-audit-domain`（保持行为不变））
...
2) Serena 证据化盘点 `SysLogEntity` 或 `SysLogModuleEntity` 引用面（重点：审计日志链路与是否跨 BC 复用）。
3) 将选定的 1 个类从 `eva-domain` 搬运归位到 `bc-audit-domain`（每次只改 1 个类；保持 `package`/类内容不变；保持行为不变）。
