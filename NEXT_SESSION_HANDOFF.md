...
- **bc-evaluation（评教）**：写侧主链路（任务发布/删除/模板）已按“用例+端口+适配器+委托壳”收敛；且已完成 `bc-evaluation-infra` 阶段 1（读侧迁移）与阶段 2（写侧 Repo 迁移；`CalculateClassTime` 迁移到 `eva-infra-shared` 以避免 `bc-evaluation-infra` 依赖 `eva-infra`），均保持包名/行为不变。
