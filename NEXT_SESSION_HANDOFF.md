...
### 0.10.1 最新状态 & 下一步建议（滚动更新至 2026-02-06；聚焦：依赖收敛（单 pom）闭环 + 评教域 gateway 逐类归位（保持行为不变））
...
- 🎯 **本次会话最新增量（2026-02-06，保持行为不变）**：
  ...
  - ✅ 可复现快照：`eva-domain` 残留 Java 文件数口径：`fd -t f -e java . eva-domain/src/main/java | wc -l`（当前为 7）。
  - ✅ 可复现快照：当前仍显式依赖 `eva-domain` 的模块口径：`rg -n "<artifactId>eva-domain</artifactId>" --glob "**/pom.xml" .`（当前至少包含 `bc-course/application`、`bc-messaging`、`eva-infra-shared`）。
  - ✅ 当前工作区应仅残留未跟踪 `.mvnd/`（本地 mvnd daemon/registry 目录；禁止提交）。

- 🎯 **下一刀建议（保持行为不变；每次只改 1 个 `pom.xml` 闭环）**：
  1) 用 Serena 证伪 `bc-course/application` 是否仍需要 `eva-domain` 提供的任何类型（重点关注 `eva-domain` 残留类型与评教域已归位类型的复用面）。
  2) 若已证伪：收敛 `bc-course/application/pom.xml`：移除对 `eva-domain` 的 Maven 编译期依赖，改为显式依赖其实际需要的 `bc-*-domain`/`shared-kernel`（保持行为不变）。
...
### 推荐版（续接主线：依赖收敛（单 pom） —— 下一刀：`bc-course/application` 去 `eva-domain` 编译期依赖（保持行为不变））
...
2) Serena 证伪 `bc-course/application` 是否仍需要 `eva-domain` 提供的任何类型（尤其是 `eva-domain` 残留类型列表）。
3) 若已证伪：修改 `bc-course/application/pom.xml`：移除 `<artifactId>eva-domain</artifactId>` 依赖（每次只改 1 个 `pom.xml`；保持行为不变）。
