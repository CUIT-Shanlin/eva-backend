...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：继续推进 **S0.2 延伸（依赖方编译期依赖收敛）**：选择下一条“依赖方编译期依赖收敛”目标（优先从 `eva-domain` / `eva-infra-shared` 等依赖方开始），用 Serena 证据化盘点引用面与依赖闭包后再收敛 `pom.xml`（每次只改 1 个 pom；保持行为不变）。
  - 📌 **当前状态快照（截至 2026-01-19，口径=可复现命令；保持行为不变）**：
    ...
    - ⏳ `eva-adapter/pom.xml` 仍**声明** `bc-audit` / `bc-ai-report` 编译期依赖（口径：`rg -n '<artifactId>bc-(audit|ai-report)</artifactId>' eva-adapter/pom.xml`）。其中 `bc-audit` 过去可由残留的 `LogController`（引用 `edu.cuit.client.api.ILogService`）解释；现 `LogController` 已归位，下一会话可用 Serena 证据化盘点后按“单 pom 收敛”推进（每次只改 1 个 `pom.xml`；保持行为不变）。
...
1) 评估收敛 `eva-adapter/pom.xml` 对 `bc-audit` 的编译期依赖（现已无 `LogController` 残留；先 Serena 证据化盘点引用面/依赖闭包，再做“单 pom 收敛”）。
2) 同理评估 `bc-ai-report`：先 Serena 证据化盘点后再收敛（每次只改 1 个 `pom.xml`；保持行为不变）。
