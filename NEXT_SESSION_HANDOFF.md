...
- ✅ 快照证据（防回归，保持行为不变）：`rg -n "<artifactId>bc-course-domain</artifactId>" eva-domain/pom.xml` 无命中。
...
- ✅ 证据化盘点（Serena，保持行为不变）：`MsgEntity` 引用面覆盖 `bc-messaging/**`（UseCase/Port/FacadeImpl/PortImpl/测试）、`bc-evaluation/infrastructure`（`MsgServiceImpl`）、`eva-infra-shared`（`MsgBizConvertor/MsgConvertor`）以及 `eva-domain` 残留接口（`MsgGateway/MessageUseCaseFacade`）；且其类签名对外暴露 `UserEntity` 并使用 `@Entity`，因此后续搬运时需保证承接模块具备 `bc-iam-domain` 与 `cola-component-domain-starter` 的编译闭合。
...
- ✅ 最小回归（保持行为不变）：已执行 `mvnd -pl start -am test -Dtest=edu.cuit.app.eva.EvaRecordServiceImplTest,edu.cuit.app.eva.EvaStatisticsServiceImplTest ...`，`BUILD SUCCESS`（对应代码提交：`51d5a042`；本会话该回归后未再修改代码）。
...
- ✅ 快照（防回归，预期无命中）：`rg -n "<artifactId>bc-course-domain</artifactId>" eva-domain/pom.xml`（本次实际无命中）。
...
>
> 阶段性策略微调（2025-12-29，持续有效）：允许“微调”（仅结构性重构；不改业务语义；缓存/日志/异常文案/副作用顺序完全不变）。在“评教统计导出基础设施归位”与“课程课表解析归位/端口化”闭环后，`bc-messaging` 的“归位 + 依赖收敛”已阶段性闭环（见 0.9）；`bc-course` 的 **S0（旧 gateway 压扁为委托壳）** 已推进到阶段性闭环（见 0.9/0.10）；**S0.2 主目标**（`eva-domain` 去 `bc-course-domain` 编译期依赖）已闭环；当前主线切换为 **评教域：逐类把 `eva-domain` 内残留的 `edu.cuit.domain.gateway.eva.*` 归位到 `bc-evaluation-domain`**，仍按“小步可回滚 + 每步闭环”推进。
>

### 0.10.1 最新状态 & 下一步建议（滚动更新至 2026-02-06；聚焦：审计域类型逐类归位 + `eva-domain` 表面积收敛（保持行为不变））

- 🎯 **本次会话最新增量（2026-02-06，保持行为不变）**：
  ...
  - ✅ 可复现快照：`eva-domain` 残留 Java 文件数口径：`fd -t f -e java . eva-domain/src/main/java | wc -l`（当前为 3；清单口径：`fd -t f -e java . eva-domain/src/main/java`）。
  - ✅ 可复现快照：当前仍显式依赖 `eva-domain` 的模块口径：`rg -n "<artifactId>eva-domain</artifactId>" --glob "**/pom.xml" .`（当前至少包含 `bc-messaging`、`eva-infra-shared`）。
  - ✅ 当前工作区应仅残留未跟踪 `.mvnd/`（本地 mvnd daemon/registry 目录；禁止提交）。

- 🎯 **下一刀建议（保持行为不变；每次只改 1 个类闭环）**：
  1) 先跑快照证据（预期无命中，作为防回归）：`rg -n "<artifactId>bc-course-domain</artifactId>" eva-domain/pom.xml`。
  2) 用 Serena 证据化盘点 `MsgEntity` 引用面（重点：是否集中在消息域/是否跨 BC 复用；以及其对 `UserEntity`/`@Entity` 的编译期依赖面）。
  3) 将 `MsgEntity` 从 `eva-domain` 搬运归位到 `bc-messaging-contract`（每次只改 1 个类；保持 `package` 与类内容不变；确保全仓库该 FQCN 仅存在一份；保持行为不变）。
  4) 最小回归命令（Java17 + mvnd）通过后：`git commit`（代码）→ 同步三文档 → `git commit`（文档）→ `git push`。
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：
  - A（优先：类，每次只改 1 个类）：消息域类型逐类归位 —— 证伪并归位 `MsgEntity` → `bc-messaging-contract`（保持 `package`/类内容不变；确保全仓库该 FQCN 仅存在一份；最小回归通过后落地提交并同步三文档）。
  - B（次优先：类，每次只改 1 个类）：继续 IAM S0.2 延伸——收敛依赖方对 `edu.cuit.domain.entity.user.biz.UserEntity` 的编译期依赖，优先处理 `bc-course/infrastructure` 的读侧仓储 `CourseQueryRepository`（见上方 0.10.1 的 I/J 两刀）。
  - C（次优先：pom，每次只改 1 个 `pom.xml`）：继续做“依赖方 pom 收敛”，优先选择“仅类型引用”的依赖方模块；移除前必须 Serena + `rg` 双证据，避免增量编译/缓存导致误判（保持行为不变）。
  - D（并行注意事项，保持行为不变）：不要直接把 `bc-ai-report/infrastructure` 对 `bc-evaluation` 的依赖替换为 `bc-evaluation-contract` 来“省依赖”；评教导出端口签名当前会触发 Maven 循环依赖（原因见 0.9 / `DDD_REFACTOR_PLAN.md` 10.3）。
...
### 推荐版（续接主线：消息域类型逐类归位 —— 下一刀：证伪并归位 `MsgEntity` → `bc-messaging-contract`（保持行为不变））
...
- 每次只改 1 个类（允许新增/搬运单个类）或只改 1 个 `pom.xml`

本轮主线（保持行为不变）：
1) 先跑快照证据：  
   - `rg -n "<artifactId>bc-course-domain</artifactId>" eva-domain/pom.xml`（预期无命中：上一刀已闭环）
2) Serena 证据化盘点 `MsgEntity` 引用面（重点：是否集中在消息域/是否跨 BC 复用；以及其对 `UserEntity`/`@Entity` 的编译期依赖面）。
   - 背景：当前 `eva-domain` 残留消息域类型：`MsgEntity/MsgGateway/MessageUseCaseFacade`；且 `bc-messaging-contract` 已补齐承接 `MsgEntity` 的最小编译期依赖（`bc-iam-domain`、`cola-component-domain-starter`）。
3) 将 `MsgEntity` 从 `eva-domain` 搬运归位到 `bc-messaging-contract`（每次只改 1 个类；保持 `package`/类内容不变；保持行为不变）。
4) 最小回归命令（每步结束必跑，Java17，必须 mvnd）：  
...
5) `git commit`（代码）→ 同步三文档 → `git commit`（文档）→ `git push`

风险提示（保持行为不变）：
- `CourseEntity` 当前编译期依赖 `UserEntity`（已在 `bc-iam-domain`），因此 `bc-course-domain` 仍保留对 `bc-iam-domain` 的编译期依赖作为过渡（保持行为不变；见 0.9）。
