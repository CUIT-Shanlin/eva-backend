...
- 📊 **整体未完成清单（更新至 2026-02-17；保持行为不变）**：
  ...
  - `eva-infra-shared`：当前仍有 `1` 个 Java（仅 `UserConverter`），且 `bc-course/infrastructure`、`bc-evaluation/infrastructure`、`bc-iam/infrastructure` 仍直依赖该模块（可复现口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l` + `rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | sort`）。
  ...
  - `eva-infra-shared` 残留结构（Serena + `rg` 证据化，保持行为不变）：仅剩 `UserConverter` 这 1 个 MapStruct Convertor（`CourseBizConvertor/CourseConvertor` 已归位到 `bc-course/infrastructure`，见 0.9/4.2），但其引用面仍跨 `bc-course/bc-evaluation/bc-iam`，且编译期依赖 `bc-iam-domain` 的实体类型；因此 **不建议直接下沉 `shared-kernel`**（仍存在 Maven 环依赖风险）。退场需走“先收敛跨 BC 使用点到最小 Port（建议归口 bc-iam）→ 再单类搬运归位 → 再逐 pom 去依赖 → 最后 reactor 退场”的节奏。LDAP 子簇已完成 `EvaLdapProperties/LdapGroupDO/LdapGroupRepo/LdapConstants/EvaLdapUtils` 下沉到 `shared-kernel`（保持包名不变）。
...
1) 证据口径（先跑快照，避免口径漂移）：
   ...
   - `fd -t f -e java . eva-infra-shared/src/main/java | wc -l`（当前预期=1）
   - `fd -t f -e java . eva-infra-shared/src/main/java | sort`（预期仅 1 个：`.../UserConverter.java`）
...
3) 🎯 本轮主线（瘦身 `eva-infra-shared`，保持行为不变）：
   ...
   - 备选主线（更稳，非 LDAP）：对 `UserConverter` 做 Serena 引用面证据化后，按“最小 Port 收敛跨 BC 使用点 → 搬运归位 → 逐 pom 去依赖 → reactor 退场”的节奏收尾 `eva-infra-shared`（**不建议** 直接下沉 `shared-kernel`：仍存在 Maven 环依赖风险；详见 0.10.1 的证据化结论）。
   - 🎯 Convertor 收尾路线（当前唯一阻塞：`UserConverter`，保持行为不变；每次只改 1 个文件闭环）：
     1) Serena：证据化 `UserConverter` 引用面（当前跨 `bc-course/infrastructure`、`bc-evaluation/infrastructure`、`bc-iam/infrastructure`）。
     2) 单类（Port 定义）：在 `bc-iam/application` 新增“最小字段读取/桥接 Port”（命名建议：`UserEntityFieldExtractPort`，但以仓库既有命名风格为准），仅暴露跨 BC 真正需要的桥接方法（优先从 `userIdOf/usernameOf/statusOf/...` 中按引用面最小集裁剪；保持异常文案/空值形态不变）。
     3) 单类（Port Adapter）：在 `bc-iam/infrastructure` 增加 Port Adapter 实现，内部 **直接委托** `UserConverter.*`（保持调用次数/顺序与异常形态不变）。
     4) 单类（调用侧收敛，逐点推进）：优先改 `bc-evaluation/infrastructure` 的 `EvaTaskQueryRepository` / `EvaRecordQueryRepository` 与 `bc-course/infrastructure` 的 `CourseQueryRepository`，改为依赖该 Port（不再直接注入 `UserConverter`）。
     5) 单类搬运（归位）：Serena 证伪跨 BC 引用面清零后，将 `UserConverter` 从 `eva-infra-shared` 搬运归位到 `bc-iam/infrastructure`（保持 `package` 不变），随后再逐 pom 收敛依赖并评估 `eva-infra-shared` reactor 退场。
4) 每刀结束必须闭环：
