...
> 下一步建议（读侧优先，单类闭环，保持行为不变）：继续清理评教读侧/评教侧对课程域其它 Mapper 的编译期直连，优先 `SubjectMapper`（候选：`EvaTaskQueryRepository`、`EvaRecordQueryRepository`）。端口前置 `SubjectObjectDirectQueryPort` 已补齐（仅接口，无实现/无调用点，避免行为漂移），下一刀按“补适配器（`bc-course/infrastructure`，单类）→ 改调用侧（`bc-evaluation/infrastructure`，单类）”推进。端口前置 `CourseIdsByTeacherIdAndSemesterIdQueryPort` 仍无实现/无调用点，可按需后续补齐。
...
1) 读侧优先（保持行为不变）：评教读侧 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/bcevaluation/query` 已清零对课程域 `CourseMapper/SemesterMapper` 的编译期直连；评教侧 `bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/gateway/impl/eva/EvaUpdateGatewayImpl.java` 也已清零对 `CourseMapper.selectById(...).getSemesterId()` 的编译期直连（详见 0.9/4.2）。
   - 下一刀建议（保持行为不变；每次只改 1 个类闭环）：继续清理评教读侧对课程域 `SubjectMapper` 的编译期直连（候选：`EvaTaskQueryRepository`、`EvaRecordQueryRepository`），按“补端口（`bc-course/application`，单类）→ 补适配器（`bc-course/infrastructure`，单类）→ 改调用侧（`bc-evaluation/infrastructure`，单类）”推进。
   - 进展提示（保持行为不变）：`CourseAndSemesterObjectDirectQueryPort`（含适配器）已就位并被评教读侧/评教侧使用；端口前置 `CourseIdsByTeacherIdAndSemesterIdQueryPort` 仍无实现/无调用点，可按需后续补齐（用于收敛“按 teacherId + semesterId 查询课程ID列表”场景）。
