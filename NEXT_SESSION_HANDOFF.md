...
2) 🎯 下一刀候选（单类，优先“无 `eva-infra-shared` 内部依赖”的叶子类）：从剩余 `9` 个 Java 中先做 Serena 证据化盘点，命中叶子类后执行单类搬运；若本轮无叶子类可搬运，则切回 `eva-infra-dal` 执行“单资源 XML 归位”。  
   - 执行策略：先用 Serena 对候选类做“引用面 + 内部依赖”双证伪；满足叶子约束则单类搬运，否则按“单 XML（Mapper 仅单 BC 引用）”推进。
