...
5) 🎯 **IAM 并行（按 10.3，保持行为不变）**：继续挑选一个依赖方类，把其对 `UserQueryGateway` 的编译期依赖收敛为 `bc-iam-contract` 的最小 Port（优先复用 `UserBasicQueryPort`/`UserNameQueryPort`；若缺口明显且稳定，再在 `bc-iam-contract` 新增“最小接口”补齐）。
...
- B（次优先：类，每次只改 1 个类）：继续推进 IAM 并行（10.3）：在依赖方模块内选 1 个类，将其对 `UserQueryGateway` 的依赖收敛为 `bc-iam-contract` 的最小 Port（行为不变；必要时先补 `pom.xml` 显式依赖）。
