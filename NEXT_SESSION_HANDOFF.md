...
5) 🎯 **下一刀聚焦（保持行为不变）**：用 Serena + `rg` 证据化盘点 `bc-iam/**` 残留的 `UserEntity` 编译期依赖点，择一闭环（每次只改 1 个类）。当前快照：预计仅剩旧 `UserQueryGatewayImpl`（高风险：承载缓存/切面触发点）。
...
下一刀（保持行为不变，优先顺序 A → B → C → D → E；每次只改 1 个 pom 或 1 个类）：
...
- 🎯 E（下一刀建议，优先做“证据化盘点 + 可执行方案”，保持行为不变）：用 Serena 盘点 `bc-iam/infrastructure/src/main/java/edu/cuit/infra/gateway/impl/user/UserQueryGatewayImpl.java` 的“对外签名/调用面/缓存注解触发点”，再决定是否要把该类也纳入“去 `UserEntity` 编译期依赖”的收尾；若其接口签名仍返回 `UserEntity`，则单类变更大概率无法彻底消除编译期依赖，需拆成多步推进（仍按每次只改 1 个类闭环）。
