...
**2026-01-13（补充：课程域去编译期耦合推进——学期/教室旧入口归位，保持行为不变）**
...
- ✅ **课程（依赖收敛前置：旧入口归位，学期，保持行为不变）**：将 `SemesterServiceImpl` 从 `eva-app` 搬运归位到 `bc-course-infra`（保持 `package edu.cuit.app.service.impl` 不变；仍实现 `ISemesterService` 并委托 `SemesterQueryUseCase`；保留 `@Transactional`；仅改变类所在 Maven 模块以减少 `eva-app` → `bc-course` 的编译期耦合面；最小回归通过）；落地提交：`8eddc643`。
- ✅ **S0.2 延伸（次优先：其它依赖收敛点盘点，证伪结论，保持行为不变）**：当前阻塞主要剩余课程域：Serena + `rg` 证据化确认 `eva-app` 仍直接 `import edu.cuit.bc.course.application.usecase.*`（`ICourseTypeServiceImpl`），因此暂不可将 `eva-app/pom.xml` 的 `bc-course` 编译期依赖移除/替换为更轻的协议层依赖；建议优先把剩余 1 个旧入口类按“保持包名不变”的方式搬运归位到 `bc-course-infra`，再回到 `pom.xml` 收敛（保持行为不变）。
  ...
  - ✅ **`eva-client` 退场**：已从 reactor 移除并从仓库删除；跨 BC 通用对象已开始沉淀 `shared-kernel`（见 10.5）。
	  - ✅ **已闭环（本阶段目标 B：基础设施旧 gateway 归位，保持行为不变）**：`eva-infra/src/main/java/edu/cuit/infra/gateway/impl` 下残留旧 `*GatewayImpl.java` 已清零（Serena 盘点口径）；归位闭环包含：
	    ...
	    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
		  - ⏳ **仍未完成（核心阻塞项）**：
			    1) `eva-app` 仍保留 **13 个** `*ServiceImpl.java`（口径：`fd -t f 'ServiceImpl\\.java$' eva-app/src/main/java/edu/cuit/app/service/impl | wc -l`），现已全部退化为“仅切面/登录态解析/委托 UseCase/网关”的壳（保持行为不变）。
				       - 已显式委托 UseCase（共 12 个，口径：`rg -l \"\\bUseCase\\b|application\\.usecase\" eva-app/src/main/java/edu/cuit/app/service/impl`）：`EvaRecordServiceImpl/EvaStatisticsServiceImpl/EvaTaskServiceImpl/EvaTemplateServiceImpl/UserEvaServiceImpl/RoleServiceImpl/MenuServiceImpl/LogServiceImpl/UserServiceImpl/UserAuthServiceImpl/DepartmentServiceImpl/ICourseTypeServiceImpl`
...
- **只改 1 个类（优先）**：将 `eva-app` 的 `ICourseTypeServiceImpl` 搬运归位到 `bc-course-infra`（保持 `package edu.cuit.app.service.impl.course` 不变；行为不变）。
- **在上述类完成后（只改 1 个 pom）**：Serena + `rg` 证伪 `eva-app` 不再 `import edu.cuit.bc.course.*` 后，再收敛 `eva-app/pom.xml`：移除对 `bc-course` 的编译期依赖（改为显式依赖 `shared-kernel`；保持行为不变）。
