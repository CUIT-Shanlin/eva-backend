...
- ✅ **IAM（S0.2 延伸：去 `eva-domain` 前置拆解，保持行为不变）进展**：已将 `MenuQueryGateway`、`LdapPersonEntity`（以及前置的 `MenuEntity/UserUpdateGateway/...`）逐类归位到 `bc-iam-domain`（均保持 `package`/签名/注解不变，仅改变 Maven 模块归属）；下一刀建议优先归位 `LdapPersonGateway`（接口，签名依赖 `LdapPersonEntity`，风险更低）。
...
4) 🎯 **IAM 优先（保持行为不变）**：已完成 `bc-iam-infra` 去 `bc-evaluation` 编译期依赖（详见 0.9）；且已逐类归位 `MenuQueryGateway`、`LdapPersonEntity` 到 `bc-iam-domain`（保持 `package`/签名/注解不变）。下一刀建议优先归位 **`LdapPersonGateway`**（接口），避免提前触碰跨 BC 复用的 `RoleEntity/RoleQueryGateway`。

下一刀（保持行为不变，二选一，先做 A 再做 B）：
- A（优先：pom，每次只改 1 个 `pom.xml`）：选择一个“仅类型引用”的依赖方模块做编译期依赖收敛；移除前先 Serena 证据化并补一条 `rg` 兜底证据，避免误判重演。
- B（备选：类，每次只改 1 个类）：继续推进 IAM 去 `eva-domain` 的前置搬运；优先从 **`LdapPersonGateway`** 开始（接口，签名依赖 `LdapPersonEntity`，搬运后更利于收敛 `bc-iam(application)` → `eva-domain` 的依赖边界）。
...
- `rg -n \"interface\\s+LdapPersonGateway\\b\" eva-domain/src/main/java/edu/cuit/domain/gateway/user/LdapPersonGateway.java`（应命中：下一刀候选接口仍在 `eva-domain`，便于 Serena 证据化后搬运）
