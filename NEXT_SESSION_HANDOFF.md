...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：继续推进 `eva-app` 的 **websocket** 编译期耦合面收敛，按“先归位配置/拦截器/支撑类 → 再收敛 pom 依赖”的套路推进。
  ...
  5) ✅ 已完成（保持行为不变）：拦截器已归位：`WebSocketInterceptor` 已从 `eva-app` 归位到 `eva-infra-shared`（保持 `package edu.cuit.app.config` 不变；落地：`df06f0e6`）。
  6) ✅ 已完成（保持行为不变）：配置已归位：`WebSocketConfig` 已从 `eva-app` 归位到 `eva-infra-shared`（保持 `package edu.cuit.app.config` 不变；落地：`e03e60f9`）。
  7) ✅ 已完成（保持行为不变；只改 1 个 pom）：当 `rg -n '^import\\s+org\\.springframework\\.web\\.socket' eva-app/src/main/java` 证伪为 0 后，已移除 `eva-app/pom.xml` 对 `spring-boot-starter-websocket` 的编译期依赖（落地：`4213a95a`）；运行期由组合根 `start` 已显式兜底。
- ✅ **完成条件（评估点）**：
...
- 快速证据口径（新会话先复核，避免口径漂移）：
  ...
  - websocket：`rg -n '^import\\s+org\\.springframework\\.web\\.socket' eva-app/src/main/java`（当前应为 0；用于后续依赖收敛评估点）。
...
### G 推荐版（当前主线：S0.2 延伸（websocket：旧入口/装配归位 → 依赖收敛前置），优先）
...
2) 🎯 当前主线：继续推进 **S0.2 延伸（websocket：旧入口/装配归位 → 依赖收敛前置）**：逐步减少 `eva-app` 对 `spring-boot-starter-websocket` 的编译期耦合面，方式为“websocket 相关配置/拦截器/通道处理器逐个归位到 `eva-infra-shared`（保持 `package` 不变）”，并在引用面清零后收敛 `eva-app/pom.xml` 去 `spring-boot-starter-websocket` 编译期依赖（每次只改 1 个类或 1 个 pom；保持行为不变）。补充：审计（`bc-audit`）与 AI 报告（`bc-ai-report`）的 `eva-app` 编译期依赖收敛已在本会话闭环（详见 0.9）。
...
- `rg -n '^import\\s+org\\.springframework\\.web\\.socket' eva-app/src/main/java`（当前应为 0；用于后续依赖收敛评估点）
