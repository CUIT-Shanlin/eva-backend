...
- 📊 **整体未完成清单（更新至 2026-02-07；保持行为不变）**：
  ...
  - `eva-infra-shared`：仍剩 `32` 个 Java，且多 BC 仍直依赖该模块（可复现口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l` + `rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | head`）。
