...
- 📅 **下一步排期建议（按“刀”计，保持行为不变；按产能 2–4 刀/天粗估）**：
  ...
  - P1（约 2–4 刀）：优先继续拆散 `eva-infra-dal`（当前 `7` 个 Java）→ 逐类归位/下沉 `DO` 与少量公共工具类（每刀 1 类；保持行为不变；`package` 不变）。建议顺序：`SubjectDO`（大概率跨 BC → 下沉 `shared-kernel`）→ 评教侧 `CourOneEvaTemplateDO/FormTemplateDO/FormRecordDO/EvaTaskDO` → 再评估 `CourseTypeDO`（需谨慎）→ `EntityFactory`（若引用面收敛为单 BC 再归位，否则继续留作共享）。
...
3) 🎯 本轮主线（承接已归位 XML，继续拆散 `eva-infra-dal`，保持行为不变）：
   - ✅ 已闭环（保持行为不变）：`SysUserMapper` 归位 + 非 IAM 依赖清零已完成（详见 `NEXT_SESSION_HANDOFF.md` 0.9/0.10.1；本轮无需再动）。
   - 下一刀建议（每次只改 1 个类；保持行为不变）：`SubjectDO`（Serena 证据化；若引用面跨 BC → 下沉到 `shared-kernel`；若可证伪为单 BC 引用 → 归位对应 BC `infrastructure`；`package` 不变）。
   - 然后逐类（每次只改 1 个类；保持行为不变）：`CourOneEvaTemplateDO` → `FormTemplateDO` → `FormRecordDO` → `EvaTaskDO`。
   - 风险提示（保持行为不变）：`CourseTypeDO` 引用面可能跨 `bc-course/**` 与 `bc-evaluation/**`，需谨慎评估，避免依赖/装配边界漂移。
   - `EntityFactory`（保持行为不变）：仅当引用面可证伪为“单 BC/单模块”时再归位，否则继续留在 `eva-infra-dal` 作为共享。
4) 每刀结束必须闭环：
