...
- 🎯 **下一刀（建议，仍按“每次只改 1 个 pom.xml”）**：继续盘点 `eva-infra/pom.xml` 中对 `bc-evaluation` / `bc-iam` / `bc-audit` 等业务模块的编译期依赖，优先尝试收敛到各自的 `*-contract` 或 `*-domain`（前置必须 Serena + `rg` 证伪：仅类型引用、无实现/副作用耦合；保持行为不变）。
...
### F 推荐版（当前主线：S0.2 延伸（依赖方 pom 依赖收敛 / 装配责任上推），优先）
...
本轮目标（F，优先；保持行为不变）：
1) ✅ 已完成：**课程（S0.2 延伸——继续削减 `eva-app` 对 `bc-course` 的编译期耦合）**：已通过“只搬运旧入口类（保持包名不变）”清空 `eva-app` 对 `edu.cuit.bc.course.application.usecase.*` 的直接引用，并收敛 `eva-app/pom.xml` 移除 `bc-course` 编译期依赖（显式依赖 `shared-kernel`；保持行为不变）。
2) ✅ 已完成：**模板/课程（S0.2 延伸——依赖层级收敛，保持行为不变）**：已将模板锁定相关类型（`CourseTemplateLockQueryPort`/`CourseTemplateLockService`/`TemplateLockedException`）下沉到 `bc-template-domain`（包名不变），并收敛多个依赖方的 pom（例如 `eva-infra`、`bc-course(application)`）将对 `bc-template` 的编译期依赖替换为 `bc-template-domain`。
3) 🎯 下一步：继续推进 **S0.2 延伸（依赖方 `pom.xml` 依赖收敛）**：逐步减少依赖方对业务实现模块的编译期耦合（每次只改 1 个 `pom.xml`；保持行为不变；需先 Serena + `rg` 证据化确认仅类型引用、无实现/副作用耦合）。优先候选：从 `eva-infra/pom.xml` 的 `bc-evaluation/bc-iam/bc-audit` 等依赖入手，评估能否收敛到 `*-contract` 或 `*-domain`。

下一刀（保持行为不变）：
- ✅ 已完成：将 `eva-app` 的 `ClassroomServiceImpl` 搬运归位到 `bc-course-infra`（保持 `package edu.cuit.app.service.impl` 不变；仍实现原接口并委托 `ClassroomQueryUseCase`；不改缓存/日志/异常文案/副作用顺序；保持行为不变）。
- ✅ 已完成：将 `eva-app` 的 `ICourseTypeServiceImpl` 搬运归位到 `bc-course-infra`（保持 `package edu.cuit.app.service.impl.course` 不变；行为不变）。
- ✅ 已完成：收敛 `eva-app/pom.xml`：移除对 `bc-course` 的编译期依赖（`shared-kernel` 显式依赖保留；保持行为不变）。
- ✅ 已完成：收敛 `bc-course/application/pom.xml`：将对 `bc-template` 的编译期依赖替换为 `bc-template-domain`（保持行为不变）。
- 下一刀：优先从 `eva-infra/pom.xml` 的业务模块依赖（`bc-evaluation/bc-iam/bc-audit` 等）入手，逐个证伪后每次只改 1 个 `pom.xml` 收敛编译期依赖边界（保持行为不变）。
