...
**2026-02-04（本次会话：IAM 并行（10.3）：测试过渡收敛；保持行为不变）**
...
- ⚠️ **踩坑记录（避免复现）**：曾尝试把 `RoleQueryGateway` 从 `eva-domain` 归位到 `bc-iam-domain`，但其接口签名依赖 `RoleEntity`（当前跨 BC 复用：审计/课程/评教等均引用），会导致 `bc-iam-domain` 编译失败；因此暂不动 `RoleQueryGateway/RoleEntity`，优先继续推进 “IAM-only 的 LDAP 相关类型/接口”。
...
- 🎯 **本次会话最新增量（2026-02-04，保持行为不变）**：
  ...
  - ⏳ 当前 `bc-evaluation/infrastructure` 仍残留 `UserEntity` import：`EvaTaskQueryRepository`、`EvaRecordQueryRepository`（可复现口径：`rg -n "import\\s+edu\\.cuit\\.domain\\.entity\\.user\\.biz\\.UserEntity;" bc-evaluation/infrastructure/src/main/java`）。

- 🎯 **下一刀建议（保持行为不变；每次只改 1 个类闭环）**：
  1) A（类，前置）：`eva-infra-shared/src/main/java/edu/cuit/infra/convertor/user/UserConverter.java` 增加桥接方法 `toUserEntityObject(...)`（返回 `Object`）与 `userIdOf(Object)`（返回 `Integer`；内部仍使用 `UserEntity` 做强转，以保持历史空值/异常表现尽量一致）。
  2) B（类，前置）：`eva-infra-shared/src/main/java/edu/cuit/infra/convertor/course/CourseConvertor.java` 增加桥接方法 `toCourseEntityWithTeacherObject(...)`（teacher Supplier 改为 `Supplier<?>`，内部桥接到既有 `toCourseEntity`）。
  3) C（类，主线）：`bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/bcevaluation/query/EvaTaskQueryRepository.java` 去 `UserEntity` import：调用侧改走上述桥接方法与 `userIdOf(Object)`，并尽量保持 DB 查询/遍历顺序不变。
  4) D（类，主线）：`bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/bcevaluation/query/EvaRecordQueryRepository.java` 同上。
...
- ✅ **IAM（S0.2 延伸：去 `eva-domain` 前置拆解，保持行为不变）进展**：已将 `MenuQueryGateway`、`LdapPersonEntity`、`LdapPersonGateway`（以及前置的 `MenuEntity/UserUpdateGateway/...`）逐类归位到 `bc-iam-domain`（均保持 `package`/签名/注解不变，仅改变 Maven 模块归属）；下一刀建议：继续按“引用面仅在 IAM”逐类归位剩余 `edu.cuit.domain.*` 子集（避免提前触碰跨 BC 复用的 `RoleEntity/RoleQueryGateway`）。
...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：
  - A（优先：类，每次只改 1 个类）：继续 IAM 并行（10.3）——优先清理 `eva-infra-shared` 侧对 `UserQueryGateway` 的编译期依赖。
    - 候选 1：`eva-infra-shared/src/main/java/edu/cuit/app/security/StpInterfaceImpl.java`（Sa-Token 权限接口实现；优先复用现有 `bc-iam-contract` 端口；若缺口需拆成“新增最小 Port → 新增端口适配器 → 再改本类”三刀闭环）
    - 候选 2：`eva-infra-shared/src/main/java/edu/cuit/app/convertor/MsgBizConvertor.java`（MapStruct Convertor；先证据化其使用 `UserQueryGateway` 的方法面，再决定复用现有 Port 或新增最小 Port）
  - C（pom，每次只改 1 个 `pom.xml`）：继续做“依赖方 pom 收敛”，优先选择“仅类型引用”的依赖方模块。
  ...
  - 📌 **当前状态快照（截至 2026-01-29，口径=可复现命令；保持行为不变）**：
    ...
    - ⏳ “全量整合 `eva-*`（并从 reactor 移除）”仍未具备落地条件：root reactor 仍包含 `eva-domain/eva-infra-dal/eva-infra-shared/eva-base`（口径：`rg -n "<module>eva-" pom.xml`），且仍有多个 `bc-*` 模块编译期依赖 `eva-domain` 与 `eva-infra-(shared|dal)`（口径：`rg -n "<artifactId>eva-domain</artifactId>" --glob "**/pom.xml" .` 与 `rg -n "<artifactId>eva-infra-(shared|dal)</artifactId>" --glob "**/pom.xml" .`）。补充：`eva-infra` 已从 root reactor 退场（落地：`0aab4516`），组合根 `start` 已移除 `eva-infra(runtime)` 兜底依赖（落地：`1e2ffa89`）。整合判定标准与路线见 `DDD_REFACTOR_PLAN.md` 10.5（坚持“每步只改 1 个类/1 个 pom + 最小回归 + 可回滚”）。
  - ✅ **完成条件（评估点）**：
		  ...
		  - ✅ **已闭环（本阶段目标 B：基础设施旧 gateway 归位，保持行为不变）**：`eva-infra/src/main/java/edu/cuit/infra/gateway/impl` 下残留旧 `*GatewayImpl.java` 已清零（Serena 盘点口径）；归位闭环包含：
		    ...
		    - 支撑类（编译闭合前置）：`StaticConfigProperties` → `eva-infra-shared`（保持 `package` 不变；用于闭合 `EvaConfigGatewayImpl` 归位）
			  - ⏳ **仍未完成（核心阻塞项）**：
			    ...
			    2.2) ⏳ **`eva-*` 技术切片仍未具备“全量整合进各 BC”的前置条件**：root reactor 仍包含 `eva-domain` / `eva-infra-dal` / `eva-infra-shared` / `eva-base`（可复现口径：`rg -n "<module>eva-" pom.xml`）。保持行为不变前提下，当前仍不能直接移除这些模块。补充“可量化快照”（口径=可复现命令）：`eva-domain` 29 个 Java 文件、`eva-infra-dal` 36 个、`eva-infra-shared` 47 个。补充：`eva-infra` 已从 root reactor 退场（落地：`0aab4516`），目录下仅剩 3 个 `package-info.java`，可后置清理（独立提交）。粗略工作量估算：若坚持“root reactor 不含任何 `eva-*` module（含 `eva-domain/eva-infra-dal/eva-infra-shared/eva-base`）”，仅按当前 `eva-*` 下的 Java 文件计，至少约 112 次“单文件闭环”；若按 `DDD_REFACTOR_PLAN.md` 10.5 推荐先把共享技术模块（`eva-infra-dal/eva-infra-shared`）视为平台模块保留，则可优先聚焦“逐 BC 清空对 `eva-domain` 的编译期依赖”，阶段性收益更高且风险更低（保持行为不变）。
			    2.3) ⏳ **`eva-domain` 仍被多个 BC 编译期依赖（因此暂不能移除/整合）**：当前至少包含 `bc-course/application`、`bc-evaluation/application`、`bc-iam/application`、`bc-audit/application`、`bc-messaging`、`bc-ai-report/application` 等仍显式依赖 `eva-domain`（可复现口径：`rg -n "<artifactId>eva-domain</artifactId>" --glob "**/pom.xml" .`）。下一阶段的“整合 eva-*”需要先把 `edu.cuit.domain.*` 的类型/接口按 BC 归位（每次只改 1 个类或 1 个 pom；保持行为不变）。建议优先从“引用面集中且依赖最少”的纯 POJO/纯接口入手，逐步让每个依赖方先做到“可移除 `eva-domain` 编译期依赖”，最后再评估 `eva-domain` 模块退场。
  ...
  5) **路线 B（后置，成本更高）**：若后续发现 `shared-kernel` 承载课程域接口/CO 规模继续膨胀，可再引入 `bc-course-contract`（或更中立的 contract 模块）承载这些接口/CO（保持 `package` 不变），并逐步把依赖方从 `shared-kernel` 切到该 contract（每步闭环；保持行为不变）。
	...
	- Q：什么时候可以把 `eva-*` 技术切片“全部整合进各业务 bc-* 模块”？
	  - A：不要用固定日期衡量，按 **可验证的 DoD**（见 `DDD_REFACTOR_PLAN.md` 10.5）：
	    ...
	    4) **现状（更新至 2026-02-02）**：root reactor 仍包含 `eva-domain/eva-infra-dal/eva-infra-shared/eva-base`，且多个 `bc-*` 仍编译期依赖 `eva-domain`（例如 `bc-iam/application`、`bc-course/application`、`bc-evaluation/application`、`bc-messaging`）。因此短期目标应先聚焦：继续按 S0.2 延伸做“类型/端口归位 + 单 pom 依赖收敛”，把依赖面逐步压缩后再评估移除模块（保持行为不变）。
...
### 推荐版（续接主线：IAM S0.2 延伸 —— 收敛 `bc-iam/application` 对 `UserEntity` 等旧领域类型的编译期依赖（行为不变））
...
4) 🎯 **IAM 优先（保持行为不变）**：已完成 `bc-iam-infra` 去 `bc-evaluation` 编译期依赖（详见 0.9）；且已逐类归位 `MenuQueryGateway`、`LdapPersonEntity`、`LdapPersonGateway` 到 `bc-iam-domain`（保持 `package`/签名/注解不变）。下一刀建议：继续按 Serena 证据化盘点，逐类归位“仅 IAM 使用”的 `edu.cuit.domain.*`（避免提前触碰跨 BC 复用的 `RoleEntity/RoleQueryGateway`）。
5) 🎯 **IAM 并行（10.3，保持行为不变）阶段性结论**：
   - ✅ 已闭环：`eva-infra-shared` 已清零对 `UserQueryGateway` 的编译期依赖并收敛为依赖 `bc-iam-contract` 最小 Port；端口适配器内部仍委托旧 `UserQueryGateway` 以保持缓存/切面触发点不变（见 0.9 与 `docs/DDD_REFACTOR_BACKLOG.md` 4.2/4.3）。
   - 🎯 下一刀转向：继续收敛 `bc-iam/application` 对 `UserEntity` 等旧领域实体的编译期依赖（优先复用“登录校验”闭环经验：新增最小 Port → 复用/补齐适配器委托旧 gateway → 再改依赖方；保持调用次数/缓存触发点不变）。
...
下一刀（保持行为不变，优先顺序 A → B → C；每次只改 1 个 pom 或 1 个类）：
- A（优先：类，每次只改 1 个类）：`bc-iam/application/src/main/java/edu/cuit/bc/iam/application/usecase/FindUserByIdUseCase.java` 去 `UserEntity` 编译期依赖（先用 Serena 证据化“方法面”使用，再决定复用现有 Port 或新增最小 Port；端口适配器内部仍委托旧 gateway，保持缓存/切面触发点不变）。
- B（次优先：类，每次只改 1 个类）：`bc-iam/application/src/main/java/edu/cuit/bc/iam/application/usecase/FindUserByUsernameUseCase.java` 去 `UserEntity` 编译期依赖（策略同 A）。
- C（备选：pom，每次只改 1 个 `pom.xml`）：选择一个“仅类型引用”的依赖方模块做编译期依赖收敛；移除前先 Serena 证据化并补一条 `rg` 兜底证据，避免误判重演。
...
- `rg -n \"interface\\s+UserEntityByIdQueryPort\\b\" bc-iam/contract/src/main/java/edu/cuit/bc/iam/application/port/UserEntityByIdQueryPort.java`（确认最小端口已存在）
- `rg -n \"class\\s+UserEntityByIdQueryPortImpl\\b\" bc-iam/infrastructure/src/main/java/edu/cuit/infra/bciam/adapter/UserEntityByIdQueryPortImpl.java`（确认端口适配器已存在）
- `rg -n \"UserQueryGateway\" bc-messaging/src/main/java/edu/cuit/infra/bcmessaging/adapter/MessageQueryPortImpl.java || true`（应为空：确认消息查询端口已去旧 gateway 编译期依赖）
- `rg -n \"\\bUserQueryGateway\\b\" --glob \"*.java\" bc-* | head`（应仅命中 `bc-iam/**` 的旧 gateway 与端口适配器，避免依赖方回潮）
- `rg -n \"\\bUserQueryGateway\\b\" eva-infra-shared/src/main/java || true`（应为空：复核 `eva-infra-shared` 已去旧 gateway 编译期依赖）
- `rg -n \"import edu\\.cuit\\.domain\\.entity\\.user\\.biz\\.UserEntity\" bc-iam/application/src/main/java`（定位 `bc-iam/application` 仍残留的 `UserEntity` 依赖点，用于下一刀排期）
- `rg -n \"<artifactId>bc-evaluation</artifactId>\" bc-iam/infrastructure/pom.xml`（应为空：复核 `bc-iam-infra` 已去 `bc-evaluation` 编译期依赖）
- `rg -n \"<artifactId>bc-iam-domain</artifactId>\" bc-iam/application/pom.xml`（应命中：复核 `bc-iam(application)` 已显式依赖 `bc-iam-domain`，用于后续逐类搬运）
- `rg -n \"<artifactId>bc-iam-contract</artifactId>\" bc-iam/domain/pom.xml`（应命中：复核 `bc-iam-domain` 已显式依赖 `bc-iam-contract`，用于后续搬运 gateway 接口）
- `rg -n \"class\\s+LdapPersonEntity\\b\" bc-iam/domain/src/main/java/edu/cuit/domain/entity/user/LdapPersonEntity.java`（应命中：复核 `LdapPersonEntity` 已归位到 `bc-iam-domain`）
- `rg -n \"interface\\s+LdapPersonGateway\\b\" bc-iam/domain/src/main/java/edu/cuit/domain/gateway/user/LdapPersonGateway.java`（应命中：复核 `LdapPersonGateway` 已归位到 `bc-iam-domain`）
- `rg -n \"interface\\s+EvaRecordCountQueryPort\\b\" bc-evaluation/contract/src/main/java/edu/cuit/bc/evaluation/application/port/EvaRecordCountQueryPort.java`（复核端口接口已下沉到 contract）
...
3) 下一刀建议（每次只改 1 个类；保持行为不变；按“前置桥接 → 改调用侧”的顺序）：
   - A：`eva-infra-shared/src/main/java/edu/cuit/infra/convertor/user/UserConverter.java` 增加 `toUserEntityObject(...)` + `userIdOf(Object)`。
   - B：`eva-infra-shared/src/main/java/edu/cuit/infra/convertor/course/CourseConvertor.java` 增加 `toCourseEntityWithTeacherObject(...)`。
   - C：`bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/bcevaluation/query/EvaTaskQueryRepository.java` 去 `UserEntity` 编译期依赖。
   - D：`bc-evaluation/infrastructure/src/main/java/edu/cuit/infra/bcevaluation/query/EvaRecordQueryRepository.java` 去 `UserEntity` 编译期依赖。
