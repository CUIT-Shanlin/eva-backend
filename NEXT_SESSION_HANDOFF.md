...
## 0.9 本次会话增量总结（2025-12-23，更新至 `HEAD`）
...
- ✅ 提交点 C5-1（读侧实现继续拆，统计主题）：新增 `EvaStatisticsQueryRepository` 承接 `EvaStatisticsQueryRepo` 实现，`EvaQueryRepository` 中对应方法退化为委托（口径/异常文案不变；落地提交：`9e0a8d28`）。
- ✅ 提交点 C5-2（读侧实现继续拆，记录主题）：新增 `EvaRecordQueryRepository` 承接 `EvaRecordQueryRepo` 实现，`EvaQueryRepository` 中对应方法退化为委托（口径/异常文案不变；落地提交：`985f7802`）。
...
- ✅ 提交点 C5-1（读侧实现继续拆，统计主题）：已完成（抽出 `EvaStatisticsQueryRepository`；落地提交：`9e0a8d28`）。
- ✅ 提交点 C5-2（读侧实现继续拆，记录主题）：已完成（抽出 `EvaRecordQueryRepository`；落地提交：`985f7802`）。

下一会话建议（继续按“每步=回归+提交+三文档同步”）：
1) **C5-3（读侧实现继续拆，任务主题）**：抽出 `EvaTaskQueryRepository` 承接 `EvaTaskQueryRepo` 实现，并让 `EvaQueryRepository` 的任务主题方法退化为委托（保持口径/异常文案不变）。
2) **C5-4（读侧实现继续拆，模板主题）**：抽出 `EvaTemplateQueryRepository` 承接 `EvaTemplateQueryRepo` 实现，并让 `EvaQueryRepository` 的模板主题方法退化为委托（保持口径/异常文案不变）。
...
- **bc-evaluation（评教）**：写侧主链路（任务发布/删除/模板）已按“用例+端口+适配器+委托壳”收敛；读侧已完成 QueryPort 拆分与 `EvaQueryRepo` 的四主题接口拆分（统计/记录/任务/模板），且已完成 C5-1/C5-2（统计/记录主题实现抽离），下一步继续拆任务/模板实现。
...
下一步提交点（建议优先级）：
1) C5-3：继续拆 `EvaQueryRepository` 的实现（任务主题，行为不变）
2) C5-4：继续拆 `EvaQueryRepository` 的实现（模板主题，行为不变）
