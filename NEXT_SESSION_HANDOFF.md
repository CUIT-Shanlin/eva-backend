...
- 🎯 **下一刀（建议，保持行为不变；每次只改 1 个类或 1 个 pom）**：继续推进 **S0.2 延伸（依赖方编译期依赖收敛）**：选择下一条“依赖方编译期依赖收敛”目标（优先从 `eva-domain` / `eva-infra-shared` 等依赖方开始），用 Serena 证据化盘点引用面与依赖闭包后再收敛 `pom.xml`（每次只改 1 个 pom；保持行为不变）。
  - 📌 **当前仍未完成（截至 2026-01-16，口径=可复现命令；保持行为不变）**：
    ...
    - `eva-app` 仍作为“过渡装配/支撑类”模块存在：当前源码侧仅剩 **10 个** Java 文件（口径：`fd -t f -e java . eva-app/src/main/java | wc -l`；包括切面配置、若干 Convertor/Exec 等）；组合根 `start` 仍对 `eva-app` 有编译期依赖（见 `start/pom.xml`）。
...
- 下一刀（每次只改 1 个 pom.xml 或 1 个类；保持行为不变）：
  1) Serena 证据化盘点：对 `eva-app/src/main/java` 残留支撑类（当前口径为 10 个 Java 文件）做“引用面 + 依赖闭包”盘点，优先挑选 1 个最独立、最不易引起装配漂移的类先归位（建议优先从 `AspectConfig` 或 `StpInterfaceImpl` 开始）。
