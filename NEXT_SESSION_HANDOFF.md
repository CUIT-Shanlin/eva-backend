...
- ✅ Serena（证据化，保持行为不变）：引用面命中 `bc-iam/infrastructure` 的 `LdapPersonGatewayImpl`（`Optional<LdapGroupDO>`）、以及 `eva-infra-shared` 的 `LdapGroupRepo` / `EvaLdapUtils`。
...
- 📊 **整体未完成清单（更新至 2026-02-17；保持行为不变）**：
  ...
  - `eva-infra-shared`：当前仍有 `6` 个 Java，且多 BC 仍直依赖该模块（可复现口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l` + `rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | head`）。
  ...
  - `eva-infra-shared` 残留结构（Serena + `rg` 证据化，保持行为不变）：`CourseBizConvertor/CourseConvertor/UserConverter` 在 `eva-infra-shared/src/main/java` 内均仅自引用（无内部依赖）；LDAP 子簇 `LdapGroupRepo/LdapConstants/EvaLdapUtils` 仍互相依赖（`EvaLdapUtils` ↔ `LdapConstants` + `LdapGroupRepo`），暂不适合无前置直接单类搬运；其中配置类 `EvaLdapProperties` 已下沉到 `shared-kernel`（保持包名不变），LDAP DO `LdapGroupDO` 已下沉到 `shared-kernel`（保持包名不变）。
...
- 下一刀建议（shared 瘦身，LDAP 子簇续接，保持行为不变；每刀只改 1 个文件闭环）：
  - 刀 1（单 pom，依赖前置）：若要下沉 `LdapGroupRepo` 到 `shared-kernel`，优先先在 `shared-kernel/pom.xml` 增加 `spring-data-ldap(optional)`（或等价依赖）以闭合 `org.springframework.data.ldap.repository.LdapRepository` 编译期依赖（先 Serena 证据化再改 pom）。
  - 刀 2（单类搬运）：`LdapGroupRepo`：`eva-infra-shared` → `shared-kernel`（保持 `package` 与接口签名不变；保持行为不变）。
...
1) 证据口径（先跑快照，避免口径漂移）：
   ...
   - `fd -t f -e java . eva-infra-shared/src/main/java | wc -l`（当前预期=6）
...
3) 🎯 本轮主线（瘦身 `eva-infra-shared`，保持行为不变）：
   ...
   - 🎯 下一刀建议（优先，LDAP 子簇续接；每次只改 1 个文件闭环）：
     1) 单 pom（依赖前置）：`shared-kernel/pom.xml` 补齐 `spring-data-ldap(optional)`（或等价依赖）以承接 `LdapGroupRepo` 的 `LdapRepository` 编译期依赖；
     2) 单类：`LdapGroupRepo` 从 `eva-infra-shared` 下沉到 `shared-kernel`（保持 `package`/签名不变）；
     3) 后置：`LdapConstants` / `EvaLdapUtils` 静态初始化互引风险高，需分刀推进，不建议直接动。
