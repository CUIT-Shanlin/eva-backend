...
### 0.10.1 最新状态 & 下一步建议（滚动更新至 2026-02-15；聚焦：S0.2 延伸（`eva-infra-dal` 按 BC 拆散 + `eva-infra-shared` 瘦身），保持行为不变）

- 📊 **整体未完成清单（更新至 2026-02-15；保持行为不变）**：
...
- 📦 **stash 快照（更新至 2026-02-15；保持行为不变）**：
...
- 🎯 **SysUserMapper 归位（进行中，保持行为不变）**：
  ...
  - 下一刀建议（每刀 1 类，保持行为不变）：继续清零 `bc-course/infrastructure` 内对 `SysUserMapper` 的编译期依赖（改为 `@Qualifier("sysUserMapper") Object` + 反射调用 `selectById(Serializable)` / `selectList(...)` + `getId()`/`getName()`），推荐顺序：
    1) `UpdateSelfCourseRepositoryImpl`
    2) `UpdateSingleCourseRepositoryImpl`
    3) `CourseImportExce`
    4) `CourseRecommendExce`
...
### 推荐版（2026-02-15 收口续接：承接已归位 XML，逐类归位 `eva-infra-dal`/瘦身 `eva-infra-shared`，保持行为不变）
...
3) 🎯 本轮主线（承接已归位 XML，逐类归位 `eva-infra-dal`，保持行为不变）：
   ...
   - 下一刀建议（每刀 1 类，保持行为不变）：继续清零 `bc-course/infrastructure` 内对 `SysUserMapper` 的编译期依赖（优先 `UpdateSelfCourseRepositoryImpl`；`@Qualifier("sysUserMapper") Object` + 反射调用）；完成清零后再做“单类搬运”：`SysUserMapper.java` 从 `eva-infra-dal` 归位到 `bc-iam/infrastructure`（保持 `package` 不变，避免 classpath 重复）。
