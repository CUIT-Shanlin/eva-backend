...
### 推荐版（续接主线：IAM 并行（10.3）——继续清理 `UserQueryGateway` 编译期依赖（优先 eva-infra-shared））
...
5) 🎯 **IAM 并行（按 10.3，保持行为不变）**：
   - ✅ 已闭环：`bc-iam/infrastructure` 的 `UserServiceImpl` 已从编译期依赖 `UserQueryGateway` 收敛为依赖 `bc-iam-contract` 端口 `UserEntityByIdQueryPort/UserEntityByUsernameQueryPort/UserBasicQueryPort/UserDirectoryPageQueryPort`，且测试 `UserServiceImplTest` 已适配端口构造（见 0.9）。
   - ⏳ 未完成（优先继续）：清理 `eva-infra-shared/src/main/java` 中对 `UserQueryGateway` 的编译期依赖（优先 `StpInterfaceImpl`，再处理 `MsgBizConvertor`）。
...
- A（优先：类，每次只改 1 个类）：`eva-infra-shared/src/main/java/edu/cuit/app/security/StpInterfaceImpl.java` 去 `UserQueryGateway` 编译期依赖（优先复用现有 `bc-iam-contract` 端口；若缺口则先新增最小 Port 与端口适配器，再改该类）。
- B（次优先：类，每次只改 1 个类）：`eva-infra-shared/src/main/java/edu/cuit/app/convertor/MsgBizConvertor.java` 去 `UserQueryGateway` 编译期依赖（MapStruct：先证据化其使用方法面，再决定复用现有 Port 或新增最小 Port）。
...
- `rg -n \"\\bUserQueryGateway\\b\" eva-infra-shared/src/main/java | head`（定位待收敛点：优先 `StpInterfaceImpl/MsgBizConvertor`）
