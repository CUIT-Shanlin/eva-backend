...
- 📊 **整体未完成清单（更新至 2026-02-19；保持行为不变）**：
  ...
  - `eva-infra-shared`：源码已清零（Java=`0`；口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l`），但 root reactor 仍包含 `<module>eva-infra-shared</module>`，且仍有 1 处 BC 基础设施 pom 引用（当前应仅命中 `bc-iam/infrastructure`；口径：`rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | sort`）。下一步按“逐 pom 去依赖 → root reactor 移除 module → 删除 `eva-infra-shared/pom.xml`”收尾（每次只改 1 个文件闭环；保持行为不变）。
...
1) 证据口径（先跑快照，避免口径漂移）：
   ...
   - `rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | sort`（当前预期仅 1 处：`bc-iam/infrastructure`）
...
3) 🎯 本轮主线（`eva-infra-shared` 退场收尾；每次只改 1 个文件闭环；保持行为不变）：
   - 第 1 刀（单 pom，优先）：移除 `bc-iam/infrastructure/pom.xml` 对 `eva-infra-shared` 的依赖（前置：Serena + `rg` 证伪 `bc-iam/**` 无 `eva-infra-shared` 类型引用；最小回归；代码提交；三文档同步提交；push）。
   - 第 2 刀（单 pom）：从 root `pom.xml` 的 reactor 中移除 `<module>eva-infra-shared</module>`（仅改变聚合构建边界；保持行为不变）。
   - 第 3 刀（单文件）：删除 `eva-infra-shared/pom.xml`（前置：全仓库 `**/pom.xml` 不再出现 `<artifactId>eva-infra-shared</artifactId>`）。
