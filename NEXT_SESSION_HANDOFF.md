...
- 下一刀（每次只改 1 个类；保持行为不变）：
  ...
  2) 逐个搬运评教旧入口壳 `EvaRecordServiceImpl/EvaTaskServiceImpl/EvaTemplateServiceImpl/EvaStatisticsServiceImpl/UserEvaServiceImpl` 到 `bc-evaluation-infra`（保持 `package` 不变；类内容不变；副作用顺序不变；若遇支撑类依赖，先逐个搬运支撑类；仍遵守“每次只改 1 个类”）。
  3) 当 Serena + `rg` 证伪 `eva-app/src/main/java` 不再 `import edu.cuit.bc.evaluation.*` 后，再评估是否可以收敛 `eva-app/pom.xml` 对 `bc-evaluation` 的编译期依赖（每次只改 1 个 `pom.xml`；保持行为不变）。
