...
- ✅ Serena（证据化，保持行为不变）：引用面命中 `bc-iam/infrastructure`（`LdapPersonGatewayImpl`）与 `eva-infra-shared`（`EvaLdapUtils`）。
...
- ✅ Serena（证据化，保持行为不变）：引用面命中 `bc-iam/infrastructure`（`LdapPersonGatewayImpl` 注入）与 `eva-infra-shared`（`EvaLdapUtils` 静态初始化取 Bean）。
...
- ✅ Serena（证据化，保持行为不变）：引用面命中 `bc-iam/infrastructure` 的 `LdapPersonGatewayImpl`（`Optional<LdapGroupDO>`）、`shared-kernel` 的 `LdapGroupRepo`、以及 `eva-infra-shared` 的 `EvaLdapUtils`。
...
- 📊 **整体未完成清单（更新至 2026-02-17；保持行为不变）**：
  ...
  - `eva-infra-shared`：当前仍有 `4` 个 Java，且多 BC 仍直依赖该模块（可复现口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l` + `rg -n "<artifactId>eva-infra-shared</artifactId>" --glob "**/pom.xml" bc-* | head`）。
  ...
  - `eva-infra-shared` 残留结构（Serena + `rg` 证据化，保持行为不变）：`CourseBizConvertor/CourseConvertor/UserConverter` 在 `eva-infra-shared/src/main/java` 内均仅自引用（无内部依赖）；LDAP 子簇当前仅剩 `EvaLdapUtils`（静态初始化副作用存在，后续需分刀评估是否可下沉到 `shared-kernel` 或归位 `bc-iam/infrastructure`）；其中 `EvaLdapProperties`、`LdapGroupDO`、`LdapGroupRepo`、`LdapConstants` 已下沉到 `shared-kernel`（保持包名不变）。
...
- 下一刀建议（shared 瘦身，LDAP 子簇续接，保持行为不变；每刀只改 1 个文件闭环）：
  ...
  - 刀 3（后置，多刀编排）：`LdapConstants` / `EvaLdapUtils` 存在静态初始化/互相引用风险（历史证据：`EvaLdapUtils` ↔ `LdapConstants`），不建议在同一刀内做“拆循环 + 搬运”，需先证据化再分刀推进。
...
- 🎯 **下一步重构计划（建议，保持行为不变；每次只改 1 个类/1 个 XML/1 个 pom 闭环）**：
  ...
  5) 风险簇（暂不建议直接动，避免破坏‘单类一刀’节奏）：LDAP 相关 `EvaLdapUtils`/`LdapConstants`/`LdapGroupRepo`/`LdapGroupDO`（配置类 `EvaLdapProperties` 已下沉到 `shared-kernel`）当前存在跨模块互相依赖与静态初始化耦合（Serena 证据化：`LdapConstants` 依赖 `EvaLdapUtils`，而 `EvaLdapUtils` 又依赖 `LdapConstants` 与 `LdapGroupRepo/DO`），若要归位到 `bc-iam/infrastructure` 需要设计“多刀编排顺序/过渡壳”，否则会导致 `eva-infra-shared` 编译断裂。
...
### 推荐版（2026-02-17 收口续接：瘦身 `eva-infra-shared`（优先 LDAP 子簇续接），保持行为不变）
...
1) 证据口径（先跑快照，避免口径漂移）：
   ...
   - `fd -t f -e java . eva-infra-shared/src/main/java | wc -l`（当前预期=4）
...
3) 🎯 本轮主线（瘦身 `eva-infra-shared`，保持行为不变）：
   ...
   - 🎯 下一刀建议（优先，LDAP 子簇续接；每次只改 1 个文件闭环）：
    ...
    2) ✅ 已完成（单类，保持行为不变）：`LdapGroupRepo` 从 `eva-infra-shared` 下沉到 `shared-kernel`（保持 `package`/签名不变；落地：`7ff087ad`）。
     3) 🎯 下一刀（后置，多刀编排，保持行为不变）：`LdapConstants` / `EvaLdapUtils` 静态初始化互引风险高，需分刀推进，不建议直接动。
        ...
        - 🎯 下一刀（单类，后置，保持行为不变）：评估并下沉 `EvaLdapUtils`（需先 Serena 证据化其引用面与静态初始化副作用边界）。
   - 备选主线（更稳，非 LDAP）：对 `CourseBizConvertor/CourseConvertor/UserConverter` 逐个做 Serena 引用面证伪后再决定“归位到单 BC”或“下沉 shared-kernel”（每刀只动 1 个类）。
4) 每刀结束必须闭环：
    ...
    - ✅ 已完成（条目 26-6-2b2）：迁移查询工具 `QueryUtils` 到 `eva-infra-shared`（保持包名不变；保持行为不变）。
      ...
      - 依赖：`eva-infra-shared/pom.xml` 增加对 `eva-client` 与 `zym-spring-boot-starter-jdbc` 的依赖（保持 MyBatis-Plus QueryWrapper/Page 等 API 不变）
	    ...
	    - ✅ 已完成（条目 26-6-2d2）：迁移 `EvaLdapUtils` 相关类型到 `eva-infra-shared`（保持包名不变；保持行为不变）。
		      - Java：`eva-infra-shared/src/main/java/edu/cuit/infra/util/EvaLdapUtils.java`、`eva-infra-shared/src/main/java/edu/cuit/infra/enums/LdapConstants.java`、`shared-kernel/src/main/java/edu/cuit/infra/property/EvaLdapProperties.java`（后续下沉 shared-kernel：`666a1b6d`）
	      - 落地提交：`3165180c`
