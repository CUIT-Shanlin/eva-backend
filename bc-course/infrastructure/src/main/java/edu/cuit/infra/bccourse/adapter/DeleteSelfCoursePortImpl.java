package edu.cuit.infra.bccourse.adapter;

import edu.cuit.bc.course.application.port.DeleteSelfCoursePort;
import edu.cuit.domain.gateway.course.CourseDeleteGateway;
import lombok.RequiredArgsConstructor;
import org.springframework.stereotype.Component;

import java.util.Map;

/**
 * bc-course：教师自助删课端口适配器（复用既有 CourseDeleteGateway，保持行为不变）。
 */
@Component
@RequiredArgsConstructor
public class DeleteSelfCoursePortImpl implements DeleteSelfCoursePort {
    private final CourseDeleteGateway courseDeleteGateway;

    @Override
    public Map<String, Map<Integer, Integer>> deleteSelfCourse(String userName, Integer courseId) {
        return courseDeleteGateway.deleteSelfCourse(userName, courseId);
    }
}

