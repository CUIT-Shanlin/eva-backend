...
5) **可选里程碑 M4：命名整合（仓库不出现 eva- 前缀）**
  ...
  - 现状评估（更新至 2026-02-15，保持行为不变）：
    ...
    - `eva-adapter`：已完成退场闭环（残留 Controller 清零 + 组合根去依赖 + root reactor 移除 + 删除模块 pom）；证据口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l` 为 0，且 `rg -n '<module>eva-adapter</module>' pom.xml` 命中为 0，且 `rg -n '<artifactId>eva-adapter</artifactId>' --glob '**/pom.xml' .` 命中为 0（保持行为不变）。
		    ...
		    - `eva-infra-shared`：仍被部分 BC 基础设施模块直依赖，当前剩余 `1` 个 Java（仅 `UserConverter`；口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l`）。补充：`CourseBizConvertor/CourseConvertor` 已归位到 `bc-course/infrastructure`（保持行为不变；详见 `NEXT_SESSION_HANDOFF.md` 0.9/0.10）。当前剩余 `UserConverter` 引用面跨 `bc-course/bc-evaluation/bc-iam`，且其桥接方法用于“在不编译期引用 `UserEntity` 的前提下读取字段”；因此不建议直接下沉 `shared-kernel`，优先走“bc-iam 最小 Port 收敛跨 BC 调用点 → 单类搬运归位 → 逐 pom 去依赖 → reactor 退场”。LDAP 子簇已清零（相关类型已下沉到 `shared-kernel`，保持包名不变）。
