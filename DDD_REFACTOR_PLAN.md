...
- 🎯 下一刀建议（更新至 2026-02-25；保持行为不变；单类；写侧优先）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 `*-contract`/`shared-kernel`）。删除链路/取消链路/自助删课链路已阶段性闭环；评教侧已补齐 `EvaTaskCourInfIdsByStatusesDirectQueryPort` 及其适配器，下一步建议回到 `bc-course/infrastructure` 改 `CourseRecommendExce` 去评教 `evaTaskMapper` 运行期依赖，并同时清零对评教 DO（`EvaTaskDO`）的编译期依赖（保持行为不变）。
