...
1) **评教任务发布写侧收敛**：把 `EvaUpdateGatewayImpl.postEvaTask` 收敛到 `bc-evaluation` 用例 + 端口，跨域副作用（消息/日志/缓存）按“事务提交后事件”固化（行为不变）。
2) **评教删除写侧收敛**：把 `EvaDeleteGatewayImpl.deleteEvaRecord/deleteEvaTemplate` 收敛到 `bc-evaluation`（行为不变）。
3) **课程读侧渐进收敛**：为 `CourseQueryGatewayImpl` 引入 `QueryPort/QueryRepo`（先结构化，再考虑 CQRS 投影表）。
...
- `bc-evaluation`：评教任务发布写侧收敛（进行中：已新增 `PostEvaTaskUseCase/PostEvaTaskRepository/EvaluationTaskPostedEvent`，`eva-infra` 端口适配器已落地，待接入应用层入口与提交后消息事件）。
