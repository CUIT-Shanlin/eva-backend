...
- 🎯 下一刀建议（更新至 2026-02-25；保持行为不变；单类；写侧优先）：继续推进“课程/模板写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 `*-contract`/`shared-kernel`）。当前 `@Qualifier("evaTaskMapper")` 残留已清到 0，但模板侧 `CourseTemplateLockQueryPortImpl` 仍残留对评教 `courOneEvaTemplateMapper/formRecordMapper` 的运行期直连与对评教 DO 的编译期依赖；评教侧 `CourseTemplateLockEvaEvidenceDirectQueryPort` contract+infra 已齐备，建议下一刀回到模板侧按端口替换直连并清零评教 DO 编译期依赖（template 单类闭环）。建议先跑口径快照：
