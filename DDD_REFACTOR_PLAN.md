...
5) **评教读侧进一步解耦**：按用例维度拆分 QueryService（任务/记录/统计/模板），将 query 端口逐步迁到 `bc-evaluation` 应用层，`eva-infra` 仅保留实现（行为不变）。  
   ...
   - 下一步建议（统计导出基础设施归位，保持行为不变；每次只迁 1 个类 + 最小回归）：先迁 `FillUserStatisticsExporterDecorator`，再迁 `EvaStatisticsExcelFactory`（包含导出异常文案与日志输出，迁移时必须完全一致），然后单独提交切换 `BcEvaluationConfiguration.evaStatisticsExportPort()` 的委托目标，最后（可选）在确认 `eva-app` 不再直接使用 POI 后移除 `eva-app/pom.xml` 对 `poi/poi-ooxml` 的依赖。
