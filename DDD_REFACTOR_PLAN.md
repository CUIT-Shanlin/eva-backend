...
- 🎯 下一刀建议（更新至 2026-02-25；保持行为不变；单类；写侧优先）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 `*-contract`/`shared-kernel`）。删除链路/取消链路/自助删课链路已阶段性闭环；下一步建议收敛 `bc-course/infrastructure` 的 `CourseRecommendExce` 去评教 `evaTaskMapper` 运行期依赖（若端口能力不足，按“先补端口（单类）→ 再补适配器（单类）→ 再改调用侧（单类）”推进；保持行为不变）。
