...
- ✅ 补充进展（2026-02-16，保持行为不变，SysUserMapper 归位前置，单类）：为后续归位 IAM 侧 `SysUserMapper` 做准备，已将 `bc-course/infrastructure` 的 `DeleteSelfCourseRepositoryImpl` 去 `SysUserMapper` 编译期依赖（改为按 `beanName` 注入 `Object` + 反射调用 `selectOne(Wrapper)`；异常文案/日志文案/副作用顺序不变；最小回归通过；落地：`71060e69`）。
  - 下一刀建议（每刀 1 类，保持行为不变）：继续清零 `bc-course/infrastructure` 内对 `SysUserMapper` 的编译期依赖（改为 `@Qualifier("sysUserMapper") Object` + 反射调用），推荐顺序：`UpdateCourseInfoRepositoryImpl` → `UpdateSelfCourseRepositoryImpl` → `UpdateSingleCourseRepositoryImpl` → `CourseImportExce` → `CourseRecommendExce`。完成全仓库“非 IAM 模块清零”后，再将 `SysUserMapper.java` 从 `eva-infra-dal` 归位到 `bc-iam/infrastructure`（保持 `package` 不变）。
