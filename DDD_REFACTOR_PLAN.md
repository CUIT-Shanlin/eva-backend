...
5) **可选里程碑 M4：命名整合（仓库不出现 eva- 前缀）**
  ...
  - 现状评估（更新至 2026-02-15，保持行为不变）：
    ...
    - `eva-adapter`：已完成退场闭环（残留 Controller 清零 + 组合根去依赖 + root reactor 移除 + 删除模块 pom）；证据口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l` 为 0，且 `rg -n '<module>eva-adapter</module>' pom.xml` 命中为 0，且 `rg -n '<artifactId>eva-adapter</artifactId>' --glob '**/pom.xml' .` 命中为 0（保持行为不变）。
		    ...
		    - `eva-infra-shared`：源码已清零（Java=`0`；口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l`），且 `UserConverter` 已按“最小 Port 收敛跨 BC 使用点 → 单类搬运归位”的路线归位到 `bc-iam/infrastructure`（保持行为不变；详见 `NEXT_SESSION_HANDOFF.md` 0.9/0.10 与 10.2 的补充进展）。当前仅剩“pom 依赖残留 + root reactor module 未退场”的收尾工作：按“逐 pom 去依赖 → root `pom.xml` 移除 `<module>eva-infra-shared</module>` → 删除 `eva-infra-shared/pom.xml`”逐刀闭环即可（每次只改 1 个文件；保持行为不变）。
