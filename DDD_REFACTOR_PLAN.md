...
- 🎯 下一步建议（写侧优先，保持行为不变）：继续清理 `bc-evaluation/infrastructure` 写侧端口适配器对课程域 `CourseMapper` 的编译期直连点（优先从 `SubmitEvaluationRepositoryImpl` 开始；若能力缺口则按“补端口（单类）→ 补适配器（单类）→ 改调用侧（单类）”推进）。
