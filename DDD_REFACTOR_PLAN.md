...
- 现状快照（更新至 2026-01-17，保持行为不变）：`eva-app` 已退场（组合根去依赖：`0a9ff564`；reactor 移除：`b5f15a4b`；删除 `eva-app/pom.xml`：`4bfa9d40`）。`eva-adapter` 下残留 `*Controller` 为 **21 个**（已将 `AuthenticationController` 归位到 `bc-iam-infra`，落地：`94a00022`），且已补齐 `eva-adapter/pom.xml` 的编译依赖闭合以避免增量编译掩盖问题（`56273162`）。下一刀建议继续按“每次只迁 1 个 Controller”推进（保持行为不变）。
...
3) **可以移除 `eva-domain` / `eva-infra*` / `eva-base*` 的判定标准（建议的 DoD）**
  ...
  - 现状评估（更新至 2026-01-17，保持行为不变）：
    ...
    - `eva-adapter`：仍保留 21 个 `*Controller.java`（口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l`），组合根 `start` 仍显式依赖 `eva-adapter`（见 `start/pom.xml`），因此“移除模块”尚未满足。
