...
- 现状快照（更新至 2026-01-17，保持行为不变）：`eva-app` 下 `*ServiceImpl` 已清零（0），且 `eva-app/src/main/java` 已清零（口径：`fd -t f -e java . eva-app/src/main/java | wc -l` 当前为 0）。`eva-adapter` 下残留 `*Controller` 为 **22 个**。✅ 组合根 `start` 已去 `eva-app` 依赖并显式引入 `eva-infra(runtime)`（落地：`0a9ff564`）。下一刀建议转向单 pom：评估从 reactor 移除 `eva-app` 模块（见 `NEXT_SESSION_HANDOFF.md` 0.10/0.11）。
