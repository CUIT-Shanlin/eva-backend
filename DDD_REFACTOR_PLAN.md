...
- 现状快照（更新至 2026-01-17，保持行为不变）：`eva-app` 已退场（组合根去依赖：`0a9ff564`；reactor 移除：`b5f15a4b`；删除 `eva-app/pom.xml`：`4bfa9d40`）。`eva-adapter` 下残留 `*Controller` 为 **22 个**，且已补齐 `eva-adapter/pom.xml` 的编译依赖闭合以避免增量编译掩盖问题（`56273162`）。下一刀建议转向单类：收敛 `eva-adapter` 的 `*Controller`（每次只改 1 个 Controller；保持行为不变）。
