...
- 下一步建议（基础设施旧 gateway 归位，保持行为不变）：继续按“每次只迁 1 个类 + 最小回归 + 三文档同步 + push”的节奏归位 `eva-infra` 残留 `*GatewayImpl.java`（当前残留清单以 `docs/DDD_REFACTOR_BACKLOG.md` 4.3 为准），优先顺序建议：`user/*GatewayImpl` → `bc-iam-infra`、`eva/*GatewayImpl` → `bc-evaluation-infra`。
- 冲突校验底层片段已收敛：
