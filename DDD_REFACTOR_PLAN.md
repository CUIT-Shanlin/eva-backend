...
- 🎯 下一刀建议（更新至 2026-02-25；保持行为不变；单类；写侧优先）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 `*-contract`/`shared-kernel`）。课程侧 `CourseImportExce/CourseRecommendExce` 已完成收敛；下一步建议收敛 `bc-template/infrastructure` 的 `CourseTemplateLockQueryPortImpl` 去评教 `evaTaskMapper` 运行期依赖（注意其属于模板 BC，同样要满足“跨 BC 只依赖 contract/shared-kernel”；保持行为不变）。
