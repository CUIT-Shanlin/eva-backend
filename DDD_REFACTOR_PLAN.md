...
- 下一步建议（更新至 2026-01-15，保持行为不变，依赖收敛：websocket）：在 `rg -n '^import\\s+org\\.springframework\\.web\\.socket' eva-app/src/main/java` 证伪为 0 后，独立提交收敛 `eva-app/pom.xml` 并移除 `spring-boot-starter-websocket` 的编译期依赖（运行期由组合根 `start` 显式兜底；保持行为不变）。
