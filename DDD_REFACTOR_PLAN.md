...
- 下一步建议（更新至 2026-01-15，保持行为不变，依赖收敛：websocket）：继续归位 `eva-app` 中残留的 websocket 配置/拦截器（`WebSocketConfig/WebSocketInterceptor`）到 `eva-infra-shared`（保持 `package edu.cuit.app.config` 不变；每次只改 1 个类）。为避免搬运 `WebSocketInterceptor` 时 `StpUtil`（Sa-Token）缺失导致编译失败，建议先独立提交补齐 `eva-infra-shared/pom.xml` 的 Sa-Token 编译期依赖（仅显式化；保持行为不变）。在 `rg` 证伪 `eva-app/src/main/java` 不再 `import org.springframework.web.socket.*` 后，再独立提交收敛 `eva-app/pom.xml` 并移除 `spring-boot-starter-websocket` 的编译期依赖（运行期由组合根 `start` 显式兜底；保持行为不变）。
...
- 现状快照（更新至 2026-01-13，保持行为不变）：`eva-app` 下残留 `*ServiceImpl` 为 **12 个**，其中已显式委托 UseCase 的为 **11 个**；`eva-adapter` 下残留 `*Controller` 为 **22 个**（Controller 已不再直接注入 `*ServiceImpl`，但仍需继续收敛为纯协议适配与参数校验）。具体清单与优先级见 `NEXT_SESSION_HANDOFF.md` 0.10。
