...
- 下一刀建议（保持行为不变，S0.2 延伸，每次只改 1 个 `pom.xml`）：优先让组合根 `start` 显式依赖 `eva-app`（保持行为不变；为后续在 `eva-adapter/pom.xml` 移除对 `eva-app` 的依赖做前置），再逐个依赖方证据化盘点“仅类型引用”的场景并收敛编译期依赖边界（仍保持行为不变）。
