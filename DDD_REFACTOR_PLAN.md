...
## 10. 本计划的下一步落地建议（你确认后我再开始改代码）

建议从最小闭环切片开始，优先顺序：
1) `bc-evaluation`：提交评教（涉及最多联动：消息清理、模板锁定、统计口径）
2) `bc-course`：改时段撤回任务（高风险链路，最需要事件化与事务边界）
3) `bc-ai-report`：教师+学期报告（可独立异步，拆服务收益高）

你只需要确认两点，我就可以把 Phase 1~2 变成具体的代码改造任务清单（到类/包/模块级）：
1) 未来拆微服务时，你倾向 **“数据库拆库”** 还是 **“先拆服务，暂时共享库”**（过渡期）？
2) day 字段：`day=1` 是否表示 **周一**？（影响 week/day → 日期的统一算法）
