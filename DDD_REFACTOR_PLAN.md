...
- 下一步（建议每步只改 1 个小包/小类簇，保持行为不变）：
  ...
  4) ✅ 延伸主线（依赖方收敛，保持行为不变）：已完成两处“依赖方收敛”闭环：`bc-evaluation-infra` 去 `bc-course` 编译期依赖（落地：`a0bcf74f`）与 `eva-infra-shared` 去 `bc-course` 编译期依赖（落地：`6ab3837a`）。同时已启动“课程域基础设施归位”：将 `edu.cuit.infra.bccourse.adapter/*PortImpl` 从 `eva-infra` 迁移到 `bc-course-infra`（保持 `package` 不变；落地：`c4179654`），为后续 `eva-infra` 去 `bc-course` 依赖铺路。下一步：继续对 `eva-adapter`/`eva-app` 做同样的“证伪 → 依赖替换”（每步只改 1 个 `pom.xml`，保持行为不变；注意 `eva-adapter` 当前仅经由 `eva-app` 间接获得 `bc-course`，需先评估是否存在“可替换的显式依赖点”再动手）。
