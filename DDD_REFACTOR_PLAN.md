...
- ✅ 补充进展（2026-02-24，保持行为不变，写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `DeleteSelfCourseRepositoryImpl` 已将“按 courInfIds 查评教任务 + 按 taskIds 删除评教记录”的反射调用，收敛为调用评教端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` + `FormRecordDeleteByTaskIdsPort`（保持异常文案/缓存失效/副作用顺序完全不变；仍保留对 `evaTaskMapper.delete(...)` 的反射调用点待后续补批量删除端口；最小回归通过；落地：`d31a2438`）。
