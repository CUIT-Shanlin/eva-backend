...
- 🎯 下一刀建议（保持行为不变；每次只改 1 个 `pom.xml`）：为后续归位 `MsgEntity` 做编译闭合前置，收敛 `bc-messaging-contract/pom.xml`：补齐承接 `MsgEntity` 所需的最小编译期依赖（预期包含 `bc-iam-domain` 与 `cola-component-domain-starter`；仅用于编译闭合）。
