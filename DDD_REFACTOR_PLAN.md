...
- 下一步建议（更新至 2026-01-15，保持行为不变，依赖收敛：websocket）：继续归位 `eva-app` 中残留的 websocket 配置（`WebSocketConfig`）到 `eva-infra-shared`（保持 `package edu.cuit.app.config` 不变；每次只改 1 个类）。在 `rg` 证伪 `eva-app/src/main/java` 不再 `import org.springframework.web.socket.*` 后，再独立提交收敛 `eva-app/pom.xml` 并移除 `spring-boot-starter-websocket` 的编译期依赖（运行期由组合根 `start` 显式兜底；保持行为不变）。
