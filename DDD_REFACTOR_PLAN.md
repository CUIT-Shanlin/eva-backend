...
- 下一刀建议（保持行为不变，建议拆 3 步闭环）：收敛 `ICourseTypeServiceImpl`（✅ 已完成：新增 `bc-course/application/usecase` 下的课程类型用例 `CourseTypeUseCase`；剩余：`BcCourseConfiguration` 装配 → 旧入口仅保留委托壳；每步最小回归 + 提交 + 三文档同步）。
