...
- ✅ 补充进展（2026-02-25，保持行为不变，写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `CourseImportExce` 已移除评教 `@Qualifier("evaTaskMapper")` 运行期依赖并改走评教端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` + `EvaTaskDeleteByCourInfIdsPort`（并将表单记录删除改走 `FormRecordDeleteByTaskIdsPort`）；同时清零对评教 DO 的编译期依赖；异常文案/缓存失效与副作用顺序完全不变（最小回归通过；落地：`60a0881a`）。
- ✅ 补充进展（2026-02-25，保持行为不变，编译期依赖收敛，单类）：课程域 `bc-course/infrastructure` 的 `CourseBizConvertor` 已清零对评教 `FormTemplateDO` 的编译期依赖（`toEvaTemplateCO(Object)` 反射读取 `getId/getIsDefault` 构造 `EvaTemplateCO`；保持 `selectById` 调用次数/顺序与异常包装口径不变；最小回归通过；落地：`6ad09180`）。
- ✅ 补充进展（2026-02-25，保持行为不变，写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `CourseRecommendExce` 已移除评教 `@Qualifier("evaTaskMapper")` 运行期依赖，并改走评教端口 `EvaTaskCourInfIdsByTeacherIdAndStatusDirectQueryPort` + `EvaTaskCourInfIdsByStatusesDirectQueryPort` + `EvaTaskBriefByCourInfIdsDirectQueryPort`（同时清零对评教 DO 的编译期依赖；保持行为不变；最小回归通过；落地：`af084dec`）。
- ✅ 补充进展（2026-02-25，保持行为不变，写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `AssignEvaTeachersRepositoryImpl` 已清零对评教 DO（`EvaTaskDO`）的编译期依赖，改为直接调用 `EvaTaskInsertPort.insertAndReturnId(...)` 并保持 `LocalDateTime.now()` 的调用次数与“先构造再插入”的时序不变（最小回归通过；落地：`8b869c33`）。
- ✅ 补充进展（2026-02-25，保持行为不变，写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `DeleteSelfCourseRepositoryImpl` 已清零对评教 DO（`EvaTaskDO`）的编译期依赖（移除无用 import；不改变业务语义/异常文案/副作用顺序；最小回归通过；落地：`63b92a27`）。
- ✅ 补充进展（2026-02-25，保持行为不变，写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `AddNotExistCoursesDetailsRepositoryImpl` 已清零对评教 DO（`FormTemplateDO`）的编译期依赖：默认模板查询仍委托 `formTemplateMapper.selectOne(...)`，仅通过反射读取 `getId` 以移除 DO 引用（不改变业务语义/异常文案/副作用顺序；最小回归通过；落地：`99a8bef1`）。
- ✅ 补充进展（2026-02-25，保持行为不变，编译闭合前置，单 `pom.xml`）：为后续将模板侧 `CourseTemplateLockQueryPortImpl` 去评教 `evaTaskMapper` 运行期依赖做准备，已在 `bc-template/infrastructure/pom.xml` 增加对 `bc-evaluation-contract` 的 Maven 编译期依赖（保持行为不变；最小回归通过；落地：`fd21b951`）。
- ✅ 补充进展（2026-02-25，保持行为不变，写侧收敛，单类）：模板侧 `bc-template/infrastructure` 的 `CourseTemplateLockQueryPortImpl` 已移除评教 `@Qualifier("evaTaskMapper")` 运行期依赖，改走评教直查端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` 获取任务信息以承接锁定判定链路（保持分支与副作用顺序不变；最小回归通过；落地：`e7ab432f`）。注意：本刀仅去除 `evaTaskMapper`，剩余对评教 `courOneEvaTemplateMapper/formRecordMapper` 的运行期直连与对评教 DO 的编译期依赖仍待后续逐刀端口化清零。
- ✅ 补充进展（2026-02-25，保持行为不变，contract 前置，单类）：在 `bc-evaluation/contract` 新增模板锁定判定证据直查端口 `CourseTemplateLockEvaEvidenceDirectQueryPort`，用于后续承接模板侧锁定判定对评教 `cour_one_eva_template/form_record` 的两处剩余直连（不引入缓存/切面副作用；入参为空/空列表 no-op；最小回归通过；落地：`754a0b0f`）。
- ✅ 补充进展（2026-02-25，保持行为不变，infra 落地，单类）：在 `bc-evaluation/infrastructure` 新增端口适配器 `CourseTemplateLockEvaEvidenceDirectQueryPortImpl`，内部仅委托 `CourOneEvaTemplateMapper/FormRecordMapper.selectCount(...)` 查询证据存在性（不引入缓存/切面副作用；查询条件与旧 Mapper 调用保持一致；入参为空/空列表 no-op；最小回归通过；落地：`10281eec`）。
- ✅ 补充进展（2026-02-25，保持行为不变，template 收敛，单类）：模板侧 `bc-template/infrastructure` 的 `CourseTemplateLockQueryPortImpl` 已改为注入并调用 `CourseTemplateLockEvaEvidenceDirectQueryPort`，替换对评教 `courOneEvaTemplateMapper/formRecordMapper` 的运行期直连，并清零对评教 DO 的编译期依赖（锁定判定分支/查询条件/异常文案/副作用顺序不变；最小回归通过；落地：`f757abd9`）。
- ✅ 补充进展（2026-02-25，保持行为不变，contract 前置，单类）：在 `bc-evaluation/contract` 新增直查端口 `EvaTaskCourInfIdsByStatusesDirectQueryPort`，用于跨 BC 按状态列表查询评教任务对应的 courInfId 列表（不引入缓存/切面副作用；入参为空 no-op；最小回归通过；落地：`2814143d`）。
- ✅ 补充进展（2026-02-25，保持行为不变，infra 落地，单类）：在 `bc-evaluation/infrastructure` 新增端口适配器 `EvaTaskCourInfIdsByStatusesDirectQueryPortImpl`，内部仅委托 `EvaTaskMapper.selectList(in status)` 并映射为 `courInfId` 列表（不引入缓存/切面副作用；查询条件/结果顺序/空值语义保持与旧 Mapper 调用一致；最小回归通过；落地：`db28b332`）。
...
- ✅ 补充进展（2026-02-23，保持行为不变，写侧去重复，单类）：`bc-evaluation/infrastructure` 的 `DeleteEvaRecordRepositoryImpl` 抽取 `rethrowInvocationTargetException(...)` 复用 `selectSysUserNameById(...)` 中反射调用异常解包逻辑，保持对 `RuntimeException/Error` 的原样抛出语义不变（最小回归通过；落地：`a1773f2b`）。
- ✅ 补充进展（2026-02-23，保持行为不变，contract 前置，单类）：在 `bc-evaluation/contract` 新增“按 courInfIds 读取任务最小视图”的协议对象 `EvaTaskBriefCO`（仅含 `id/teacherId/courInfId/status`；不引入 Mapper/DO；最小回归通过；落地：`017b16e3`）。
- ✅ 补充进展（2026-02-23，保持行为不变，contract 前置，单类）：在 `bc-evaluation/contract` 新增最小端口 `EvaTaskBriefByCourInfIdsDirectQueryPort`，用于跨 BC 按 courInfIds 查询 `EvaTaskBriefCO` 列表（不引入缓存/切面副作用；最小回归通过；落地：`9457edb1`）。
...
- ✅ 补充进展（2026-02-23，保持行为不变，infra 落地，单类）：在 `bc-evaluation/infrastructure` 新增端口适配器 `EvaTaskCascadeDeleteByTaskIdsPortImpl`，内部委托 `EvaTaskMapper.delete(in id)` + `FormRecordMapper.delete(in task_id)` 实现 taskIds 级联删除（不引入缓存/切面副作用；删除顺序保持“先任务后记录”；最小回归通过；落地：`1d69770a`）。
- ✅ 补充进展（2026-02-23，保持行为不变，infra 落地，单类）：在 `bc-evaluation/infrastructure` 新增取消端口适配器 `EvaTaskCancelByTeacherIdAndCourInfIdPortImpl`，内部仅委托 `EvaTaskMapper.update(...)` 将 `eva_task.status` 更新为 `2`（不引入缓存/切面副作用；空入参 no-op；最小回归通过；落地：`656ca526`）。
- ✅ 补充进展（2026-02-23，保持行为不变，contract 前置，单类）：在 `bc-evaluation/contract` 新增最小直查端口 `EvaTaskCourInfIdsByTeacherIdAndStatusDirectQueryPort`，用于跨 BC 以 `teacherId + status` 获取评教任务关联的 `courInfId` 列表（不引入缓存/切面副作用；入参为空 no-op；最小回归通过；落地：`d9846645`）。
- ✅ 补充进展（2026-02-23，保持行为不变，infra 落地，单类）：在 `bc-evaluation/infrastructure` 新增端口适配器 `EvaTaskCourInfIdsByTeacherIdAndStatusDirectQueryPortImpl`，内部仅委托 `EvaTaskMapper.selectList(eq teacher_id, status)` 并映射为 `courInfId` 列表（不引入缓存/切面副作用；查询条件、结果顺序与空值语义保持不变；最小回归通过；落地：`0f4fa7d3`）。
- ✅ 补充进展（2026-02-23，保持行为不变，写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `UpdateSingleCourseRepositoryImpl` 已将“取消评教任务”的评教 `evaTaskMapper` 反射调用收敛为调用评教端口（通过 `EvaTaskBriefByCourInfIdsDirectQueryPort` 获取任务最小视图并筛选 `status=0`，再调用 `EvaTaskCancelByTeacherIdAndCourInfIdPort` 执行取消；仍保持 `cour_inf_id = courINfo.getCourseId()` 的历史口径不变；最小回归通过；落地：`b9178e9a`）。
- ✅ 补充进展（2026-02-23，保持行为不变，写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `UpdateSelfCourseRepositoryImpl` 已将“自助改课时用于判断评教时间冲突”的评教任务 `courInfId` 列表查询，从评教 `evaTaskMapper.selectList(eq teacher_id, status=0)` 的反射调用收敛为调用评教直查端口 `EvaTaskCourInfIdsByTeacherIdAndStatusDirectQueryPort`（不引入缓存/切面副作用；异常文案/副作用顺序不变；最小回归通过；落地：`794bc199`）。
- ✅ 补充进展（2026-02-23，保持行为不变，contract 前置，单类）：在 `bc-evaluation/contract` 新增写侧端口 `EvaTaskDeleteByCourInfIdPort`，用于跨 BC 在课程侧“自助改课删除某些课次（cour_inf）”链路中按 `courInfId` 删除评教任务（保持旧调用语义：仅删除 `eva_task`，不做级联删除；不引入缓存/切面副作用；入参为空 no-op；最小回归通过；落地：`6b304d4b`）。
- ✅ 补充进展（2026-02-23，保持行为不变，infra 落点，单类）：在 `bc-evaluation/infrastructure` 新增端口适配器 `EvaTaskDeleteByCourInfIdPortImpl`，内部仅委托 `EvaTaskMapper.delete(eq cour_inf_id)` 按 `courInfId` 删除 `eva_task`（不引入缓存/切面副作用；空入参 no-op；最小回归通过；落地：`19814946`）。
- ✅ 补充进展（2026-02-23，保持行为不变，写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `UpdateSelfCourseRepositoryImpl` 已将按 `cour_inf_id` 的评教任务查询/删除从评教 `evaTaskMapper` 反射调用收敛为调用评教端口（读端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` + 写端口 `EvaTaskDeleteByCourInfIdPort`；保持异常文案/缓存失效/副作用顺序完全不变；最小回归通过；落地：`cd3a7bfe`）。
- ✅ 补充进展（2026-02-24，保持行为不变，contract 前置，单类）：在 `bc-evaluation/contract` 新增写侧端口 `FormRecordDeleteByTaskIdsPort`，用于跨 BC 在课程删课等链路中按 taskIds 删除评教记录（form_record）（不引入缓存/切面副作用；入参为空/空列表 no-op；最小回归通过；落地：`6df5e2ab`）。
- ✅ 补充进展（2026-02-24，保持行为不变，infra 落点，单类）：在 `bc-evaluation/infrastructure` 新增端口适配器 `FormRecordDeleteByTaskIdsPortImpl`，内部仅委托 `FormRecordMapper.delete(in task_id)` 按 taskIds 删除 `form_record`（不引入缓存/切面副作用；入参为空/空列表 no-op；最小回归通过；落地：`8faf4d0b`）。
- ✅ 补充进展（2026-02-24，保持行为不变，写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `DeleteSelfCourseRepositoryImpl` 已清零对评教 `evaTaskMapper/formRecordMapper` 的反射调用，改为调用评教端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` + `FormRecordDeleteByTaskIdsPort` + `EvaTaskDeleteByCourInfIdsPort`（保持异常文案/缓存失效/副作用顺序完全不变；最小回归通过；落地：`089be092`）。
- ✅ 补充进展（2026-02-24，保持行为不变，contract 前置，单类）：在 `bc-evaluation/contract` 新增写侧端口 `EvaTaskDeleteByCourInfIdsPort`，用于跨 BC 在课程删课等链路中按 courInfIds 批量删除评教任务（不引入缓存/切面副作用；入参为空应为 no-op；最小回归通过；落地：`c2562508`）。
- ✅ 补充进展（2026-02-24，保持行为不变，infra 落点，单类）：在 `bc-evaluation/infrastructure` 新增端口适配器 `EvaTaskDeleteByCourInfIdsPortImpl`，内部仅委托 `EvaTaskMapper.delete(in cour_inf_id)` 按 courInfIds 批量删除 `eva_task`（不引入缓存/切面副作用；入参为空应为 no-op；最小回归通过；落地：`73d98f77`）。
- ✅ 补充进展（2026-02-24，保持行为不变，contract 前置，单类）：在 `bc-evaluation/contract` 新增读侧直查端口 `EvaTaskByTeacherIdsAndStatusDirectQueryPort`，用于后续将课程写侧（如 `AssignEvaTeachersRepositoryImpl`）中对评教 `evaTaskMapper` 的反射 `selectList/selectOne` 收敛为端口调用（约束：不引入缓存/切面副作用；入参为空 no-op；保持旧查询语义/异常行为不变；最小回归通过；落地：`25235d1d`）。
- ✅ 补充进展（2026-02-24，保持行为不变，infra 落地，单类）：在 `bc-evaluation/infrastructure` 新增端口适配器 `EvaTaskByTeacherIdsAndStatusDirectQueryPortImpl`，内部仅委托 `EvaTaskMapper.selectList/selectOne` 并映射为 `courInfIds/teacherId`（不引入缓存/切面副作用；入参为空 no-op；保持旧查询语义/异常行为不变；最小回归通过；落地：`e7b2b99f`）。
- ✅ 补充进展（2026-02-24，保持行为不变，contract 前置，单类）：在 `bc-evaluation/contract` 新增写侧端口 `EvaTaskInsertPort`，用于后续将课程写侧（如 `AssignEvaTeachersRepositoryImpl`）中对评教 `evaTaskMapper.insert` 的反射调用收敛为端口调用，并显式回传生成的 `taskId`（不引入缓存/切面副作用；入参为空 no-op；保持旧 insert + 主键回填语义不变；最小回归通过；落地：`3facb7f1`）。
- ✅ 补充进展（2026-02-24，保持行为不变，infra 落地，单类）：在 `bc-evaluation/infrastructure` 新增端口适配器 `EvaTaskInsertPortImpl`，内部仅委托 `EvaTaskMapper.insert` 并回传生成的 `taskId`（不引入缓存/切面副作用；入参为空 no-op；保持旧 insert + 主键回填语义不变；最小回归通过；落地：`d7a31571`）。
- ✅ 补充进展（2026-02-23，保持行为不变，写侧收敛，单类）：课程写侧删除链路 `DeleteCourseRepositoryImpl` 已将“按 taskIds 删除评教任务 + 评教记录”的实现改为调用评教写端口 `EvaTaskCascadeDeleteByTaskIdsPort`，并移除对 `evaTaskMapper/formRecordMapper` 的反射删除调用点（保持删除顺序与缓存失效顺序不变；最小回归通过；落地：`19d9ebdc`）。
- ✅ 补充进展（2026-02-23，保持行为不变，写侧收敛，单类）：课程写侧删除链路 `DeleteCoursesRepositoryImpl` 已将“按 courInfIds 查询评教任务 + 删除评教任务/记录”的实现改为调用评教直查端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` + 写端口 `EvaTaskCascadeDeleteByTaskIdsPort`，并清零对 `edu.cuit.infra.dal.database.dataobject.eva.*` 的编译期依赖与反射删除调用点（保持删除顺序与缓存失效顺序不变；最小回归通过；落地：`55d27784`）。
...
- ✅ 补充进展（2026-02-23，保持行为不变，跨 BC 直连清零前置，单类）：在 `bc-iam-infra` 新增端口适配器 `UserDetailByUsernameDirectQueryPortImpl`，内部仅委托 `SysUserMapper.selectOne(eq username)` 并映射为 `UserDetailCO(id/name/username)`（不引入缓存/切面副作用；最小回归通过；落地：`644fe2c9`）。
- ✅ 补充进展（2026-02-23，保持行为不变，跨 BC 直连清零前置，单类）：在 `bc-iam-infra` 新增端口适配器 `UserIdByUsernameDirectQueryPortImpl`，内部仅委托 `SysUserMapper.selectOne(eq username).getId()`（不引入缓存/切面副作用；最小回归通过；落地：`1e8c8b99`）。
- ✅ 补充进展（2026-02-23，保持行为不变，跨 BC 直连清零前置，单类）：在 `bc-iam-infra` 新增端口适配器 `UserNameDirectQueryPortImpl`，内部仅委托 `SysUserMapper.selectById(id).getName()`（不引入缓存/切面副作用；最小回归通过；落地：`58a4b628`）。
- ✅ 补充进展（2026-02-23，保持行为不变，课程写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `UpdateCourseInfoRepositoryImpl` 已将“按 `teacherId` 取姓名”的跨 BC `sysUserMapper` 反射调用收敛为调用 IAM 端口 `UserNameDirectQueryPort.findNameById(...)`（不引入缓存/切面副作用；最小回归通过；落地：`d0dc8708`）。
- ✅ 补充进展（2026-02-23，保持行为不变，课程写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `DeleteSelfCourseRepositoryImpl` 已将“按用户名查用户信息”的跨 BC `sysUserMapper.selectOne(eq username)` 反射调用收敛为调用 IAM 端口 `UserDetailByUsernameDirectQueryPort.findByUsername(...)`（不引入缓存/切面副作用；最小回归通过；落地：`ff3a31c4`）。
- ✅ 补充进展（2026-02-23，保持行为不变，课程写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `UpdateSelfCourseRepositoryImpl` 已将“按用户名查用户信息/按 teacherId 查姓名”的跨 BC `sysUserMapper` 反射调用收敛为调用 IAM 直查端口 `UserDetailByUsernameDirectQueryPort/UserNameDirectQueryPort`（不引入缓存/切面副作用；最小回归通过；落地：`0439dea0`）。
- ✅ 补充进展（2026-02-23，保持行为不变，课程写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `UpdateSingleCourseRepositoryImpl` 已将“按 teacherId 取姓名”的跨 BC `sysUserMapper` 反射调用收敛为调用 IAM 端口 `UserNameDirectQueryPort.findNameById(...)`（缺失老师仍抛 `QueryException(\"老师不存在\")`；不引入缓存/切面副作用；最小回归通过；落地：`edf22c1f`）。
- ✅ 补充进展（2026-02-23，保持行为不变，课程写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `DeleteCourseRepositoryImpl` 已将“按 teacherId 取姓名”的跨 BC `sysUserMapper` 反射调用收敛为调用 IAM 端口 `UserNameDirectQueryPort.findNameById(...)`（缺失老师仍保持在删除链路后触发 NPE 的时机不变；不引入缓存/切面副作用；最小回归通过；落地：`77fd744f`）。
- ✅ 补充进展（2026-02-23，保持行为不变，课程写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `DeleteCoursesRepositoryImpl` 已将“按 teacherId 取姓名”的跨 BC `sysUserMapper` 反射调用收敛为调用 IAM 端口 `UserNameDirectQueryPort.findNameById(...)`（缺失老师仍抛 `QueryException(\"对应老师不存在\")`；不引入缓存/切面副作用；最小回归通过；落地：`ecacf6b2`）。
- ✅ 补充进展（2026-02-23，保持行为不变，课程写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `AssignEvaTeachersRepositoryImpl` 已将“按 teacherId 取姓名”的跨 BC `sysUserMapper` 反射调用收敛为调用 IAM 端口 `UserNameDirectQueryPort.findNameById(...)`（缺失老师仍抛 `QueryException(\"所分配老师中有人未在数据库中\")`；不引入缓存/切面副作用；最小回归通过；落地：`e1d9c16b`）。
- 🎯 下一刀建议（写侧优先，保持行为不变；每次只改 1 个类/1 个资源 XML/1 个 `pom.xml` 闭环）：从 `docs/DDD_REFACTOR_BACKLOG.md` 的 4.3“未收敛清单”或第 6 节建议中选 1 个可单刀闭环目标继续推进。
...
- ✅ 补充进展（2026-02-24，保持行为不变，写侧收敛，单类）：课程写侧 `bc-course/infrastructure` 的 `AssignEvaTeachersRepositoryImpl` 已移除 `@Qualifier(\"evaTaskMapper\") Object` 运行期依赖与反射 `selectList/selectOne/insert`，统一改走评教端口 `EvaTaskByTeacherIdsAndStatusDirectQueryPort` + `EvaTaskInsertPort`（保持异常文案/日志文案/缓存失效与副作用顺序完全不变；最小回归通过；落地：`6926ada2`）。
- ✅ 补充进展（2026-02-25，保持行为不变，读侧收敛，单类）：课程读侧仓储 `bc-course/infrastructure` 的 `CourseQueryRepository` 已移除 `@Qualifier(\"evaTaskMapper\") Object` 运行期依赖与反射 `selectList/selectCount` 调用，统一改走评教端口 `EvaTaskBriefByCourInfIdsDirectQueryPort`（保持异常文案/缓存/日志与副作用顺序完全不变；最小回归通过；落地：`65d1c784`）。
- 🎯 下一刀建议（更新至 2026-02-25；保持行为不变；单类；写侧优先）：继续推进“课程/模板写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 `*-contract`/`shared-kernel`）。模板侧 `CourseTemplateLockQueryPortImpl` 已完成端口化清零，下一刀建议回到课程写侧继续清零对评教 DO 的编译期依赖（口径：`rg -n "edu\\.cuit\\.infra\\.dal\\.database\\.dataobject\\.eva\\." bc-course --glob "**/*.java"`），优先从 `bc-course/infrastructure` 的 `AssignEvaTeachersRepositoryImpl` 开始逐类闭环。建议先跑口径快照：
