...
- 🎯 下一刀建议（更新至 2026-02-23；保持行为不变；单类；写侧优先）：开始推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 `*-contract`/`shared-kernel`）。推荐先走删除链路：先在 `bc-evaluation/contract` 补最小 CO（✅ 已完成：`EvaTaskBriefCO`）/端口（✅ 已完成：`EvaTaskBriefByCourInfIdsDirectQueryPort`），再在 `bc-evaluation/infrastructure` 落端口适配器（✅ 已完成：`EvaTaskBriefByCourInfIdsDirectQueryPortImpl`），最后回到 `bc-course/infrastructure` 替换一个删除类调用点（异常文案/缓存/日志/副作用顺序完全不变；每刀闭环）。
