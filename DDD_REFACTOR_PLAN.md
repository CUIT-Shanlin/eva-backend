...
- 现状快照（更新至 2026-01-17，保持行为不变）：`eva-app` 下 `*ServiceImpl` 已清零（0），且 `eva-app/src/main/java` 已清零（口径：`fd -t f -e java . eva-app/src/main/java | wc -l` 当前为 0）。`eva-adapter` 下残留 `*Controller` 为 **22 个**。下一刀建议转向单 pom：评估并收敛 `start/pom.xml` 对 `eva-app` 的依赖（见 `NEXT_SESSION_HANDOFF.md` 0.10/0.11）。
