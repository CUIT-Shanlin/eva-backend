...
5) **可选里程碑 M4：命名整合（仓库不出现 eva- 前缀）**
  ...
  - 现状评估（更新至 2026-02-15，保持行为不变）：
    ...
    - `eva-adapter`：已完成退场闭环（残留 Controller 清零 + 组合根去依赖 + root reactor 移除 + 删除模块 pom）；证据口径：`fd -t f 'Controller\\.java$' eva-adapter/src/main/java | wc -l` 为 0，且 `rg -n '<module>eva-adapter</module>' pom.xml` 命中为 0，且 `rg -n '<artifactId>eva-adapter</artifactId>' --glob '**/pom.xml' .` 命中为 0（保持行为不变）。
	    ...
	    - `eva-infra-shared`：仍被部分 BC 基础设施模块直依赖，当前剩余 `3` 个 Java（口径：`fd -t f -e java . eva-infra-shared/src/main/java | wc -l`）。补充：`CourseFormat` 已下沉到 `shared-kernel`（`dff4e751`）；剩余类为 `CourseBizConvertor/CourseConvertor/UserConverter`（MapStruct），三者之间无互相依赖，但**引用面跨** `bc-course/bc-evaluation/bc-iam`，且编译期依赖 `bc-course-domain/bc-iam-domain` 的实体类型，因此**不能直接下沉** `shared-kernel`（会形成 Maven 环依赖）。退场路线：先按 M1/M2 用最小 Port 收敛跨 BC 使用点，使其变为“单 BC 独占”，再逐类搬运归位到对应 BC `infrastructure`，最后逐 pom 收敛依赖并评估模块退场。LDAP 子簇已清零（相关类型已下沉到 `shared-kernel`，保持包名不变）。
