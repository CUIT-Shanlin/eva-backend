...
- 🎯 下一刀建议（更新至 2026-02-25；保持行为不变；单类；写侧优先）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 `*-contract`/`shared-kernel`）。删除链路/取消链路/自助删课链路已阶段性闭环；当前已补齐评教 contract 端口 `EvaTaskCourInfIdsByStatusesDirectQueryPort`，下一步建议在 `bc-evaluation/infrastructure` 补齐端口适配器 `EvaTaskCourInfIdsByStatusesDirectQueryPortImpl`（单类闭环；不引入缓存/切面副作用；保持查询条件/结果顺序/空值语义不变），再回到 `bc-course/infrastructure` 改 `CourseRecommendExce` 去 `evaTaskMapper` 运行期依赖（保持行为不变）。
