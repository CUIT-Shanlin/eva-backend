...
- 🎯 下一步建议（写侧优先，保持行为不变）：`bc-evaluation/infrastructure/repository` 写侧对课程域 `CourseMapper` 的编译期直连已清零；下一步建议转向评教读侧 QueryRepo（如 `EvaTaskQueryRepository/EvaRecordQueryRepository/EvaStatisticsQueryRepository/EvaTemplateQueryRepository`）的直连点，或进入“跨 BC 依赖边界收敛（contract 化）”阶段（每次仍只改 1 个类闭环）。
