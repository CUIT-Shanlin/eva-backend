...
- 🎯 下一刀建议（写侧优先，保持行为不变，单类闭环）：回到课程写侧 `UpdateSelfCourseRepositoryImpl`，将其按 `cour_inf_id` 的评教任务查询/删除从反射调用收敛为调用评教端口（读端口 `EvaTaskBriefByCourInfIdsDirectQueryPort` + 写端口 `EvaTaskDeleteByCourInfIdPort`），保持异常文案/缓存失效/副作用顺序不变。
