...
- 现状快照（更新至 2026-01-17，保持行为不变）：`eva-app` 下 `*ServiceImpl` 已清零（0）；但仍残留少量支撑类（口径：`fd -t f -e java . eva-app/src/main/java | wc -l` 当前为 7）。`eva-adapter` 下残留 `*Controller` 为 **22 个**。具体清单与优先级见 `NEXT_SESSION_HANDOFF.md` 0.10。
...
2) **可以移除 `eva-app` / `eva-adapter` 的判定标准（建议的 DoD）**
   ...
   - 现状评估（更新至 2026-01-16，保持行为不变）：
     - `eva-app`：`*ServiceImpl` 已清零，但仍残留少量支撑类（口径：`fd -t f -e java . eva-app/src/main/java | wc -l` 当前为 7；典型包括评教 Convertor、少量 Exec/Service 等）。组合根 `start` 仍显式依赖 `eva-app`（见 `start/pom.xml`），因此“移除模块”尚未满足。
