...
- 现状散落点（Serena 证据化盘点，2025-12-29）：
  ...
  - 迁移前置（DAL 归位，保持行为不变）：消息表数据对象 `MsgTipDO` 与 Mapper `MsgTipMapper`（含 `MsgTipMapper.xml`）已从 `eva-infra` 归位到 `eva-infra-dal`（保持 `package/namespace` 不变）。后续建议继续按“每次只迁 1 个类”推进：先 `MessageInsertionPortImpl`，再处理依赖更重的 `MessageQueryPortImpl`（依赖 `MsgConvertor/UserQueryGateway`，风险更高，后置；避免 bc-messaging 反向依赖 `eva-infra` 造成 Maven 循环依赖）。
