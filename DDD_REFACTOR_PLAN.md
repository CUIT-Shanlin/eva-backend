...
- ✅ 已完成（保持行为不变）：`UserUpdateController`（落地：`5ee37fd2`）、`DepartmentController`（落地：`fbc5fb74`）、`AuthenticationController`（落地：`fd9e4d1c`）、`MenuUpdateController`（落地：`44bc649d`）。
- ⏳ 待完成（建议顺序；每次只改 1 个类闭环）：`RoleUpdateController`。
