...
- 🎯 下一刀建议（更新至 2026-02-24；保持行为不变；单类；写侧优先）：继续推进“课程写侧去评教 infrastructure/Mapper/DO/XML 直连”（跨 BC 只允许依赖 `*-contract`/`shared-kernel`）。删除链路/取消链路/自助删课链路已阶段性闭环；当前推荐优先收敛 **分配评教老师** 链路：先改 `bc-course/infrastructure` 的 `AssignEvaTeachersRepositoryImpl` 去评教 `evaTaskMapper` 运行期直连，改走评教端口 `EvaTaskByTeacherIdsAndStatusDirectQueryPort` + `EvaTaskInsertPort`（端口已齐且已有适配器；必须保持异常文案/日志/缓存失效与副作用顺序完全不变；每刀闭环）。
