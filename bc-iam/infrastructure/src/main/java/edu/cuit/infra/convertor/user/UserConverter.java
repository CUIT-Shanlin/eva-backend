...
public interface UserConverter {
    ...
    /**
     * 过渡期桥接方法：从“用户对象（实际为 UserEntity）”中取 name。
     * <p>
     * 约束：内部仍使用 {@link UserEntity} 强转，以尽量保持历史空值/异常表现一致。
     * </p>
     * <p>
     ...
     * </p>
     */
    default String nameOf(Object user, boolean ignored) {
        return ((UserEntity) user).getName();
    }

    /**
     * 过渡期桥接方法：从“用户对象（实际为 UserEntity）”中取 perms。
     * <p>
     * 约束：内部仍使用 {@link UserEntity} 强转，以尽量保持历史空值/异常表现一致。
     * </p>
     * <p>
     ...
     * </p>
     */
    default List<String> permsOf(Object user, boolean ignored) {
        return ((UserEntity) user).getPerms();
    }

    /**
     * 过渡期桥接方法：强转为 UserEntity（仅用于保持历史 ClassCastException 触发点尽量一致）。
     * <p>
     * 说明：为避免被 MapStruct 误判为通用类型转换方法（导致编译期歧义），刻意保留一个无业务意义的形参。
     * </p>
     */
    default Object castUserEntityObject(Object user, boolean ignored) {
